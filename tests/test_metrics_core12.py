import json
from types import SimpleNamespace

import polars as pl
import pytest

from etl.l3_aggregate import run as l3_run
from metrics.core12 import compute as core12_compute
from utils.paths import path_for


def _settings(tmp_path):
    return SimpleNamespace(data_root=tmp_path)


def test_core12_compute_generates_expected_columns(tmp_path, monkeypatch):
    settings = _settings(tmp_path)
    monkeypatch.setattr("utils.paths.load_settings", lambda *args, **kwargs: settings)

    # Prepare supporting L2/L1 artifacts
    l2_path = path_for("l2", 2025, 8)
    l2_path.parent.mkdir(parents=True, exist_ok=True)
    pl.DataFrame(
        {
            "season": [2025] * 6,
            "week": [8] * 6,
            "game_id": ["G1", "G1", "G1", "G1", "G1", "G1"],
            "play_id": [1, 2, 3, 4, 5, 6],
            "TEAM": ["BAL", "BAL", "BAL", "MIA", "MIA", "MIA"],
            "OPP": ["MIA", "MIA", "MIA", "BAL", "BAL", "BAL"],
            "drive": [1, 1, 2, 1, 1, 2],
            "play_type": ["pass", "run", "interception", "field_goal", "pass", "extra_point"],
            "epa": [-0.6, 0.3, -2.0, 0.8, -0.5, 0.4],
            "success": [0.0, 1.0, 0.0, 1.0, 0.0, 1.0],
            "yards_gained": [-7.0, 12.0, -5.0, 25.0, 18.0, 10.0],
            "success_bin": [0, 1, 0, 1, 0, 1],
            "is_explosive": [0, 1, 0, 0, 1, 0],
            "is_turnover": [0, 0, 1, 0, 0, 0],
            "is_offensive_td": [1, 0, 0, 0, 1, 0],
            "is_dropback": [1, 0, 1, 1, 1, 0],
            "is_pressure": [1, 0, 0, 0, 1, 0],
            "is_third_down": [0, 1, 1, 0, 1, 0],
            "third_down_converted": [0, 1, 0, 0, 1, 0],
            "in_redzone": [1, 0, 0, 0, 1, 0],
            "play_description": [
                "Pass short left to WR for 10 yards, TOUCHDOWN.",
                "Run up the middle for 4 yards.",
                "Pass intercepted by defender, tackled.",
                "Field goal is GOOD.",
                "Pass to WR for 15 yards, TOUCHDOWN.",
                "extra point is GOOD.",
            ],
        }
    ).write_parquet(l2_path)

    l1_path = path_for("l1", 2025, 8)
    l1_path.parent.mkdir(parents=True, exist_ok=True)
    pl.DataFrame(
        {
            "season": [2025, 2025],
            "week": [8, 8],
            "posteam": ["BAL", "MIA"],
            "play_type": ["touchdown pass", "run"],
            "yardline_100": [15.0, 30.0],
        }
    ).write_parquet(l1_path)

    l3_result = l3_run(2025, 8, l2_result=l2_path)
    l3_df = pl.read_parquet(l3_result)

    result = core12_compute(l3_df, 2025, 8)
    expected_columns = {
        "season",
        "week",
        "TEAM",
        "core_epa_off",
        "core_epa_def",
        "core_sr_off",
        "core_sr_def",
        "core_ed_sr_off",
        "core_third_down_conv",
        "core_ypp_diff",
        "core_turnover_margin",
        "core_points_per_drive_diff",
        "core_redzone_td_rate",
        "core_pressure_rate_def",
        "core_explosive_play_rate_off",
    }
    assert expected_columns.issubset(set(result.columns))
    assert expected_columns.issubset(set(result.columns))
    assert result.height == 2
    for column, dtype in result.schema.items():
        if column in {"season", "week"}:
            assert dtype == pl.Int64
        elif column == "TEAM":
            assert dtype == pl.Utf8
        else:
            assert dtype == pl.Float64

    bal = result.filter(pl.col("TEAM") == "BAL").row(0, named=True)
    mia = result.filter(pl.col("TEAM") == "MIA").row(0, named=True)

    assert pytest.approx(bal["core_points_per_drive_diff"], rel=1e-6) == -2.0
    assert pytest.approx(mia["core_points_per_drive_diff"], rel=1e-6) == 2.0
    assert pytest.approx(bal["core_explosive_play_rate_off"], rel=1e-6) == pytest.approx(1 / 3)
    assert pytest.approx(mia["core_explosive_play_rate_off"], rel=1e-6) == pytest.approx(1 / 3)
    assert pytest.approx(bal["core_third_down_conv"], rel=1e-6) == pytest.approx(0.5)
    assert pytest.approx(mia["core_third_down_conv"], rel=1e-6) == pytest.approx(1.0)
    assert pytest.approx(bal["core_redzone_td_rate"], rel=1e-6) == 1.0
    assert pytest.approx(mia["core_redzone_td_rate"], rel=1e-6) == 1.0
    assert pytest.approx(bal["core_pressure_rate_def"], rel=1e-6) == pytest.approx(0.5)
    assert pytest.approx(mia["core_pressure_rate_def"], rel=1e-6) == pytest.approx(0.5)

    out_path = path_for("l4_core12", 2025, 8)
    assert out_path.exists()
    manifest = path_for("l4_core12_manifest", 2025, 8)
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    assert payload["rows"] == 2
    assert payload["layer"] == "l4_core12"


def test_core12_handles_empty_input(tmp_path, monkeypatch):
    settings = _settings(tmp_path)
    monkeypatch.setattr("utils.paths.load_settings", lambda *args, **kwargs: settings)

    empty_l3 = pl.DataFrame(
        {
            "season": [],
            "week": [],
            "TEAM": [],
            "drives": [],
            "plays": [],
            "epa_off_mean": [],
            "success_rate_off": [],
            "epa_def_mean": [],
            "success_rate_def": [],
            "tempo": [],
            "pressure_rate_def": [],
            "explosive_play_rate_off": [],
            "third_down_conv_off": [],
            "redzone_td_rate_off": [],
            "turnover_margin": [],
            "points_per_drive_off": [],
            "points_per_drive_def": [],
            "points_per_drive_diff": [],
            "ypp_off": [],
            "ypp_def": [],
            "ypp_diff": [],
        },
        schema={
            "season": pl.Int64,
            "week": pl.Int64,
            "TEAM": pl.Utf8,
            "drives": pl.Int64,
            "plays": pl.Int64,
            "epa_off_mean": pl.Float64,
            "success_rate_off": pl.Float64,
            "epa_def_mean": pl.Float64,
            "success_rate_def": pl.Float64,
            "tempo": pl.Float64,
            "pressure_rate_def": pl.Float64,
            "explosive_play_rate_off": pl.Float64,
            "third_down_conv_off": pl.Float64,
            "redzone_td_rate_off": pl.Float64,
            "turnover_margin": pl.Float64,
            "points_per_drive_off": pl.Float64,
            "points_per_drive_def": pl.Float64,
            "points_per_drive_diff": pl.Float64,
            "ypp_off": pl.Float64,
            "ypp_def": pl.Float64,
            "ypp_diff": pl.Float64,
        },
    )

    # create empty L1/L2 artifacts to avoid file-not-found
    path_for("l1", 2025, 8).parent.mkdir(parents=True, exist_ok=True)
    pl.DataFrame(
        {
            "season": pl.Series([], dtype=pl.Int64),
            "week": pl.Series([], dtype=pl.Int64),
            "posteam": pl.Series([], dtype=pl.Utf8),
            "play_type": pl.Series([], dtype=pl.Utf8),
            "yardline_100": pl.Series([], dtype=pl.Float64),
        }
    ).write_parquet(path_for("l1", 2025, 8))

    pl.DataFrame(
        {
            "season": pl.Series([], dtype=pl.Int64),
            "week": pl.Series([], dtype=pl.Int64),
            "game_id": pl.Series([], dtype=pl.Utf8),
            "play_id": pl.Series([], dtype=pl.Int64),
            "TEAM": pl.Series([], dtype=pl.Utf8),
            "OPP": pl.Series([], dtype=pl.Utf8),
            "drive": pl.Series([], dtype=pl.Int64),
            "play_type": pl.Series([], dtype=pl.Utf8),
            "epa": pl.Series([], dtype=pl.Float64),
            "success": pl.Series([], dtype=pl.Float64),
            "yards_gained": pl.Series([], dtype=pl.Float64),
            "success_bin": pl.Series([], dtype=pl.Int64),
            "is_turnover": pl.Series([], dtype=pl.Int64),
            "is_offensive_td": pl.Series([], dtype=pl.Int64),
            "is_explosive": pl.Series([], dtype=pl.Int64),
            "is_third_down": pl.Series([], dtype=pl.Int64),
            "third_down_converted": pl.Series([], dtype=pl.Int64),
            "in_redzone": pl.Series([], dtype=pl.Int64),
            "play_description": pl.Series([], dtype=pl.Utf8),
        }
    ).write_parquet(path_for("l2", 2025, 8))

    result = core12_compute(empty_l3, 2025, 8)
    assert result.is_empty()
