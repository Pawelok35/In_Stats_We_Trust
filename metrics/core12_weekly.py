...
def build_core12_weekly(season: int, week: int) -> pl.DataFrame:
    ...
    extras = pl.DataFrame(
        {
            ...
            "core_ed_sr_off": [None]
            * base.height,  # Explosive Drive Success Rate (drive-level explosiveness)
    ...
    if l2_path.exists():
        ...
        off_agg = df_l2.groupby("TEAM").agg(
            [
                # yards per play
                (pl.col("yards_gained").sum() / pl.count()).alias("ypp_off"),
                # explosive play rate off
                (pl.col("is_explosive").cast(pl.Int64).sum() / pl.count()).alias(
                ...
                (pl.col("points").sum() / pl.col("drive_id").n_unique()).alias(
        ...
        def_agg = df_l2.groupby("TEAM").agg(
            [
                ...
                (pl.col("points_allowed").sum() / pl.col("opp_drive_id").n_unique()).alias(
        ...
        combined = combined.with_columns(
            [
                ...
                (pl.col("points_per_drive_off") - pl.col("points_per_drive_def")).alias(
        ...
        if "drive_had_explosive" in df_l2.columns:
            drive_level = (
                df_l2.groupby(["TEAM", "drive_id"])
                .agg([pl.col("drive_had_explosive").max().alias("drive_explosive_flag")])
                .groupby("TEAM")
                .agg([(pl.col("drive_explosive_flag").sum() / pl.count()).alias("core_ed_sr_off")])
            )

            extras = extras.join(drive_level, on="TEAM", how="left")
        else:
            # jeśli nie mamy danych – zostaw None, raport sam pominie
            extras = extras.with_columns(pl.lit(None).alias("core_ed_sr_off"))
