...
def run(cmd: list[str], *, desc: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"Step '{desc}' failed with exit code {result.returncode}. " f"Command: {printable}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Week to fully ingest/build (L1-L4). Defaults to week-1 so latest "
            "completed games are pulled."
        ),
    )
    parser.add_argument(
        "--reference-week",
...
def main() -> None:
    ...
    reference_week = args.reference_week if args.reference_week is not None else max(1, week - 7)

    manual_path = args.manual_results.resolve()
    if not manual_path.exists():
        raise SystemExit(f"Missing manual results file: {manual_path}")

    # 1) Sync schedule and spread/total lines from nfl_data_py.
    run(
        [
            sys.executable,
            "-m",
        ...
        desc="Sync nfl_data_py schedule",
    )
    ...
    run(
        [
            ...
            "-m",
            "app.cli",
            "export-lines-from-nfl",
            "--season",
            str(season),
            "--week",
            str(week),
        ],
        desc="Export spread/total lines",
    )

    config_path = ROOT_DIR / "config" / "lines" / str(season) / f"week{week}_lines.yaml"
    if not config_path.exists():
        raise SystemExit(f"Missing config file after export: {config_path}")
    ...
    if missing:
        print(
            "[WARN] Skipping convergence analyzer - missing pick files for variants: "
            f"{', '.join(missing)}"
        )
        return
