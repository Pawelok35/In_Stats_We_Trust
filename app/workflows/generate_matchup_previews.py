...
from utils.team_aliases import normalize_team_code  # noqa: E402
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=2025,
        help="Season to process (default: 2025)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Season whose metrics should power previews. Defaults to --season. "
            "Use previous season for Week 1 preseason previews."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail if the schedule references teams without available metrics "
...
def _frames_with_fallback(
    ...
    """
    if reference_week is not None and reference_week != target_week:
        reference_frames = reports._load_metric_frames(season, reference_week)
        reference_teams = reports.available_teams(season, reference_week, frames=reference_frames)
        has_reference_metrics = any(
            df is not None and not df.is_empty() for df in reference_frames.values()
        )
        if reference_teams and has_reference_metrics:
            logger.info(
                "Using explicitly requested metrics from week %s for previews targeting week %s.",
                reference_week,
                target_week,
            )
            return reference_frames, reference_week
...
def main(argv: list[str] | None = None) -> None:
    ...
    metrics_season = args.metrics_season or args.season
    frames, source_week = _frames_with_fallback(metrics_season, args.week, args.reference_week)
    available = set(reports.available_teams(metrics_season, source_week, frames=frames))
    if not available:
        raise SystemExit(
            f"[error] No metrics available for metrics season {metrics_season} "
            f"through week {source_week}; cannot build previews."
        )

    logger.info(
        "Generating matchup previews for season=%s week=%s using metrics season=%s week=%s "
        "(schedule=%s).",
        args.season,
        args.week,
        metrics_season,
    ...
    for row in pairs.to_dicts():
        ...
        if not (
            _ensure_team_available(
                available,
                team_a,
                require_complete_schedule=args.require_complete_schedule,
            )
            and _ensure_team_available(
                available,
                team_b,
                require_complete_schedule=args.require_complete_schedule,
            )
    ...
    if generated_markdown:
        ...
        logger.warning(
            "No matchup reports produced for season=%s week=%s "
            "(after filtering unavailable teams).",
            args.season,
            args.week,
        )
    ...
    if args.summary:
        try:
            reports.generate_weekly_summary(
                args.season,
                args.week,
                comparison_reports=generated_markdown,
            )
