# Matchup Report - KC vs JAX

## Metric Comparison

| Metric | KC | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.143 | 0.027 | 0.116 |
| Core EPA Defense | 0.043 | -0.022 | 0.066 |
| Success Rate Offense | 0.482 | 0.460 | 0.022 |
| Success Rate Defense | 0.439 | 0.404 | 0.035 |
| Explosive Play Rate (Off) | 0.114 | 0.111 | 0.004 |
| Third Down Conversion | 0.428 | 0.405 | 0.023 |
| Points per Drive Differential | 0.580 | 0.063 | 0.517 |
| Yards per Play Differential | 0.621 | 0.292 | 0.329 |
| Turnover Margin | 0.375 | -0.222 | 0.597 |
| Red Zone TD Rate (Off) | 0.145 | 0.132 | 0.012 |
| Pressure Rate (Def) | 0.072 | 0.054 | 0.018 |
| Tempo | 6.818 | 7.364 | -0.545 |
| PowerScore | 1.514 | 1.584 | -0.070 |
| PowerScore | 1.514 | 1.584 | -0.070 |
| Tempo | 6.818 | 7.364 | -0.545 |

## PowerScore Breakdown (Model)

| Component | Weight | KC | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.093 | -0.043 | +0.135 |
| EPA Defense | 20% | -0.043 | 0.093 | -0.135 |
| Success Rate Offense | 25% | 50.7% | 43.2% | +7.5 pp |
| Tempo | 20% | 0.160 | 0.086 | +0.074 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | KC | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.093 | -0.043 | +0.135 |
| EPA Defense | 20% | -0.043 | 0.093 | -0.135 |
| Success Rate Offense | 15% | 50.7% | 43.2% | +7.5 pp |
| Turnover Margin | 10% | -3.000 | 3.000 | -6.000 |
| Tempo | 10% | 0.160 | 0.086 | +0.074 |
| Red Zone TD Rate (Off) | 10% | 33.3% | 14.3% | +19.0 pp |
| Pressure Rate (Def) | 10% | 11.1% | 0.0% | +11.1 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.054 (KC +0.183 vs JAX +0.129)
**7 metrics version:**
JAX edge: +0.545 (KC -0.149 vs JAX +0.396)
**Verdict:** JAX holds the edge in the extended breakdown (lead 0.545), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (KC - JAX) | -1.3 pts (favours JAX) |
| Model Win% KC | 44.6% |
| Model Win% JAX | 55.4% |
| SOS Differential (KC-JAX) | +0.016 |
*Neutral-field assumption. Based on schedule through Week 9.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| KC | 48.0% | 43.2% | 4.8% | 66.7% | 35.9% | Pass heavy |
| JAX | 55.6% | 43.2% | 12.4% | 48.9% | 36.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +8.1 pp | +10.2 pp | +9.9 pp |
| JAX | +6.9 pp | +8.1 pp | +13.6 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | -1.0 pp | -3.1 pp | -0.7 pp |
| JAX | -8.8 pp | -7.5 pp | -5.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +1.1 pp | -1.5 pp | +0.8 pp |
| JAX | +2.8 pp | +2.9 pp | +7.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +1.2 pp | +0.6 pp | +2.5 pp |
| JAX | +1.4 pp | +1.4 pp | -1.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +6.1 pp | +2.7 pp | -4.8 pp |
| JAX | +1.0 pp | +4.1 pp | +2.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +10.1 pp | +5.6 pp | +6.0 pp |
| JAX | +6.5 pp | +5.8 pp | +14.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +1.2 pp | +0.6 pp | +2.5 pp |
| JAX | +1.4 pp | +1.4 pp | -1.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +1.1 pp | -1.5 pp | +0.8 pp |
| JAX | +2.8 pp | +2.9 pp | +7.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 49.1 | 50.1 | 45.2 |
| JAX | 45.5 | 46.3 | 47.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 49.5 | 50.5 | 49.8 |
| JAX | 46.0 | 43.1 | 42.3 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | -0.4 | -0.4 | -4.7 |
| JAX | -0.5 | 3.2 | 5.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 2.78 | 2.87 | 2.54 |
| JAX | 1.98 | 1.83 | 1.96 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 2.20 | 2.37 | 1.93 |
| JAX | 1.92 | 2.31 | 2.21 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 0.58 | 0.50 | 0.61 |
| JAX | 0.06 | -0.48 | -0.25 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 8.10 | 8.47 | 8.21 |
| JAX | 7.69 | 8.26 | 8.56 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 50.5% | 51.7% | 54.5% |
| JAX | 44.0% | 42.9% | 41.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 49.5% | 48.3% | 45.5% |
| JAX | 56.0% | 57.1% | 58.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 4.09 | 4.38 | 4.47 |
| JAX | 3.38 | 3.55 | 3.55 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 4.01 | 4.09 | 3.74 |
| JAX | 4.31 | 4.71 | 5.01 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 9)

| Window | KC | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.703 | 1.687 |
| Last 5 games | 1.720 | 1.824 |
| Last 3 games | 1.659 | 1.811 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA KC | 0.037 | 0.372 | -0.027 | - declining |
| Off EPA JAX | 0.154 | 0.065 | 0.073 | - declining |
| Def EPA KC | 0.085 | 0.051 | -0.057 | + improving |
| Def EPA JAX | 0.178 | 0.157 | 0.029 | + improving |
| Off SR KC | 42.7% | 55.0% | 39.0% | - declining |
| Off SR JAX | 50.6% | 42.0% | 51.8% | + improving |
| Def SR KC | 46.0% | 44.3% | 33.3% | + improving |
| Def SR JAX | 45.8% | 51.7% | 37.8% | + improving |
| Tempo KC | 8.200 | 8.000 | 8.429 | + improving |
| Tempo JAX | 7.700 | 9.778 | 7.727 | + improving |

## Matchup Analogs

**KC analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | BUF (H) | 20-24 | BUF | 0.844 | 0.037 | 42.7% | -0.400 |
| 7 | SF (A) | 42-21 | KC | 0.836 | 0.372 | 55.0% | 2.109 |
| 9 | TEN (H) | 20-18 | KC | 0.566 | -0.027 | 39.0% | 0.121 |

**JAX analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | NYG (H) | 17-26 | NYG | 0.857 | 0.065 | 42.0% | -0.986 |
| 4 | PHI (A) | 15-29 | PHI | 0.757 | -0.211 | 43.3% | -1.167 |
| 9 | LV (H) | 27-22 | JAX | 0.713 | 0.073 | 51.8% | 0.455 |

## Core EPA Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.027 | 0.032 | 0.037 |
| KC | 0.143 | 0.150 | 0.127 |

## Core EPA Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.022 | 0.077 | 0.068 |
| KC | 0.043 | 0.063 | 0.026 |

## Success Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.1% | 46.6% | 47.5% |
| KC | 47.9% | 47.6% | 44.6% |

## Success Rate Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 41.0% | 44.3% | 45.2% |
| KC | 44.2% | 44.1% | 41.9% |

## Explosive Play Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 11.0% | 12.4% | 11.4% |
| KC | 11.2% | 10.9% | 12.5% |

## Third Down Conversion Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 41.0% | 43.5% | 46.1% |
| KC | 43.3% | 45.3% | 37.6% |

## Points per Drive Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.101 | -0.450 | -0.198 |
| KC | 0.544 | 0.483 | 0.552 |

## Yards per Play Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.292 | 0.338 | -0.064 |
| KC | 0.621 | 0.319 | 0.863 |

## Turnover Margin Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.222 | -1.000 | -0.667 |
| KC | 0.375 | 0.400 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 13.2% | 11.9% | 12.2% |
| KC | 14.5% | 14.8% | 12.4% |

## Pressure Rate (Def) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 5.4% | 4.3% | 5.9% |
| KC | 7.2% | 7.0% | 9.0% |

## Tempo Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 7.690 | 8.259 | 8.562 |
| KC | 8.099 | 8.471 | 8.210 |

## Pass Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 44.1% | 43.1% | 41.4% |
| KC | 50.9% | 52.1% | 55.4% |
