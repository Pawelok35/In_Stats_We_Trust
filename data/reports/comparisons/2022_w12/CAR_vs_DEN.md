# Matchup Report - CAR vs DEN

## Metric Comparison

| Metric | CAR | DEN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.099 | -0.089 | -0.010 |
| Core EPA Defense | -0.026 | -0.058 | 0.032 |
| Success Rate Offense | 0.390 | 0.369 | 0.021 |
| Success Rate Defense | 0.429 | 0.420 | 0.009 |
| Explosive Play Rate (Off) | 0.090 | 0.095 | -0.005 |
| Third Down Conversion | 0.250 | 0.269 | -0.019 |
| Points per Drive Differential | -0.388 | -0.153 | -0.235 |
| Yards per Play Differential | -0.269 | 0.242 | -0.511 |
| Turnover Margin | -0.455 | 0.200 | -0.655 |
| Red Zone TD Rate (Off) | 0.134 | 0.123 | 0.010 |
| Pressure Rate (Def) | 0.058 | 0.068 | -0.010 |
| Tempo | 6.308 | 5.571 | 0.736 |
| PowerScore | 1.350 | 1.150 | 0.200 |
| PowerScore | 1.350 | 1.150 | 0.200 |
| Tempo | 6.308 | 5.571 | 0.736 |

## PowerScore Breakdown (Model)

| Component | Weight | CAR | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.028 | -0.171 | +0.200 |
| EPA Defense | 20% | -0.171 | 0.028 | -0.200 |
| Success Rate Offense | 25% | 45.1% | 35.9% | +9.2 pp |
| Tempo | 20% | 0.098 | 0.077 | +0.021 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.028 | -0.171 | +0.200 |
| EPA Defense | 20% | -0.171 | 0.028 | -0.200 |
| Success Rate Offense | 15% | 45.1% | 35.9% | +9.2 pp |
| Turnover Margin | 10% | 3.000 | -3.000 | +6.000 |
| Tempo | 10% | 0.098 | 0.077 | +0.021 |
| Red Zone TD Rate (Off) | 10% | 11.1% | 7.1% | +4.0 pp |
| Pressure Rate (Def) | 10% | 7.9% | 0.0% | +7.9 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.057 (CAR +0.108 vs DEN +0.051)
**7 metrics version:**
CAR edge: +0.638 (CAR +0.369 vs DEN -0.268)
**Verdict:** CAR holds the edge in the extended breakdown (lead 0.638), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - DEN) | +3.7 pts (favours CAR) |
| Model Win% CAR | 65.0% |
| Model Win% DEN | 35.0% |
| SOS Differential (CAR-DEN) | +0.037 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 24.4% | 44.4% | -20.0% | 40.0% | 46.8% | Run heavy |
| DEN | 48.7% | 44.4% | 4.3% | 28.9% | 42.5% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -4.7 pp | -1.3 pp | -5.0 pp |
| DEN | -9.8 pp | -7.7 pp | -10.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +8.0 pp | +12.1 pp | +3.7 pp |
| DEN | -0.1 pp | +4.7 pp | +1.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.4 pp | +0.2 pp | -1.8 pp |
| DEN | -2.9 pp | -1.6 pp | -0.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.8 pp | +1.7 pp | -3.1 pp |
| DEN | -1.2 pp | -2.2 pp | -1.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -3.1 pp | -6.6 pp | -8.3 pp |
| DEN | -3.6 pp | -4.1 pp | -6.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -3.8 pp | +0.4 pp | -5.7 pp |
| DEN | -8.9 pp | -9.2 pp | -5.8 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.8 pp | +1.7 pp | -3.1 pp |
| DEN | -1.2 pp | -2.2 pp | -1.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.4 pp | +0.2 pp | -1.8 pp |
| DEN | -2.9 pp | -1.6 pp | -0.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.4 | 41.7 | 43.0 |
| DEN | 43.1 | 39.9 | 37.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.4 | 46.4 | 45.9 |
| DEN | 42.7 | 42.3 | 44.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 0.0 | -4.7 | -2.9 |
| DEN | 0.4 | -2.3 | -7.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.48 | 1.83 | 1.48 |
| DEN | 1.30 | 1.24 | 1.32 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.87 | 1.73 | 2.05 |
| DEN | 1.45 | 1.55 | 1.68 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.39 | 0.11 | -0.57 |
| DEN | -0.15 | -0.31 | -0.36 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 6.51 | 6.90 | 6.76 |
| DEN | 7.44 | 7.24 | 7.19 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 42.4% | 37.8% | 38.9% |
| DEN | 44.8% | 45.2% | 44.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 57.6% | 62.2% | 61.1% |
| DEN | 55.2% | 54.8% | 55.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.76 | 2.61 | 2.63 |
| DEN | 3.33 | 3.27 | 3.19 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.75 | 4.29 | 4.13 |
| DEN | 4.10 | 3.97 | 3.99 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | CAR | DEN |
| --- | ---: | ---: |
| Season-to-date | 1.569 | 1.532 |
| Last 5 games | 1.548 | 1.581 |
| Last 3 games | 1.584 | 1.618 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W10 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | 0.004 | 0.007 | -0.298 | - declining |
| Off EPA DEN | 0.017 | -0.099 | 0.002 | - declining |
| Def EPA CAR | 0.001 | -0.139 | -0.130 | + improving |
| Def EPA DEN | -0.027 | -0.047 | 0.078 | - declining |
| Off SR CAR | 40.4% | 40.0% | 36.1% | - declining |
| Off SR DEN | 44.9% | 34.0% | 43.2% | - declining |
| Def SR CAR | 44.0% | 40.0% | 43.7% | = stable |
| Def SR DEN | 48.9% | 42.5% | 44.6% | + improving |
| Tempo CAR | 7.615 | 7.727 | 6.545 | - declining |
| Tempo DEN | 6.500 | 7.692 | 7.364 | + improving |

## Matchup Analogs

**CAR analogs vs DEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | LA (A) | 4-24 | LA | 0.882 | -0.281 | 31.7% | -1.818 |
| 7 | TB (H) | 21-3 | CAR | 0.686 | 0.116 | 39.4% | 1.827 |
| 4 | ARI (H) | 10-26 | ARI | 0.635 | -0.326 | 40.9% | -1.333 |

**DEN analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | HOU (H) | 16-9 | DEN | 0.927 | -0.035 | 35.1% | 0.636 |
| 5 | IND (H) | 9-14 | IND | 0.904 | -0.187 | 36.2% | -0.323 |
| 7 | NYJ (H) | 9-15 | NYJ | 0.888 | -0.163 | 32.3% | -0.481 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.099 | -0.037 | -0.102 |
| DEN | -0.089 | -0.068 | -0.027 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.026 | -0.037 | -0.015 |
| DEN | -0.058 | -0.022 | 0.002 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 39.2% | 38.9% | 38.1% |
| DEN | 36.7% | 37.9% | 40.2% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.0% | 44.9% | 47.7% |
| DEN | 42.0% | 42.2% | 45.5% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.1% | 10.3% | 8.5% |
| DEN | 9.4% | 8.6% | 9.7% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 25.4% | 29.9% | 27.3% |
| DEN | 26.6% | 26.1% | 25.2% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.389 | 0.086 | -0.566 |
| DEN | -0.163 | -0.315 | -0.354 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.269 | -0.049 | -0.649 |
| DEN | 0.242 | 0.034 | -0.269 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.455 | -0.600 | -1.333 |
| DEN | 0.200 | 0.400 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 13.4% | 16.9% | 9.8% |
| DEN | 12.3% | 15.0% | 16.7% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 5.8% | 7.3% | 9.4% |
| DEN | 6.8% | 5.0% | 3.8% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.514 | 6.898 | 6.758 |
| DEN | 7.436 | 7.243 | 7.185 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 42.1% | 37.1% | 37.7% |
| DEN | 45.0% | 45.6% | 44.8% |
