# Matchup Report - SEA vs LV

## Metric Comparison

| Metric | SEA | LV | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.053 | 0.118 | ↓ -0.065 |
| Core EPA Defense | 0.118 | 0.053 | ↑ +0.065 |
| Success Rate Offense | 51.8% | 48.0% | ↑ +3.8 pp |
| Success Rate Defense | 48.0% | 51.8% | ↓ -3.8 pp |
| Explosive Play Rate (Off) | 12.9% | 12.2% | ↑ +0.7 pp |
| Third Down Conversion | 30.0% | 57.1% | ↓ -27.1 pp |
| Points per Drive Differential | -0.429 | 0.429 | ↓ -0.857 |
| Yards per Play Differential | -1.501 | 1.501 | ↓ -3.002 |
| Turnover Margin | 0.000 | 0.000 | ± 0.000 |
| Red Zone TD Rate (Off) | 17.6% | 11.1% | ↑ +6.5 pp |
| Pressure Rate (Def) | 2.7% | 7.3% | ↓ -4.6 pp |
| Tempo | 6.071 | 7.000 | ↓ -0.929 |
| PowerScore | 0.198 | 0.196 | ↑ +0.001 |

## PowerScore Breakdown (Model)

| Component | Weight | SEA | LV | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.053 | 0.118 | -0.065 |
| EPA Defense | 20% | 0.118 | 0.053 | +0.065 |
| Success Rate Offense | 25% | 51.8% | 48.0% | +3.8 pp |
| Tempo | 20% | 0.129 | 0.122 | +0.007 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | SEA | LV | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.053 | 0.118 | -0.065 |
| EPA Defense | 20% | 0.118 | 0.053 | +0.065 |
| Success Rate Offense | 15% | 51.8% | 48.0% | +3.8 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.129 | 0.122 | +0.007 |
| Red Zone TD Rate (Off) | 10% | 17.6% | 11.1% | +6.5 pp |
| Pressure Rate (Def) | 10% | 2.7% | 7.3% | -4.6 pp |

## PowerScore Summary

**Model (4 metrics):**
SEA edge: +0.001 (SEA +0.198 vs LV +0.196)
**7 metrics version:**
SEA edge: +0.005 (SEA +0.148 vs LV +0.143)
**Verdict:** SEA holds the edge in the extended breakdown (lead 0.005), driven by EPA Defense (20% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (SEA - LV) | +0.0 pts (favours SEA) |
| Model Win% SEA | 50.1% |
| Model Win% LV | 49.9% |
| SOS Differential (SEA-LV) | -0.009 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| SEA | 48.2% | 44.4% | 3.8% | 58.5% | 45.5% | Pass heavy |
| LV | 37.8% | 44.4% | -6.7% | 59.5% | 41.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -7.6 pp | -19.4 pp | -17.9 pp |
| LV | -6.2 pp | -4.7 pp | -12.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -7.6 pp | -1.4 pp | -0.1 pp |
| LV | -3.2 pp | -2.4 pp | -8.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -3.2 pp | -5.5 pp | -3.1 pp |
| LV | +1.7 pp | +4.1 pp | +4.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +1.1 pp | +0.5 pp | -0.4 pp |
| LV | +0.2 pp | +1.3 pp | +0.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +2.5 pp | +4.0 pp | +4.7 pp |
| LV | +4.3 pp | +8.1 pp | +7.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -1.5 pp | -6.8 pp | -7.1 pp |
| LV | -2.3 pp | +0.5 pp | -3.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +1.1 pp | +0.5 pp | -0.4 pp |
| LV | +0.2 pp | +1.3 pp | +0.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -3.2 pp | -5.5 pp | -3.1 pp |
| LV | +1.7 pp | +4.1 pp | +4.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 48.2 | 43.4 | 45.7 |
| LV | 47.2 | 49.5 | 49.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 49.8 | 49.6 | 46.0 |
| LV | 49.1 | 44.9 | 43.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -1.6 | -6.2 | -0.4 |
| LV | -1.9 | 4.6 | 6.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.26 | 2.17 | 2.15 |
| LV | 2.16 | 1.95 | 2.07 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.18 | 1.33 | 1.55 |
| LV | 2.36 | 2.28 | 2.06 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 0.08 | 0.84 | 0.60 |
| LV | -0.20 | -0.33 | 0.01 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 7.53 | 6.96 | 7.00 |
| LV | 8.13 | 8.07 | 8.00 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 45.1% | 44.7% | 48.6% |
| LV | 49.3% | 49.5% | 48.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 54.9% | 55.3% | 51.4% |
| LV | 50.7% | 50.5% | 51.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 3.39 | 3.11 | 3.40 |
| LV | 4.01 | 3.99 | 3.91 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 4.14 | 3.84 | 3.60 |
| LV | 4.12 | 4.08 | 4.09 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | SEA | LV |
| --- | ---: | ---: |
| Season-to-date | 0.138 | 0.146 |
| Last 5 games | 0.082 | 0.154 |
| Last 3 games | 0.112 | 0.159 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA SEA | 0.090 | 0.043 | 0.049 | - declining |
| Off EPA LV | -0.309 | 0.029 | -0.046 | + improving |
| Def EPA SEA | -0.081 | -0.139 | 0.090 | - declining |
| Def EPA LV | 0.081 | 0.073 | 0.050 | + improving |
| Off SR SEA | 41.2% | 47.8% | 54.5% | + improving |
| Off SR LV | 27.4% | 37.8% | 39.8% | + improving |
| Def SR SEA | 32.9% | 35.7% | 45.5% | - declining |
| Def SR LV | 51.4% | 51.8% | 46.8% | + improving |
| Tempo SEA | 6.154 | 7.500 | 7.333 | + improving |
| Tempo LV | 8.111 | 7.400 | 8.300 | + improving |

## Matchup Analogs

**SEA analogs vs LV profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | NYG (H) | 27-15 | SEA | 0.918 | 0.090 | 41.2% | 0.895 |
| 4 | DET (A) | 42-41 | SEA | 0.911 | 0.326 | 52.8% | 0.109 |
| 3 | ATL (H) | 23-24 | ATL | 0.862 | 0.146 | 55.2% | -0.125 |

**LV analogs vs SEA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | JAX (A) | 20-25 | JAX | 0.905 | 0.029 | 37.8% | -0.455 |
| 5 | KC (A) | 29-27 | LV | 0.842 | 0.198 | 43.0% | 0.222 |
| 8 | NO (A) | 0-27 | NO | 0.503 | -0.309 | 27.4% | -3.000 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 0.015 | -0.002 | 0.020 |
| SEA | 0.047 | 0.040 | 0.061 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 0.026 | 0.040 | 0.042 |
| SEA | 0.014 | -0.102 | -0.043 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 42.2% | 40.8% | 40.8% |
| SEA | 46.1% | 46.8% | 47.5% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 45.2% | 47.4% | 47.3% |
| SEA | 40.5% | 35.9% | 38.1% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 11.0% | 11.1% | 10.4% |
| SEA | 12.0% | 11.3% | 10.6% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 35.3% | 34.1% | 31.2% |
| SEA | 38.6% | 34.7% | 34.5% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | -0.193 | -0.309 | 0.006 |
| SEA | 0.171 | 0.909 | 0.687 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | -0.280 | -0.616 | -0.327 |
| SEA | 0.123 | 0.556 | 0.634 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | -0.100 | 0.200 | 0.333 |
| SEA | 0.400 | -0.200 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 11.0% | 9.7% | 5.1% |
| SEA | 9.2% | 12.3% | 15.2% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 3.6% | 2.8% | 4.0% |
| SEA | 7.2% | 9.2% | 8.8% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 8.131 | 8.067 | 8.000 |
| SEA | 7.527 | 6.955 | 6.996 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LV | 48.9% | 49.4% | 48.8% |
| SEA | 44.6% | 44.1% | 47.9% |
