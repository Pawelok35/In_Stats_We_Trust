# Matchup Report - CAR vs PIT

## Metric Comparison

| Metric | CAR | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.072 | -0.046 | -0.027 |
| Core EPA Defense | -0.036 | 0.028 | -0.064 |
| Success Rate Offense | 0.403 | 0.424 | -0.020 |
| Success Rate Defense | 0.422 | 0.443 | -0.021 |
| Explosive Play Rate (Off) | 0.091 | 0.078 | 0.013 |
| Third Down Conversion | 0.260 | 0.358 | -0.099 |
| Points per Drive Differential | -0.222 | -0.425 | 0.203 |
| Yards per Play Differential | -0.176 | -0.696 | 0.520 |
| Turnover Margin | 0.000 | 0.154 | -0.154 |
| Red Zone TD Rate (Off) | 0.130 | 0.137 | -0.007 |
| Pressure Rate (Def) | 0.061 | 0.071 | -0.010 |
| Tempo | 7.750 | 9.333 | -1.583 |
| PowerScore | 1.691 | 2.062 | -0.371 |
| PowerScore | 1.691 | 2.062 | -0.371 |
| Tempo | 7.750 | 9.333 | -1.583 |

## PowerScore Breakdown (Model)

| Component | Weight | CAR | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.052 | 0.171 | -0.119 |
| EPA Defense | 20% | 0.171 | 0.052 | +0.119 |
| Success Rate Offense | 25% | 35.5% | 50.0% | -14.5 pp |
| Tempo | 20% | 0.097 | 0.071 | +0.025 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.052 | 0.171 | -0.119 |
| EPA Defense | 20% | 0.171 | 0.052 | +0.119 |
| Success Rate Offense | 15% | 35.5% | 50.0% | -14.5 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.097 | 0.071 | +0.025 |
| Red Zone TD Rate (Off) | 10% | 8.3% | 17.6% | -9.3 pp |
| Pressure Rate (Def) | 10% | 4.2% | 14.8% | -10.6 pp |

## PowerScore Summary

**Model (4 metrics):**
PIT edge: +0.049 (CAR +0.160 vs PIT +0.209)
**7 metrics version:**
PIT edge: +0.045 (CAR +0.123 vs PIT +0.168)
**Verdict:** PIT holds the edge in the extended breakdown (lead 0.045), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - PIT) | -6.9 pts (favours PIT) |
| Model Win% CAR | 24.2% |
| Model Win% PIT | 75.8% |
| SOS Differential (CAR-PIT) | -0.065 |
*Neutral-field assumption. Based on schedule through Week 14.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 43.5% | 43.4% | 0.1% | 44.4% | 28.6% | Balanced |
| PIT | 28.6% | 43.4% | -14.9% | 50.0% | 50.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -9.1 pp | +1.7 pp | +1.3 pp |
| PIT | -0.7 pp | +9.6 pp | +11.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -2.2 pp | -12.6 pp | -4.1 pp |
| PIT | +1.5 pp | -0.2 pp | +3.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.5 pp | +3.9 pp | +2.3 pp |
| PIT | -1.5 pp | +0.6 pp | +2.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.9 pp | -0.4 pp | -0.2 pp |
| PIT | -2.8 pp | -3.6 pp | -3.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.3 pp | -3.2 pp | -2.6 pp |
| PIT | +0.6 pp | +3.9 pp | +5.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -8.7 pp | -3.5 pp | -1.0 pp |
| PIT | -1.7 pp | +0.4 pp | +8.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.9 pp | -0.4 pp | -0.2 pp |
| PIT | -2.8 pp | -3.6 pp | -3.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.5 pp | +3.9 pp | +2.3 pp |
| PIT | -1.5 pp | +0.6 pp | +2.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 46.0 | 45.6 | 44.5 |
| PIT | 45.9 | 47.4 | 47.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.7 | 46.6 | 45.4 |
| PIT | 44.7 | 45.9 | 45.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 0.3 | -0.9 | -1.0 |
| PIT | 1.3 | 1.5 | 2.4 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.60 | 1.79 | 1.59 |
| PIT | 1.56 | 2.04 | 2.02 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.82 | 1.85 | 1.40 |
| PIT | 1.98 | 1.83 | 1.77 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.22 | -0.06 | 0.19 |
| PIT | -0.42 | 0.21 | 0.26 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 6.63 | 6.97 | 7.04 |
| PIT | 7.77 | 8.41 | 8.50 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 40.1% | 34.2% | 35.1% |
| PIT | 45.0% | 40.7% | 40.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 59.9% | 65.8% | 64.9% |
| PIT | 55.0% | 59.3% | 59.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.66 | 2.38 | 2.48 |
| PIT | 3.49 | 3.42 | 3.44 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.98 | 4.59 | 4.57 |
| PIT | 4.28 | 4.99 | 5.07 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 14)

| Window | CAR | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.535 | 1.600 |
| Last 5 games | 1.489 | 1.580 |
| Last 3 games | 1.383 | 1.716 |

## Trend Summary (last 3 weeks)

| Metric | W11 | W12 | W14 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | -0.298 | 0.028 | 0.122 | + improving |
| Off EPA PIT | 0.018 | 0.097 | -0.043 | - declining |
| Def EPA CAR | -0.130 | -0.171 | -0.008 | - declining |
| Def EPA PIT | 0.185 | -0.037 | 0.041 | + improving |
| Off SR CAR | 36.1% | 45.1% | 50.5% | + improving |
| Off SR PIT | 43.9% | 48.8% | 46.4% | + improving |
| Def SR CAR | 43.7% | 35.9% | 40.8% | + improving |
| Def SR PIT | 51.8% | 46.2% | 42.5% | + improving |
| Tempo CAR | 6.545 | 6.308 | 8.273 | + improving |
| Tempo PIT | 7.538 | 8.400 | 7.667 | + improving |

## Matchup Analogs

**CAR analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | ARI (H) | 10-26 | ARI | 0.897 | -0.326 | 40.9% | -1.333 |
| 6 | LA (A) | 4-24 | LA | 0.849 | -0.281 | 31.7% | -1.818 |
| 7 | TB (H) | 21-3 | CAR | 0.799 | 0.116 | 39.4% | 1.827 |

**PIT analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | IND (A) | 24-17 | PIT | 0.888 | 0.097 | 48.8% | 0.700 |
| 4 | NYJ (H) | 20-28 | NYJ | 0.865 | -0.050 | 41.2% | -0.643 |
| 2 | NE (H) | 14-15 | NE | 0.823 | -0.050 | 39.7% | -0.144 |

## Core EPA Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.072 | -0.031 | -0.049 |
| PIT | -0.046 | 0.031 | 0.045 |

## Core EPA Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.036 | -0.045 | -0.103 |
| PIT | 0.028 | 0.003 | 0.005 |

## Success Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 40.7% | 42.4% | 44.5% |
| PIT | 42.3% | 46.4% | 47.9% |

## Success Rate Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 42.4% | 44.3% | 40.3% |
| PIT | 44.1% | 45.8% | 46.2% |

## Explosive Play Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.2% | 9.1% | 9.0% |
| PIT | 7.9% | 7.4% | 6.3% |

## Third Down Conversion Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 26.6% | 29.4% | 28.6% |
| PIT | 35.6% | 45.5% | 47.0% |

## Points per Drive Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.213 | -0.024 | 0.240 |
| PIT | -0.455 | 0.185 | 0.271 |

## Yards per Play Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.176 | -0.257 | -0.010 |
| PIT | -0.696 | 0.001 | 0.148 |

## Turnover Margin Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.000 | 0.200 | 1.000 |
| PIT | 0.154 | 0.800 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 13.0% | 10.3% | 7.4% |
| PIT | 13.7% | 13.5% | 14.2% |

## Pressure Rate (Def) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.1% | 8.7% | 7.9% |
| PIT | 7.1% | 8.7% | 8.4% |

## Tempo Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.633 | 6.971 | 7.042 |
| PIT | 7.768 | 8.410 | 8.504 |

## Pass Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 39.5% | 33.1% | 34.3% |
| PIT | 45.0% | 40.5% | 39.9% |
