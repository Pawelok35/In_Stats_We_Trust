# Matchup Report - JAX vs DAL

## Metric Comparison

| Metric | JAX | DAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.022 | 0.033 | -0.011 |
| Core EPA Defense | 0.010 | -0.103 | 0.114 |
| Success Rate Offense | 0.446 | 0.438 | 0.008 |
| Success Rate Defense | 0.433 | 0.385 | 0.047 |
| Explosive Play Rate (Off) | 0.108 | 0.112 | -0.005 |
| Third Down Conversion | 0.375 | 0.412 | -0.036 |
| Points per Drive Differential | -0.152 | 0.772 | -0.924 |
| Yards per Play Differential | -0.109 | 0.668 | -0.777 |
| Turnover Margin | 0.385 | 0.615 | -0.231 |
| Red Zone TD Rate (Off) | 0.130 | 0.160 | -0.029 |
| Pressure Rate (Def) | 0.050 | 0.106 | -0.055 |
| Tempo | 6.429 | 7.833 | -1.405 |
| PowerScore | 1.466 | 1.749 | -0.284 |
| PowerScore | 1.466 | 1.749 | -0.284 |
| Tempo | 6.429 | 7.833 | -1.405 |

## PowerScore Breakdown (Model)

| Component | Weight | JAX | DAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.103 | 0.068 | +0.034 |
| EPA Defense | 20% | 0.068 | 0.103 | -0.034 |
| Success Rate Offense | 25% | 52.2% | 55.3% | -3.1 pp |
| Tempo | 20% | 0.144 | 0.106 | +0.038 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | JAX | DAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.103 | 0.068 | +0.034 |
| EPA Defense | 20% | 0.068 | 0.103 | -0.034 |
| Success Rate Offense | 15% | 52.2% | 55.3% | -3.1 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.144 | 0.106 | +0.038 |
| Red Zone TD Rate (Off) | 10% | 18.8% | 23.5% | -4.8 pp |
| Pressure Rate (Def) | 10% | 9.1% | 2.3% | +6.8 pp |

## PowerScore Summary

**Model (4 metrics):**
JAX edge: +0.005 (JAX +0.209 vs DAL +0.204)
**7 metrics version:**
DAL edge: +0.197 (JAX +0.060 vs DAL +0.257)
**Verdict:** DAL holds the edge in the extended breakdown (lead 0.197), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (JAX - DAL) | -5.2 pts (favours DAL) |
| Model Win% JAX | 29.4% |
| Model Win% DAL | 70.6% |
| SOS Differential (JAX-DAL) | +0.131 |
*Neutral-field assumption. Based on schedule through Week 14.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| JAX | 47.8% | 43.4% | 4.3% | 53.5% | 51.1% | Pass heavy |
| DAL | 35.1% | 43.4% | -8.3% | 60.6% | 52.5% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +6.7 pp | +5.7 pp | -1.0 pp |
| DAL | -0.5 pp | -0.6 pp | -6.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +2.1 pp | +0.6 pp | -4.9 pp |
| DAL | -1.0 pp | +2.9 pp | +5.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +5.5 pp | +3.3 pp | -0.0 pp |
| DAL | +1.0 pp | +1.8 pp | +2.6 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.2 pp | +1.8 pp | +2.3 pp |
| DAL | +0.4 pp | +1.0 pp | +3.1 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +4.7 pp | +0.2 pp | -5.1 pp |
| DAL | +2.4 pp | +0.1 pp | -6.2 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +7.5 pp | +11.7 pp | +6.6 pp |
| DAL | +0.4 pp | -0.6 pp | -2.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.2 pp | +1.8 pp | +2.3 pp |
| DAL | +0.4 pp | +1.0 pp | +3.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +5.5 pp | +3.3 pp | -0.0 pp |
| DAL | +1.0 pp | +1.8 pp | +2.6 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 49.0 | 55.4 | 58.8 |
| DAL | 46.0 | 46.3 | 47.5 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 46.7 | 47.8 | 51.4 |
| DAL | 49.0 | 51.9 | 53.4 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.3 | 7.5 | 7.4 |
| DAL | -3.0 | -5.6 | -5.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.04 | 2.22 | 2.37 |
| DAL | 2.31 | 2.98 | 2.91 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.19 | 2.64 | 2.91 |
| DAL | 1.54 | 1.56 | 1.71 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -0.15 | -0.42 | -0.54 |
| DAL | 0.77 | 1.42 | 1.21 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 7.66 | 7.61 | 7.66 |
| DAL | 7.32 | 7.83 | 7.68 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 46.4% | 48.9% | 50.5% |
| DAL | 40.6% | 40.3% | 39.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 53.6% | 51.1% | 49.5% |
| DAL | 59.4% | 59.7% | 60.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.55 | 3.72 | 3.87 |
| DAL | 2.97 | 3.16 | 3.02 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 4.11 | 3.89 | 3.79 |
| DAL | 4.35 | 4.67 | 4.66 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 14)

| Window | JAX | DAL |
| --- | ---: | ---: |
| Season-to-date | 1.704 | 1.574 |
| Last 5 games | 1.715 | 1.489 |
| Last 3 games | 1.820 | 1.467 |

## Trend Summary (last 3 weeks)

| Metric | W12 | W13 | W14 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA JAX | 0.052 | -0.108 | 0.139 | + improving |
| Off EPA DAL | 0.072 | 0.163 | -0.009 | - declining |
| Def EPA JAX | 0.089 | 0.244 | -0.091 | + improving |
| Def EPA DAL | -0.026 | -0.259 | -0.034 | = stable |
| Off SR JAX | 40.7% | 34.2% | 47.8% | + improving |
| Off SR DAL | 42.9% | 45.3% | 44.3% | + improving |
| Def SR JAX | 45.5% | 54.9% | 47.6% | - declining |
| Def SR DAL | 40.0% | 36.7% | 39.3% | = stable |
| Tempo JAX | 7.364 | 8.111 | 7.500 | + improving |
| Tempo DAL | 9.100 | 6.615 | 7.333 | - declining |

## Matchup Analogs

**JAX analogs vs DAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | PHI (A) | 15-29 | PHI | 0.861 | -0.211 | 43.3% | -1.167 |
| 12 | BAL (H) | 28-27 | JAX | 0.751 | 0.052 | 40.7% | 0.091 |
| 1 | WAS (A) | 22-26 | WAS | 0.738 | 0.040 | 45.9% | -0.333 |

**DAL analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | DET (H) | 24-6 | DAL | 0.893 | -0.022 | 48.8% | 1.636 |
| 2 | CIN (H) | 20-19 | DAL | 0.834 | 0.025 | 40.3% | 0.118 |
| 3 | NYG (A) | 23-18 | DAL | 0.794 | 0.090 | 45.8% | 0.491 |

## Core EPA Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 0.033 | 0.111 | 0.075 |
| JAX | 0.022 | 0.023 | 0.028 |

## Core EPA Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | -0.103 | -0.103 | -0.106 |
| JAX | 0.010 | 0.073 | 0.081 |

## Success Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 43.9% | 46.1% | 44.2% |
| JAX | 44.8% | 43.9% | 41.4% |

## Success Rate Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 38.5% | 36.9% | 38.6% |
| JAX | 43.7% | 47.6% | 49.4% |

## Explosive Play Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 11.3% | 12.9% | 14.0% |
| JAX | 10.7% | 10.5% | 10.7% |

## Third Down Conversion Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 41.1% | 49.6% | 49.0% |
| JAX | 38.1% | 36.5% | 32.2% |

## Points per Drive Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 0.767 | 1.347 | 1.261 |
| JAX | -0.079 | -0.296 | -0.344 |

## Yards per Play Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 0.668 | 1.014 | 0.906 |
| JAX | -0.109 | -0.728 | -0.485 |

## Turnover Margin Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 0.615 | 0.400 | 1.000 |
| JAX | 0.385 | 1.400 | 1.333 |

## Red Zone TD Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 16.0% | 19.3% | 21.4% |
| JAX | 13.0% | 14.1% | 12.0% |

## Pressure Rate (Def) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 10.6% | 8.5% | 5.0% |
| JAX | 5.0% | 4.4% | 5.5% |

## Tempo Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 7.320 | 7.833 | 7.683 |
| JAX | 7.658 | 7.613 | 7.658 |

## Pass Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 40.8% | 40.5% | 39.2% |
| JAX | 46.4% | 48.8% | 50.4% |
