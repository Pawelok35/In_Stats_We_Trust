# Matchup Report - CLE vs CHI

## Metric Comparison

| Metric | CLE | CHI | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.097 | -0.077 | -0.020 |
| Core EPA Defense | -0.148 | -0.048 | -0.099 |
| Success Rate Offense | 0.394 | 0.403 | -0.009 |
| Success Rate Defense | 0.356 | 0.407 | -0.051 |
| Explosive Play Rate (Off) | 0.084 | 0.102 | -0.018 |
| Third Down Conversion | 0.293 | 0.404 | -0.111 |
| Points per Drive Differential | 0.184 | -0.037 | 0.221 |
| Yards per Play Differential | 0.306 | -0.054 | 0.360 |
| Turnover Margin | -0.385 | -0.077 | -0.308 |
| Red Zone TD Rate (Off) | 0.140 | 0.146 | -0.006 |
| Pressure Rate (Def) | 0.099 | 0.042 | 0.057 |
| Tempo | 5.294 | 5.938 | -0.643 |
| PowerScore | 1.009 | 1.114 | -0.105 |
| PowerScore | 1.009 | 1.114 | -0.105 |
| Tempo | 5.294 | 5.938 | -0.643 |

## PowerScore Breakdown (Model)

| Component | Weight | CLE | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.253 | -0.253 | +0.001 |
| EPA Defense | 20% | -0.253 | -0.253 | -0.001 |
| Success Rate Offense | 25% | 35.6% | 26.3% | +9.2 pp |
| Tempo | 20% | 0.067 | 0.074 | -0.007 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CLE | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.253 | -0.253 | +0.001 |
| EPA Defense | 20% | -0.253 | -0.253 | -0.001 |
| Success Rate Offense | 15% | 35.6% | 26.3% | +9.2 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.067 | 0.074 | -0.007 |
| Red Zone TD Rate (Off) | 10% | 5.6% | 10.0% | -4.4 pp |
| Pressure Rate (Def) | 10% | 7.0% | 8.2% | -1.2 pp |

## PowerScore Summary

**Model (4 metrics):**
CLE edge: +0.022 (CLE -0.037 vs CHI -0.059)
**7 metrics version:**
CHI edge: +0.392 (CLE -0.241 vs CHI +0.151)
**Verdict:** CHI holds the edge in the extended breakdown (lead 0.392), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CLE - CHI) | -2.0 pts (favours CHI) |
| Model Win% CLE | 41.9% |
| Model Win% CHI | 58.1% |
| SOS Differential (CLE-CHI) | -0.296 |
*Neutral-field assumption. Based on schedule through Week 14.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CLE | 54.4% | 44.7% | 9.7% | 38.8% | 31.7% | Pass heavy |
| CHI | 45.3% | 44.7% | 0.6% | 25.6% | 26.9% | Balanced |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -10.4 pp | -9.3 pp | -18.5 pp |
| CHI | +15.3 pp | +9.8 pp | +1.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.0 pp | -7.2 pp | -13.3 pp |
| CHI | +2.8 pp | -3.5 pp | -12.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.7 pp | +0.3 pp | +2.2 pp |
| CHI | +1.0 pp | +1.0 pp | -3.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | +0.3 pp | +1.0 pp | -0.6 pp |
| CHI | +2.2 pp | +1.4 pp | +1.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | +4.6 pp | +3.8 pp | +0.5 pp |
| CHI | +3.4 pp | -1.6 pp | +1.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -7.3 pp | -1.1 pp | -2.4 pp |
| CHI | +7.3 pp | +4.4 pp | -1.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | +0.3 pp | +1.0 pp | -0.6 pp |
| CHI | +2.2 pp | +1.4 pp | +1.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.7 pp | +0.3 pp | +2.2 pp |
| CHI | +1.0 pp | +1.0 pp | -3.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 42.8 | 45.5 | 43.7 |
| CHI | 47.7 | 43.7 | 45.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 47.9 | 49.8 | 49.8 |
| CHI | 51.4 | 52.6 | 52.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -5.1 | -4.3 | -6.1 |
| CHI | -3.7 | -8.9 | -6.8 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.64 | 1.59 | 1.47 |
| CHI | 1.91 | 1.89 | 1.97 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.46 | 1.90 | 2.15 |
| CHI | 1.95 | 1.49 | 1.58 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 0.18 | -0.31 | -0.68 |
| CHI | -0.04 | 0.40 | 0.39 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 7.15 | 7.42 | 6.91 |
| CHI | 7.65 | 7.84 | 7.89 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 43.7% | 48.8% | 51.5% |
| CHI | 41.9% | 40.7% | 42.2% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 56.3% | 51.2% | 48.5% |
| CHI | 58.1% | 59.3% | 57.8% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.12 | 3.62 | 3.56 |
| CHI | 3.20 | 3.20 | 3.33 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 4.02 | 3.80 | 3.35 |
| CHI | 4.44 | 4.65 | 4.56 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 14)

| Window | CLE | CHI |
| --- | ---: | ---: |
| Season-to-date | 1.237 | 1.533 |
| Last 5 games | 1.373 | 1.424 |
| Last 3 games | 1.392 | 1.409 |

## Trend Summary (last 3 weeks)

| Metric | W11 | W12 | W14 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CLE | -0.131 | -0.188 | -0.096 | + improving |
| Off EPA CHI | 0.036 | -0.172 | 0.017 | - declining |
| Def EPA CLE | -0.163 | 0.022 | -0.134 | - declining |
| Def EPA CHI | 0.016 | -0.312 | -0.200 | + improving |
| Off SR CLE | 39.1% | 34.8% | 41.0% | + improving |
| Off SR CHI | 45.7% | 36.1% | 33.3% | - declining |
| Def SR CLE | 30.7% | 43.7% | 34.3% | - declining |
| Def SR CHI | 50.6% | 32.8% | 34.9% | + improving |
| Tempo CLE | 7.667 | 7.417 | 5.882 | - declining |
| Tempo CHI | 8.364 | 8.300 | 7.000 | - declining |

## Matchup Analogs

**CLE analogs vs CHI profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | PIT (A) | 22-14 | CLE | 0.686 | -0.216 | 37.1% | 0.533 |
| 11 | PIT (H) | 13-9 | CLE | 0.686 | -0.131 | 39.1% | 0.314 |
| 4 | BAL (H) | 3-33 | BAL | 0.470 | -0.324 | 31.5% | -2.119 |

**CHI analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | NO (A) | 17-22 | NO | 0.969 | -0.213 | 37.8% | -0.455 |
| 6 | MIN (H) | 13-14 | MIN | 0.932 | -0.246 | 46.3% | -0.083 |
| 12 | MIN (A) | 12-9 | CHI | 0.932 | -0.172 | 36.1% | 0.291 |

## Core EPA Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.077 | -0.089 | -0.040 |
| CLE | -0.097 | -0.093 | -0.130 |

## Core EPA Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.048 | -0.158 | -0.165 |
| CLE | -0.148 | -0.041 | -0.015 |

## Success Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 40.5% | 39.1% | 38.6% |
| CLE | 39.4% | 40.1% | 38.5% |

## Success Rate Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 40.9% | 38.5% | 39.6% |
| CLE | 35.9% | 39.1% | 39.8% |

## Explosive Play Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 10.3% | 9.8% | 10.4% |
| CLE | 8.4% | 8.2% | 8.3% |

## Third Down Conversion Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 40.4% | 38.5% | 37.5% |
| CLE | 29.2% | 29.3% | 24.1% |

## Points per Drive Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.029 | 0.419 | 0.416 |
| CLE | 0.139 | -0.275 | -0.570 |

## Yards per Play Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.054 | 0.291 | 0.068 |
| CLE | 0.306 | -0.162 | -0.095 |

## Turnover Margin Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.077 | 0.600 | 2.667 |
| CLE | -0.385 | -0.200 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 14.6% | 9.1% | 6.4% |
| CLE | 14.0% | 10.0% | 10.4% |

## Pressure Rate (Def) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 4.2% | 5.7% | 7.1% |
| CLE | 9.9% | 6.7% | 4.2% |

## Tempo Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 7.647 | 7.842 | 7.888 |
| CLE | 7.146 | 7.422 | 6.905 |

## Pass Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 41.7% | 40.5% | 41.7% |
| CLE | 43.7% | 48.7% | 51.4% |
