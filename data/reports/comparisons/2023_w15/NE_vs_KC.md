# Matchup Report - NE vs KC

## Metric Comparison

| Metric | NE | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.167 | 0.025 | -0.193 |
| Core EPA Defense | -0.043 | -0.050 | 0.007 |
| Success Rate Offense | 0.378 | 0.441 | -0.063 |
| Success Rate Defense | 0.410 | 0.392 | 0.017 |
| Explosive Play Rate (Off) | 0.074 | 0.102 | -0.028 |
| Third Down Conversion | 0.303 | 0.425 | -0.121 |
| Points per Drive Differential | -0.478 | 0.490 | -0.968 |
| Yards per Play Differential | -0.017 | 0.582 | -0.599 |
| Turnover Margin | -0.692 | -0.231 | -0.462 |
| Red Zone TD Rate (Off) | 0.122 | 0.142 | -0.020 |
| Pressure Rate (Def) | 0.052 | 0.097 | -0.045 |
| Tempo | 5.071 | 5.786 | -0.714 |
| PowerScore | 0.964 | 1.144 | -0.180 |
| PowerScore | 0.964 | 1.144 | -0.180 |
| Tempo | 5.071 | 5.786 | -0.714 |

## PowerScore Breakdown (Model)

| Component | Weight | NE | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.331 | -0.113 | -0.218 |
| EPA Defense | 20% | -0.113 | -0.331 | +0.218 |
| Success Rate Offense | 25% | 35.2% | 37.0% | -1.8 pp |
| Tempo | 20% | 0.085 | 0.099 | -0.014 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | NE | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.331 | -0.113 | -0.218 |
| EPA Defense | 20% | -0.113 | -0.331 | +0.218 |
| Success Rate Offense | 15% | 35.2% | 37.0% | -1.8 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.085 | 0.099 | -0.014 |
| Red Zone TD Rate (Off) | 10% | 18.2% | 14.3% | +3.9 pp |
| Pressure Rate (Def) | 10% | 6.8% | 11.4% | -4.6 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.040 (NE -0.033 vs KC +0.007)
**7 metrics version:**
NE edge: +0.184 (NE +0.081 vs KC -0.103)
**Verdict:** NE holds the edge in the extended breakdown (lead 0.184), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (NE - KC) | -3.3 pts (favours KC) |
| Model Win% NE | 36.5% |
| Model Win% KC | 63.5% |
| SOS Differential (NE-KC) | -0.115 |
*Neutral-field assumption. Based on schedule through Week 14.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| NE | 49.3% | 44.7% | 4.6% | 34.3% | 36.1% | Pass heavy |
| KC | 54.3% | 44.7% | 9.6% | 40.9% | 32.4% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -4.9 pp | -7.8 pp | -18.2 pp |
| KC | +8.3 pp | +8.5 pp | +22.5 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -5.7 pp | -6.5 pp | +4.5 pp |
| KC | +3.4 pp | +7.1 pp | +3.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +2.7 pp | -1.0 pp | -4.7 pp |
| KC | +1.6 pp | +1.3 pp | +2.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -1.3 pp | -2.6 pp | -1.8 pp |
| KC | +2.5 pp | +2.5 pp | +3.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -4.1 pp | -2.2 pp | -1.3 pp |
| KC | +3.6 pp | +7.4 pp | +10.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +2.9 pp | -0.8 pp | -6.0 pp |
| KC | +1.6 pp | -0.2 pp | +6.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -1.3 pp | -2.6 pp | -1.8 pp |
| KC | +2.5 pp | +2.5 pp | +3.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +2.7 pp | -1.0 pp | -4.7 pp |
| KC | +1.6 pp | +1.3 pp | +2.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 44.2 | 35.2 | 33.5 |
| KC | 44.0 | 45.7 | 49.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 42.7 | 37.8 | 39.0 |
| KC | 49.0 | 47.2 | 50.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 1.5 | -2.6 | -5.5 |
| KC | -4.9 | -1.5 | -0.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 1.15 | 0.84 | 0.73 |
| KC | 2.15 | 1.97 | 2.39 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 1.62 | 1.07 | 0.89 |
| KC | 1.66 | 1.96 | 2.30 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -0.48 | -0.22 | -0.16 |
| KC | 0.49 | 0.01 | 0.09 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 6.99 | 6.87 | 6.48 |
| KC | 7.97 | 8.02 | 8.53 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 45.6% | 44.1% | 41.7% |
| KC | 49.6% | 48.2% | 50.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 54.4% | 55.9% | 58.3% |
| KC | 50.4% | 51.8% | 50.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 3.19 | 3.03 | 2.70 |
| KC | 3.96 | 3.86 | 4.26 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 3.80 | 3.83 | 3.78 |
| KC | 4.01 | 4.15 | 4.27 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 14)

| Window | NE | KC |
| --- | ---: | ---: |
| Season-to-date | 1.492 | 1.607 |
| Last 5 games | 1.390 | 1.623 |
| Last 3 games | 1.249 | 1.825 |

## Trend Summary (last 3 weeks)

| Metric | W12 | W13 | W14 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA NE | -0.189 | -0.267 | -0.138 | + improving |
| Off EPA KC | 0.132 | 0.134 | -0.102 | - declining |
| Def EPA NE | -0.191 | -0.156 | -0.173 | - declining |
| Def EPA KC | -0.100 | 0.233 | -0.027 | - declining |
| Off SR NE | 39.5% | 35.1% | 32.4% | - declining |
| Off SR KC | 48.1% | 48.1% | 45.0% | - declining |
| Def SR NE | 33.8% | 40.3% | 40.7% | - declining |
| Def SR KC | 31.0% | 47.5% | 39.8% | - declining |
| Tempo NE | 6.750 | 7.000 | 5.692 | - declining |
| Tempo KC | 7.700 | 9.875 | 8.000 | + improving |

## Matchup Analogs

**NE analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | DAL (A) | 3-26 | DAL | 0.933 | -0.468 | 25.7% | -2.091 |
| 7 | BUF (H) | 29-25 | NE | 0.923 | 0.077 | 48.7% | 0.400 |
| 2 | MIA (H) | 17-22 | MIA | 0.852 | -0.035 | 51.7% | -0.455 |

**KC analogs vs NE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | LV (A) | 31-17 | KC | 0.983 | 0.132 | 48.1% | 1.400 |
| 4 | NYJ (A) | 23-18 | KC | 0.916 | 0.075 | 46.4% | 0.455 |
| 3 | CHI (H) | 41-11 | KC | 0.759 | 0.171 | 51.1% | 2.727 |

## Core EPA Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.025 | 0.014 | 0.055 |
| NE | -0.167 | -0.132 | -0.198 |

## Core EPA Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | -0.050 | -0.007 | 0.035 |
| NE | -0.043 | -0.093 | -0.173 |

## Success Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 44.0% | 43.7% | 47.0% |
| NE | 38.0% | 38.8% | 35.8% |

## Success Rate Defense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 39.4% | 40.6% | 39.8% |
| NE | 41.1% | 40.4% | 38.5% |

## Explosive Play Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 10.3% | 9.5% | 10.2% |
| NE | 7.4% | 7.4% | 8.2% |

## Third Down Conversion Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 42.7% | 42.4% | 50.5% |
| NE | 30.9% | 28.5% | 26.2% |

## Points per Drive Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.501 | 0.035 | 0.172 |
| NE | -0.483 | -0.202 | -0.131 |

## Yards per Play Differential Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.582 | -0.033 | -0.022 |
| NE | -0.017 | 0.277 | 0.489 |

## Turnover Margin Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | -0.231 | -0.600 | -0.667 |
| NE | -0.692 | -0.600 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 14.2% | 17.3% | 13.2% |
| NE | 12.2% | 14.7% | 16.2% |

## Pressure Rate (Def) Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 9.7% | 7.9% | 4.8% |
| NE | 5.2% | 6.0% | 7.9% |

## Tempo Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 7.970 | 8.015 | 8.525 |
| NE | 6.989 | 6.865 | 6.481 |

## Pass Rate Offense Form (up to Week 14)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 49.4% | 48.0% | 50.0% |
| NE | 45.8% | 44.2% | 41.8% |
