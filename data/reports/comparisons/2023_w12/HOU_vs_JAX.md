# Matchup Report - HOU vs JAX

## Metric Comparison

| Metric | HOU | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.018 | -0.016 | 0.034 |
| Core EPA Defense | -0.023 | -0.037 | 0.015 |
| Success Rate Offense | 0.411 | 0.433 | -0.021 |
| Success Rate Defense | 0.415 | 0.404 | 0.011 |
| Explosive Play Rate (Off) | 0.126 | 0.088 | 0.038 |
| Third Down Conversion | 0.403 | 0.297 | 0.106 |
| Points per Drive Differential | 0.258 | 0.104 | 0.153 |
| Yards per Play Differential | 0.456 | -0.353 | 0.809 |
| Turnover Margin | 0.000 | 0.100 | -0.100 |
| Red Zone TD Rate (Off) | 0.116 | 0.115 | 0.001 |
| Pressure Rate (Def) | 0.069 | 0.052 | 0.017 |
| Tempo | 7.700 | 8.273 | -0.573 |
| PowerScore | 0.138 | 0.135 | 0.003 |
| PowerScore | 0.138 | 0.135 | 0.003 |
| Tempo | 7.700 | 8.273 | -0.573 |

## PowerScore Breakdown (Model)

| Component | Weight | HOU | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.029 | 0.055 | -0.084 |
| EPA Defense | 20% | 0.055 | -0.029 | +0.084 |
| Success Rate Offense | 25% | 41.6% | 41.8% | -0.2 pp |
| Tempo | 20% | 0.169 | 0.088 | +0.081 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | HOU | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.029 | 0.055 | -0.084 |
| EPA Defense | 20% | 0.055 | -0.029 | +0.084 |
| Success Rate Offense | 15% | 41.6% | 41.8% | -0.2 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.169 | 0.088 | +0.081 |
| Red Zone TD Rate (Off) | 10% | 27.3% | 15.4% | +11.9 pp |
| Pressure Rate (Def) | 10% | 0.0% | 10.0% | -10.0 pp |

## PowerScore Summary

**Model (4 metrics):**
HOU edge: +0.003 (HOU +0.138 vs JAX +0.135)
**7 metrics version:**
HOU edge: +0.205 (HOU +0.210 vs JAX +0.005)
**Verdict:** HOU holds the edge in the extended breakdown (lead 0.205), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (HOU - JAX) | +0.1 pts (favours HOU) |
| Model Win% HOU | 50.2% |
| Model Win% JAX | 49.8% |
| SOS Differential (HOU-JAX) | +0.145 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| HOU | 51.9% | 46.5% | 5.5% | 42.5% | 40.5% | Pass heavy |
| JAX | 44.0% | 46.5% | -2.5% | 52.5% | 33.3% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +8.7 pp | +12.9 pp | +14.3 pp |
| JAX | -8.2 pp | -5.0 pp | +1.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +0.1 pp | +6.5 pp | +7.0 pp |
| JAX | -2.1 pp | +2.2 pp | -9.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -0.2 pp | -0.7 pp | +1.3 pp |
| JAX | +0.4 pp | +2.2 pp | -0.3 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +3.0 pp | +5.9 pp | +8.3 pp |
| JAX | -1.1 pp | -0.9 pp | -2.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -1.9 pp | +0.9 pp | -4.8 pp |
| JAX | +2.7 pp | +0.7 pp | -1.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +3.4 pp | +3.5 pp | +7.3 pp |
| JAX | +0.3 pp | -0.3 pp | +4.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +3.0 pp | +5.9 pp | +8.3 pp |
| JAX | -1.1 pp | -0.9 pp | -2.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -0.2 pp | -0.7 pp | +1.3 pp |
| JAX | +0.4 pp | +2.2 pp | -0.3 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 44.9 | 45.1 | 47.0 |
| JAX | 46.1 | 47.4 | 44.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 47.4 | 44.7 | 48.3 |
| JAX | 45.3 | 47.5 | 46.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -2.6 | 0.3 | -1.3 |
| JAX | 0.8 | -0.1 | -1.8 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 2.10 | 2.07 | 2.41 |
| JAX | 1.81 | 2.02 | 1.84 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 1.84 | 1.86 | 2.20 |
| JAX | 1.71 | 1.73 | 1.81 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 0.26 | 0.21 | 0.20 |
| JAX | 0.10 | 0.28 | 0.03 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 7.52 | 7.13 | 7.09 |
| JAX | 7.32 | 7.10 | 7.81 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 45.2% | 45.0% | 49.6% |
| JAX | 44.3% | 43.4% | 43.7% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 54.8% | 55.0% | 50.4% |
| JAX | 55.7% | 56.6% | 56.3% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 3.40 | 3.21 | 3.51 |
| JAX | 3.24 | 3.08 | 3.41 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 4.12 | 3.92 | 3.57 |
| JAX | 4.07 | 4.02 | 4.40 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | HOU | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.613 | 1.468 |
| Last 5 games | 1.620 | 1.482 |
| Last 3 games | 1.489 | 1.447 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W10 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA HOU | -0.043 | 0.095 | -0.021 | + improving |
| Off EPA JAX | -0.019 | -0.289 | 0.312 | + improving |
| Def EPA HOU | -0.021 | 0.052 | -0.048 | + improving |
| Def EPA JAX | -0.190 | 0.232 | 0.045 | - declining |
| Off SR HOU | 39.4% | 43.6% | 44.3% | + improving |
| Off SR JAX | 49.4% | 30.1% | 48.8% | = stable |
| Def SR HOU | 41.0% | 41.0% | 39.4% | + improving |
| Def SR JAX | 37.6% | 46.7% | 38.3% | = stable |
| Tempo HOU | 7.889 | 7.231 | 7.182 | - declining |
| Tempo JAX | 7.727 | 7.300 | 8.400 | + improving |

## Matchup Analogs

**HOU analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | JAX (A) | 37-17 | HOU | 1.000 | 0.201 | 48.0% | 1.818 |
| 1 | BAL (A) | 9-27 | BAL | 0.670 | -0.231 | 30.9% | -1.523 |
| 6 | NO (H) | 20-16 | HOU | 0.639 | 0.025 | 43.6% | 0.367 |

**JAX analogs vs HOU profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | HOU (H) | 17-37 | HOU | 1.000 | -0.118 | 43.0% | -1.818 |
| 10 | SF (H) | 3-31 | SF | 0.772 | -0.289 | 30.1% | -2.791 |
| 5 | BUF (A) | 25-20 | JAX | 0.729 | 0.142 | 46.7% | 0.417 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 0.018 | 0.041 | 0.074 |
| JAX | -0.016 | 0.009 | 0.001 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | -0.023 | 0.019 | 0.058 |
| JAX | -0.037 | -0.032 | 0.029 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 40.9% | 42.3% | 42.7% |
| JAX | 43.6% | 42.7% | 43.4% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 41.7% | 42.0% | 41.1% |
| JAX | 40.0% | 39.3% | 40.9% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 12.7% | 15.3% | 17.9% |
| JAX | 8.9% | 9.1% | 9.9% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 39.8% | 37.0% | 37.7% |
| JAX | 30.3% | 26.6% | 30.3% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 0.223 | 0.213 | 0.196 |
| JAX | 0.177 | 0.349 | 0.067 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 0.456 | 0.740 | 1.266 |
| JAX | -0.353 | -0.265 | -0.232 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 0.000 | -1.000 | -1.333 |
| JAX | 0.100 | -0.200 | -1.667 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 11.6% | 15.4% | 15.9% |
| JAX | 11.5% | 12.0% | 5.3% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 6.9% | 8.8% | 8.1% |
| JAX | 5.2% | 5.8% | 7.4% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 7.518 | 7.130 | 7.086 |
| JAX | 7.318 | 7.099 | 7.809 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| HOU | 45.6% | 45.3% | 49.2% |
| JAX | 44.2% | 43.2% | 43.4% |
