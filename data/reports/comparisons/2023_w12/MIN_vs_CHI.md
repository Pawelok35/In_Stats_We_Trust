# Matchup Report - MIN vs CHI

## Metric Comparison

| Metric | MIN | CHI | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.312 | -0.172 | ↓ -0.139 |
| Core EPA Defense | -0.172 | -0.312 | ↑ +0.139 |
| Success Rate Offense | 32.8% | 36.1% | ↓ -3.3 pp |
| Success Rate Defense | 36.1% | 32.8% | ↑ +3.3 pp |
| Explosive Play Rate (Off) | 10.4% | 9.6% | ↑ +0.8 pp |
| Third Down Conversion | 18.2% | 44.4% | ↓ -26.3 pp |
| Points per Drive Differential | -0.291 | 0.291 | ↓ -0.582 |
| Yards per Play Differential | -0.207 | 0.207 | ↓ -0.415 |
| Turnover Margin | -2.000 | 2.000 | ↓ -4.000 |
| Red Zone TD Rate (Off) | 25.0% | 0.0% | ↑ +25.0 pp |
| Pressure Rate (Def) | 7.0% | 5.9% | ↑ +1.1 pp |
| Tempo | 6.091 | 8.300 | ↓ -2.209 |
| PowerScore | -0.041 | -0.013 | ↓ -0.028 |

## PowerScore Breakdown (Model)

| Component | Weight | MIN | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.312 | -0.172 | -0.139 |
| EPA Defense | 20% | -0.172 | -0.312 | +0.139 |
| Success Rate Offense | 25% | 32.8% | 36.1% | -3.3 pp |
| Tempo | 20% | 0.104 | 0.096 | +0.008 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | MIN | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.312 | -0.172 | -0.139 |
| EPA Defense | 20% | -0.172 | -0.312 | +0.139 |
| Success Rate Offense | 15% | 32.8% | 36.1% | -3.3 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.104 | 0.096 | +0.008 |
| Red Zone TD Rate (Off) | 10% | 25.0% | 0.0% | +25.0 pp |
| Pressure Rate (Def) | 10% | 7.0% | 5.9% | +1.1 pp |

## PowerScore Summary

**Model (4 metrics):**
CHI edge: +0.028 (MIN -0.041 vs CHI -0.013)
**7 metrics version:**
CHI edge: +0.385 (MIN -0.221 vs CHI +0.164)
**Verdict:** CHI holds the edge in the extended breakdown (lead 0.385), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (MIN - CHI) | -0.5 pts (favours CHI) |
| Model Win% MIN | 47.9% |
| Model Win% CHI | 52.1% |
| SOS Differential (MIN-CHI) | -0.003 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| MIN | 50.7% | 46.5% | 4.3% | 23.5% | 42.4% | Pass heavy |
| CHI | 51.8% | 46.5% | 5.3% | 30.2% | 42.5% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -3.0 pp | +6.3 pp | -7.0 pp |
| CHI | +3.3 pp | +9.3 pp | +6.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -1.5 pp | +0.1 pp | +1.9 pp |
| CHI | +2.7 pp | +1.1 pp | -4.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -2.3 pp | -2.2 pp | -1.9 pp |
| CHI | -1.8 pp | +0.8 pp | +0.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +3.0 pp | +5.7 pp | +5.4 pp |
| CHI | +1.9 pp | -0.2 pp | -0.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +4.7 pp | +3.6 pp | +1.9 pp |
| CHI | +2.2 pp | +1.0 pp | +3.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -1.2 pp | -0.2 pp | +1.9 pp |
| CHI | -0.8 pp | +2.8 pp | +9.8 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +3.0 pp | +5.7 pp | +5.4 pp |
| CHI | +1.9 pp | -0.2 pp | -0.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -2.3 pp | -2.2 pp | -1.9 pp |
| CHI | -1.8 pp | +0.8 pp | +0.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 45.6 | 44.1 | 47.3 |
| CHI | 48.3 | 46.4 | 43.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 50.1 | 54.3 | 54.6 |
| CHI | 51.8 | 52.8 | 54.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.5 | -10.2 | -7.3 |
| CHI | -3.4 | -6.3 | -11.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 2.08 | 2.30 | 2.21 |
| CHI | 1.94 | 1.90 | 1.97 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.85 | 1.65 | 1.79 |
| CHI | 2.11 | 1.93 | 1.78 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 0.22 | 0.64 | 0.42 |
| CHI | -0.18 | -0.04 | 0.19 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 7.95 | 8.34 | 7.88 |
| CHI | 7.65 | 8.01 | 7.97 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.8% | 44.5% | 40.6% |
| CHI | 40.6% | 38.7% | 35.2% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 51.2% | 55.5% | 59.4% |
| CHI | 59.4% | 61.3% | 64.8% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.88 | 3.71 | 3.20 |
| CHI | 3.10 | 3.10 | 2.80 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 4.07 | 4.63 | 4.68 |
| CHI | 4.54 | 4.91 | 5.17 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | MIN | CHI |
| --- | ---: | ---: |
| Season-to-date | 0.098 | 0.101 |
| Last 5 games | 0.092 | 0.073 |
| Last 3 games | 0.080 | 0.063 |

## Trend Summary (last 3 weeks)

| Metric | W9 | W10 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA MIN | -0.019 | 0.009 | -0.055 | - declining |
| Off EPA CHI | -0.213 | -0.114 | 0.036 | + improving |
| Def EPA MIN | -0.072 | -0.077 | -0.070 | = stable |
| Def EPA CHI | -0.153 | -0.142 | 0.016 | - declining |
| Off SR MIN | 40.2% | 44.6% | 43.8% | + improving |
| Off SR CHI | 37.8% | 41.6% | 45.7% | + improving |
| Def SR MIN | 34.8% | 33.7% | 39.4% | - declining |
| Def SR CHI | 43.6% | 30.4% | 50.6% | - declining |
| Tempo MIN | 7.077 | 7.667 | 8.900 | + improving |
| Tempo CHI | 7.455 | 8.091 | 8.364 | + improving |

## Matchup Analogs

**MIN analogs vs CHI profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | CHI (A) | 13-12 | MIN | 1.000 | -0.165 | 41.3% | 0.083 |
| 9 | ATL (A) | 31-26 | MIN | 0.772 | -0.019 | 40.2% | 0.385 |
| 4 | CAR (A) | 15-7 | MIN | 0.736 | -0.088 | 50.0% | 0.800 |

**CHI analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | MIN (H) | 13-14 | MIN | 1.000 | -0.246 | 46.3% | -0.083 |
| 3 | KC (A) | 10-37 | KC | 0.866 | -0.263 | 32.4% | -2.727 |
| 11 | DET (A) | 26-29 | DET | 0.855 | 0.036 | 45.7% | -0.273 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.077 | -0.078 | -0.097 |
| MIN | -0.020 | 0.009 | -0.022 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.011 | -0.074 | -0.093 |
| MIN | -0.051 | -0.076 | -0.073 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 41.6% | 41.2% | 41.8% |
| MIN | 43.0% | 42.3% | 42.9% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 42.0% | 41.2% | 41.5% |
| MIN | 41.3% | 39.0% | 35.8% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 10.1% | 9.0% | 8.7% |
| MIN | 10.8% | 11.3% | 11.0% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 40.3% | 43.1% | 37.0% |
| MIN | 39.6% | 46.4% | 38.7% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.173 | -0.056 | 0.191 |
| MIN | 0.238 | 0.636 | 0.439 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.153 | 0.083 | 0.154 |
| MIN | 0.462 | 0.464 | 0.304 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.545 | 0.000 | -0.667 |
| MIN | -0.364 | 0.200 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 16.3% | 14.8% | 11.9% |
| MIN | 14.9% | 12.2% | 12.8% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 3.4% | 2.9% | 4.1% |
| MIN | 7.3% | 5.3% | 4.8% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 7.646 | 8.015 | 7.970 |
| MIN | 7.954 | 8.340 | 7.881 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 40.4% | 38.4% | 35.0% |
| MIN | 48.9% | 44.4% | 40.7% |
