# Matchup Report - BAL vs CLE

## Metric Comparison

| Metric | BAL | CLE | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.100 | -0.227 | ↑ +0.327 |
| Core EPA Defense | -0.227 | 0.100 | ↓ -0.327 |
| Success Rate Offense | 43.6% | 43.5% | ↑ +0.1 pp |
| Success Rate Defense | 43.5% | 43.6% | ↓ -0.1 pp |
| Explosive Play Rate (Off) | 9.0% | 5.4% | ↑ +3.5 pp |
| Third Down Conversion | 23.1% | 36.8% | ↓ -13.8 pp |
| Points per Drive Differential | 1.276 | -1.276 | ↑ +2.551 |
| Yards per Play Differential | -0.397 | 0.397 | ↓ -0.795 |
| Turnover Margin | 2.000 | -2.000 | ↑ +4.000 |
| Red Zone TD Rate (Off) | 11.8% | 33.3% | ↓ -21.6 pp |
| Pressure Rate (Def) | 4.0% | 8.6% | ↓ -4.6 pp |
| Tempo | 6.000 | 7.667 | ↓ -1.667 |
| PowerScore | 0.116 | 0.060 | ↑ +0.056 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | CLE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.100 | -0.227 | +0.327 |
| EPA Defense | 20% | -0.227 | 0.100 | -0.327 |
| Success Rate Offense | 25% | 43.6% | 43.5% | +0.1 pp |
| Tempo | 20% | 0.090 | 0.054 | +0.035 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | CLE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.100 | -0.227 | +0.327 |
| EPA Defense | 20% | -0.227 | 0.100 | -0.327 |
| Success Rate Offense | 15% | 43.6% | 43.5% | +0.1 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.090 | 0.054 | +0.035 |
| Red Zone TD Rate (Off) | 10% | 11.8% | 33.3% | -21.6 pp |
| Pressure Rate (Def) | 10% | 4.0% | 8.6% | -4.6 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.056 (BAL +0.116 vs CLE +0.060)
**7 metrics version:**
BAL edge: +0.394 (BAL +0.270 vs CLE -0.124)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.394), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - CLE) | +1.0 pts (favours BAL) |
| Model Win% BAL | 54.3% |
| Model Win% CLE | 45.7% |
| SOS Differential (BAL-CLE) | +0.212 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 44.9% | 44.7% | 0.2% | 42.9% | 44.2% | Balanced |
| CLE | 54.3% | 44.7% | 9.7% | 32.0% | 57.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -3.8 pp | -3.8 pp | -3.8 pp |
| CLE | +8.5 pp | +8.5 pp | +8.5 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -6.7 pp | -6.7 pp | -6.7 pp |
| CLE | -3.9 pp | -3.9 pp | -3.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.0 pp | +2.0 pp | +2.0 pp |
| CLE | -2.4 pp | -2.4 pp | -2.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +21.1 pp | +21.1 pp | +21.1 pp |
| CLE | -5.6 pp | -5.6 pp | -5.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +17.1 pp | +17.1 pp | +17.1 pp |
| CLE | -24.2 pp | -24.2 pp | -24.2 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.5 pp | +1.5 pp | +1.5 pp |
| CLE | +5.5 pp | +5.5 pp | +5.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +21.1 pp | +21.1 pp | +21.1 pp |
| CLE | -5.6 pp | -5.6 pp | -5.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.0 pp | +2.0 pp | +2.0 pp |
| CLE | -2.4 pp | -2.4 pp | -2.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 51.6 | 51.6 | 51.6 |
| CLE | 41.7 | 41.7 | 41.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 53.8 | 53.8 | 53.8 |
| CLE | 51.7 | 51.7 | 51.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -2.2 | -2.2 | -2.2 |
| CLE | -10.0 | -10.0 | -10.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 3.64 | 3.64 | 3.64 |
| CLE | 1.60 | 1.60 | 1.60 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 3.15 | 3.15 | 3.15 |
| CLE | 1.70 | 1.70 | 1.70 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.48 | 0.48 | 0.48 |
| CLE | -0.10 | -0.10 | -0.10 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 6.45 | 6.45 | 6.45 |
| CLE | 9.10 | 9.10 | 9.10 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 29.6% | 29.6% | 29.6% |
| CLE | 51.6% | 51.6% | 51.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 70.4% | 70.4% | 70.4% |
| CLE | 48.4% | 48.4% | 48.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.91 | 1.91 | 1.91 |
| CLE | 4.70 | 4.70 | 4.70 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.55 | 4.55 | 4.55 |
| CLE | 4.40 | 4.40 | 4.40 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | BAL | CLE |
| --- | ---: | ---: |
| Season-to-date | 0.285 | 0.073 |
| Last 5 games | 0.285 | 0.073 |
| Last 3 games | 0.285 | 0.073 |

## Matchup Analogs

**BAL analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | BUF (A) | 40-35 | BAL | 0.139 | 0.340 | 56.3% | 0.483 |

**CLE analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CIN (H) | 16-17 | CIN | 0.127 | -0.087 | 39.6% | -0.100 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.340 | 0.340 | 0.340 |
| CLE | -0.087 | -0.087 | -0.087 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.202 | 0.202 | 0.202 |
| CLE | -0.066 | -0.066 | -0.066 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 56.3% | 56.3% | 56.3% |
| CLE | 39.6% | 39.6% | 39.6% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 48.6% | 48.6% | 48.6% |
| CLE | 44.1% | 44.1% | 44.1% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 22.5% | 22.5% | 22.5% |
| CLE | 6.6% | 6.6% | 6.6% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 46.2% | 46.2% | 46.2% |
| CLE | 47.4% | 47.4% | 47.4% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.483 | 0.483 | 0.483 |
| CLE | -0.100 | -0.100 | -0.100 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.440 | 1.440 | 1.440 |
| CLE | 1.520 | 1.520 | 1.520 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -1.000 | -1.000 | -1.000 |
| CLE | -2.000 | -2.000 | -2.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 10.0% | 10.0% | 10.0% |
| CLE | 13.3% | 13.3% | 13.3% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.9% | 1.9% | 1.9% |
| CLE | 11.5% | 11.5% | 11.5% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 6.455 | 6.455 | 6.455 |
| CLE | 9.100 | 9.100 | 9.100 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 29.6% | 29.6% | 29.6% |
| CLE | 51.6% | 51.6% | 51.6% |
