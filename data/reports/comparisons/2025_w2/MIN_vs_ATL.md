# Matchup Report - MIN vs ATL

## Metric Comparison

| Metric | MIN | ATL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.064 | 0.011 | -0.075 |
| Core EPA Defense | -0.109 | 0.075 | -0.184 |
| Success Rate Offense | 0.446 | 0.473 | -0.027 |
| Success Rate Defense | 0.374 | 0.413 | -0.040 |
| Explosive Play Rate (Off) | 0.095 | 0.055 | 0.040 |
| Third Down Conversion | 0.200 | 0.375 | -0.175 |
| Points per Drive Differential | 0.791 | -0.300 | 1.091 |
| Yards per Play Differential | -0.024 | 0.467 | -0.491 |
| Red Zone TD Rate (Off) | 0.167 | 0.067 | 0.100 |
| Pressure Rate (Def) | 0.054 | 0.029 | 0.025 |
| Tempo | 5.909 | 7.545 | -1.636 |
| PowerScore | 1.151 | 1.557 | -0.406 |
| PowerScore | 1.151 | 1.557 | -0.406 |
| Tempo | 5.909 | 7.545 | -1.636 |

## PowerScore Breakdown (Model)

| Component | Weight | MIN | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.369 | 0.012 | -0.381 |
| EPA Defense | 20% | 0.012 | -0.369 | +0.381 |
| Success Rate Offense | 25% | 38.5% | 47.0% | -8.5 pp |
| Tempo | 20% | 0.077 | 0.120 | -0.044 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | MIN | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.369 | 0.012 | -0.381 |
| EPA Defense | 20% | 0.012 | -0.369 | +0.381 |
| Success Rate Offense | 15% | 38.5% | 47.0% | -8.5 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.077 | 0.120 | -0.044 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 4.5% | -4.5 pp |
| Pressure Rate (Def) | 10% | 12.0% | 22.2% | -10.2 pp |

## PowerScore Summary

**Model (4 metrics):**
ATL edge: +0.087 (MIN -0.015 vs ATL +0.072)
**7 metrics version:**
ATL edge: +0.251 (MIN -0.112 vs ATL +0.138)
**Verdict:** ATL holds the edge in the extended breakdown (lead 0.251), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (MIN - ATL) | -7.5 pts (favours ATL) |
| Model Win% MIN | 22.3% |
| Model Win% ATL | 77.7% |
| SOS Differential (MIN-ATL) | -0.290 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| MIN | 41.5% | 44.7% | -3.1% | 33.3% | 42.1% | Run heavy |
| ATL | 30.1% | 44.7% | -14.5% | 32.0% | 53.4% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -30.0 pp | -30.0 pp | -30.0 pp |
| ATL | +16.1 pp | +16.1 pp | +16.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -3.3 pp | -3.3 pp | -3.3 pp |
| ATL | -18.3 pp | -18.3 pp | -18.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -9.1 pp | -9.1 pp | -9.1 pp |
| ATL | +3.1 pp | +3.1 pp | +3.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +1.5 pp | +1.5 pp | +1.5 pp |
| ATL | -5.5 pp | -5.5 pp | -5.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +0.6 pp | +0.6 pp | +0.6 pp |
| ATL | +9.5 pp | +9.5 pp | +9.5 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +4.7 pp | +4.7 pp | +4.7 pp |
| ATL | +9.5 pp | +9.5 pp | +9.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +1.5 pp | +1.5 pp | +1.5 pp |
| ATL | -5.5 pp | -5.5 pp | -5.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -9.1 pp | -9.1 pp | -9.1 pp |
| ATL | +3.1 pp | +3.1 pp | +3.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 45.3 | 45.3 | 45.3 |
| ATL | 46.2 | 46.2 | 46.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.5 | 48.5 | 48.5 |
| ATL | 49.2 | 49.2 | 49.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -3.2 | -3.2 | -3.2 |
| ATL | -3.0 | -3.0 | -3.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 2.08 | 2.08 | 2.08 |
| ATL | 2.00 | 2.00 | 2.00 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.29 | 1.29 | 1.29 |
| ATL | 2.30 | 2.30 | 2.30 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 0.79 | 0.79 | 0.79 |
| ATL | -0.30 | -0.30 | -0.30 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 5.69 | 5.69 | 5.69 |
| ATL | 9.10 | 9.10 | 9.10 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 33.8% | 33.8% | 33.8% |
| ATL | 48.4% | 48.4% | 48.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 66.2% | 66.2% | 66.2% |
| ATL | 51.6% | 51.6% | 51.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.92 | 1.92 | 1.92 |
| ATL | 4.40 | 4.40 | 4.40 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.77 | 3.77 | 3.77 |
| ATL | 4.70 | 4.70 | 4.70 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | MIN | ATL |
| --- | ---: | ---: |
| Season-to-date | 1.342 | 1.632 |
| Last 5 games | 1.342 | 1.632 |
| Last 3 games | 1.342 | 1.632 |

## Matchup Analogs

**MIN analogs vs ATL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CHI (A) | 27-17 | MIN | 0.370 | -0.064 | 44.6% | 0.791 |

**ATL analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | TB (H) | 20-23 | TB | 0.331 | 0.011 | 47.3% | -0.300 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.011 | 0.011 | 0.011 |
| MIN | -0.064 | -0.064 | -0.064 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.075 | 0.075 | 0.075 |
| MIN | -0.109 | -0.109 | -0.109 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 47.3% | 47.3% | 47.3% |
| MIN | 44.6% | 44.6% | 44.6% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 41.3% | 41.3% | 41.3% |
| MIN | 37.4% | 37.4% | 37.4% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 5.5% | 5.5% | 5.5% |
| MIN | 9.5% | 9.5% | 9.5% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 37.5% | 37.5% | 37.5% |
| MIN | 20.0% | 20.0% | 20.0% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.300 | -0.300 | -0.300 |
| MIN | 0.791 | 0.791 | 0.791 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.467 | 0.467 | 0.467 |
| MIN | -0.024 | -0.024 | -0.024 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.000 | 0.000 | 0.000 |
| MIN | 0.000 | 0.000 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 6.7% | 6.7% | 6.7% |
| MIN | 16.7% | 16.7% | 16.7% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 2.9% | 2.9% | 2.9% |
| MIN | 5.4% | 5.4% | 5.4% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 9.100 | 9.100 | 9.100 |
| MIN | 5.692 | 5.692 | 5.692 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 48.4% | 48.4% | 48.4% |
| MIN | 33.8% | 33.8% | 33.8% |
