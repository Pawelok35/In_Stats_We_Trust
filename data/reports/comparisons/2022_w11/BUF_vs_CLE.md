# Matchup Report - BUF vs CLE

## Metric Comparison

| Metric | BUF | CLE | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.086 | 0.040 | 0.046 |
| Core EPA Defense | -0.071 | 0.066 | -0.137 |
| Success Rate Offense | 0.493 | 0.448 | 0.045 |
| Success Rate Defense | 0.432 | 0.442 | -0.009 |
| Explosive Play Rate (Off) | 0.128 | 0.118 | 0.010 |
| Third Down Conversion | 0.470 | 0.397 | 0.073 |
| Points per Drive Differential | 0.854 | -0.177 | 1.030 |
| Yards per Play Differential | 1.271 | 0.114 | 1.158 |
| Turnover Margin | -0.333 | -0.333 | 0.000 |
| Red Zone TD Rate (Off) | 0.132 | 0.143 | -0.011 |
| Pressure Rate (Def) | 0.089 | 0.069 | 0.020 |
| Tempo | 7.364 | 8.636 | -1.273 |
| PowerScore | 1.682 | 1.890 | -0.208 |
| PowerScore | 1.682 | 1.890 | -0.208 |
| Tempo | 7.364 | 8.636 | -1.273 |

## PowerScore Breakdown (Model)

| Component | Weight | BUF | CLE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.191 | 0.017 | +0.174 |
| EPA Defense | 20% | 0.017 | 0.191 | -0.174 |
| Success Rate Offense | 25% | 55.6% | 47.4% | +8.2 pp |
| Tempo | 20% | 0.136 | 0.126 | +0.009 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BUF | CLE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.191 | 0.017 | +0.174 |
| EPA Defense | 20% | 0.017 | 0.191 | -0.174 |
| Success Rate Offense | 15% | 55.6% | 47.4% | +8.2 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.136 | 0.126 | +0.009 |
| Red Zone TD Rate (Off) | 10% | 10.0% | 12.5% | -2.5 pp |
| Pressure Rate (Def) | 10% | 2.3% | 6.7% | -4.3 pp |

## PowerScore Summary

**Model (4 metrics):**
BUF edge: +0.048 (BUF +0.236 vs CLE +0.188)
**7 metrics version:**
BUF edge: +0.215 (BUF +0.260 vs CLE +0.045)
**Verdict:** BUF holds the edge in the extended breakdown (lead 0.215), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BUF - CLE) | -3.8 pts (favours CLE) |
| Model Win% BUF | 34.5% |
| Model Win% CLE | 65.5% |
| SOS Differential (BUF-CLE) | +0.057 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BUF | 37.0% | 43.0% | -6.0% | 56.7% | 54.9% | Run heavy |
| CLE | 45.3% | 43.0% | 2.3% | 55.8% | 40.4% | Slight pass lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +11.5 pp | +5.2 pp | -1.4 pp |
| CLE | +1.9 pp | -0.7 pp | -1.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -3.7 pp | -3.5 pp | -6.7 pp |
| CLE | +2.5 pp | +9.5 pp | +7.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.8 pp | +1.8 pp | +1.2 pp |
| CLE | +2.9 pp | -0.8 pp | -3.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.1 pp | +2.1 pp | +1.0 pp |
| CLE | +2.3 pp | -0.3 pp | -2.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +5.4 pp | +7.8 pp | +6.6 pp |
| CLE | +3.5 pp | +1.9 pp | -2.6 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +6.2 pp | +0.4 pp | +1.2 pp |
| CLE | +1.1 pp | -0.8 pp | +1.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.1 pp | +2.1 pp | +1.0 pp |
| CLE | +2.3 pp | -0.3 pp | -2.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.8 pp | +1.8 pp | +1.2 pp |
| CLE | +2.9 pp | -0.8 pp | -3.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 44.9 | 46.3 | 49.0 |
| CLE | 50.0 | 51.2 | 50.0 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 51.0 | 49.0 | 51.3 |
| CLE | 49.4 | 52.6 | 54.4 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -6.1 | -2.8 | -2.3 |
| CLE | 0.7 | -1.4 | -4.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.45 | 2.37 | 2.24 |
| CLE | 2.23 | 2.17 | 2.27 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 1.60 | 1.66 | 2.01 |
| CLE | 2.40 | 2.63 | 2.57 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 0.85 | 0.71 | 0.23 |
| CLE | -0.18 | -0.46 | -0.31 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 7.78 | 7.26 | 7.49 |
| CLE | 8.13 | 7.97 | 8.14 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 50.3% | 48.2% | 45.8% |
| CLE | 41.1% | 44.1% | 40.5% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 49.7% | 51.8% | 54.2% |
| CLE | 58.9% | 55.9% | 59.5% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 3.91 | 3.50 | 3.43 |
| CLE | 3.34 | 3.52 | 3.30 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 3.87 | 3.76 | 4.06 |
| CLE | 4.79 | 4.46 | 4.84 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | BUF | CLE |
| --- | ---: | ---: |
| Season-to-date | 1.659 | 1.602 |
| Last 5 games | 1.718 | 1.657 |
| Last 3 games | 1.753 | 1.683 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W8 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BUF | 0.085 | 0.120 | -0.022 | - declining |
| Off EPA CLE | -0.186 | 0.088 | -0.020 | + improving |
| Def EPA BUF | 0.037 | -0.016 | 0.032 | = stable |
| Def EPA CLE | 0.132 | -0.185 | 0.265 | - declining |
| Off SR BUF | 46.0% | 51.4% | 55.7% | + improving |
| Off SR CLE | 32.2% | 51.2% | 44.4% | + improving |
| Def SR BUF | 42.7% | 44.3% | 43.6% | = stable |
| Def SR CLE | 38.7% | 41.8% | 56.6% | - declining |
| Tempo BUF | 8.700 | 7.200 | 6.929 | - declining |
| Tempo CLE | 7.250 | 7.818 | 9.000 | + improving |

## Matchup Analogs

**BUF analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | MIA (A) | 17-24 | MIA | 0.944 | 0.042 | 48.1% | -0.736 |
| 6 | KC (A) | 24-20 | BUF | 0.867 | 0.085 | 46.0% | 0.400 |
| 4 | BAL (A) | 23-18 | BUF | 0.846 | 0.016 | 50.0% | 0.482 |

**CLE analogs vs BUF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | BAL (A) | 20-21 | BAL | 0.837 | 0.015 | 44.7% | -0.091 |
| 8 | CIN (H) | 32-14 | CLE | 0.823 | 0.088 | 51.2% | 1.609 |
| 10 | MIA (A) | 17-39 | MIA | 0.694 | -0.020 | 44.4% | -2.444 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.086 | 0.077 | 0.034 |
| CLE | 0.040 | 0.004 | 0.028 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | -0.071 | -0.038 | 0.014 |
| CLE | 0.066 | 0.087 | 0.036 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 49.4% | 49.4% | 51.6% |
| CLE | 44.7% | 44.2% | 46.9% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 43.1% | 44.4% | 47.5% |
| CLE | 44.2% | 45.0% | 47.8% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 12.8% | 14.6% | 13.9% |
| CLE | 11.7% | 11.4% | 10.7% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 47.0% | 44.4% | 44.0% |
| CLE | 39.8% | 36.6% | 40.9% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.951 | 0.818 | 0.246 |
| CLE | -0.147 | -0.424 | -0.174 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 1.271 | 0.893 | 0.143 |
| CLE | 0.114 | 0.063 | 0.240 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | -0.333 | -0.600 | -1.333 |
| CLE | -0.333 | -0.800 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 13.2% | 12.6% | 13.2% |
| CLE | 14.3% | 16.3% | 16.3% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 8.9% | 7.0% | 6.9% |
| CLE | 6.9% | 7.6% | 10.1% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.783 | 7.261 | 7.487 |
| CLE | 8.126 | 7.974 | 8.139 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 50.7% | 48.1% | 45.9% |
| CLE | 41.0% | 44.2% | 40.3% |
