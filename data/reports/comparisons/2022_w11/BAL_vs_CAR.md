# Matchup Report - BAL vs CAR

## Metric Comparison

| Metric | BAL | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.130 | -0.298 | ↑ +0.168 |
| Core EPA Defense | -0.298 | -0.130 | ↓ -0.168 |
| Success Rate Offense | 43.7% | 36.1% | ↑ +7.6 pp |
| Success Rate Defense | 36.1% | 43.7% | ↓ -7.6 pp |
| Explosive Play Rate (Off) | 8.0% | 6.9% | ↑ +1.1 pp |
| Third Down Conversion | 33.3% | 20.0% | ↑ +13.3 pp |
| Points per Drive Differential | 0.811 | -0.811 | ↑ +1.621 |
| Yards per Play Differential | 0.693 | -0.693 | ↑ +1.386 |
| Turnover Margin | 2.000 | -2.000 | ↑ +4.000 |
| Red Zone TD Rate (Off) | 11.1% | 0.0% | ↑ +11.1 pp |
| Pressure Rate (Def) | 10.8% | 8.1% | ↑ +2.7 pp |
| Tempo | 7.250 | 6.545 | ↑ +0.705 |
| PowerScore | 0.020 | -0.026 | ↑ +0.046 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.130 | -0.298 | +0.168 |
| EPA Defense | 20% | -0.298 | -0.130 | -0.168 |
| Success Rate Offense | 25% | 43.7% | 36.1% | +7.6 pp |
| Tempo | 20% | 0.080 | 0.069 | +0.011 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.130 | -0.298 | +0.168 |
| EPA Defense | 20% | -0.298 | -0.130 | -0.168 |
| Success Rate Offense | 15% | 43.7% | 36.1% | +7.6 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.080 | 0.069 | +0.011 |
| Red Zone TD Rate (Off) | 10% | 11.1% | 0.0% | +11.1 pp |
| Pressure Rate (Def) | 10% | 10.8% | 8.1% | +2.7 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.046 (BAL +0.020 vs CAR -0.026)
**7 metrics version:**
BAL edge: +0.435 (BAL +0.203 vs CAR -0.231)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.435), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - CAR) | +0.9 pts (favours BAL) |
| Model Win% BAL | 53.6% |
| Model Win% CAR | 46.4% |
| SOS Differential (BAL-CAR) | +0.043 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 42.5% | 43.0% | -0.5% | 43.2% | 44.0% | Balanced |
| CAR | 51.4% | 43.0% | 8.4% | 29.7% | 42.9% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +4.6 pp | +10.3 pp | +8.5 pp |
| CAR | -7.4 pp | -1.5 pp | +5.5 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +0.6 pp | -0.3 pp | -0.4 pp |
| CAR | +2.2 pp | +7.5 pp | +7.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.8 pp | -2.3 pp | -2.5 pp |
| CAR | +1.1 pp | +6.4 pp | +8.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.3 pp | +1.7 pp | -0.2 pp |
| CAR | -0.6 pp | +2.2 pp | +1.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +5.7 pp | +7.9 pp | +4.8 pp |
| CAR | +0.8 pp | -3.4 pp | +1.6 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.0 pp | -6.3 pp | -7.2 pp |
| CAR | -11.5 pp | -8.7 pp | -8.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.3 pp | +1.7 pp | -0.2 pp |
| CAR | -0.6 pp | +2.2 pp | +1.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.8 pp | -2.3 pp | -2.5 pp |
| CAR | +1.1 pp | +6.4 pp | +8.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 48.0 | 47.8 | 49.7 |
| CAR | 46.5 | 44.9 | 48.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 49.6 | 50.2 | 48.1 |
| CAR | 45.8 | 44.6 | 51.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.6 | -2.5 | 1.6 |
| CAR | 0.7 | 0.3 | -2.4 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.39 | 2.26 | 2.45 |
| CAR | 1.61 | 1.85 | 2.27 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.11 | 1.96 | 1.83 |
| CAR | 1.95 | 1.95 | 2.43 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.28 | 0.30 | 0.61 |
| CAR | -0.35 | -0.10 | -0.16 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 7.42 | 7.99 | 8.07 |
| CAR | 6.51 | 6.68 | 7.11 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 39.7% | 37.7% | 33.7% |
| CAR | 41.5% | 35.5% | 34.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 60.3% | 62.3% | 66.3% |
| CAR | 58.5% | 64.5% | 65.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.95 | 3.01 | 2.72 |
| CAR | 2.71 | 2.37 | 2.44 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.48 | 4.98 | 5.35 |
| CAR | 3.81 | 4.31 | 4.68 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | BAL | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.151 | 0.108 |
| Last 5 games | 0.137 | 0.122 |
| Last 3 games | 0.137 | 0.152 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BAL | 0.029 | 0.158 | 0.121 | + improving |
| Off EPA CAR | 0.116 | 0.004 | -0.015 | - declining |
| Def EPA BAL | 0.015 | 0.080 | -0.150 | + improving |
| Def EPA CAR | -0.140 | 0.001 | 0.224 | - declining |
| Off SR BAL | 43.8% | 51.1% | 48.8% | + improving |
| Off SR CAR | 39.4% | 40.4% | 37.9% | - declining |
| Def SR BAL | 44.7% | 43.0% | 38.6% | + improving |
| Def SR CAR | 37.3% | 44.0% | 58.1% | - declining |
| Tempo BAL | 7.273 | 7.833 | 9.111 | + improving |
| Tempo CAR | 6.600 | 7.615 | 6.000 | - declining |

## Matchup Analogs

**BAL analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | NYJ (A) | 24-10 | BAL | 0.812 | 0.016 | 40.8% | 1.096 |
| 8 | TB (A) | 27-26 | BAL | 0.656 | 0.158 | 51.1% | 0.050 |
| 3 | NE (A) | 37-28 | BAL | 0.639 | 0.170 | 45.5% | 0.750 |

**CAR analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CLE (H) | 24-28 | CLE | 0.846 | 0.023 | 36.5% | -0.364 |
| 2 | NYG (A) | 16-17 | NYG | 0.836 | -0.101 | 46.7% | -0.129 |
| 8 | ATL (A) | 34-29 | CAR | 0.721 | 0.004 | 40.4% | 0.401 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.087 | 0.068 | 0.103 |
| CAR | -0.079 | -0.034 | -0.001 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.010 | -0.007 | -0.018 |
| CAR | -0.016 | -0.015 | 0.029 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 45.7% | 46.6% | 48.0% |
| CAR | 39.4% | 38.3% | 39.6% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 44.0% | 43.7% | 42.2% |
| CAR | 43.0% | 44.7% | 47.7% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 13.2% | 13.3% | 12.1% |
| CAR | 9.3% | 10.6% | 10.4% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 41.8% | 45.0% | 48.6% |
| CAR | 25.9% | 30.2% | 31.8% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.277 | 0.260 | 0.528 |
| CAR | -0.349 | -0.112 | -0.130 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.135 | 0.511 | -0.022 |
| CAR | -0.226 | -0.101 | -0.289 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.556 | 0.000 | 0.333 |
| CAR | -0.300 | -0.200 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 13.1% | 11.4% | 13.1% |
| CAR | 14.7% | 16.9% | 14.8% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.9% | 10.7% | 11.3% |
| CAR | 5.5% | 6.2% | 8.8% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.424 | 7.987 | 8.072 |
| CAR | 6.511 | 6.679 | 7.114 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 39.6% | 37.9% | 34.4% |
| CAR | 41.2% | 34.8% | 33.6% |
