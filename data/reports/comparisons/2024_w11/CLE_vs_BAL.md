# Matchup Report - CLE vs BAL

## Metric Comparison

| Metric | CLE | BAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.128 | 0.164 | -0.292 |
| Core EPA Defense | -0.040 | 0.066 | -0.106 |
| Success Rate Offense | 0.407 | 0.525 | -0.118 |
| Success Rate Defense | 0.417 | 0.468 | -0.051 |
| Explosive Play Rate (Off) | 0.070 | 0.141 | -0.071 |
| Third Down Conversion | 0.250 | 0.423 | -0.173 |
| Points per Drive Differential | -0.616 | 0.638 | -1.254 |
| Yards per Play Differential | -0.894 | 1.025 | -1.919 |
| Turnover Margin | -0.778 | 0.400 | -1.178 |
| Red Zone TD Rate (Off) | 0.105 | 0.223 | -0.117 |
| Pressure Rate (Def) | 0.088 | 0.075 | 0.014 |
| Tempo | 8.273 | 6.077 | 2.196 |
| PowerScore | 1.770 | 1.242 | 0.529 |
| PowerScore | 1.770 | 1.242 | 0.529 |
| Tempo | 8.273 | 6.077 | 2.196 |

## PowerScore Breakdown (Model)

| Component | Weight | CLE | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.073 | -0.165 | +0.092 |
| EPA Defense | 20% | 0.213 | -0.102 | +0.315 |
| Success Rate Offense | 25% | 39.6% | 41.8% | -2.2 pp |
| Tempo | 20% | 0.099 | 0.114 | -0.015 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CLE | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.073 | -0.165 | +0.092 |
| EPA Defense | 20% | 0.213 | -0.102 | +0.315 |
| Success Rate Offense | 15% | 39.6% | 41.8% | -2.2 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.099 | 0.114 | -0.015 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 40.0% | -40.0 pp |
| Pressure Rate (Def) | 10% | 3.1% | 9.5% | -6.4 pp |

## PowerScore Summary

**Model (4 metrics):**
CLE edge: +0.087 (CLE +0.136 vs BAL +0.049)
**7 metrics version:**
CLE edge: +0.435 (CLE +0.297 vs BAL -0.138)
**Verdict:** CLE holds the edge in the extended breakdown (lead 0.435), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CLE - BAL) | +9.8 pts (favours CLE) |
| Model Win% CLE | 83.6% |
| Model Win% BAL | 16.4% |
| SOS Differential (CLE-BAL) | -0.349 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CLE | 57.1% | 45.2% | 11.9% | 28.6% | 50.0% | Pass heavy |
| BAL | 46.8% | 45.2% | 1.6% | 46.9% | 54.8% | Slight pass lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -15.6 pp | -15.3 pp | -5.2 pp |
| BAL | +13.7 pp | +10.5 pp | +14.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.4 pp | -2.8 pp | +0.6 pp |
| BAL | +7.5 pp | +4.0 pp | -0.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.0 pp | -4.9 pp | -1.1 pp |
| BAL | +4.6 pp | +4.0 pp | +7.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.5 pp | -2.9 pp | -2.5 pp |
| BAL | +2.6 pp | +1.2 pp | -0.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.6 pp | -6.2 pp | -4.8 pp |
| BAL | +7.3 pp | +7.0 pp | +10.6 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -10.3 pp | -8.8 pp | -2.4 pp |
| BAL | +17.0 pp | +22.3 pp | +17.8 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.5 pp | -2.9 pp | -2.5 pp |
| BAL | +2.6 pp | +1.2 pp | -0.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.0 pp | -4.9 pp | -1.1 pp |
| BAL | +4.6 pp | +4.0 pp | +7.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 47.6 | 46.7 | 43.2 |
| BAL | 47.1 | 48.1 | 48.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 43.3 | 44.8 | 44.1 |
| BAL | 51.5 | 53.2 | 49.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 4.2 | 1.9 | -0.9 |
| BAL | -4.4 | -5.1 | -0.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.30 | 1.25 | 1.40 |
| BAL | 2.88 | 3.25 | 3.07 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.92 | 2.19 | 1.84 |
| BAL | 2.25 | 2.33 | 2.17 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -0.62 | -0.94 | -0.44 |
| BAL | 0.64 | 0.92 | 0.90 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 7.35 | 7.24 | 7.18 |
| BAL | 7.61 | 7.68 | 7.38 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 49.0% | 48.5% | 54.8% |
| BAL | 36.3% | 36.8% | 38.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 51.0% | 51.5% | 45.2% |
| BAL | 63.7% | 63.2% | 61.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.60 | 3.52 | 3.94 |
| BAL | 2.76 | 2.83 | 2.87 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.75 | 3.73 | 3.24 |
| BAL | 4.85 | 4.85 | 4.50 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | CLE | BAL |
| --- | ---: | ---: |
| Season-to-date | 1.383 | 1.732 |
| Last 5 games | 1.400 | 1.852 |
| Last 3 games | 1.287 | 1.808 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CLE | -0.187 | 0.117 | -0.266 | - declining |
| Off EPA BAL | 0.293 | 0.054 | 0.333 | + improving |
| Def EPA CLE | -0.102 | 0.054 | -0.063 | - declining |
| Def EPA BAL | 0.088 | 0.117 | -0.118 | + improving |
| Off SR CLE | 35.7% | 51.2% | 42.7% | + improving |
| Off SR BAL | 52.6% | 48.9% | 58.9% | + improving |
| Def SR CLE | 36.5% | 48.9% | 32.9% | + improving |
| Def SR BAL | 52.0% | 51.2% | 36.0% | + improving |
| Tempo CLE | 7.000 | 7.167 | 7.385 | + improving |
| Tempo BAL | 6.909 | 8.000 | 7.300 | + improving |

## Matchup Analogs

**CLE analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | BAL (A) | 16-12 | CLE | 1.000 | 0.048 | 44.3% | 0.478 |
| 4 | DET (A) | 10-18 | DET | 0.749 | -0.114 | 39.7% | -0.818 |
| 3 | GB (H) | 15-20 | GB | 0.576 | -0.252 | 43.9% | -0.329 |

**BAL analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | CLE (H) | 23-26 | CLE | 1.000 | -0.034 | 44.9% | -0.250 |
| 8 | CHI (H) | 24-27 | CHI | 0.959 | 0.054 | 48.9% | -0.235 |
| 5 | HOU (H) | 41-36 | BAL | 0.837 | 0.252 | 58.9% | 0.385 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.164 | 0.213 | 0.197 |
| CLE | -0.128 | -0.119 | -0.112 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.066 | 0.080 | 0.055 |
| CLE | -0.040 | -0.015 | -0.037 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 52.4% | 52.9% | 52.7% |
| CLE | 40.4% | 41.5% | 42.9% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 46.9% | 48.7% | 46.6% |
| CLE | 41.8% | 40.8% | 39.9% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 14.0% | 14.4% | 12.3% |
| CLE | 7.0% | 7.9% | 8.2% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 42.3% | 38.9% | 35.5% |
| CLE | 25.1% | 29.8% | 40.1% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.600 | 0.866 | 0.798 |
| CLE | -0.629 | -0.934 | -0.458 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.025 | 1.006 | 0.441 |
| CLE | -0.894 | -1.011 | -0.253 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.400 | 0.600 | 1.000 |
| CLE | -0.778 | -1.000 | -2.000 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 22.3% | 19.4% | 20.3% |
| CLE | 10.5% | 5.8% | 7.4% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.5% | 7.1% | 6.6% |
| CLE | 8.8% | 9.4% | 11.4% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.608 | 7.675 | 7.378 |
| CLE | 7.349 | 7.242 | 7.184 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 36.9% | 37.2% | 39.5% |
| CLE | 49.4% | 49.2% | 55.0% |
