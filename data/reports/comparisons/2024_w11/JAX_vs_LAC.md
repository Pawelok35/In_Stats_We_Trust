# Matchup Report - JAX vs LAC

## Metric Comparison

| Metric | JAX | LAC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.015 | -0.005 | 0.020 |
| Core EPA Defense | 0.103 | -0.127 | 0.231 |
| Success Rate Offense | 0.463 | 0.420 | 0.042 |
| Success Rate Defense | 0.471 | 0.439 | 0.032 |
| Explosive Play Rate (Off) | 0.107 | 0.104 | 0.003 |
| Third Down Conversion | 0.316 | 0.373 | -0.057 |
| Points per Drive Differential | -0.738 | 0.638 | -1.376 |
| Yards per Play Differential | -0.373 | 0.142 | -0.514 |
| Turnover Margin | -0.700 | 1.111 | -1.811 |
| Red Zone TD Rate (Off) | 0.155 | 0.101 | 0.054 |
| Pressure Rate (Def) | 0.055 | 0.092 | -0.038 |
| Tempo | 7.444 | 6.692 | 0.752 |
| PowerScore | 1.617 | 1.475 | 0.142 |
| PowerScore | 1.617 | 1.475 | 0.142 |
| Tempo | 7.444 | 6.692 | 0.752 |

## PowerScore Breakdown (Model)

| Component | Weight | JAX | LAC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.193 | 0.041 | -0.235 |
| EPA Defense | 20% | 0.439 | 0.009 | +0.430 |
| Success Rate Offense | 25% | 43.3% | 48.3% | -5.0 pp |
| Tempo | 20% | 0.045 | 0.161 | -0.116 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | JAX | LAC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.193 | 0.041 | -0.235 |
| EPA Defense | 20% | 0.439 | 0.009 | +0.430 |
| Success Rate Offense | 15% | 43.3% | 48.3% | -5.0 pp |
| Turnover Margin | 10% | -1.000 | -1.000 | +0.000 |
| Tempo | 10% | 0.045 | 0.161 | -0.116 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 6.7% | -6.7 pp |
| Pressure Rate (Def) | 10% | 0.0% | 5.7% | -5.7 pp |

## PowerScore Summary

**Model (4 metrics):**
LAC edge: +0.032 (JAX +0.137 vs LAC +0.169)
**7 metrics version:**
LAC edge: +0.004 (JAX +0.009 vs LAC +0.013)
**Verdict:** LAC holds the edge in the extended breakdown (lead 0.004), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (JAX - LAC) | +2.6 pts (favours JAX) |
| Model Win% JAX | 60.8% |
| Model Win% LAC | 39.2% |
| SOS Differential (JAX-LAC) | +0.287 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| JAX | 44.8% | 45.2% | -0.5% | 37.7% | 46.0% | Balanced |
| LAC | 43.7% | 45.2% | -1.5% | 74.3% | 61.4% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +3.0 pp | +14.3 pp | +3.4 pp |
| LAC | -1.9 pp | +3.7 pp | -3.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +4.7 pp | +1.6 pp | +16.8 pp |
| LAC | -3.1 pp | -3.4 pp | +7.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +2.3 pp | +4.2 pp | +8.3 pp |
| LAC | -3.4 pp | -3.2 pp | -5.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +0.2 pp | -0.6 pp | +2.4 pp |
| LAC | -0.8 pp | +1.1 pp | +2.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.0 pp | -4.8 pp | -1.9 pp |
| LAC | -5.1 pp | -1.5 pp | -3.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +5.3 pp | +8.2 pp | +5.9 pp |
| LAC | -5.7 pp | -4.9 pp | -6.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +0.2 pp | -0.6 pp | +2.4 pp |
| LAC | -0.8 pp | +1.1 pp | +2.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +2.3 pp | +4.2 pp | +8.3 pp |
| LAC | -3.4 pp | -3.2 pp | -5.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 48.3 | 48.2 | 46.6 |
| LAC | 39.8 | 37.4 | 36.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 44.4 | 43.2 | 41.8 |
| LAC | 45.1 | 47.6 | 49.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.9 | 5.0 | 4.8 |
| LAC | -5.3 | -10.2 | -13.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 1.73 | 1.74 | 1.40 |
| LAC | 1.86 | 2.18 | 2.31 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.47 | 2.30 | 1.85 |
| LAC | 1.22 | 1.28 | 1.04 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -0.74 | -0.56 | -0.45 |
| LAC | 0.64 | 0.89 | 1.27 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 6.87 | 6.60 | 5.69 |
| LAC | 7.21 | 7.70 | 6.67 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 45.0% | 44.8% | 47.2% |
| LAC | 39.9% | 42.4% | 38.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 55.0% | 55.2% | 52.8% |
| LAC | 60.1% | 57.6% | 61.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.09 | 2.96 | 2.69 |
| LAC | 2.88 | 3.26 | 2.58 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.78 | 3.64 | 3.01 |
| LAC | 4.34 | 4.44 | 4.08 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | JAX | LAC |
| --- | ---: | ---: |
| Season-to-date | 1.775 | 1.488 |
| Last 5 games | 1.740 | 1.596 |
| Last 3 games | 1.665 | 1.592 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA JAX | 0.061 | -0.069 | -0.190 | - declining |
| Off EPA LAC | 0.041 | -0.063 | 0.190 | + improving |
| Def EPA JAX | 0.131 | 0.011 | -0.009 | + improving |
| Def EPA LAC | -0.203 | -0.266 | 0.041 | - declining |
| Off SR JAX | 50.0% | 41.8% | 41.8% | - declining |
| Off SR LAC | 38.1% | 32.9% | 52.1% | + improving |
| Def SR JAX | 48.9% | 42.7% | 45.9% | + improving |
| Def SR LAC | 37.2% | 42.7% | 50.0% | - declining |
| Tempo JAX | 6.000 | 5.583 | 5.500 | - declining |
| Tempo LAC | 6.462 | 5.429 | 8.111 | + improving |

## Matchup Analogs

**JAX analogs vs LAC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 10 | MIN (-) | 7-11 | MIN | 0.934 | -0.190 | 41.8% | -0.391 |
| 3 | HOU (H) | 10-47 | HOU | 0.759 | -0.163 | 42.7% | -3.700 |
| 1 | MIA (A) | 17-18 | MIA | 0.649 | -0.042 | 43.1% | -0.121 |

**LAC analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | NYG (A) | 10-15 | NYG | 0.661 | -0.220 | 32.0% | -0.545 |
| 5 | WAS (H) | n/a | n/a | 0.620 | n/a | n/a | n/a |
| 9 | TEN (A) | 27-11 | LAC | 0.575 | -0.063 | 32.9% | 1.159 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.015 | 0.009 | -0.066 |
| LAC | -0.005 | 0.072 | 0.056 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.103 | 0.076 | 0.044 |
| LAC | -0.127 | -0.079 | -0.143 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.4% | 47.0% | 45.0% |
| LAC | 42.1% | 43.5% | 40.8% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 47.0% | 46.4% | 45.8% |
| LAC | 43.9% | 45.5% | 42.8% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 10.7% | 11.4% | 12.5% |
| LAC | 10.4% | 12.0% | 13.3% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 30.9% | 36.5% | 25.2% |
| LAC | 38.1% | 40.4% | 32.4% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.707 | -0.570 | -0.450 |
| LAC | 0.723 | 0.964 | 1.284 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.373 | -0.489 | -0.860 |
| LAC | 0.142 | 0.449 | 0.797 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.700 | -0.400 | -0.333 |
| LAC | 1.111 | 1.000 | 1.000 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 15.5% | 17.3% | 23.2% |
| LAC | 10.1% | 10.4% | 15.7% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 5.5% | 5.8% | 5.1% |
| LAC | 9.2% | 10.2% | 15.2% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 6.869 | 6.599 | 5.694 |
| LAC | 7.214 | 7.696 | 6.667 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 45.3% | 44.7% | 47.0% |
| LAC | 40.0% | 42.5% | 39.1% |
