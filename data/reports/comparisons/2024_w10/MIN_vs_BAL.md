# Matchup Report - MIN vs BAL

## Metric Comparison

| Metric | MIN | BAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.009 | 0.203 | ↓ -0.213 |
| Core EPA Defense | -0.190 | 0.168 | ↓ -0.358 |
| Success Rate Offense | 45.9% | 51.2% | ↓ -5.3 pp |
| Success Rate Defense | 41.8% | 52.1% | ↓ -10.3 pp |
| Explosive Play Rate (Off) | 8.2% | 9.8% | ↓ -1.6 pp |
| Third Down Conversion | 33.3% | 53.8% | ↓ -20.5 pp |
| Points per Drive Differential | 0.391 | -0.174 | ↑ +0.565 |
| Yards per Play Differential | 1.502 | -0.128 | ↑ +1.630 |
| Turnover Margin | 0.000 | 1.000 | ↓ -1.000 |
| Red Zone TD Rate (Off) | 0.0% | 22.2% | ↓ -22.2 pp |
| Pressure Rate (Def) | 12.0% | 5.0% | ↑ +7.0 pp |
| Tempo | 8.909 | 6.833 | ↑ +2.076 |
| PowerScore | 0.090 | 0.252 | ↓ -0.162 |

## PowerScore Breakdown (Model)

| Component | Weight | MIN | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.009 | 0.203 | -0.213 |
| EPA Defense | 20% | -0.190 | 0.168 | -0.358 |
| Success Rate Offense | 25% | 45.9% | 51.2% | -5.3 pp |
| Tempo | 20% | 0.082 | 0.098 | -0.016 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | MIN | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.009 | 0.203 | -0.213 |
| EPA Defense | 20% | -0.190 | 0.168 | -0.358 |
| Success Rate Offense | 15% | 45.9% | 51.2% | -5.3 pp |
| Turnover Margin | 10% | 0.000 | 1.000 | -1.000 |
| Tempo | 10% | 0.082 | 0.098 | -0.016 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 22.2% | -22.2 pp |
| Pressure Rate (Def) | 10% | 12.0% | 5.0% | +7.0 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.162 (MIN +0.090 vs BAL +0.252)
**7 metrics version:**
BAL edge: +0.250 (MIN +0.049 vs BAL +0.298)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.250), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (MIN - BAL) | -3.0 pts (favours BAL) |
| Model Win% MIN | 37.7% |
| Model Win% BAL | 62.3% |
| SOS Differential (MIN-BAL) | -0.098 |
*Neutral-field assumption. Based on schedule through Week 9.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| MIN | 41.8% | 43.3% | -1.4% | 46.7% | 61.1% | Slight run lean |
| BAL | 41.5% | 43.3% | -1.8% | 32.0% | 50.0% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -3.9 pp | -11.8 pp | -11.3 pp |
| BAL | +12.8 pp | +11.6 pp | -1.4 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +0.5 pp | +2.8 pp | +5.5 pp |
| BAL | +10.8 pp | +7.0 pp | +9.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -2.0 pp | -2.3 pp | -3.4 pp |
| BAL | +3.4 pp | +0.1 pp | +0.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +1.3 pp | +0.8 pp | +1.3 pp |
| BAL | +4.1 pp | +4.1 pp | +2.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +1.5 pp | -1.8 pp | -3.9 pp |
| BAL | +6.2 pp | +5.0 pp | +7.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +0.0 pp | -1.8 pp | +3.8 pp |
| BAL | +11.8 pp | +13.5 pp | +11.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +1.3 pp | +0.8 pp | +1.3 pp |
| BAL | +4.1 pp | +4.1 pp | +2.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -2.0 pp | -2.3 pp | -3.4 pp |
| BAL | +3.4 pp | +0.1 pp | +0.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 44.2 | 45.7 | 46.2 |
| BAL | 46.9 | 47.8 | 48.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.4 | 49.0 | 50.1 |
| BAL | 52.2 | 55.0 | 52.9 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.2 | -3.3 | -3.9 |
| BAL | -5.2 | -7.2 | -4.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 2.06 | 1.87 | 2.02 |
| BAL | 2.88 | 3.30 | 3.34 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.59 | 1.96 | 2.19 |
| BAL | 2.15 | 2.26 | 2.00 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 0.48 | -0.10 | -0.17 |
| BAL | 0.73 | 1.03 | 1.34 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 6.72 | 6.81 | 7.15 |
| BAL | 7.69 | 7.77 | 7.40 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 42.1% | 42.1% | 44.2% |
| BAL | 35.7% | 37.8% | 35.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 57.9% | 57.9% | 55.8% |
| BAL | 64.3% | 62.2% | 64.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 2.83 | 2.87 | 3.16 |
| BAL | 2.75 | 2.94 | 2.64 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.89 | 3.94 | 3.99 |
| BAL | 4.94 | 4.83 | 4.76 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 9)

| Window | MIN | BAL |
| --- | ---: | ---: |
| Season-to-date | 0.090 | 0.188 |
| Last 5 games | 0.110 | 0.216 |
| Last 3 games | 0.126 | 0.196 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA MIN | 0.008 | 0.094 | -0.132 | - declining |
| Off EPA BAL | 0.293 | 0.054 | 0.333 | + improving |
| Def EPA MIN | 0.043 | 0.146 | -0.279 | + improving |
| Def EPA BAL | 0.088 | 0.117 | -0.118 | + improving |
| Off SR MIN | 37.7% | 49.2% | 51.2% | + improving |
| Off SR BAL | 52.6% | 48.9% | 58.9% | + improving |
| Def SR MIN | 47.4% | 49.5% | 39.1% | + improving |
| Def SR BAL | 52.0% | 51.2% | 36.0% | + improving |
| Tempo MIN | 6.417 | 7.222 | 7.818 | + improving |
| Tempo BAL | 6.909 | 8.000 | 7.300 | + improving |

## Matchup Analogs

**MIN analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | DET (A) | 21-7 | MIN | 0.799 | -0.132 | 51.2% | 1.273 |
| 2 | ATL (H) | 23-19 | MIN | 0.770 | -0.053 | 52.2% | 0.391 |
| 7 | PHI (A) | 23-34 | PHI | 0.742 | 0.008 | 37.7% | -0.902 |

**BAL analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | DET (H) | 28-27 | BAL | 0.817 | 0.124 | 46.3% | 0.061 |
| 8 | CHI (A) | 24-27 | CHI | 0.788 | 0.054 | 48.9% | -0.235 |
| 1 | KC (A) | 20-27 | KC | 0.639 | 0.021 | 55.2% | -0.700 |

## Core EPA Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.159 | 0.223 | 0.227 |
| MIN | 0.007 | -0.020 | -0.010 |

## Core EPA Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.054 | 0.084 | 0.029 |
| MIN | -0.119 | -0.065 | -0.030 |

## Success Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 52.6% | 54.6% | 53.2% |
| MIN | 47.0% | 46.1% | 46.1% |

## Success Rate Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 46.3% | 48.0% | 46.7% |
| MIN | 43.8% | 45.8% | 45.8% |

## Explosive Play Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 14.4% | 15.1% | 14.8% |
| MIN | 11.8% | 12.0% | 14.0% |

## Third Down Conversion Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 41.0% | 42.3% | 29.8% |
| MIN | 35.7% | 33.4% | 37.8% |

## Points per Drive Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.694 | 0.981 | 1.281 |
| MIN | 0.459 | -0.068 | -0.151 |

## Yards per Play Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.153 | 1.161 | 1.102 |
| MIN | 0.455 | 0.116 | 0.500 |

## Turnover Margin Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.333 | 0.400 | 1.000 |
| MIN | 0.750 | 0.800 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 22.3% | 20.8% | 19.3% |
| MIN | 13.2% | 11.8% | 10.7% |

## Pressure Rate (Def) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.7% | 7.5% | 7.5% |
| MIN | 8.1% | 5.9% | 7.5% |

## Tempo Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.694 | 7.770 | 7.403 |
| MIN | 6.720 | 6.807 | 7.152 |

## Pass Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 36.4% | 38.5% | 36.3% |
| MIN | 41.9% | 42.0% | 44.3% |
