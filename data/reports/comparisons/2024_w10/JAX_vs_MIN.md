# Matchup Report - JAX vs MIN

## Metric Comparison

| Metric | JAX | MIN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.038 | 0.007 | 0.031 |
| Core EPA Defense | 0.116 | -0.119 | 0.235 |
| Success Rate Offense | 0.467 | 0.471 | -0.004 |
| Success Rate Defense | 0.473 | 0.436 | 0.036 |
| Explosive Play Rate (Off) | 0.109 | 0.119 | -0.010 |
| Third Down Conversion | 0.301 | 0.360 | -0.059 |
| Points per Drive Differential | -0.776 | 0.478 | -1.254 |
| Yards per Play Differential | -0.247 | 0.455 | -0.702 |
| Turnover Margin | -0.778 | 0.750 | -1.528 |
| Red Zone TD Rate (Off) | 0.150 | 0.132 | 0.018 |
| Pressure Rate (Def) | 0.058 | 0.081 | -0.023 |
| Tempo | 5.500 | 8.909 | -3.409 |
| PowerScore | 0.054 | 0.090 | -0.036 |
| PowerScore | 0.054 | 0.090 | -0.036 |
| Tempo | 5.500 | 8.909 | -3.409 |

## PowerScore Breakdown (Model)

| Component | Weight | JAX | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.190 | -0.009 | -0.181 |
| EPA Defense | 20% | -0.009 | -0.190 | +0.181 |
| Success Rate Offense | 25% | 41.8% | 45.9% | -4.1 pp |
| Tempo | 20% | 0.091 | 0.082 | +0.009 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | JAX | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.190 | -0.009 | -0.181 |
| EPA Defense | 20% | -0.009 | -0.190 | +0.181 |
| Success Rate Offense | 15% | 41.8% | 45.9% | -4.1 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.091 | 0.082 | +0.009 |
| Red Zone TD Rate (Off) | 10% | 20.0% | 0.0% | +20.0 pp |
| Pressure Rate (Def) | 10% | 2.4% | 12.0% | -9.6 pp |

## PowerScore Summary

**Model (4 metrics):**
MIN edge: +0.036 (JAX +0.054 vs MIN +0.090)
**7 metrics version:**
MIN edge: +0.004 (JAX +0.045 vs MIN +0.049)
**Verdict:** MIN holds the edge in the extended breakdown (lead 0.004), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (JAX - MIN) | -0.7 pts (favours MIN) |
| Model Win% JAX | 47.3% |
| Model Win% MIN | 52.7% |
| SOS Differential (JAX-MIN) | +0.172 |
*Neutral-field assumption. Based on schedule through Week 9.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| JAX | 45.5% | 43.3% | 2.2% | 32.0% | 50.0% | Slight pass lean |
| MIN | 41.8% | 43.3% | -1.4% | 43.9% | 47.4% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.9 pp | +5.2 pp | +0.2 pp |
| MIN | -3.9 pp | -6.1 pp | -1.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +3.6 pp | +2.2 pp | +10.0 pp |
| MIN | -1.5 pp | -4.4 pp | -2.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.8 pp | +2.3 pp | +4.7 pp |
| MIN | -3.9 pp | -3.0 pp | -5.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +0.4 pp | -0.2 pp | +0.7 pp |
| MIN | +0.4 pp | -0.1 pp | +2.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +3.8 pp | +1.2 pp | +6.3 pp |
| MIN | -0.8 pp | -4.8 pp | -2.6 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.8 pp | +4.3 pp | +2.8 pp |
| MIN | +1.0 pp | +3.4 pp | +7.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +0.4 pp | -0.2 pp | +0.7 pp |
| MIN | +0.4 pp | -0.1 pp | +2.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +1.8 pp | +2.3 pp | +4.7 pp |
| MIN | -3.9 pp | -3.0 pp | -5.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 48.9 | 51.2 | 49.6 |
| MIN | 44.2 | 45.7 | 46.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 45.6 | 45.7 | 46.0 |
| MIN | 48.4 | 49.0 | 50.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.4 | 5.4 | 3.6 |
| MIN | -4.2 | -3.3 | -3.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 1.84 | 2.21 | 2.13 |
| MIN | 2.06 | 1.87 | 2.02 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.62 | 2.60 | 2.30 |
| MIN | 1.59 | 1.96 | 2.19 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -0.78 | -0.39 | -0.17 |
| MIN | 0.48 | -0.10 | -0.17 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 7.02 | 6.77 | 6.56 |
| MIN | 6.72 | 6.81 | 7.15 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 45.0% | 44.7% | 41.6% |
| MIN | 42.1% | 42.1% | 44.2% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 55.0% | 55.3% | 58.4% |
| MIN | 57.9% | 57.9% | 55.8% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.16 | 3.02 | 2.73 |
| MIN | 2.83 | 2.87 | 3.16 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.86 | 3.74 | 3.83 |
| MIN | 3.89 | 3.94 | 3.99 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 9)

| Window | JAX | MIN |
| --- | ---: | ---: |
| Season-to-date | 1.766 | 1.594 |
| Last 5 games | 1.701 | 1.551 |
| Last 3 games | 1.671 | 1.666 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA JAX | 0.229 | 0.061 | -0.069 | - declining |
| Off EPA MIN | 0.008 | 0.094 | -0.132 | - declining |
| Def EPA JAX | -0.019 | 0.131 | 0.011 | - declining |
| Def EPA MIN | 0.043 | 0.146 | -0.279 | + improving |
| Off SR JAX | 56.2% | 50.0% | 41.8% | - declining |
| Off SR MIN | 37.7% | 49.2% | 51.2% | + improving |
| Def SR JAX | 41.0% | 48.9% | 42.7% | - declining |
| Def SR MIN | 47.4% | 49.5% | 39.1% | + improving |
| Tempo JAX | 8.111 | 6.000 | 5.583 | - declining |
| Tempo MIN | 6.417 | 7.222 | 7.818 | + improving |

## Matchup Analogs

**JAX analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | PHI (A) | 17-26 | PHI | 0.803 | -0.069 | 41.8% | -0.737 |
| 6 | CHI (A) | 16-35 | CHI | 0.788 | 0.012 | 43.6% | -1.900 |
| 5 | IND (H) | 37-31 | JAX | 0.670 | 0.235 | 52.6% | 0.468 |

**MIN analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | LA (A) | 20-28 | LA | 0.734 | 0.094 | 49.2% | -0.889 |
| 1 | NYG (A) | 22-6 | MIN | 0.627 | 0.093 | 45.6% | 1.455 |
| 2 | SF (H) | 23-19 | MIN | 0.526 | -0.053 | 52.2% | 0.391 |

## Core EPA Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.038 | 0.094 | 0.074 |
| MIN | 0.007 | -0.020 | -0.010 |

## Core EPA Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.116 | 0.112 | 0.041 |
| MIN | -0.119 | -0.065 | -0.030 |

## Success Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.7% | 48.9% | 49.5% |
| MIN | 47.0% | 46.1% | 46.1% |

## Success Rate Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 47.2% | 47.1% | 44.3% |
| MIN | 43.8% | 45.8% | 45.8% |

## Explosive Play Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 10.8% | 11.0% | 13.3% |
| MIN | 11.8% | 12.0% | 14.0% |

## Third Down Conversion Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 29.8% | 35.2% | 31.7% |
| MIN | 35.7% | 33.4% | 37.8% |

## Points per Drive Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.739 | -0.379 | -0.231 |
| MIN | 0.459 | -0.068 | -0.151 |

## Yards per Play Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.247 | 0.158 | 0.052 |
| MIN | 0.455 | 0.116 | 0.500 |

## Turnover Margin Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.778 | -0.600 | -0.333 |
| MIN | 0.750 | 0.800 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 15.0% | 15.9% | 20.3% |
| MIN | 13.2% | 11.8% | 10.7% |

## Pressure Rate (Def) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 5.8% | 6.9% | 6.0% |
| MIN | 8.1% | 5.9% | 7.5% |

## Tempo Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 7.021 | 6.766 | 6.565 |
| MIN | 6.720 | 6.807 | 7.152 |

## Pass Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 45.3% | 44.6% | 41.3% |
| MIN | 41.9% | 42.0% | 44.3% |
