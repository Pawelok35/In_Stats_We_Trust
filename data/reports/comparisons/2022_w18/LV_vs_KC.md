# Matchup Report - LV vs KC

## Metric Comparison

| Metric | LV | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.089 | 0.186 | ↓ -0.275 |
| Core EPA Defense | 0.186 | -0.089 | ↑ +0.275 |
| Success Rate Offense | 40.2% | 54.7% | ↓ -14.4 pp |
| Success Rate Defense | 54.7% | 40.2% | ↑ +14.4 pp |
| Explosive Play Rate (Off) | 9.8% | 12.0% | ↓ -2.2 pp |
| Third Down Conversion | 47.1% | 45.5% | ↑ +1.6 pp |
| Points per Drive Differential | -1.656 | 1.656 | ↓ -3.311 |
| Yards per Play Differential | -1.251 | 1.251 | ↓ -2.502 |
| Turnover Margin | -2.000 | 2.000 | ↓ -4.000 |
| Red Zone TD Rate (Off) | 6.7% | 21.1% | ↓ -14.4 pp |
| Pressure Rate (Def) | 6.7% | 14.3% | ↓ -7.6 pp |
| Tempo | 9.111 | 7.500 | ↑ +1.611 |
| PowerScore | 0.126 | 0.208 | ↓ -0.082 |

## PowerScore Breakdown (Model)

| Component | Weight | LV | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.089 | 0.186 | -0.275 |
| EPA Defense | 20% | 0.186 | -0.089 | +0.275 |
| Success Rate Offense | 25% | 40.2% | 54.7% | -14.4 pp |
| Tempo | 20% | 0.098 | 0.120 | -0.022 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LV | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.089 | 0.186 | -0.275 |
| EPA Defense | 20% | 0.186 | -0.089 | +0.275 |
| Success Rate Offense | 15% | 40.2% | 54.7% | -14.4 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.098 | 0.120 | -0.022 |
| Red Zone TD Rate (Off) | 10% | 6.7% | 21.1% | -14.4 pp |
| Pressure Rate (Def) | 10% | 6.7% | 14.3% | -7.6 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.082 (LV +0.126 vs KC +0.208)
**7 metrics version:**
KC edge: +0.460 (LV -0.102 vs KC +0.358)
**Verdict:** KC holds the edge in the extended breakdown (lead 0.460), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LV - KC) | -1.5 pts (favours KC) |
| Model Win% LV | 43.7% |
| Model Win% KC | 56.3% |
| SOS Differential (LV-KC) | -0.020 |
*Neutral-field assumption. Based on schedule through Week 17.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LV | 51.2% | 42.9% | 8.3% | 38.1% | 42.5% | Pass heavy |
| KC | 40.0% | 42.9% | -2.9% | 40.0% | 64.4% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +3.3 pp | +6.7 pp | +21.2 pp |
| KC | +3.7 pp | +9.0 pp | +0.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -8.0 pp | -4.9 pp | -2.4 pp |
| KC | +2.1 pp | +10.2 pp | +10.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +3.2 pp | +3.2 pp | +0.3 pp |
| KC | +0.5 pp | +0.1 pp | -2.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +1.3 pp | +0.4 pp | -0.1 pp |
| KC | +1.5 pp | +1.7 pp | -1.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.1 pp | +0.3 pp | -1.2 pp |
| KC | +4.4 pp | +5.0 pp | -3.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +1.4 pp | +4.3 pp | +12.1 pp |
| KC | +4.5 pp | +13.1 pp | +16.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +1.3 pp | +0.4 pp | -0.1 pp |
| KC | +1.5 pp | +1.7 pp | -1.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | +3.2 pp | +3.2 pp | +0.3 pp |
| KC | +0.5 pp | +0.1 pp | -2.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 47.6 | 48.2 | 50.2 |
| KC | 47.8 | 47.2 | 45.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 48.7 | 46.1 | 45.6 |
| KC | 49.8 | 47.9 | 44.9 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -1.2 | 2.0 | 4.5 |
| KC | -2.0 | -0.7 | 0.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 2.14 | 1.94 | 1.89 |
| KC | 2.66 | 2.39 | 2.31 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 2.16 | 1.69 | 1.88 |
| KC | 2.06 | 2.03 | 1.54 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.02 | 0.26 | 0.01 |
| KC | 0.60 | 0.36 | 0.77 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 7.76 | 7.16 | 7.06 |
| KC | 7.79 | 6.96 | 6.75 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 45.5% | 39.6% | 44.1% |
| KC | 49.3% | 48.9% | 48.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 54.5% | 60.4% | 55.9% |
| KC | 50.7% | 51.1% | 51.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 3.53 | 2.84 | 3.11 |
| KC | 3.84 | 3.40 | 3.30 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 4.22 | 4.32 | 3.95 |
| KC | 3.95 | 3.56 | 3.46 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 17)

| Window | LV | KC |
| --- | ---: | ---: |
| Season-to-date | 0.134 | 0.154 |
| Last 5 games | 0.097 | 0.130 |
| Last 3 games | 0.108 | 0.101 |

## Trend Summary (last 3 weeks)

| Metric | W15 | W16 | W17 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LV | -0.109 | -0.169 | 0.067 | + improving |
| Off EPA KC | 0.056 | 0.029 | 0.037 | - declining |
| Def EPA LV | -0.134 | -0.105 | 0.162 | - declining |
| Def EPA KC | -0.015 | -0.159 | 0.018 | - declining |
| Off SR LV | 39.8% | 37.9% | 49.4% | + improving |
| Off SR KC | 53.8% | 47.8% | 46.8% | - declining |
| Def SR LV | 35.3% | 40.7% | 53.5% | - declining |
| Def SR KC | 36.2% | 35.5% | 41.9% | - declining |
| Tempo LV | 7.154 | 6.600 | 7.417 | + improving |
| Tempo KC | 7.750 | 6.091 | 6.417 | - declining |

## Matchup Analogs

**LV analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | KC (A) | 29-27 | LV | 1.000 | 0.198 | 43.0% | 0.222 |
| 9 | JAX (A) | 20-25 | JAX | 0.892 | 0.029 | 37.8% | -0.455 |
| 12 | SEA (A) | 40-34 | LV | 0.877 | 0.118 | 48.0% | 0.429 |

**KC analogs vs LV profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | LV (H) | 30-32 | LV | 1.000 | 0.155 | 48.9% | -0.222 |
| 1 | ARI (A) | 44-19 | KC | 0.664 | 0.366 | 59.0% | 2.491 |
| 16 | SEA (H) | 24-8 | KC | 0.664 | 0.029 | 47.8% | 1.413 |

## Core EPA Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.120 | 0.076 | 0.041 |
| LV | 0.001 | -0.052 | -0.070 |

## Core EPA Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.015 | -0.009 | -0.052 |
| LV | 0.005 | -0.048 | -0.026 |

## Success Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 49.1% | 50.3% | 49.8% |
| LV | 42.8% | 42.9% | 42.7% |

## Success Rate Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 42.6% | 40.0% | 37.8% |
| LV | 44.4% | 41.4% | 43.3% |

## Explosive Play Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 12.1% | 12.1% | 10.1% |
| LV | 10.6% | 9.3% | 8.1% |

## Third Down Conversion Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 43.5% | 40.2% | 33.8% |
| LV | 37.5% | 36.9% | 40.2% |

## Points per Drive Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.556 | 0.372 | 0.754 |
| LV | 0.017 | 0.285 | 0.034 |

## Yards per Play Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 1.118 | 1.318 | 1.607 |
| LV | -0.065 | 0.050 | -0.463 |

## Turnover Margin Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.188 | 0.000 | 0.667 |
| LV | -0.375 | -1.000 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 17.2% | 21.3% | 24.2% |
| LV | 11.6% | 12.8% | 15.2% |

## Pressure Rate (Def) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 7.9% | 6.5% | 5.8% |
| LV | 4.0% | 3.9% | 0.0% |

## Tempo Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 7.787 | 6.955 | 6.753 |
| LV | 7.756 | 7.156 | 7.057 |

## Pass Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 49.7% | 49.2% | 48.9% |
| LV | 45.2% | 39.6% | 43.5% |
