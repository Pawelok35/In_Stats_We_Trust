# Matchup Report - SEA vs PIT

## Metric Comparison

| Metric | SEA | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.085 | 0.192 | ↓ -0.107 |
| Core EPA Defense | 0.192 | 0.085 | ↑ +0.107 |
| Success Rate Offense | 46.4% | 52.9% | ↓ -6.5 pp |
| Success Rate Defense | 52.9% | 46.4% | ↑ +6.5 pp |
| Explosive Play Rate (Off) | 15.9% | 12.6% | ↑ +3.3 pp |
| Third Down Conversion | 30.0% | 46.2% | ↓ -16.2 pp |
| Points per Drive Differential | -0.444 | 0.444 | ↓ -0.889 |
| Yards per Play Differential | -0.031 | 0.031 | ↓ -0.063 |
| Turnover Margin | -1.000 | 1.000 | ↓ -2.000 |
| Red Zone TD Rate (Off) | 18.2% | 10.0% | ↑ +8.2 pp |
| Pressure Rate (Def) | 3.8% | 2.9% | ↑ +0.9 pp |
| Tempo | 7.667 | 8.700 | ↓ -1.033 |
| PowerScore | 0.216 | 0.242 | ↓ -0.026 |

## PowerScore Breakdown (Model)

| Component | Weight | SEA | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.085 | 0.192 | -0.107 |
| EPA Defense | 20% | 0.192 | 0.085 | +0.107 |
| Success Rate Offense | 25% | 46.4% | 52.9% | -6.5 pp |
| Tempo | 20% | 0.159 | 0.126 | +0.033 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | SEA | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.085 | 0.192 | -0.107 |
| EPA Defense | 20% | 0.192 | 0.085 | +0.107 |
| Success Rate Offense | 15% | 46.4% | 52.9% | -6.5 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.159 | 0.126 | +0.033 |
| Red Zone TD Rate (Off) | 10% | 18.2% | 10.0% | +8.2 pp |
| Pressure Rate (Def) | 10% | 3.8% | 2.9% | +0.9 pp |

## PowerScore Summary

**Model (4 metrics):**
PIT edge: +0.026 (SEA +0.216 vs PIT +0.242)
**7 metrics version:**
PIT edge: +0.203 (SEA +0.067 vs PIT +0.270)
**Verdict:** PIT holds the edge in the extended breakdown (lead 0.203), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (SEA - PIT) | -0.5 pts (favours PIT) |
| Model Win% SEA | 48.0% |
| Model Win% PIT | 52.0% |
| SOS Differential (SEA-PIT) | +0.052 |
*Neutral-field assumption. Based on schedule through Week 16.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| SEA | 49.3% | 44.0% | 5.3% | 52.9% | 40.0% | Pass heavy |
| PIT | 29.9% | 44.0% | -14.1% | 50.0% | 54.1% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -3.1 pp | +2.5 pp | -0.6 pp |
| PIT | -9.7 pp | -12.9 pp | -15.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -2.8 pp | -9.3 pp | -9.6 pp |
| PIT | -5.4 pp | -3.1 pp | -1.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +0.3 pp | -1.0 pp | -1.5 pp |
| PIT | +1.5 pp | +2.4 pp | +2.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -0.4 pp | -0.4 pp | +0.3 pp |
| PIT | -1.0 pp | -2.8 pp | -3.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +3.6 pp | +10.4 pp | +12.3 pp |
| PIT | -4.9 pp | -13.7 pp | -13.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +2.0 pp | +0.5 pp | -3.6 pp |
| PIT | -4.9 pp | +1.8 pp | +1.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -0.4 pp | -0.4 pp | +0.3 pp |
| PIT | -1.0 pp | -2.8 pp | -3.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +0.3 pp | -1.0 pp | -1.5 pp |
| PIT | +1.5 pp | +2.4 pp | +2.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 48.5 | 45.4 | 40.7 |
| PIT | 43.9 | 42.9 | 44.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 45.8 | 47.1 | 43.2 |
| PIT | 44.6 | 46.4 | 47.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.6 | -1.7 | -2.5 |
| PIT | -0.7 | -3.5 | -2.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 1.87 | 2.07 | 2.06 |
| PIT | 1.40 | 1.54 | 1.73 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.13 | 2.49 | 2.05 |
| PIT | 1.74 | 1.73 | 1.78 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -0.26 | -0.43 | 0.01 |
| PIT | -0.34 | -0.19 | -0.05 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 7.41 | 8.07 | 8.34 |
| PIT | 6.79 | 6.81 | 6.16 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 46.8% | 47.7% | 48.3% |
| PIT | 43.6% | 41.1% | 41.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 53.2% | 52.3% | 51.7% |
| PIT | 56.4% | 58.9% | 58.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 3.47 | 3.85 | 4.03 |
| PIT | 2.96 | 2.80 | 2.56 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 3.94 | 4.22 | 4.31 |
| PIT | 3.82 | 4.01 | 3.59 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 16)

| Window | SEA | PIT |
| --- | ---: | ---: |
| Season-to-date | 0.137 | 0.085 |
| Last 5 games | 0.191 | 0.066 |
| Last 3 games | 0.187 | 0.051 |

## Trend Summary (last 3 weeks)

| Metric | W14 | W15 | W16 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA SEA | -0.024 | 0.092 | 0.166 | + improving |
| Off EPA PIT | -0.173 | -0.324 | 0.153 | + improving |
| Def EPA SEA | 0.124 | 0.069 | 0.080 | + improving |
| Def EPA PIT | -0.138 | -0.059 | -0.174 | + improving |
| Off SR SEA | 39.2% | 42.5% | 46.1% | + improving |
| Off SR PIT | 40.7% | 30.1% | 45.3% | + improving |
| Def SR SEA | 43.1% | 48.2% | 48.2% | - declining |
| Def SR PIT | 32.4% | 43.4% | 39.0% | - declining |
| Tempo SEA | 6.167 | 8.000 | 10.857 | + improving |
| Tempo PIT | 6.143 | 6.083 | 6.250 | + improving |

## Matchup Analogs

**SEA analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | NYG (A) | 18-3 | SEA | 0.845 | -0.134 | 44.9% | 1.269 |
| 8 | CLE (H) | 24-20 | SEA | 0.819 | 0.008 | 44.6% | 0.333 |
| 3 | CAR (H) | 37-29 | SEA | 0.708 | 0.163 | 52.7% | 0.596 |

**PIT analogs vs SEA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | CIN (A) | 16-9 | PIT | 0.935 | 0.029 | 42.9% | 0.691 |
| 16 | CIN (H) | 34-12 | PIT | 0.935 | 0.153 | 45.3% | 1.833 |
| 9 | TEN (H) | 20-16 | PIT | 0.909 | 0.131 | 48.8% | 0.444 |

## Core EPA Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | -0.095 | -0.098 | -0.114 |
| SEA | -0.004 | 0.043 | 0.078 |

## Core EPA Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | -0.054 | -0.088 | -0.124 |
| SEA | 0.025 | 0.116 | 0.091 |

## Success Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 39.0% | 40.1% | 38.9% |
| SEA | 43.9% | 43.3% | 42.6% |

## Success Rate Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 40.9% | 38.4% | 38.5% |
| SEA | 44.1% | 47.9% | 46.7% |

## Explosive Play Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 8.4% | 8.3% | 7.7% |
| SEA | 10.2% | 9.7% | 10.9% |

## Third Down Conversion Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 32.4% | 35.6% | 32.3% |
| SEA | 32.5% | 38.3% | 35.8% |

## Points per Drive Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | -0.340 | -0.201 | -0.062 |
| SEA | -0.286 | -0.551 | -0.134 |

## Yards per Play Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | -0.409 | -0.014 | -0.679 |
| SEA | -0.207 | -0.712 | -0.957 |

## Turnover Margin Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 0.800 | -0.200 | 0.000 |
| SEA | 0.467 | 0.400 | 0.667 |

## Red Zone TD Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 11.4% | 13.4% | 16.8% |
| SEA | 9.7% | 6.9% | 4.4% |

## Pressure Rate (Def) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 7.4% | 8.5% | 7.5% |
| SEA | 7.5% | 7.8% | 9.3% |

## Tempo Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 6.786 | 6.812 | 6.159 |
| SEA | 7.409 | 8.072 | 8.341 |

## Pass Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| PIT | 43.7% | 41.3% | 41.9% |
| SEA | 47.0% | 47.7% | 48.3% |
