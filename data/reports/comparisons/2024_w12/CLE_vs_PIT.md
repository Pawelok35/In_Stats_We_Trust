# Matchup Report - CLE vs PIT

## Metric Comparison

| Metric | CLE | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.123 | 0.027 | -0.150 |
| Core EPA Defense | -0.015 | -0.090 | 0.075 |
| Success Rate Offense | 0.406 | 0.451 | -0.045 |
| Success Rate Defense | 0.426 | 0.458 | -0.031 |
| Explosive Play Rate (Off) | 0.073 | 0.097 | -0.024 |
| Third Down Conversion | 0.247 | 0.344 | -0.097 |
| Points per Drive Differential | -0.745 | 0.476 | -1.222 |
| Yards per Play Differential | -0.955 | -0.024 | -0.930 |
| Turnover Margin | -0.500 | 0.900 | -1.400 |
| Red Zone TD Rate (Off) | 0.095 | 0.114 | -0.019 |
| Pressure Rate (Def) | 0.083 | 0.071 | 0.012 |
| Tempo | 7.400 | 7.727 | -0.327 |
| PowerScore | 0.107 | 0.096 | 0.011 |
| PowerScore | 0.107 | 0.096 | 0.011 |
| Tempo | 7.400 | 7.727 | -0.327 |

## PowerScore Breakdown (Model)

| Component | Weight | CLE | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.047 | -0.053 | +0.006 |
| EPA Defense | 20% | -0.053 | -0.047 | -0.006 |
| Success Rate Offense | 25% | 45.9% | 40.0% | +5.9 pp |
| Tempo | 20% | 0.095 | 0.118 | -0.023 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CLE | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.047 | -0.053 | +0.006 |
| EPA Defense | 20% | -0.053 | -0.047 | -0.006 |
| Success Rate Offense | 15% | 45.9% | 40.0% | +5.9 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.095 | 0.118 | -0.023 |
| Red Zone TD Rate (Off) | 10% | 17.6% | 10.0% | +7.6 pp |
| Pressure Rate (Def) | 10% | 11.8% | 3.3% | +8.4 pp |

## PowerScore Summary

**Model (4 metrics):**
CLE edge: +0.011 (CLE +0.107 vs PIT +0.096)
**7 metrics version:**
PIT edge: +0.377 (CLE -0.115 vs PIT +0.262)
**Verdict:** PIT holds the edge in the extended breakdown (lead 0.377), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CLE - PIT) | +0.2 pts (favours CLE) |
| Model Win% CLE | 50.9% |
| Model Win% PIT | 49.1% |
| SOS Differential (CLE-PIT) | -0.132 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CLE | 40.5% | 45.1% | -4.6% | 46.7% | 45.5% | Run heavy |
| PIT | 40.0% | 45.1% | -5.1% | 41.2% | 39.2% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -6.2 pp | +3.8 pp | +3.5 pp |
| PIT | +2.6 pp | +1.6 pp | -1.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.0 pp | -14.3 pp | -17.8 pp |
| PIT | -3.4 pp | -6.5 pp | -13.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.0 pp | -4.2 pp | -0.3 pp |
| PIT | -0.4 pp | -0.6 pp | -0.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.6 pp | -1.6 pp | -0.2 pp |
| PIT | -2.5 pp | -3.0 pp | -5.1 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.3 pp | -5.4 pp | -7.4 pp |
| PIT | +3.0 pp | +4.0 pp | +0.2 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -6.5 pp | -1.5 pp | +4.9 pp |
| PIT | +1.0 pp | -4.5 pp | -3.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.6 pp | -1.6 pp | -0.2 pp |
| PIT | -2.5 pp | -3.0 pp | -5.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.0 pp | -4.2 pp | -0.3 pp |
| PIT | -0.4 pp | -0.6 pp | -0.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 46.9 | 44.3 | 44.2 |
| PIT | 45.6 | 48.8 | 47.9 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 43.1 | 44.1 | 44.2 |
| PIT | 45.9 | 48.7 | 47.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.8 | 0.2 | 0.0 |
| PIT | -0.4 | 0.0 | 0.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.30 | 1.31 | 1.49 |
| PIT | 2.00 | 2.26 | 1.68 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 2.04 | 2.26 | 2.43 |
| PIT | 1.53 | 1.62 | 1.89 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -0.75 | -0.94 | -0.94 |
| PIT | 0.48 | 0.65 | -0.20 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 7.44 | 7.65 | 7.61 |
| PIT | 7.57 | 7.36 | 7.18 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 49.8% | 51.7% | 54.2% |
| PIT | 37.4% | 37.8% | 39.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 50.2% | 48.3% | 45.8% |
| PIT | 62.6% | 62.2% | 60.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.71 | 3.96 | 4.12 |
| PIT | 2.83 | 2.79 | 2.82 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.74 | 3.70 | 3.49 |
| PIT | 4.74 | 4.58 | 4.36 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | CLE | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.398 | 1.530 |
| Last 5 games | 1.418 | 1.468 |
| Last 3 games | 1.464 | 1.510 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CLE | -0.187 | 0.117 | -0.073 | + improving |
| Off EPA PIT | 0.176 | 0.051 | -0.102 | - declining |
| Def EPA CLE | -0.102 | 0.054 | 0.213 | - declining |
| Def EPA PIT | -0.118 | -0.027 | -0.165 | + improving |
| Off SR CLE | 35.7% | 51.2% | 39.6% | + improving |
| Off SR PIT | 45.2% | 49.4% | 40.8% | - declining |
| Def SR CLE | 36.5% | 48.9% | 51.4% | - declining |
| Def SR PIT | 43.4% | 43.5% | 41.8% | + improving |
| Tempo CLE | 7.000 | 7.167 | 8.273 | + improving |
| Tempo PIT | 7.636 | 7.083 | 7.538 | - declining |

## Matchup Analogs

**CLE analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | LAC (H) | 10-25 | LAC | 0.953 | -0.266 | 42.7% | -1.159 |
| 6 | PHI (A) | 10-23 | PHI | 0.897 | -0.069 | 42.1% | -1.489 |
| 11 | NO (A) | 14-35 | NO | 0.781 | -0.073 | 39.6% | -1.909 |

**PIT analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | LV (A) | 32-13 | PIT | 0.842 | 0.083 | 44.0% | 1.727 |
| 4 | IND (A) | 24-27 | IND | 0.832 | -0.019 | 51.8% | -0.273 |
| 5 | DAL (H) | 17-24 | DAL | 0.804 | 0.031 | 50.0% | -0.677 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.123 | -0.096 | -0.074 |
| PIT | 0.027 | 0.053 | 0.002 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.015 | 0.018 | 0.068 |
| PIT | -0.090 | -0.110 | -0.055 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 40.3% | 42.1% | 44.3% |
| PIT | 44.9% | 43.8% | 43.2% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 42.7% | 43.1% | 44.5% |
| PIT | 45.9% | 45.9% | 46.4% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 7.3% | 8.3% | 9.2% |
| PIT | 9.7% | 10.7% | 9.3% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 24.7% | 32.7% | 34.6% |
| PIT | 34.4% | 33.3% | 35.1% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.747 | -0.886 | -0.924 |
| PIT | 0.435 | 0.567 | -0.197 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.955 | -0.740 | -0.898 |
| PIT | -0.024 | 0.079 | -0.038 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.500 | -0.800 | -0.667 |
| PIT | 0.900 | 1.000 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 9.5% | 4.4% | 2.2% |
| PIT | 11.4% | 9.1% | 5.9% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 8.3% | 8.1% | 9.2% |
| PIT | 7.1% | 5.5% | 7.6% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 7.442 | 7.654 | 7.608 |
| PIT | 7.570 | 7.365 | 7.183 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 50.2% | 52.3% | 54.2% |
| PIT | 37.5% | 37.9% | 39.3% |
