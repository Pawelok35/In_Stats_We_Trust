# Matchup Report - CAR vs KC

## Metric Comparison

| Metric | CAR | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.125 | 0.052 | -0.177 |
| Core EPA Defense | 0.099 | -0.013 | 0.112 |
| Success Rate Offense | 0.436 | 0.515 | -0.079 |
| Success Rate Defense | 0.504 | 0.456 | 0.048 |
| Explosive Play Rate (Off) | 0.080 | 0.081 | -0.000 |
| Third Down Conversion | 0.292 | 0.477 | -0.185 |
| Points per Drive Differential | -1.262 | 0.365 | -1.627 |
| Yards per Play Differential | -0.978 | 0.291 | -1.269 |
| Turnover Margin | -0.700 | -0.500 | -0.200 |
| Red Zone TD Rate (Off) | 0.137 | 0.128 | 0.009 |
| Pressure Rate (Def) | 0.034 | 0.058 | -0.024 |
| Tempo | 9.222 | 11.125 | -1.903 |
| PowerScore | 0.242 | 0.284 | -0.042 |
| PowerScore | 0.242 | 0.284 | -0.042 |
| Tempo | 9.222 | 11.125 | -1.903 |

## PowerScore Breakdown (Model)

| Component | Weight | CAR | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.135 | 0.244 | -0.109 |
| EPA Defense | 20% | 0.244 | 0.135 | +0.109 |
| Success Rate Offense | 25% | 50.6% | 59.6% | -8.9 pp |
| Tempo | 20% | 0.096 | 0.112 | -0.016 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.135 | 0.244 | -0.109 |
| EPA Defense | 20% | 0.244 | 0.135 | +0.109 |
| Success Rate Offense | 15% | 50.6% | 59.6% | -8.9 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.096 | 0.112 | -0.016 |
| Red Zone TD Rate (Off) | 10% | 6.5% | 8.3% | -1.9 pp |
| Pressure Rate (Def) | 10% | 11.9% | 5.4% | +6.5 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.042 (CAR +0.242 vs KC +0.284)
**7 metrics version:**
KC edge: +0.016 (CAR +0.187 vs KC +0.202)
**Verdict:** KC holds the edge in the extended breakdown (lead 0.016), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - KC) | -0.8 pts (favours KC) |
| Model Win% CAR | 46.8% |
| Model Win% KC | 53.2% |
| SOS Differential (CAR-KC) | -0.052 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 44.6% | 45.1% | -0.5% | 43.2% | 56.5% | Balanced |
| KC | 47.2% | 45.1% | 2.1% | 50.0% | 68.1% | Slight pass lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -5.8 pp | -3.3 pp | -10.6 pp |
| KC | +2.3 pp | +9.0 pp | +1.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.0 pp | +2.7 pp | +1.5 pp |
| KC | -3.4 pp | +3.2 pp | +2.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.1 pp | +1.3 pp | -0.3 pp |
| KC | -2.7 pp | -3.7 pp | -4.3 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.3 pp | +0.7 pp | +0.3 pp |
| KC | -3.3 pp | -3.8 pp | -3.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +5.1 pp | +3.6 pp | +5.2 pp |
| KC | +0.5 pp | -4.5 pp | -1.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -11.1 pp | -11.4 pp | -10.5 pp |
| KC | +2.4 pp | +6.2 pp | +4.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.3 pp | +0.7 pp | +0.3 pp |
| KC | -3.3 pp | -3.8 pp | -3.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.1 pp | +1.3 pp | -0.3 pp |
| KC | -2.7 pp | -3.7 pp | -4.3 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 50.7 | 51.4 | 45.4 |
| KC | 46.3 | 45.2 | 46.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.9 | 45.1 | 41.8 |
| KC | 50.2 | 51.6 | 47.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 4.7 | 6.3 | 3.6 |
| KC | -3.9 | -6.3 | -1.6 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.45 | 1.50 | 1.60 |
| KC | 2.30 | 2.43 | 2.20 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.72 | 2.50 | 1.88 |
| KC | 1.94 | 2.22 | 2.41 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.26 | -1.00 | -0.28 |
| KC | 0.36 | 0.22 | -0.20 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 6.82 | 6.83 | 6.45 |
| KC | 8.20 | 8.47 | 8.36 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 43.5% | 40.7% | 41.8% |
| KC | 45.9% | 48.1% | 51.7% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 56.5% | 59.3% | 58.2% |
| KC | 54.1% | 51.9% | 48.3% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.97 | 2.78 | 2.70 |
| KC | 3.76 | 4.07 | 4.32 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.85 | 4.05 | 3.75 |
| KC | 4.44 | 4.40 | 4.04 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | CAR | KC |
| --- | ---: | ---: |
| Season-to-date | 1.651 | 1.702 |
| Last 5 games | 1.709 | 1.744 |
| Last 3 games | 1.664 | 1.830 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | -0.123 | 0.031 | -0.016 | + improving |
| Off EPA KC | 0.071 | 0.140 | 0.019 | - declining |
| Def EPA CAR | 0.059 | 0.064 | -0.064 | + improving |
| Def EPA KC | -0.042 | 0.133 | -0.013 | - declining |
| Off SR CAR | 44.9% | 43.1% | 42.5% | - declining |
| Off SR KC | 52.3% | 53.5% | 42.0% | - declining |
| Def SR CAR | 47.1% | 43.8% | 55.9% | - declining |
| Def SR KC | 50.0% | 44.4% | 42.7% | + improving |
| Tempo CAR | 7.091 | 6.545 | 5.714 | - declining |
| Tempo KC | 9.556 | 8.417 | 9.000 | - declining |

## Matchup Analogs

**CAR analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | WAS (A) | 7-34 | WAS | 0.799 | -0.309 | 45.2% | -2.700 |
| 4 | CIN (H) | 24-37 | CIN | 0.677 | 0.044 | 52.3% | -1.218 |
| 8 | DEN (A) | 14-26 | DEN | 0.536 | -0.123 | 44.9% | -1.061 |

**KC analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | LV (A) | 27-20 | KC | 0.895 | 0.071 | 52.3% | 0.778 |
| 3 | ATL (A) | 22-19 | KC | 0.746 | 0.085 | 56.8% | 0.300 |
| 2 | CIN (H) | 20-23 | CIN | 0.592 | -0.006 | 53.2% | -0.273 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.125 | -0.101 | -0.036 |
| KC | 0.052 | 0.065 | 0.067 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.099 | 0.095 | 0.020 |
| KC | -0.013 | 0.021 | 0.086 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 44.0% | 44.2% | 43.5% |
| KC | 51.7% | 51.2% | 49.8% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 50.3% | 51.4% | 48.9% |
| KC | 46.0% | 46.6% | 44.7% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 8.2% | 8.5% | 8.7% |
| KC | 8.0% | 8.1% | 7.6% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 29.8% | 36.0% | 35.5% |
| KC | 48.3% | 58.4% | 53.1% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -1.237 | -0.902 | -0.249 |
| KC | 0.368 | 0.220 | -0.200 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.978 | -1.023 | -0.613 |
| KC | 0.291 | 0.046 | -0.104 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.700 | -0.400 | 0.333 |
| KC | -0.500 | -0.200 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 13.7% | 18.4% | 20.5% |
| KC | 12.8% | 16.5% | 15.7% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 3.4% | 3.2% | 4.2% |
| KC | 5.8% | 6.3% | 4.9% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.822 | 6.830 | 6.450 |
| KC | 8.198 | 8.473 | 8.361 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.8% | 41.0% | 41.7% |
| KC | 45.9% | 47.9% | 51.4% |
