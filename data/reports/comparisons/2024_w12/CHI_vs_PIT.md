# Matchup Report - CHI vs PIT

## Metric Comparison

| Metric | CHI | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.041 | 0.027 | -0.069 |
| Core EPA Defense | -0.034 | -0.090 | 0.057 |
| Success Rate Offense | 0.429 | 0.451 | -0.022 |
| Success Rate Defense | 0.450 | 0.458 | -0.008 |
| Explosive Play Rate (Off) | 0.081 | 0.097 | -0.016 |
| Third Down Conversion | 0.269 | 0.344 | -0.075 |
| Points per Drive Differential | -0.036 | 0.476 | -0.513 |
| Yards per Play Differential | -0.970 | -0.024 | -0.945 |
| Turnover Margin | 0.900 | 0.900 | 0.000 |
| Red Zone TD Rate (Off) | 0.101 | 0.114 | -0.013 |
| Pressure Rate (Def) | 0.076 | 0.071 | 0.005 |
| Tempo | 6.857 | 7.727 | -0.870 |
| PowerScore | 1.535 | 1.617 | -0.083 |
| PowerScore | 1.535 | 1.617 | -0.083 |
| Tempo | 6.857 | 7.727 | -0.870 |

## PowerScore Breakdown (Model)

| Component | Weight | CHI | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.044 | -0.053 | +0.097 |
| EPA Defense | 20% | 0.141 | -0.047 | +0.188 |
| Success Rate Offense | 25% | 47.9% | 40.0% | +7.9 pp |
| Tempo | 20% | 0.083 | 0.118 | -0.034 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CHI | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.044 | -0.053 | +0.097 |
| EPA Defense | 20% | 0.141 | -0.047 | +0.188 |
| Success Rate Offense | 15% | 47.9% | 40.0% | +7.9 pp |
| Turnover Margin | 10% | 2.000 | 2.000 | +0.000 |
| Tempo | 10% | 0.083 | 0.118 | -0.034 |
| Red Zone TD Rate (Off) | 10% | 18.8% | 10.0% | +8.8 pp |
| Pressure Rate (Def) | 10% | 7.9% | 3.3% | +4.6 pp |

## PowerScore Summary

**Model (4 metrics):**
CHI edge: +0.085 (CHI +0.180 vs PIT +0.096)
**7 metrics version:**
CHI edge: +0.084 (CHI +0.346 vs PIT +0.262)
**Verdict:** CHI holds the edge in the extended breakdown (lead 0.084), driven by EPA Defense (20% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CHI - PIT) | -1.5 pts (favours PIT) |
| Model Win% CHI | 43.7% |
| Model Win% PIT | 56.3% |
| SOS Differential (CHI-PIT) | -0.070 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CHI | 55.2% | 45.1% | 10.1% | 46.7% | 45.5% | Pass heavy |
| PIT | 40.0% | 45.1% | -5.1% | 52.6% | 48.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -3.9 pp | -2.9 pp | -4.3 pp |
| PIT | +3.6 pp | -0.5 pp | +3.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -2.4 pp | -9.2 pp | -16.7 pp |
| PIT | +4.0 pp | +1.0 pp | -5.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -3.5 pp | -7.2 pp | -7.1 pp |
| PIT | -1.1 pp | -1.4 pp | -2.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -1.8 pp | -0.3 pp | -2.2 pp |
| PIT | -2.6 pp | -1.9 pp | -5.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -2.0 pp | -3.4 pp | -12.0 pp |
| PIT | +1.6 pp | +1.8 pp | -3.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -4.1 pp | -3.8 pp | -2.9 pp |
| PIT | -2.2 pp | -11.4 pp | -12.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -1.8 pp | -0.3 pp | -2.2 pp |
| PIT | -2.6 pp | -1.9 pp | -5.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -3.5 pp | -7.2 pp | -7.1 pp |
| PIT | -1.1 pp | -1.4 pp | -2.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 45.3 | 43.9 | 44.0 |
| PIT | 45.6 | 48.8 | 47.9 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 44.9 | 45.0 | 47.3 |
| PIT | 45.9 | 48.7 | 47.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 0.4 | -1.1 | -3.4 |
| PIT | -0.4 | 0.0 | 0.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 1.70 | 1.65 | 1.13 |
| PIT | 2.00 | 2.26 | 1.68 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 1.74 | 1.98 | 2.30 |
| PIT | 1.53 | 1.62 | 1.89 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | -0.04 | -0.32 | -1.17 |
| PIT | 0.48 | 0.65 | -0.20 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 7.84 | 8.21 | 8.56 |
| PIT | 7.57 | 7.36 | 7.18 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 44.2% | 44.0% | 47.2% |
| PIT | 37.4% | 37.8% | 39.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 55.8% | 56.0% | 52.8% |
| PIT | 62.6% | 62.2% | 60.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 3.46 | 3.61 | 4.04 |
| PIT | 2.83 | 2.79 | 2.82 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CHI | 4.38 | 4.60 | 4.52 |
| PIT | 4.74 | 4.58 | 4.36 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | CHI | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.461 | 1.530 |
| Last 5 games | 1.576 | 1.468 |
| Last 3 games | 1.550 | 1.510 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W10 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CHI | -0.081 | -0.297 | 0.134 | + improving |
| Off EPA PIT | 0.051 | 0.056 | -0.102 | - declining |
| Def EPA CHI | -0.006 | -0.036 | 0.262 | - declining |
| Def EPA PIT | -0.027 | 0.029 | -0.165 | + improving |
| Off SR CHI | 43.2% | 35.0% | 48.8% | + improving |
| Off SR PIT | 49.4% | 40.2% | 40.8% | - declining |
| Def SR CHI | 41.3% | 48.1% | 53.6% | - declining |
| Def SR PIT | 43.5% | 54.3% | 41.8% | + improving |
| Tempo CHI | 7.364 | 7.273 | 10.750 | + improving |
| Tempo PIT | 7.083 | 6.929 | 7.538 | + improving |

## Matchup Analogs

**CHI analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 11 | MIN (A) | 19-23 | MIN | 0.933 | 0.134 | 48.8% | -0.482 |
| 7 | NO (H) | n/a | n/a | 0.781 | n/a | n/a | n/a |
| 2 | DET (A) | 13-19 | DET | 0.750 | -0.130 | 40.0% | -0.462 |

**PIT analogs vs CHI profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | CLE (H) | 32-13 | PIT | 0.962 | 0.083 | 44.0% | 1.727 |
| 10 | LAC (A) | 28-32 | LAC | 0.806 | 0.056 | 40.2% | -0.250 |
| 9 | IND (H) | n/a | n/a | 0.801 | n/a | n/a | n/a |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.041 | -0.031 | -0.113 |
| PIT | 0.027 | 0.053 | 0.002 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.034 | 0.055 | 0.090 |
| PIT | -0.090 | -0.110 | -0.055 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 42.7% | 42.2% | 38.4% |
| PIT | 44.9% | 43.8% | 43.2% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 44.6% | 46.8% | 50.5% |
| PIT | 45.9% | 45.9% | 46.4% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 8.2% | 9.5% | 7.4% |
| PIT | 9.7% | 10.7% | 9.3% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 27.6% | 25.8% | 27.4% |
| PIT | 34.4% | 33.3% | 35.1% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.059 | -0.384 | -1.249 |
| PIT | 0.435 | 0.567 | -0.197 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.970 | -1.371 | -2.180 |
| PIT | -0.024 | 0.079 | -0.038 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 0.900 | 0.800 | 1.333 |
| PIT | 0.900 | 1.000 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 10.1% | 9.5% | 3.3% |
| PIT | 11.4% | 9.1% | 5.9% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 7.6% | 7.3% | 7.3% |
| PIT | 7.1% | 5.5% | 7.6% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 7.838 | 8.211 | 8.563 |
| PIT | 7.570 | 7.365 | 7.183 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 44.5% | 44.2% | 47.3% |
| PIT | 37.5% | 37.9% | 39.3% |
