# Matchup Report - LA vs TB

## Metric Comparison

| Metric | LA | TB | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.015 | 0.211 | ↓ -0.226 |
| Core EPA Defense | 0.275 | -0.177 | ↑ +0.452 |
| Success Rate Offense | 50.6% | 50.0% | ↑ +0.6 pp |
| Success Rate Defense | 51.7% | 42.5% | ↑ +9.2 pp |
| Explosive Play Rate (Off) | 10.1% | 13.2% | ↓ -3.0 pp |
| Third Down Conversion | 0.0% | 42.9% | ↓ -42.9 pp |
| Points per Drive Differential | -1.700 | 2.633 | ↓ -4.333 |
| Yards per Play Differential | -1.734 | 2.578 | ↓ -4.312 |
| Turnover Margin | -1.000 | 0.000 | ↓ -1.000 |
| Red Zone TD Rate (Off) | 18.2% | 25.0% | ↓ -6.8 pp |
| Pressure Rate (Def) | 4.0% | 10.8% | ↓ -6.8 pp |
| Tempo | 7.900 | 8.444 | ↓ -0.544 |
| PowerScore | 0.197 | 0.190 | ↑ +0.007 |

## PowerScore Breakdown (Model)

| Component | Weight | LA | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.015 | 0.211 | -0.226 |
| EPA Defense | 20% | 0.275 | -0.177 | +0.452 |
| Success Rate Offense | 25% | 50.6% | 50.0% | +0.6 pp |
| Tempo | 20% | 0.101 | 0.132 | -0.030 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LA | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.015 | 0.211 | -0.226 |
| EPA Defense | 20% | 0.275 | -0.177 | +0.452 |
| Success Rate Offense | 15% | 50.6% | 50.0% | +0.6 pp |
| Turnover Margin | 10% | -1.000 | 0.000 | -1.000 |
| Tempo | 10% | 0.101 | 0.132 | -0.030 |
| Red Zone TD Rate (Off) | 10% | 18.2% | 25.0% | -6.8 pp |
| Pressure Rate (Def) | 10% | 4.0% | 10.8% | -6.8 pp |

## PowerScore Summary

**Model (4 metrics):**
LA edge: +0.007 (LA +0.197 vs TB +0.190)
**7 metrics version:**
TB edge: +0.082 (LA +0.060 vs TB +0.141)
**Verdict:** TB holds the edge in the extended breakdown (lead 0.082), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LA - TB) | +0.1 pts (favours LA) |
| Model Win% LA | 50.5% |
| Model Win% TB | 49.5% |
| SOS Differential (LA-TB) | -0.039 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LA | 53.2% | 45.1% | 8.0% | 37.8% | 47.2% | Pass heavy |
| TB | 39.5% | 45.1% | -5.6% | 48.0% | 53.1% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -9.5 pp | -21.3 pp | -30.3 pp |
| TB | +10.7 pp | +18.6 pp | +8.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -4.7 pp | -3.4 pp | -5.3 pp |
| TB | +2.5 pp | +4.6 pp | +3.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | +1.5 pp | +6.0 pp | +4.8 pp |
| TB | +0.8 pp | +5.0 pp | +6.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -0.7 pp | -0.6 pp | +0.1 pp |
| TB | +1.7 pp | +1.6 pp | -3.1 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -3.2 pp | -7.9 pp | -9.5 pp |
| TB | +2.5 pp | +8.2 pp | +6.5 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -2.3 pp | -5.6 pp | -7.3 pp |
| TB | +1.0 pp | -0.0 pp | -4.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -0.7 pp | -0.6 pp | +0.1 pp |
| TB | +1.7 pp | +1.6 pp | -3.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | +1.5 pp | +6.0 pp | +4.8 pp |
| TB | +0.8 pp | +5.0 pp | +6.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 49.2 | 50.5 | 49.8 |
| TB | 49.3 | 51.1 | 51.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 45.8 | 44.3 | 45.5 |
| TB | 51.2 | 50.7 | 47.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 3.3 | 6.3 | 4.3 |
| TB | -1.9 | 0.3 | 3.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 1.88 | 1.94 | 1.83 |
| TB | 2.54 | 2.53 | 2.22 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 2.31 | 1.83 | 1.93 |
| TB | 2.37 | 2.61 | 2.63 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | -0.43 | 0.10 | -0.10 |
| TB | 0.17 | -0.08 | -0.41 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 7.76 | 7.35 | 7.03 |
| TB | 7.51 | 7.67 | 7.85 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 46.4% | 44.9% | 49.3% |
| TB | 45.8% | 46.1% | 46.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 53.6% | 55.1% | 50.7% |
| TB | 54.2% | 53.9% | 53.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 3.60 | 3.30 | 3.46 |
| TB | 3.44 | 3.54 | 3.68 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LA | 4.16 | 4.05 | 3.57 |
| TB | 4.07 | 4.13 | 4.17 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | LA | TB |
| --- | ---: | ---: |
| Season-to-date | 0.148 | 0.187 |
| Last 5 games | 0.102 | 0.204 |
| Last 3 games | 0.117 | 0.227 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LA | 0.146 | -0.040 | -0.199 | - declining |
| Off EPA TB | 0.040 | 0.133 | 0.058 | + improving |
| Def EPA LA | 0.094 | -0.153 | -0.065 | + improving |
| Def EPA TB | 0.159 | 0.140 | 0.133 | + improving |
| Off SR LA | 49.5% | 41.5% | 48.3% | - declining |
| Off SR TB | 50.0% | 44.4% | 49.4% | = stable |
| Def SR LA | 49.2% | 43.1% | 44.1% | + improving |
| Def SR TB | 57.9% | 53.5% | 57.5% | = stable |
| Tempo LA | 10.111 | 6.267 | 7.909 | - declining |
| Tempo TB | 8.364 | 8.000 | 7.182 | - declining |

## Matchup Analogs

**LA analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | SF (H) | 13-22 | SF | 0.939 | -0.074 | 53.8% | -0.882 |
| 10 | SF (A) | 15-23 | SF | 0.939 | -0.199 | 48.3% | -0.727 |
| 6 | BAL (H) | n/a | n/a | 0.836 | n/a | n/a | n/a |

**TB analogs vs LA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | SEA (A) | 30-30 | TIE | 0.749 | 0.202 | 55.2% | 0.000 |
| 10 | NE (H) | 20-25 | NE | 0.699 | 0.058 | 49.4% | -0.482 |
| 6 | SF (A) | 51-21 | TB | 0.636 | 0.072 | 53.6% | 2.000 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | -0.017 | -0.041 | -0.010 |
| TB | 0.077 | 0.078 | 0.077 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 0.030 | -0.063 | -0.049 |
| TB | 0.064 | 0.095 | 0.144 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 49.3% | 47.0% | 47.2% |
| TB | 50.5% | 50.2% | 48.1% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 48.3% | 44.7% | 46.0% |
| TB | 51.4% | 53.1% | 56.0% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 9.5% | 10.2% | 10.0% |
| TB | 12.0% | 11.6% | 7.8% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 30.4% | 22.6% | 21.1% |
| TB | 45.5% | 50.6% | 45.1% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | -0.423 | 0.066 | -0.089 |
| TB | 0.194 | 0.041 | -0.448 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | -0.095 | 0.425 | 0.600 |
| TB | -0.199 | -0.355 | -0.929 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 0.400 | 1.200 | 1.333 |
| TB | -0.400 | -0.800 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 11.5% | 13.0% | 10.0% |
| TB | 17.7% | 19.3% | 22.6% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 8.0% | 9.6% | 11.1% |
| TB | 7.1% | 8.3% | 7.4% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 7.765 | 7.345 | 7.025 |
| TB | 7.507 | 7.669 | 7.848 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LA | 46.8% | 45.3% | 49.6% |
| TB | 46.2% | 46.4% | 47.3% |
