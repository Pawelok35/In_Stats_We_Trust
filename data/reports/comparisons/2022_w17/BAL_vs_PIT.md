# Matchup Report - BAL vs PIT

## Metric Comparison

| Metric | BAL | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.027 | -0.035 | 0.062 |
| Core EPA Defense | -0.035 | 0.017 | -0.052 |
| Success Rate Offense | 0.442 | 0.428 | 0.014 |
| Success Rate Defense | 0.430 | 0.433 | -0.003 |
| Explosive Play Rate (Off) | 0.119 | 0.080 | 0.039 |
| Third Down Conversion | 0.388 | 0.377 | 0.012 |
| Points per Drive Differential | 0.204 | -0.312 | 0.516 |
| Yards per Play Differential | 0.175 | -0.485 | 0.660 |
| Turnover Margin | 0.400 | 0.267 | 0.133 |
| Red Zone TD Rate (Off) | 0.109 | 0.144 | -0.035 |
| Pressure Rate (Def) | 0.080 | 0.078 | 0.003 |
| Tempo | 8.375 | 9.333 | -0.958 |
| PowerScore | 1.818 | 2.042 | -0.224 |
| PowerScore | 1.818 | 2.042 | -0.224 |
| Tempo | 8.375 | 9.333 | -0.958 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.040 | 0.105 | -0.065 |
| EPA Defense | 20% | 0.105 | 0.040 | +0.065 |
| Success Rate Offense | 25% | 43.3% | 52.4% | -9.1 pp |
| Tempo | 20% | 0.119 | 0.083 | +0.036 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.040 | 0.105 | -0.065 |
| EPA Defense | 20% | 0.105 | 0.040 | +0.065 |
| Success Rate Offense | 15% | 43.3% | 52.4% | -9.1 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.119 | 0.083 | +0.036 |
| Red Zone TD Rate (Off) | 10% | 12.5% | 5.6% | +6.9 pp |
| Pressure Rate (Def) | 10% | 6.5% | 4.5% | +1.9 pp |

## PowerScore Summary

**Model (4 metrics):**
PIT edge: +0.025 (BAL +0.167 vs PIT +0.192)
**7 metrics version:**
PIT edge: +0.204 (BAL +0.027 vs PIT +0.231)
**Verdict:** PIT holds the edge in the extended breakdown (lead 0.204), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - PIT) | -4.1 pts (favours PIT) |
| Model Win% BAL | 33.4% |
| Model Win% PIT | 66.6% |
| SOS Differential (BAL-PIT) | +0.059 |
*Neutral-field assumption. Based on schedule through Week 16.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 32.8% | 43.1% | -10.2% | 50.0% | 40.0% | Run heavy |
| PIT | 36.9% | 43.1% | -6.2% | 38.7% | 60.4% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.8 pp | +2.5 pp | -4.8 pp |
| PIT | +6.4 pp | +17.4 pp | +12.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -4.6 pp | -6.7 pp | -9.4 pp |
| PIT | +4.6 pp | +9.0 pp | +10.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +0.5 pp | +2.9 pp | +4.9 pp |
| PIT | +1.2 pp | +3.8 pp | +3.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.3 pp | +1.6 pp | +2.9 pp |
| PIT | -1.3 pp | -1.5 pp | +0.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +3.4 pp | +2.1 pp | +9.2 pp |
| PIT | +0.8 pp | +6.6 pp | -1.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -2.7 pp | -5.0 pp | -6.5 pp |
| PIT | -2.0 pp | +1.7 pp | +3.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.3 pp | +1.6 pp | +2.9 pp |
| PIT | -1.3 pp | -1.5 pp | +0.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +0.5 pp | +2.9 pp | +4.9 pp |
| PIT | +1.2 pp | +3.8 pp | +3.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 45.9 | 43.0 | 41.6 |
| PIT | 45.5 | 45.6 | 44.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 47.7 | 47.1 | 46.9 |
| PIT | 45.0 | 45.9 | 43.3 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.9 | -4.1 | -5.3 |
| PIT | 0.5 | -0.3 | 1.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.98 | 1.42 | 1.27 |
| PIT | 1.61 | 1.98 | 1.80 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.78 | 1.47 | 1.33 |
| PIT | 1.92 | 1.66 | 1.53 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.20 | -0.05 | -0.06 |
| PIT | -0.31 | 0.32 | 0.27 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 7.48 | 7.62 | 7.58 |
| PIT | 7.85 | 8.44 | 8.12 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 39.5% | 38.6% | 35.6% |
| PIT | 44.1% | 39.6% | 41.5% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 60.5% | 61.4% | 64.4% |
| PIT | 55.9% | 60.4% | 58.5% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.96 | 2.94 | 2.70 |
| PIT | 3.46 | 3.34 | 3.37 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.52 | 4.67 | 4.88 |
| PIT | 4.39 | 5.10 | 4.75 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 16)

| Window | BAL | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.647 | 1.588 |
| Last 5 games | 1.633 | 1.635 |
| Last 3 games | 1.761 | 1.532 |

## Trend Summary (last 3 weeks)

| Metric | W14 | W15 | W16 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BAL | 0.041 | -0.236 | -0.045 | - declining |
| Off EPA PIT | -0.043 | 0.171 | -0.105 | - declining |
| Def EPA BAL | -0.043 | -0.083 | -0.140 | + improving |
| Def EPA PIT | 0.041 | 0.052 | -0.169 | + improving |
| Off SR BAL | 42.5% | 43.2% | 40.0% | - declining |
| Off SR PIT | 46.4% | 50.0% | 40.7% | - declining |
| Def SR BAL | 46.4% | 48.7% | 39.0% | + improving |
| Def SR PIT | 42.5% | 35.5% | 37.9% | + improving |
| Tempo BAL | 7.300 | 8.222 | 7.222 | - declining |
| Tempo PIT | 7.667 | 9.333 | 7.364 | - declining |

## Matchup Analogs

**BAL analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 14 | PIT (A) | 16-16 | TIE | 1.000 | 0.041 | 42.5% | 0.044 |
| 8 | TB (A) | 27-26 | BAL | 0.744 | 0.158 | 51.1% | 0.050 |
| 6 | NYG (A) | 20-22 | NYG | 0.730 | 0.007 | 42.1% | -0.178 |

**PIT analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 14 | BAL (H) | 14-14 | TIE | 1.000 | -0.043 | 46.4% | -0.044 |
| 5 | BUF (A) | 3-33 | BUF | 0.942 | -0.270 | 37.4% | -2.464 |
| 8 | PHI (A) | 13-35 | PHI | 0.941 | -0.044 | 41.5% | -2.200 |

## Core EPA Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.027 | -0.051 | -0.080 |
| PIT | -0.035 | 0.040 | 0.008 |

## Core EPA Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.035 | -0.063 | -0.089 |
| PIT | 0.017 | -0.021 | -0.025 |

## Success Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 44.3% | 42.0% | 42.0% |
| PIT | 42.7% | 46.9% | 45.7% |

## Success Rate Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 43.0% | 42.3% | 44.5% |
| PIT | 43.3% | 42.7% | 38.8% |

## Explosive Play Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 11.8% | 10.1% | 11.3% |
| PIT | 8.0% | 7.4% | 8.1% |

## Third Down Conversion Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 39.0% | 35.1% | 33.3% |
| PIT | 37.4% | 48.1% | 48.1% |

## Points per Drive Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.215 | -0.047 | -0.056 |
| PIT | -0.351 | 0.325 | 0.262 |

## Yards per Play Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.175 | 0.142 | 0.286 |
| PIT | -0.485 | 0.444 | 0.770 |

## Turnover Margin Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.400 | -0.200 | 0.667 |
| PIT | 0.267 | 0.400 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 10.9% | 6.8% | 5.1% |
| PIT | 14.4% | 16.1% | 18.6% |

## Pressure Rate (Def) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 8.0% | 7.7% | 6.9% |
| PIT | 7.8% | 9.8% | 12.3% |

## Tempo Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.476 | 7.616 | 7.581 |
| PIT | 7.845 | 8.442 | 8.121 |

## Pass Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 39.7% | 39.2% | 35.8% |
| PIT | 44.1% | 39.2% | 41.0% |
