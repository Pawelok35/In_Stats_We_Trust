# Matchup Report - LV vs JAX

## Metric Comparison

| Metric | LV | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.141 | 0.000 | -0.141 |
| Core EPA Defense | 0.034 | 0.007 | 0.026 |
| Success Rate Offense | 0.431 | 0.455 | -0.024 |
| Success Rate Defense | 0.472 | 0.472 | 0.000 |
| Explosive Play Rate (Off) | 0.088 | 0.085 | 0.003 |
| Third Down Conversion | 0.305 | 0.311 | -0.006 |
| Points per Drive Differential | -1.129 | -0.352 | -0.777 |
| Yards per Play Differential | -0.396 | -0.348 | -0.048 |
| Turnover Margin | -0.714 | 1.143 | -1.857 |
| Red Zone TD Rate (Off) | 0.092 | 0.103 | -0.010 |
| Pressure Rate (Def) | 0.057 | 0.028 | 0.029 |
| Tempo | 7.900 | 10.778 | -2.878 |
| PowerScore | 1.808 | 2.380 | -0.572 |
| PowerScore | 1.808 | 2.380 | -0.572 |
| Tempo | 7.900 | 10.778 | -2.878 |

## PowerScore Breakdown (Model)

| Component | Weight | LV | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.165 | 0.156 | +0.009 |
| EPA Defense | 20% | 0.156 | 0.165 | -0.009 |
| Success Rate Offense | 25% | 55.7% | 54.6% | +1.1 pp |
| Tempo | 20% | 0.076 | 0.124 | -0.048 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LV | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.165 | 0.156 | +0.009 |
| EPA Defense | 20% | 0.156 | 0.165 | -0.009 |
| Success Rate Offense | 15% | 55.7% | 54.6% | +1.1 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.076 | 0.124 | -0.048 |
| Red Zone TD Rate (Off) | 10% | 16.7% | 10.0% | +6.7 pp |
| Pressure Rate (Def) | 10% | 2.9% | 4.8% | -1.9 pp |

## PowerScore Summary

**Model (4 metrics):**
JAX edge: +0.006 (LV +0.243 vs JAX +0.249)
**7 metrics version:**
LV edge: +0.002 (LV +0.183 vs JAX +0.181)
**Verdict:** LV holds the edge in the extended breakdown (lead 0.002), driven by Red Zone TD Rate (Off) (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LV - JAX) | -10.6 pts (favours JAX) |
| Model Win% LV | 14.7% |
| Model Win% JAX | 85.3% |
| SOS Differential (LV-JAX) | +0.168 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LV | 53.2% | 45.1% | 8.1% | 61.9% | 48.6% | Pass heavy |
| JAX | 36.1% | 45.1% | -9.0% | 45.7% | 59.7% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -7.0 pp | -7.0 pp | +3.8 pp |
| JAX | -8.8 pp | -18.2 pp | -18.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -1.9 pp | -3.3 pp | -14.4 pp |
| JAX | -2.2 pp | -4.4 pp | -11.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -5.4 pp | -5.9 pp | -7.6 pp |
| JAX | -0.6 pp | -3.3 pp | -6.3 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.2 pp | -1.7 pp | -6.3 pp |
| JAX | -0.3 pp | -1.2 pp | -0.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -3.6 pp | -1.1 pp | -4.3 pp |
| JAX | +2.3 pp | +1.5 pp | +7.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -4.4 pp | -6.7 pp | -16.5 pp |
| JAX | -6.2 pp | -9.4 pp | -9.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.2 pp | -1.7 pp | -6.3 pp |
| JAX | -0.3 pp | -1.2 pp | -0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -5.4 pp | -5.9 pp | -7.6 pp |
| JAX | -0.6 pp | -3.3 pp | -6.3 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 46.4 | 48.3 | 48.6 |
| JAX | 45.0 | 44.5 | 44.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 48.9 | 50.4 | 45.4 |
| JAX | 43.9 | 41.0 | 39.4 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -2.5 | -2.1 | 3.2 |
| JAX | 1.2 | 3.5 | 4.8 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 1.39 | 1.43 | 0.83 |
| JAX | 1.71 | 1.42 | 1.38 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 2.52 | 2.93 | 3.15 |
| JAX | 2.06 | 2.12 | 2.46 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -1.13 | -1.49 | -2.32 |
| JAX | -0.35 | -0.70 | -1.08 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 6.99 | 6.80 | 7.00 |
| JAX | 7.61 | 7.55 | 7.83 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 43.1% | 39.5% | 41.4% |
| JAX | 46.9% | 47.8% | 50.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 56.9% | 60.5% | 58.6% |
| JAX | 53.1% | 52.2% | 49.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 3.02 | 2.69 | 2.90 |
| JAX | 3.57 | 3.61 | 3.95 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 3.98 | 4.11 | 4.10 |
| JAX | 4.05 | 3.94 | 3.88 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | LV | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.713 | 1.545 |
| Last 5 games | 1.780 | 1.506 |
| Last 3 games | 1.930 | 1.605 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W6 | W7 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LV | -0.223 | -0.068 | -0.332 | - declining |
| Off EPA JAX | 0.162 | -0.087 | -0.114 | - declining |
| Def EPA LV | 0.242 | -0.216 | 0.194 | + improving |
| Def EPA JAX | 0.128 | 0.039 | 0.245 | - declining |
| Off SR LV | 50.0% | 43.6% | 33.3% | - declining |
| Off SR JAX | 56.2% | 37.2% | 45.7% | - declining |
| Def SR LV | 50.7% | 40.8% | 50.0% | = stable |
| Def SR JAX | 61.0% | 43.1% | 51.7% | + improving |
| Tempo LV | 9.111 | 7.091 | 4.800 | - declining |
| Tempo JAX | 7.300 | 7.833 | 8.364 | + improving |

## Matchup Analogs

**LV analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | TEN (H) | 20-11 | LV | 0.772 | -0.068 | 43.6% | 0.818 |
| 4 | CHI (H) | 24-25 | CHI | 0.504 | -0.172 | 54.5% | -0.091 |
| 1 | NE (A) | 20-14 | LV | 0.437 | 0.081 | 44.4% | 0.485 |

**JAX analogs vs LV profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CAR (H) | 26-10 | JAX | 0.737 | 0.116 | 47.6% | 1.600 |
| 2 | CIN (A) | 27-34 | CIN | 0.601 | 0.075 | 50.0% | -0.568 |
| 3 | HOU (H) | 17-10 | JAX | 0.549 | -0.122 | 37.1% | 0.538 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.000 | -0.038 | -0.013 |
| LV | -0.141 | -0.163 | -0.208 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.007 | 0.016 | 0.137 |
| LV | 0.034 | 0.074 | 0.074 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 45.1% | 43.7% | 45.6% |
| LV | 43.4% | 44.7% | 43.8% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 47.5% | 48.1% | 52.3% |
| LV | 47.2% | 48.0% | 47.3% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 8.4% | 7.8% | 8.1% |
| LV | 9.2% | 8.7% | 4.8% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 30.5% | 25.3% | 26.9% |
| LV | 32.6% | 29.5% | 34.3% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.367 | -0.664 | -1.093 |
| LV | -0.984 | -1.329 | -2.148 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.348 | -0.804 | -0.853 |
| LV | -0.396 | -0.415 | -1.115 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 1.143 | 1.000 | -0.333 |
| LV | -0.714 | -0.600 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 10.3% | 9.4% | 5.0% |
| LV | 9.2% | 8.9% | 3.7% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 2.8% | 2.1% | 1.1% |
| LV | 5.7% | 5.1% | 6.2% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 7.613 | 7.552 | 7.832 |
| LV | 6.993 | 6.800 | 7.001 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 47.3% | 48.3% | 51.4% |
| LV | 43.8% | 39.9% | 41.8% |
