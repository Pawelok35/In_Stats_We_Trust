# Matchup Report - BUF vs KC

## Metric Comparison

| Metric | BUF | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.134 | 0.140 | -0.007 |
| Core EPA Defense | 0.017 | -0.016 | 0.034 |
| Success Rate Offense | 0.495 | 0.502 | -0.007 |
| Success Rate Defense | 0.488 | 0.480 | 0.007 |
| Explosive Play Rate (Off) | 0.123 | 0.119 | 0.005 |
| Third Down Conversion | 0.394 | 0.396 | -0.002 |
| Points per Drive Differential | 0.776 | 1.153 | -0.378 |
| Yards per Play Differential | 0.553 | 0.746 | -0.193 |
| Turnover Margin | 0.286 | 0.625 | -0.339 |
| Red Zone TD Rate (Off) | 0.171 | 0.178 | -0.007 |
| Pressure Rate (Def) | 0.107 | 0.069 | 0.038 |
| Tempo | 7.900 | 7.400 | 0.500 |
| PowerScore | 1.791 | 1.651 | 0.140 |
| PowerScore | 1.791 | 1.651 | 0.140 |
| Tempo | 7.900 | 7.400 | 0.500 |

## PowerScore Breakdown (Model)

| Component | Weight | BUF | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.151 | 0.066 | +0.085 |
| EPA Defense | 20% | 0.066 | 0.151 | -0.085 |
| Success Rate Offense | 25% | 58.2% | 47.3% | +10.9 pp |
| Tempo | 20% | 0.127 | 0.122 | +0.005 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BUF | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.151 | 0.066 | +0.085 |
| EPA Defense | 20% | 0.066 | 0.151 | -0.085 |
| Success Rate Offense | 15% | 58.2% | 47.3% | +10.9 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.127 | 0.122 | +0.005 |
| Red Zone TD Rate (Off) | 10% | 27.3% | 14.3% | +13.0 pp |
| Pressure Rate (Def) | 10% | 7.9% | 10.0% | -2.1 pp |

## PowerScore Summary

**Model (4 metrics):**
BUF edge: +0.041 (BUF +0.237 vs KC +0.196)
**7 metrics version:**
BUF edge: +0.232 (BUF +0.286 vs KC +0.054)
**Verdict:** BUF holds the edge in the extended breakdown (lead 0.232), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BUF - KC) | +2.6 pts (favours BUF) |
| Model Win% BUF | 60.6% |
| Model Win% KC | 39.4% |
| SOS Differential (BUF-KC) | -0.081 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BUF | 38.0% | 45.1% | -7.1% | 60.0% | 57.1% | Run heavy |
| KC | 51.4% | 45.1% | 6.3% | 34.2% | 61.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +7.1 pp | +7.8 pp | +8.8 pp |
| KC | +4.7 pp | +2.7 pp | +10.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +4.2 pp | +7.5 pp | +8.2 pp |
| KC | +4.9 pp | +8.3 pp | +10.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +0.2 pp | -1.6 pp | -3.2 pp |
| KC | +6.0 pp | +5.7 pp | +7.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.5 pp | +4.6 pp | +5.0 pp |
| KC | -0.1 pp | +1.9 pp | +2.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -0.8 pp | -1.3 pp | +2.7 pp |
| KC | -4.3 pp | -4.2 pp | -3.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +5.1 pp | +6.8 pp | +8.2 pp |
| KC | +10.1 pp | +15.1 pp | +9.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.5 pp | +4.6 pp | +5.0 pp |
| KC | -0.1 pp | +1.9 pp | +2.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +0.2 pp | -1.6 pp | -3.2 pp |
| KC | +6.0 pp | +5.7 pp | +7.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 45.1 | 44.8 | 45.5 |
| KC | 45.5 | 45.0 | 39.9 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 51.1 | 50.6 | 46.2 |
| KC | 49.6 | 50.1 | 49.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -6.0 | -5.9 | -0.8 |
| KC | -4.1 | -5.1 | -9.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.77 | 2.71 | 2.42 |
| KC | 2.89 | 3.28 | 3.30 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.00 | 1.87 | 1.71 |
| KC | 1.73 | 1.50 | 1.00 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 0.78 | 0.84 | 0.72 |
| KC | 1.15 | 1.78 | 2.30 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 7.76 | 7.58 | 7.55 |
| KC | 8.89 | 8.89 | 9.15 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 39.2% | 37.9% | 37.5% |
| KC | 47.4% | 47.0% | 46.5% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 60.8% | 62.1% | 62.5% |
| KC | 52.6% | 53.0% | 53.5% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 3.04 | 2.87 | 2.83 |
| KC | 4.21 | 4.18 | 4.26 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 4.71 | 4.71 | 4.72 |
| KC | 4.68 | 4.71 | 4.89 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | BUF | KC |
| --- | ---: | ---: |
| Season-to-date | 1.624 | 1.705 |
| Last 5 games | 1.683 | 1.574 |
| Last 3 games | 1.603 | 1.541 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W6 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BUF | 0.019 | -0.088 | 0.283 | + improving |
| Off EPA KC | 0.128 | 0.227 | 0.216 | + improving |
| Def EPA BUF | 0.069 | 0.053 | -0.210 | + improving |
| Def EPA KC | 0.162 | 0.067 | -0.108 | + improving |
| Off SR BUF | 50.6% | 49.3% | 51.9% | + improving |
| Off SR KC | 61.0% | 56.0% | 50.0% | - declining |
| Def SR BUF | 51.9% | 50.0% | 47.4% | + improving |
| Def SR KC | 56.2% | 56.5% | 46.9% | + improving |
| Tempo BUF | 8.300 | 6.455 | 7.900 | - declining |
| Tempo KC | 8.200 | 8.333 | 8.667 | + improving |

## Matchup Analogs

**BUF analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | NE (H) | 20-23 | NE | 0.821 | 0.019 | 50.6% | -0.300 |
| 1 | BAL (H) | 41-47 | BAL | 0.585 | 0.202 | 48.6% | -0.483 |
| 3 | MIA (H) | 31-21 | BUF | 0.306 | 0.221 | 54.1% | 1.111 |

**KC analogs vs BUF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | DET (H) | 30-19 | KC | 0.736 | 0.227 | 56.0% | 1.208 |
| 4 | BAL (H) | 37-20 | KC | 0.690 | 0.182 | 46.6% | 1.700 |
| 8 | WAS (H) | 28-8 | KC | 0.666 | 0.216 | 50.0% | 2.236 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.134 | 0.111 | 0.071 |
| KC | 0.140 | 0.189 | 0.212 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.017 | -0.010 | -0.030 |
| KC | -0.016 | -0.051 | -0.124 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 49.4% | 50.5% | 50.6% |
| KC | 50.2% | 52.5% | 51.8% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 48.9% | 48.6% | 49.8% |
| KC | 48.7% | 49.5% | 47.0% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 12.2% | 13.3% | 12.0% |
| KC | 11.7% | 12.5% | 13.0% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 39.5% | 42.0% | 42.8% |
| KC | 40.0% | 42.6% | 41.6% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.715 | 0.802 | 0.672 |
| KC | 1.176 | 1.745 | 2.296 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.553 | 0.648 | 0.370 |
| KC | 0.746 | 1.050 | 1.514 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.286 | 0.000 | -0.333 |
| KC | 0.625 | 0.800 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 17.1% | 18.4% | 17.6% |
| KC | 17.8% | 20.3% | 19.3% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 10.7% | 9.9% | 13.4% |
| KC | 6.9% | 6.8% | 4.9% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.756 | 7.575 | 7.552 |
| KC | 8.893 | 8.889 | 9.148 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 39.6% | 37.8% | 37.3% |
| KC | 47.3% | 46.8% | 46.2% |
