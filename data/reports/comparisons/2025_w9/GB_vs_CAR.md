# Matchup Report - GB vs CAR

## Metric Comparison

| Metric | GB | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.000 | 0.066 | ↓ -0.067 |
| Core EPA Defense | 0.066 | -0.000 | ↑ +0.067 |
| Success Rate Offense | 50.0% | 49.3% | ↑ +0.7 pp |
| Success Rate Defense | 49.3% | 50.0% | ↓ -0.7 pp |
| Explosive Play Rate (Off) | 9.2% | 8.5% | ↑ +0.8 pp |
| Third Down Conversion | 50.0% | 33.3% | ↑ +16.7 pp |
| Points per Drive Differential | -0.661 | 0.661 | ↓ -1.321 |
| Yards per Play Differential | 1.123 | -1.123 | ↑ +2.246 |
| Turnover Margin | -1.000 | 1.000 | ↓ -2.000 |
| Red Zone TD Rate (Off) | 5.6% | 11.8% | ↓ -6.2 pp |
| Pressure Rate (Def) | 4.8% | 2.6% | ↑ +2.1 pp |
| Tempo | 9.500 | 10.143 | ↓ -0.643 |
| PowerScore | 0.157 | 0.163 | ↓ -0.007 |

## PowerScore Breakdown (Model)

| Component | Weight | GB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.000 | 0.066 | -0.067 |
| EPA Defense | 20% | 0.066 | -0.000 | +0.067 |
| Success Rate Offense | 25% | 50.0% | 49.3% | +0.7 pp |
| Tempo | 20% | 0.092 | 0.085 | +0.008 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | GB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.000 | 0.066 | -0.067 |
| EPA Defense | 20% | 0.066 | -0.000 | +0.067 |
| Success Rate Offense | 15% | 50.0% | 49.3% | +0.7 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.092 | 0.085 | +0.008 |
| Red Zone TD Rate (Off) | 10% | 5.6% | 11.8% | -6.2 pp |
| Pressure Rate (Def) | 10% | 4.8% | 2.6% | +2.1 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.007 (GB +0.157 vs CAR +0.163)
**7 metrics version:**
CAR edge: +0.206 (GB +0.008 vs CAR +0.213)
**Verdict:** CAR holds the edge in the extended breakdown (lead 0.206), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (GB - CAR) | -0.1 pts (favours CAR) |
| Model Win% GB | 49.5% |
| Model Win% CAR | 50.5% |
| SOS Differential (GB-CAR) | +0.024 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| GB | 50.0% | 45.1% | 4.9% | 44.7% | 55.3% | Pass heavy |
| CAR | 29.6% | 45.1% | -15.5% | 47.6% | 50.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +0.4 pp | +2.2 pp | -3.3 pp |
| CAR | +1.7 pp | +1.9 pp | -0.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +9.0 pp | +9.7 pp | +14.2 pp |
| CAR | -2.6 pp | +0.0 pp | -4.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +2.6 pp | +4.8 pp | +5.4 pp |
| CAR | -2.7 pp | -3.2 pp | -1.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +5.4 pp | +5.1 pp | +6.0 pp |
| CAR | +2.3 pp | +4.3 pp | +3.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +10.4 pp | +10.4 pp | +10.1 pp |
| CAR | -6.2 pp | -6.2 pp | -5.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +8.5 pp | +8.5 pp | +9.0 pp |
| CAR | +0.3 pp | +0.3 pp | +0.3 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +5.4 pp | +5.1 pp | +6.0 pp |
| CAR | +2.3 pp | +4.3 pp | +3.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | +2.6 pp | +4.8 pp | +5.4 pp |
| CAR | -2.7 pp | -3.2 pp | -1.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 49.8 | 49.8 | 49.8 |
| CAR | 45.0 | 45.0 | 47.0 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 51.1 | 51.1 | 51.1 |
| CAR | 49.1 | 49.1 | 48.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -1.3 | -1.3 | -1.3 |
| CAR | -4.0 | -4.0 | -1.8 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 2.67 | 2.69 | 2.87 |
| CAR | 1.85 | 1.84 | 1.67 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 2.06 | 2.29 | 2.28 |
| CAR | 2.24 | 2.64 | 2.52 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 0.61 | 0.39 | 0.60 |
| CAR | -0.39 | -0.81 | -0.85 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 7.77 | 8.10 | 7.54 |
| CAR | 8.08 | 7.97 | 7.92 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 42.1% | 42.1% | 42.1% |
| CAR | 37.9% | 37.9% | 37.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 57.9% | 57.9% | 57.9% |
| CAR | 62.1% | 62.1% | 62.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 3.27 | 3.41 | 3.17 |
| CAR | 3.06 | 3.02 | 2.98 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 4.50 | 4.69 | 4.37 |
| CAR | 5.02 | 4.95 | 4.95 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | GB | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.162 | 0.139 |
| Last 5 games | 0.180 | 0.163 |
| Last 3 games | 0.187 | 0.147 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA GB | 0.216 | 0.123 | 0.169 | - declining |
| Off EPA CAR | 0.153 | -0.053 | -0.210 | - declining |
| Def EPA GB | 0.096 | 0.039 | 0.017 | + improving |
| Def EPA CAR | 0.177 | -0.195 | 0.283 | - declining |
| Off SR GB | 58.7% | 53.4% | 48.8% | - declining |
| Off SR CAR | 56.5% | 40.0% | 47.4% | - declining |
| Def SR GB | 53.1% | 48.9% | 43.8% | + improving |
| Def SR CAR | 45.2% | 35.1% | 51.9% | - declining |
| Tempo GB | 8.333 | 7.300 | 7.000 | - declining |
| Tempo CAR | 8.500 | 8.182 | 7.091 | - declining |

## Matchup Analogs

**GB analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | CLE (-) | 10-12 | CLE | 0.741 | -0.094 | 39.8% | -0.182 |
| 2 | WAS (-) | 27-20 | GB | 0.643 | 0.046 | 48.8% | 0.614 |
| 6 | CIN (H) | 27-18 | GB | 0.494 | 0.216 | 58.7% | 1.000 |

**CAR analogs vs GB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | BUF (H) | 9-44 | BUF | 0.953 | -0.210 | 47.4% | -3.182 |
| 4 | NE (-) | 19-36 | NE | 0.703 | -0.179 | 47.1% | -1.545 |
| 6 | DAL (H) | 30-30 | TIE | 0.695 | 0.153 | 56.5% | 0.000 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.028 | -0.035 | -0.037 |
| GB | 0.127 | 0.132 | 0.169 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.051 | 0.126 | 0.088 |
| GB | 0.024 | 0.058 | 0.051 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 46.6% | 47.2% | 47.8% |
| GB | 50.1% | 50.5% | 53.4% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.3% | 44.0% | 44.2% |
| GB | 46.6% | 47.7% | 48.6% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.3% | 11.2% | 10.7% |
| GB | 12.3% | 12.1% | 12.9% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 32.3% | 32.6% | 30.3% |
| GB | 42.8% | 44.6% | 39.2% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.421 | -0.822 | -0.875 |
| GB | 0.589 | 0.391 | 0.586 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.441 | -0.112 | -0.285 |
| GB | 0.988 | 0.704 | 1.233 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.375 | -0.800 | -0.667 |
| GB | 0.143 | 0.000 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 12.5% | 15.2% | 10.5% |
| GB | 18.5% | 19.2% | 21.7% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 4.9% | 7.0% | 7.7% |
| GB | 7.4% | 6.8% | 8.8% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 8.083 | 7.973 | 7.924 |
| GB | 7.775 | 8.096 | 7.544 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 37.9% | 37.9% | 37.5% |
| GB | 42.2% | 42.2% | 42.2% |
