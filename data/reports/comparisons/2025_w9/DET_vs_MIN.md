# Matchup Report - DET vs MIN

## Metric Comparison

| Metric | DET | MIN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.087 | -0.081 | 0.168 |
| Core EPA Defense | -0.035 | -0.035 | -0.000 |
| Success Rate Offense | 0.501 | 0.455 | 0.045 |
| Success Rate Defense | 0.448 | 0.449 | -0.000 |
| Explosive Play Rate (Off) | 0.095 | 0.099 | -0.003 |
| Third Down Conversion | 0.363 | 0.296 | 0.066 |
| Points per Drive Differential | 0.463 | -0.218 | 0.681 |
| Yards per Play Differential | 0.604 | -0.206 | 0.810 |
| Turnover Margin | 0.857 | -0.286 | 1.143 |
| Red Zone TD Rate (Off) | 0.167 | 0.130 | 0.037 |
| Pressure Rate (Def) | 0.083 | 0.089 | -0.006 |
| Tempo | 7.636 | 6.750 | 0.886 |
| PowerScore | 1.619 | 1.448 | 0.171 |
| PowerScore | 1.619 | 1.448 | 0.171 |
| Tempo | 7.636 | 6.750 | 0.886 |

## PowerScore Breakdown (Model)

| Component | Weight | DET | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.062 | 0.017 | -0.079 |
| EPA Defense | 20% | 0.017 | -0.062 | +0.079 |
| Success Rate Offense | 25% | 44.0% | 42.0% | +2.1 pp |
| Tempo | 20% | 0.119 | 0.111 | +0.008 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | DET | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.062 | 0.017 | -0.079 |
| EPA Defense | 20% | 0.017 | -0.062 | +0.079 |
| Success Rate Offense | 15% | 44.0% | 42.0% | +2.1 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.119 | 0.111 | +0.008 |
| Red Zone TD Rate (Off) | 10% | 6.7% | 18.8% | -12.1 pp |
| Pressure Rate (Def) | 10% | 14.7% | 11.9% | +2.8 pp |

## PowerScore Summary

**Model (4 metrics):**
MIN edge: +0.005 (DET +0.115 vs MIN +0.121)
**7 metrics version:**
MIN edge: +0.009 (DET +0.087 vs MIN +0.096)
**Verdict:** MIN holds the edge in the extended breakdown (lead 0.009), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (DET - MIN) | +3.2 pts (favours DET) |
| Model Win% DET | 62.9% |
| Model Win% MIN | 37.1% |
| SOS Differential (DET-MIN) | -0.026 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| DET | 50.0% | 45.1% | 4.9% | 38.1% | 50.0% | Pass heavy |
| MIN | 42.0% | 45.1% | -3.1% | 29.4% | 51.1% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.2 pp | +0.3 pp | -7.9 pp |
| MIN | -5.3 pp | -0.8 pp | +3.6 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +3.6 pp | +4.7 pp | +6.9 pp |
| MIN | -1.5 pp | +1.9 pp | -1.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +3.5 pp | +3.2 pp | -1.5 pp |
| MIN | -3.5 pp | -0.3 pp | -2.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | -1.2 pp | -0.9 pp | -2.0 pp |
| MIN | -1.7 pp | -0.4 pp | -0.3 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +1.1 pp | +0.8 pp | -0.7 pp |
| MIN | -3.6 pp | -3.4 pp | -7.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +11.9 pp | +8.5 pp | +9.5 pp |
| MIN | +5.0 pp | +7.8 pp | +5.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | -1.2 pp | -0.9 pp | -2.0 pp |
| MIN | -1.7 pp | -0.4 pp | -0.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +3.5 pp | +3.2 pp | -1.5 pp |
| MIN | -3.5 pp | -0.3 pp | -2.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 47.6 | 47.5 | 47.1 |
| MIN | 48.0 | 47.2 | 49.0 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 47.8 | 45.9 | 43.8 |
| MIN | 48.4 | 49.7 | 48.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | -0.2 | 1.6 | 3.3 |
| MIN | -0.3 | -2.4 | 0.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 2.57 | 2.47 | 2.30 |
| MIN | 1.78 | 1.96 | 1.65 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 2.10 | 2.02 | 1.99 |
| MIN | 1.99 | 2.14 | 2.58 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 0.46 | 0.44 | 0.30 |
| MIN | -0.22 | -0.17 | -0.93 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 7.15 | 7.06 | 7.23 |
| MIN | 6.94 | 7.40 | 7.19 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 40.0% | 38.1% | 40.2% |
| MIN | 46.1% | 49.5% | 52.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 60.0% | 61.9% | 59.8% |
| MIN | 53.9% | 50.5% | 48.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 2.85 | 2.69 | 2.91 |
| MIN | 3.20 | 3.66 | 3.74 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 4.29 | 4.37 | 4.32 |
| MIN | 3.74 | 3.74 | 3.45 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | DET | MIN |
| --- | ---: | ---: |
| Season-to-date | 1.491 | 1.517 |
| Last 5 games | 1.490 | 1.544 |
| Last 3 games | 1.521 | 1.732 |

## Trend Summary (last 3 weeks)

| Metric | W4 | W5 | W7 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA DET | -0.082 | 0.186 | -0.092 | = stable |
| Off EPA MIN | 0.056 | 0.044 | -0.019 | - declining |
| Def EPA DET | -0.382 | -0.019 | -0.292 | - declining |
| Def EPA MIN | 0.139 | -0.008 | 0.065 | + improving |
| Off SR DET | 42.5% | 53.8% | 38.6% | - declining |
| Off SR MIN | 47.3% | 46.8% | 54.2% | + improving |
| Def SR DET | 30.2% | 48.8% | 34.1% | - declining |
| Def SR MIN | 58.0% | 41.1% | 42.6% | + improving |
| Tempo DET | 5.714 | 6.667 | 6.385 | + improving |
| Tempo MIN | 9.100 | 6.583 | 8.300 | - declining |

## Matchup Analogs

**DET analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | CLE (H) | 28-16 | DET | 0.929 | -0.082 | 42.5% | 0.857 |
| 7 | TB (H) | 24-8 | DET | 0.842 | -0.092 | 38.6% | 1.203 |
| 2 | CHI (H) | 52-19 | DET | 0.460 | 0.304 | 57.1% | 2.718 |

**MIN analogs vs DET profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | LAC (A) | 10-41 | LAC | 0.705 | -0.255 | 37.3% | -3.111 |
| 4 | PIT (A) | 21-22 | PIT | 0.491 | 0.056 | 47.3% | -0.082 |
| 7 | PHI (H) | 22-22 | TIE | 0.479 | -0.019 | 54.2% | 0.000 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.087 | 0.072 | 0.054 |
| MIN | -0.081 | -0.027 | -0.077 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.035 | -0.062 | -0.028 |
| MIN | -0.035 | -0.030 | 0.076 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 49.9% | 49.9% | 49.1% |
| MIN | 46.0% | 47.5% | 46.7% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 44.2% | 44.2% | 45.8% |
| MIN | 44.7% | 45.9% | 46.3% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 9.5% | 9.5% | 10.3% |
| MIN | 10.0% | 10.4% | 10.5% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 36.1% | 36.0% | 29.3% |
| MIN | 29.7% | 31.0% | 33.5% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.620 | 0.586 | 0.514 |
| MIN | -0.097 | -0.035 | -0.847 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.604 | 0.578 | 0.574 |
| MIN | -0.206 | -0.107 | -0.707 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.857 | 1.000 | 0.667 |
| MIN | -0.286 | -0.200 | -1.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 16.7% | 17.1% | 16.1% |
| MIN | 13.0% | 14.8% | 13.4% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 8.3% | 9.4% | 6.7% |
| MIN | 8.9% | 9.0% | 8.1% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 7.145 | 7.060 | 7.225 |
| MIN | 6.942 | 7.398 | 7.194 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 39.9% | 37.8% | 40.1% |
| MIN | 46.4% | 49.5% | 52.0% |
