# Matchup Report - NE vs ATL

## Metric Comparison

| Metric | NE | ATL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.076 | -0.057 | 0.134 |
| Core EPA Defense | -0.050 | 0.010 | -0.060 |
| Success Rate Offense | 0.470 | 0.454 | 0.016 |
| Success Rate Defense | 0.463 | 0.487 | -0.023 |
| Explosive Play Rate (Off) | 0.117 | 0.100 | 0.017 |
| Third Down Conversion | 0.333 | 0.361 | -0.027 |
| Points per Drive Differential | 0.684 | -0.531 | 1.216 |
| Yards per Play Differential | 0.454 | 0.484 | -0.030 |
| Turnover Margin | 0.250 | -0.286 | 0.536 |
| Red Zone TD Rate (Off) | 0.133 | 0.112 | 0.021 |
| Pressure Rate (Def) | 0.065 | 0.085 | -0.020 |
| Tempo | 7.455 | 7.300 | 0.155 |
| PowerScore | 1.615 | 1.580 | 0.035 |
| PowerScore | 1.615 | 1.580 | 0.035 |
| Tempo | 7.455 | 7.300 | 0.155 |

## PowerScore Breakdown (Model)

| Component | Weight | NE | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.022 | 0.016 | +0.006 |
| EPA Defense | 20% | 0.016 | 0.022 | -0.006 |
| Success Rate Offense | 25% | 45.1% | 43.8% | +1.3 pp |
| Tempo | 20% | 0.110 | 0.096 | +0.014 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | NE | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.022 | 0.016 | +0.006 |
| EPA Defense | 20% | 0.016 | 0.022 | -0.006 |
| Success Rate Offense | 15% | 45.1% | 43.8% | +1.3 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.110 | 0.096 | +0.014 |
| Red Zone TD Rate (Off) | 10% | 18.8% | 17.6% | +1.1 pp |
| Pressure Rate (Def) | 10% | 2.6% | 15.8% | -13.2 pp |

## PowerScore Summary

**Model (4 metrics):**
NE edge: +0.007 (NE +0.146 vs ATL +0.139)
**7 metrics version:**
ATL edge: +0.408 (NE -0.091 vs ATL +0.317)
**Verdict:** ATL holds the edge in the extended breakdown (lead 0.408), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (NE - ATL) | +0.6 pts (favours NE) |
| Model Win% NE | 52.7% |
| Model Win% ATL | 47.3% |
| SOS Differential (NE-ATL) | -0.035 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| NE | 46.3% | 45.1% | 1.3% | 42.1% | 47.7% | Slight pass lean |
| ATL | 52.1% | 45.1% | 7.0% | 42.1% | 45.7% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -0.6 pp | -2.2 pp | -10.9 pp |
| ATL | +2.9 pp | +2.9 pp | +5.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -2.5 pp | -7.0 pp | -15.0 pp |
| ATL | -5.2 pp | -1.0 pp | +4.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -1.7 pp | -3.4 pp | -3.7 pp |
| ATL | +1.7 pp | +1.8 pp | +2.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +2.6 pp | +3.2 pp | +0.5 pp |
| ATL | +0.2 pp | +1.4 pp | +2.0 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -8.5 pp | -11.8 pp | -10.0 pp |
| ATL | +1.8 pp | -0.2 pp | +1.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +9.2 pp | +7.2 pp | +5.4 pp |
| ATL | -5.3 pp | -2.6 pp | +0.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | +2.6 pp | +3.2 pp | +0.5 pp |
| ATL | +0.2 pp | +1.4 pp | +2.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | -1.7 pp | -3.4 pp | -3.7 pp |
| ATL | +1.7 pp | +1.8 pp | +2.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 49.5 | 50.5 | 49.3 |
| ATL | 46.1 | 47.0 | 43.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 47.7 | 48.1 | 49.9 |
| ATL | 49.2 | 48.1 | 44.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 1.8 | 2.4 | -0.5 |
| ATL | -3.1 | -1.2 | -1.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 2.47 | 2.63 | 2.53 |
| ATL | 1.63 | 1.48 | 1.34 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 1.79 | 1.64 | 1.50 |
| ATL | 2.16 | 2.46 | 2.30 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 0.68 | 0.99 | 1.03 |
| ATL | -0.53 | -0.97 | -0.96 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 7.85 | 7.49 | 7.91 |
| ATL | 7.76 | 7.53 | 7.33 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 41.5% | 38.0% | 37.5% |
| ATL | 44.1% | 46.1% | 47.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 58.5% | 62.0% | 62.5% |
| ATL | 55.9% | 53.9% | 53.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 3.26 | 2.84 | 2.97 |
| ATL | 3.42 | 3.47 | 3.44 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NE | 4.59 | 4.64 | 4.95 |
| ATL | 4.33 | 4.06 | 3.88 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | NE | ATL |
| --- | ---: | ---: |
| Season-to-date | 1.581 | 1.616 |
| Last 5 games | 1.554 | 1.705 |
| Last 3 games | 1.425 | 1.703 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA NE | 0.091 | 0.161 | -0.008 | - declining |
| Off EPA ATL | 0.053 | -0.077 | -0.227 | - declining |
| Def EPA NE | 0.007 | -0.131 | -0.306 | + improving |
| Def EPA ATL | -0.088 | 0.043 | 0.176 | - declining |
| Off SR NE | 48.1% | 46.3% | 48.2% | = stable |
| Off SR ATL | 50.0% | 44.2% | 41.8% | - declining |
| Def SR NE | 49.3% | 47.8% | 33.8% | + improving |
| Def SR ATL | 49.3% | 51.9% | 53.6% | - declining |
| Tempo NE | 9.000 | 8.200 | 6.538 | - declining |
| Tempo ATL | 7.333 | 8.556 | 6.091 | - declining |

## Matchup Analogs

**NE analogs vs ATL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | LV (H) | 13-18 | LV | 0.902 | -0.049 | 44.0% | -0.485 |
| 6 | NO (A) | 25-19 | NE | 0.845 | 0.091 | 48.1% | 0.667 |
| 7 | TEN (A) | 25-13 | NE | 0.809 | 0.161 | 46.3% | 1.200 |

**ATL analogs vs NE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | TB (H) | 20-23 | TB | 0.734 | 0.011 | 47.3% | -0.300 |
| 6 | BUF (H) | 24-15 | ATL | 0.723 | 0.053 | 50.0% | 0.727 |
| 2 | MIN (A) | 22-6 | ATL | 0.560 | 0.012 | 47.0% | 1.455 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.057 | -0.085 | -0.084 |
| NE | 0.076 | 0.112 | 0.081 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.010 | 0.073 | 0.044 |
| NE | -0.050 | -0.118 | -0.144 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 45.6% | 44.9% | 45.7% |
| NE | 47.0% | 47.6% | 47.6% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 48.9% | 52.2% | 51.7% |
| NE | 46.4% | 46.0% | 43.7% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 10.0% | 10.6% | 9.5% |
| NE | 11.6% | 12.8% | 12.9% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 36.6% | 36.2% | 31.3% |
| NE | 33.2% | 28.6% | 29.9% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.514 | -0.964 | -0.896 |
| NE | 0.715 | 1.017 | 1.060 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.484 | 0.408 | -0.073 |
| NE | 0.454 | 0.727 | 0.904 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.286 | -0.600 | 0.333 |
| NE | 0.250 | 1.000 | 1.000 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 11.2% | 13.4% | 14.4% |
| NE | 13.3% | 11.0% | 7.1% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 8.5% | 6.9% | 7.9% |
| NE | 6.5% | 5.7% | 7.6% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 7.757 | 7.531 | 7.327 |
| NE | 7.854 | 7.488 | 7.913 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 43.9% | 45.7% | 46.6% |
| NE | 41.9% | 38.1% | 37.5% |
