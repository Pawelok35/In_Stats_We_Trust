# Matchup Report - PHI vs DET

## PowerScore Breakdown (Model)

| Component | Weight | PHI | DET | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.051 | -0.082 | +0.031 |
| EPA Defense | 20% | -0.146 | -0.382 | +0.236 |
| Success Rate Offense | 25% | 46.2% | 42.5% | +3.7 pp |
| Tempo | 20% | 0.090 | 0.062 | +0.027 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | PHI | DET | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.051 | -0.082 | +0.031 |
| EPA Defense | 20% | -0.146 | -0.382 | +0.236 |
| Success Rate Offense | 15% | 46.2% | 42.5% | +3.7 pp |
| Turnover Margin | 10% | 2.000 | 2.000 | +0.000 |
| Tempo | 10% | 0.090 | 0.062 | +0.027 |
| Red Zone TD Rate (Off) | 10% | 27.3% | 20.0% | +7.3 pp |
| Pressure Rate (Def) | 10% | 4.8% | 7.7% | -2.9 pp |

## PowerScore Summary

**Model (4 metrics):**
PHI edge: +0.073 (PHI +0.086 vs DET +0.014)
**7 metrics version:**
PHI edge: +0.067 (PHI +0.268 vs DET +0.201)
**Verdict:** PHI holds the edge in the extended breakdown (lead 0.067), driven by EPA Defense (20% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (PHI - DET) | +1.3 pts (favours PHI) |
| Model Win% PHI | 55.6% |
| Model Win% DET | 44.4% |
| SOS Differential (PHI-DET) | +0.022 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| PHI | 33.3% | n/a | n/a | 25.6% | 34.0% | n/a |
| DET | 36.2% | n/a | n/a | 33.3% | 44.7% | n/a |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -4.1 pp | -5.0 pp | -9.8 pp |
| DET | -3.5 pp | -7.1 pp | -17.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | +2.5 pp | +0.7 pp | -1.6 pp |
| DET | +3.1 pp | +3.1 pp | -2.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -0.6 pp | -2.6 pp | -1.4 pp |
| DET | +0.3 pp | +0.4 pp | +1.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -1.5 pp | +1.6 pp | +4.0 pp |
| DET | -0.3 pp | +0.2 pp | +0.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -1.0 pp | +0.7 pp | +3.0 pp |
| DET | -5.5 pp | -4.6 pp | -7.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | +3.3 pp | +8.4 pp | +6.5 pp |
| DET | +9.2 pp | +5.4 pp | +1.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -1.5 pp | +1.6 pp | +4.0 pp |
| DET | -0.3 pp | +0.2 pp | +0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -0.6 pp | -2.6 pp | -1.4 pp |
| DET | +0.3 pp | +0.4 pp | +1.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 48.2 | 48.0 | 51.0 |
| DET | 47.9 | 47.9 | 46.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 49.2 | 49.0 | 51.8 |
| DET | 47.8 | 44.3 | 46.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -1.0 | -1.0 | -0.8 |
| DET | 0.1 | 3.6 | -0.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 2.30 | 2.29 | 2.71 |
| DET | 2.52 | 2.21 | 2.05 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 2.39 | 2.36 | 2.61 |
| DET | 2.12 | 1.87 | 2.08 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | -0.09 | -0.07 | 0.10 |
| DET | 0.40 | 0.34 | -0.02 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 7.35 | 7.07 | 7.56 |
| DET | 7.21 | 7.01 | 7.55 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 40.9% | 42.8% | 41.5% |
| DET | 41.2% | 41.4% | 44.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 59.1% | 57.2% | 58.5% |
| DET | 58.8% | 58.6% | 55.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 3.01 | 3.03 | 3.14 |
| DET | 2.97 | 2.90 | 3.35 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PHI | 4.34 | 4.04 | 4.42 |
| DET | 4.24 | 4.11 | 4.20 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | PHI | DET |
| --- | ---: | ---: |
| Season-to-date | 0.159 | 0.138 |
| Last 5 games | 0.161 | 0.095 |
| Last 3 games | 0.198 | 0.119 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W6 | W7 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA PHI | -0.005 | -0.032 | 0.065 | + improving |
| Off EPA DET | 0.186 | 0.067 | -0.092 | - declining |
| Def EPA PHI | 0.063 | 0.170 | -0.019 | + improving |
| Def EPA DET | -0.019 | 0.227 | -0.292 | + improving |
| Off SR PHI | 41.2% | 50.7% | 42.6% | + improving |
| Off SR DET | 53.8% | 56.5% | 38.6% | - declining |
| Def SR PHI | 48.4% | 47.1% | 54.2% | - declining |
| Def SR DET | 48.8% | 56.0% | 34.1% | + improving |
| Tempo PHI | 6.667 | 7.100 | 6.800 | + improving |
| Tempo DET | 6.667 | 8.625 | 6.385 | - declining |

## Matchup Analogs

**PHI analogs vs DET profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | LA (H) | 27-35 | LA | 0.952 | -0.048 | 38.6% | -0.659 |
| 5 | DEN (H) | 17-23 | DEN | 0.818 | -0.005 | 41.2% | -0.492 |
| 10 | GB (A) | n/a | n/a | 0.715 | n/a | n/a | n/a |

**DET analogs vs PHI profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | KC (H) | 17-27 | KC | 0.701 | 0.067 | 56.5% | -1.208 |
| 2 | CHI (H) | 52-19 | DET | 0.638 | 0.304 | 57.1% | 2.718 |
| 10 | WAS (A) | n/a | n/a | 0.634 | n/a | n/a | n/a |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.069 | 0.003 | -0.029 |
| PHI | 0.054 | 0.057 | 0.113 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.029 | -0.090 | -0.016 |
| PHI | 0.020 | 0.033 | 0.083 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 49.1% | 46.7% | 45.8% |
| PHI | 46.6% | 47.9% | 50.9% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 43.9% | 41.8% | 43.6% |
| PHI | 48.4% | 47.4% | 50.0% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 9.8% | 9.8% | 11.4% |
| PHI | 9.9% | 11.4% | 14.2% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 34.8% | 30.8% | 29.7% |
| PHI | 30.3% | 22.4% | 20.2% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.537 | 0.487 | 0.163 |
| PHI | -0.143 | -0.127 | 0.034 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.585 | 0.547 | 0.543 |
| PHI | -0.126 | 0.286 | 1.035 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.750 | 0.800 | 0.000 |
| PHI | 0.500 | 0.400 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 15.5% | 15.0% | 11.7% |
| PHI | 17.5% | 14.9% | 11.0% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 9.1% | 8.5% | 10.0% |
| PHI | 6.5% | 8.6% | 10.3% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 7.207 | 7.005 | 7.549 |
| PHI | 7.353 | 7.069 | 7.559 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 41.2% | 41.4% | 44.5% |
| PHI | 40.9% | 42.8% | 41.3% |
