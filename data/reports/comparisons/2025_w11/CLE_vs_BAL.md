# Matchup Report - CLE vs BAL

## Metric Comparison

| Metric | CLE | BAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.155 | 0.051 | -0.205 |
| Core EPA Defense | -0.051 | 0.064 | -0.116 |
| Success Rate Offense | 0.382 | 0.495 | -0.113 |
| Success Rate Defense | 0.422 | 0.492 | -0.070 |
| Explosive Play Rate (Off) | 0.070 | 0.123 | -0.053 |
| Third Down Conversion | 0.271 | 0.361 | -0.091 |
| Points per Drive Differential | -0.448 | -0.201 | -0.247 |
| Yards per Play Differential | -0.417 | -0.053 | -0.364 |
| Turnover Margin | 0.000 | -0.375 | 0.375 |
| Red Zone TD Rate (Off) | 0.136 | 0.123 | 0.013 |
| Pressure Rate (Def) | 0.092 | 0.035 | 0.057 |
| Tempo | 5.615 | 6.154 | -0.538 |
| PowerScore | 0.046 | 0.079 | -0.032 |
| PowerScore | 0.046 | 0.079 | -0.032 |
| Tempo | 5.615 | 6.154 | -0.538 |

## PowerScore Breakdown (Model)

| Component | Weight | CLE | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.184 | -0.055 | -0.129 |
| EPA Defense | 20% | -0.055 | -0.184 | +0.129 |
| Success Rate Offense | 25% | 41.1% | 45.0% | -3.9 pp |
| Tempo | 20% | 0.096 | 0.113 | -0.017 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CLE | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.184 | -0.055 | -0.129 |
| EPA Defense | 20% | -0.055 | -0.184 | +0.129 |
| Success Rate Offense | 15% | 41.1% | 45.0% | -3.9 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.096 | 0.113 | -0.017 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 5.6% | -5.6 pp |
| Pressure Rate (Def) | 10% | 15.6% | 10.3% | +5.3 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.032 (CLE +0.046 vs BAL +0.079)
**7 metrics version:**
BAL edge: +0.014 (CLE +0.030 vs BAL +0.044)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.014), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CLE - BAL) | -0.6 pts (favours BAL) |
| Model Win% CLE | 47.5% |
| Model Win% BAL | 52.5% |
| SOS Differential (CLE-BAL) | -0.101 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CLE | 39.7% | 44.1% | -4.3% | 24.1% | 52.3% | Run heavy |
| BAL | 40.0% | 44.1% | -4.1% | 37.5% | 50.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -11.2 pp | -13.8 pp | -6.4 pp |
| BAL | +5.5 pp | +7.9 pp | +13.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -1.3 pp | +0.7 pp | +6.2 pp |
| BAL | -2.3 pp | -2.3 pp | +2.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.7 pp | -2.8 pp | -2.9 pp |
| BAL | -0.4 pp | -0.4 pp | +1.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.2 pp | -3.0 pp | -4.8 pp |
| BAL | +3.0 pp | -0.9 pp | -1.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -7.0 pp | -4.0 pp | +0.1 pp |
| BAL | +9.8 pp | +8.3 pp | +10.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -14.9 pp | -20.3 pp | -14.4 pp |
| BAL | +3.7 pp | +3.4 pp | +5.8 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -3.2 pp | -3.0 pp | -4.8 pp |
| BAL | +3.0 pp | -0.9 pp | -1.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -2.7 pp | -2.8 pp | -2.9 pp |
| BAL | -0.4 pp | -0.4 pp | +1.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 48.1 | 48.9 | 53.4 |
| BAL | 52.1 | 51.5 | 45.9 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 43.5 | 42.1 | 42.0 |
| BAL | 49.4 | 47.9 | 45.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 4.6 | 6.7 | 11.4 |
| BAL | 2.7 | 3.6 | 0.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.37 | 1.35 | 1.39 |
| BAL | 2.32 | 1.84 | 2.07 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.81 | 1.82 | 1.79 |
| BAL | 2.52 | 2.42 | 1.34 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -0.45 | -0.48 | -0.40 |
| BAL | -0.20 | -0.58 | 0.73 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 7.54 | 7.25 | 7.54 |
| BAL | 7.30 | 7.71 | 8.41 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 47.5% | 45.4% | 47.5% |
| BAL | 39.8% | 39.1% | 36.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 52.5% | 54.6% | 52.5% |
| BAL | 60.2% | 60.9% | 63.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.58 | 3.29 | 3.58 |
| BAL | 2.90 | 3.01 | 3.10 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.95 | 3.96 | 3.95 |
| BAL | 4.39 | 4.70 | 5.31 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | CLE | BAL |
| --- | ---: | ---: |
| Season-to-date | 0.075 | 0.176 |
| Last 5 games | 0.070 | 0.153 |
| Last 3 games | 0.061 | 0.115 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W6 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CLE | -0.008 | -0.080 | -0.306 | - declining |
| Off EPA BAL | -0.250 | -0.263 | 0.227 | + improving |
| Def EPA CLE | 0.044 | 0.114 | -0.008 | + improving |
| Def EPA BAL | 0.313 | -0.084 | 0.041 | + improving |
| Off SR CLE | 41.1% | 43.3% | 33.8% | - declining |
| Off SR BAL | 43.5% | 40.4% | 55.0% | + improving |
| Def SR CLE | 46.8% | 42.7% | 48.2% | - declining |
| Def SR BAL | 61.2% | 46.2% | 46.9% | + improving |
| Tempo CLE | 7.500 | 10.778 | 5.667 | - declining |
| Tempo BAL | 6.200 | 9.889 | 8.889 | + improving |

## Matchup Analogs

**CLE analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | BAL (A) | 17-32 | BAL | 1.000 | -0.227 | 43.5% | -1.276 |
| 1 | CIN (H) | 16-17 | CIN | 0.837 | -0.087 | 39.6% | -0.100 |
| 3 | GB (H) | 13-11 | CLE | 0.785 | -0.092 | 40.0% | 0.182 |

**BAL analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 2 | CLE (H) | 35-18 | BAL | 1.000 | 0.100 | 43.6% | 1.276 |
| 10 | MIN (A) | n/a | n/a | 0.942 | n/a | n/a | n/a |
| 5 | HOU (H) | 10-44 | HOU | 0.866 | -0.250 | 43.5% | -3.400 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.051 | -0.039 | 0.034 |
| CLE | -0.155 | -0.166 | -0.147 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.064 | 0.051 | -0.079 |
| CLE | -0.051 | -0.070 | -0.104 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 49.3% | 48.0% | 48.8% |
| CLE | 38.5% | 36.9% | 37.7% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 49.3% | 48.9% | 45.1% |
| CLE | 42.3% | 42.3% | 40.7% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 12.2% | 10.5% | 11.2% |
| CLE | 6.9% | 7.7% | 6.7% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 35.9% | 33.8% | 34.9% |
| CLE | 27.6% | 22.5% | 25.2% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.120 | -0.557 | 0.817 |
| CLE | -0.435 | -0.435 | -0.294 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.053 | -0.174 | -0.011 |
| CLE | -0.417 | -1.049 | -1.280 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.375 | -0.600 | 0.667 |
| CLE | 0.000 | 0.800 | 1.333 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 12.3% | 12.8% | 13.5% |
| CLE | 13.6% | 11.0% | 12.2% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 3.5% | 4.4% | 4.7% |
| CLE | 9.2% | 7.5% | 9.9% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.298 | 7.706 | 8.411 |
| CLE | 7.537 | 7.251 | 7.537 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 39.9% | 39.1% | 37.1% |
| CLE | 47.9% | 45.8% | 48.5% |
