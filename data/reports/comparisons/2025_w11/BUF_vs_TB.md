# Matchup Report - BUF vs TB

## Metric Comparison

| Metric | BUF | TB | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.320 | 0.142 | ↑ +0.178 |
| Core EPA Defense | 0.142 | 0.320 | ↓ -0.178 |
| Success Rate Offense | 54.2% | 49.5% | ↑ +4.7 pp |
| Success Rate Defense | 49.5% | 54.2% | ↓ -4.7 pp |
| Explosive Play Rate (Off) | 13.9% | 9.7% | ↑ +4.2 pp |
| Third Down Conversion | 46.2% | 56.2% | ↓ -10.1 pp |
| Points per Drive Differential | 1.538 | -1.538 | ↑ +3.077 |
| Yards per Play Differential | 1.804 | -1.804 | ↑ +3.608 |
| Turnover Margin | 1.000 | -1.000 | ↑ +2.000 |
| Red Zone TD Rate (Off) | 21.4% | 10.0% | ↑ +11.4 pp |
| Pressure Rate (Def) | 6.2% | 0.0% | ↑ +6.2 pp |
| Tempo | 6.545 | 7.154 | ↓ -0.608 |
| PowerScore | 0.304 | 0.257 | ↑ +0.047 |

## PowerScore Breakdown (Model)

| Component | Weight | BUF | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.320 | 0.142 | +0.178 |
| EPA Defense | 20% | 0.142 | 0.320 | -0.178 |
| Success Rate Offense | 25% | 54.2% | 49.5% | +4.7 pp |
| Tempo | 20% | 0.139 | 0.097 | +0.042 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BUF | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.320 | 0.142 | +0.178 |
| EPA Defense | 20% | 0.142 | 0.320 | -0.178 |
| Success Rate Offense | 15% | 54.2% | 49.5% | +4.7 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.139 | 0.097 | +0.042 |
| Red Zone TD Rate (Off) | 10% | 21.4% | 10.0% | +11.4 pp |
| Pressure Rate (Def) | 10% | 6.2% | 0.0% | +6.2 pp |

## PowerScore Summary

**Model (4 metrics):**
BUF edge: +0.047 (BUF +0.304 vs TB +0.257)
**7 metrics version:**
BUF edge: +0.238 (BUF +0.331 vs TB +0.093)
**Verdict:** BUF holds the edge in the extended breakdown (lead 0.238), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BUF - TB) | +0.9 pts (favours BUF) |
| Model Win% BUF | 53.6% |
| Model Win% TB | 46.4% |
| SOS Differential (BUF-TB) | +0.062 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BUF | 43.1% | 44.1% | -1.0% | 48.4% | 58.5% | Balanced |
| TB | 34.4% | 44.1% | -9.6% | 37.5% | 55.7% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +5.7 pp | +2.0 pp | +7.6 pp |
| TB | +0.5 pp | +0.8 pp | -4.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +5.1 pp | +2.5 pp | +13.1 pp |
| TB | -0.9 pp | -0.3 pp | +0.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.4 pp | -0.6 pp | +1.0 pp |
| TB | +4.7 pp | +5.6 pp | +4.6 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.5 pp | +3.6 pp | +3.6 pp |
| TB | -2.3 pp | -1.8 pp | -1.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +6.1 pp | +8.6 pp | +11.8 pp |
| TB | -9.1 pp | -10.2 pp | -11.6 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.2 pp | +0.1 pp | +8.1 pp |
| TB | +2.5 pp | +1.7 pp | +0.8 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.5 pp | +3.6 pp | +3.6 pp |
| TB | -2.3 pp | -1.8 pp | -1.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.4 pp | -0.6 pp | +1.0 pp |
| TB | +4.7 pp | +5.6 pp | +4.6 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 45.4 | 45.3 | 47.1 |
| TB | 48.8 | 49.0 | 48.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 50.6 | 49.3 | 44.4 |
| TB | 48.3 | 47.5 | 43.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -5.3 | -4.0 | 2.7 |
| TB | 0.5 | 1.6 | 4.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.78 | 2.58 | 2.69 |
| TB | 2.19 | 2.16 | 1.64 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.01 | 1.82 | 1.64 |
| TB | 1.86 | 1.89 | 1.34 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 0.77 | 0.75 | 1.05 |
| TB | 0.32 | 0.27 | 0.30 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 7.77 | 7.51 | 7.42 |
| TB | 7.26 | 6.59 | 6.33 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 39.1% | 37.4% | 37.3% |
| TB | 44.5% | 46.7% | 46.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 60.9% | 62.6% | 62.7% |
| TB | 55.5% | 53.3% | 53.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 3.04 | 2.81 | 2.77 |
| TB | 3.23 | 3.08 | 2.93 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 4.74 | 4.70 | 4.65 |
| TB | 4.03 | 3.51 | 3.39 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | BUF | TB |
| --- | ---: | ---: |
| Season-to-date | 0.181 | 0.119 |
| Last 5 games | 0.161 | 0.113 |
| Last 3 games | 0.155 | 0.054 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W6 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BUF | 0.019 | -0.088 | 0.283 | + improving |
| Off EPA TB | 0.339 | 0.157 | -0.078 | - declining |
| Def EPA BUF | 0.069 | 0.053 | -0.210 | + improving |
| Def EPA TB | 0.243 | 0.016 | -0.394 | + improving |
| Off SR BUF | 50.6% | 49.3% | 51.9% | + improving |
| Off SR TB | 53.9% | 53.6% | 35.6% | - declining |
| Def SR BUF | 51.9% | 50.0% | 47.4% | + improving |
| Def SR TB | 57.1% | 50.0% | 38.4% | + improving |
| Tempo BUF | 8.300 | 6.455 | 7.900 | - declining |
| Tempo TB | 7.600 | 6.273 | 6.636 | - declining |

## Matchup Analogs

**BUF analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | ATL (H) | 14-22 | ATL | 0.750 | -0.088 | 49.3% | -0.727 |
| 5 | NE (H) | 20-23 | NE | 0.719 | 0.019 | 50.6% | -0.300 |
| 4 | NO (H) | 31-21 | BUF | 0.663 | 0.122 | 46.8% | 0.918 |

**TB analogs vs BUF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | SF (A) | 30-21 | TB | 0.816 | 0.157 | 53.6% | 0.827 |
| 5 | SEA (A) | 38-35 | TB | 0.723 | 0.339 | 53.9% | 0.300 |
| 10 | NE (H) | n/a | n/a | 0.721 | n/a | n/a | n/a |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.136 | 0.097 | 0.115 |
| TB | 0.001 | -0.004 | -0.071 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.023 | -0.010 | -0.031 |
| TB | -0.042 | -0.056 | -0.157 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 50.5% | 51.4% | 53.3% |
| TB | 43.4% | 42.9% | 40.5% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 48.7% | 49.1% | 48.3% |
| TB | 46.3% | 45.9% | 42.4% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 12.3% | 12.6% | 12.2% |
| TB | 9.6% | 9.2% | 8.8% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 41.7% | 42.9% | 43.6% |
| TB | 33.4% | 31.1% | 26.4% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.713 | 0.729 | 0.994 |
| TB | 0.272 | 0.189 | 0.174 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.605 | 0.595 | 0.697 |
| TB | -0.023 | 0.070 | -0.310 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.375 | 0.000 | 0.667 |
| TB | 0.875 | 1.000 | 1.667 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 18.3% | 19.1% | 22.8% |
| TB | 12.1% | 10.3% | 10.2% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 10.3% | 11.5% | 12.1% |
| TB | 8.5% | 8.4% | 11.5% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.774 | 7.511 | 7.418 |
| TB | 7.257 | 6.588 | 6.327 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 39.4% | 37.3% | 37.1% |
| TB | 44.6% | 47.4% | 47.6% |
