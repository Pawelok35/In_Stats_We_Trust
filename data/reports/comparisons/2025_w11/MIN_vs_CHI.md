# Matchup Report - MIN vs CHI

## Metric Comparison

| Metric | MIN | CHI | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.069 | 0.041 | -0.110 |
| Core EPA Defense | -0.038 | 0.043 | -0.081 |
| Success Rate Offense | 0.451 | 0.460 | -0.009 |
| Success Rate Defense | 0.448 | 0.509 | -0.062 |
| Explosive Play Rate (Off) | 0.100 | 0.124 | -0.024 |
| Third Down Conversion | 0.295 | 0.380 | -0.085 |
| Points per Drive Differential | -0.182 | -0.143 | -0.039 |
| Yards per Play Differential | -0.236 | -0.347 | 0.111 |
| Turnover Margin | -0.250 | 1.625 | -1.875 |
| Red Zone TD Rate (Off) | 0.137 | 0.126 | 0.011 |
| Pressure Rate (Def) | 0.093 | 0.067 | 0.026 |
| Tempo | 5.462 | 7.917 | -2.455 |
| PowerScore | 0.038 | 0.060 | -0.022 |
| PowerScore | 0.038 | 0.060 | -0.022 |
| Tempo | 5.462 | 7.917 | -2.455 |

## PowerScore Breakdown (Model)

| Component | Weight | MIN | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.181 | -0.084 | -0.098 |
| EPA Defense | 20% | -0.084 | -0.181 | +0.098 |
| Success Rate Offense | 25% | 39.4% | 44.2% | -4.8 pp |
| Tempo | 20% | 0.099 | 0.074 | +0.025 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | MIN | CHI | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.181 | -0.084 | -0.098 |
| EPA Defense | 20% | -0.084 | -0.181 | +0.098 |
| Success Rate Offense | 15% | 39.4% | 44.2% | -4.8 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.099 | 0.074 | +0.025 |
| Red Zone TD Rate (Off) | 10% | 25.0% | 8.3% | +16.7 pp |
| Pressure Rate (Def) | 10% | 5.9% | 0.0% | +5.9 pp |

## PowerScore Summary

**Model (4 metrics):**
CHI edge: +0.022 (MIN +0.038 vs CHI +0.060)
**7 metrics version:**
CHI edge: +0.387 (MIN -0.162 vs CHI +0.225)
**Verdict:** CHI holds the edge in the extended breakdown (lead 0.387), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (MIN - CHI) | -0.4 pts (favours CHI) |
| Model Win% MIN | 48.3% |
| Model Win% CHI | 51.7% |
| SOS Differential (MIN-CHI) | -0.066 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| MIN | 45.1% | 44.1% | 1.0% | 37.5% | 41.0% | Slight pass lean |
| CHI | 35.8% | 44.1% | -8.3% | 38.2% | 47.5% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -0.8 pp | -1.4 pp | -1.2 pp |
| CHI | +5.0 pp | -1.0 pp | -5.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -2.3 pp | -2.0 pp | -2.9 pp |
| CHI | +0.3 pp | -0.1 pp | +1.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -5.5 pp | -3.0 pp | -2.0 pp |
| CHI | +4.4 pp | +4.3 pp | +6.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -1.1 pp | -1.5 pp | -1.2 pp |
| CHI | +1.5 pp | +1.5 pp | +2.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.9 pp | -5.2 pp | -3.9 pp |
| CHI | +0.9 pp | +0.8 pp | +3.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -7.6 pp | -7.2 pp | -10.5 pp |
| CHI | +2.1 pp | -0.7 pp | -1.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -1.1 pp | -1.5 pp | -1.2 pp |
| CHI | +1.5 pp | +1.5 pp | +2.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -5.5 pp | -3.0 pp | -2.0 pp |
| CHI | +4.4 pp | +4.3 pp | +6.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.0 | 46.8 | 52.8 |
| CHI | 51.5 | 54.1 | 51.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.6 | 49.6 | 51.0 |
| CHI | 49.9 | 51.1 | 52.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -0.6 | -2.8 | 1.8 |
| CHI | 1.6 | 3.1 | -0.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.84 | 1.86 | 1.82 |
| CHI | 2.33 | 2.53 | 2.62 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 2.02 | 2.42 | 2.83 |
| CHI | 2.47 | 2.39 | 2.54 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -0.18 | -0.56 | -1.01 |
| CHI | -0.14 | 0.13 | 0.09 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 6.92 | 7.49 | 7.25 |
| CHI | 7.65 | 8.15 | 8.31 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 45.6% | 51.3% | 49.6% |
| CHI | 41.7% | 41.8% | 40.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 54.4% | 48.7% | 50.4% |
| CHI | 58.3% | 58.2% | 60.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.15 | 3.84 | 3.59 |
| CHI | 3.19 | 3.40 | 3.32 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.77 | 3.65 | 3.66 |
| CHI | 4.46 | 4.74 | 4.98 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | MIN | CHI |
| --- | ---: | ---: |
| Season-to-date | 0.106 | 0.173 |
| Last 5 games | 0.158 | 0.174 |
| Last 3 games | 0.146 | 0.201 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA MIN | -0.019 | -0.255 | 0.017 | + improving |
| Off EPA CHI | -0.010 | 0.041 | 0.306 | + improving |
| Def EPA MIN | 0.065 | 0.170 | -0.062 | + improving |
| Def EPA CHI | -0.231 | 0.227 | 0.247 | - declining |
| Off SR MIN | 54.2% | 37.3% | 42.0% | - declining |
| Off SR CHI | 43.8% | 46.9% | 55.9% | + improving |
| Def SR MIN | 42.6% | 54.5% | 44.0% | - declining |
| Def SR CHI | 42.9% | 55.0% | 55.7% | - declining |
| Tempo MIN | 8.300 | 6.700 | 6.750 | - declining |
| Tempo CHI | 7.417 | 9.000 | 8.500 | + improving |

## Matchup Analogs

**MIN analogs vs CHI profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CHI (A) | 27-17 | MIN | 1.000 | -0.064 | 44.6% | 0.791 |
| 10 | BAL (H) | n/a | n/a | 0.848 | n/a | n/a | n/a |
| 3 | CIN (H) | 36-10 | MIN | 0.747 | 0.042 | 50.0% | 2.000 |

**CHI analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | MIN (H) | 18-29 | MIN | 1.000 | -0.109 | 37.4% | -0.791 |
| 7 | NO (H) | 26-15 | CHI | 0.820 | -0.010 | 43.8% | 0.894 |
| 4 | LV (A) | 25-24 | CHI | 0.714 | -0.072 | 44.8% | 0.091 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 0.041 | 0.072 | 0.113 |
| MIN | -0.069 | -0.032 | -0.086 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 0.043 | 0.021 | 0.081 |
| MIN | -0.038 | 0.061 | 0.058 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 46.0% | 48.6% | 49.3% |
| MIN | 45.5% | 45.9% | 45.0% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 51.0% | 51.7% | 51.7% |
| MIN | 44.6% | 47.9% | 47.5% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 12.5% | 13.0% | 14.7% |
| MIN | 10.1% | 10.2% | 10.4% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 37.8% | 35.5% | 36.1% |
| MIN | 29.6% | 31.0% | 34.7% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.192 | 0.221 | 0.234 |
| MIN | -0.075 | -0.502 | -0.947 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | -0.347 | -0.152 | 0.292 |
| MIN | -0.236 | -0.598 | -1.135 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 1.625 | 2.200 | 1.667 |
| MIN | -0.250 | -1.200 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 12.6% | 8.6% | 9.6% |
| MIN | 13.7% | 14.1% | 12.2% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 6.7% | 7.4% | 9.0% |
| MIN | 9.3% | 8.8% | 10.1% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 7.652 | 8.145 | 8.306 |
| MIN | 6.918 | 7.487 | 7.250 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CHI | 41.5% | 41.6% | 39.7% |
| MIN | 45.8% | 51.4% | 49.4% |
