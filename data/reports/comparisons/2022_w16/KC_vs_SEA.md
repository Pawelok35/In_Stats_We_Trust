# Matchup Report - KC vs SEA

## Metric Comparison

| Metric | KC | SEA | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.132 | 0.037 | 0.095 |
| Core EPA Defense | 0.027 | 0.030 | -0.003 |
| Success Rate Offense | 0.496 | 0.461 | 0.035 |
| Success Rate Defense | 0.429 | 0.422 | 0.008 |
| Explosive Play Rate (Off) | 0.128 | 0.114 | 0.014 |
| Third Down Conversion | 0.459 | 0.369 | 0.090 |
| Points per Drive Differential | 0.568 | -0.041 | 0.608 |
| Yards per Play Differential | 1.125 | 0.014 | 1.111 |
| Turnover Margin | 0.000 | 0.071 | -0.071 |
| Red Zone TD Rate (Off) | 0.161 | 0.113 | 0.048 |
| Pressure Rate (Def) | 0.080 | 0.071 | 0.009 |
| Tempo | 6.091 | 7.154 | -1.063 |
| PowerScore | 1.316 | 1.470 | -0.154 |
| PowerScore | 1.316 | 1.470 | -0.154 |
| Tempo | 6.091 | 7.154 | -1.063 |

## PowerScore Breakdown (Model)

| Component | Weight | KC | SEA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.029 | -0.159 | +0.188 |
| EPA Defense | 20% | -0.159 | 0.029 | -0.188 |
| Success Rate Offense | 25% | 47.8% | 35.5% | +12.3 pp |
| Tempo | 20% | 0.075 | 0.075 | -0.001 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | KC | SEA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.029 | -0.159 | +0.188 |
| EPA Defense | 20% | -0.159 | 0.029 | -0.188 |
| Success Rate Offense | 15% | 47.8% | 35.5% | +12.3 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.075 | 0.075 | -0.001 |
| Red Zone TD Rate (Off) | 10% | 27.3% | 10.0% | +17.3 pp |
| Pressure Rate (Def) | 10% | 4.8% | 3.4% | +1.3 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.059 (KC +0.113 vs SEA +0.054)
**7 metrics version:**
KC edge: +0.246 (KC +0.187 vs SEA -0.060)
**Verdict:** KC holds the edge in the extended breakdown (lead 0.246), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (KC - SEA) | -2.8 pts (favours SEA) |
| Model Win% KC | 38.4% |
| Model Win% SEA | 61.6% |
| SOS Differential (KC-SEA) | -0.024 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| KC | 43.3% | 44.3% | -1.0% | 44.8% | 50.0% | Slight run lean |
| SEA | 45.2% | 44.3% | 0.8% | 26.2% | 43.1% | Balanced |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +4.0 pp | -0.3 pp | +11.8 pp |
| SEA | +0.5 pp | -11.4 pp | -1.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +2.0 pp | +1.3 pp | +4.5 pp |
| SEA | -8.3 pp | -1.3 pp | -0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +3.3 pp | +1.7 pp | +2.8 pp |
| SEA | +0.9 pp | +0.9 pp | -1.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +2.0 pp | +2.9 pp | +4.3 pp |
| SEA | +2.0 pp | +1.4 pp | +0.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +5.9 pp | +6.0 pp | +3.1 pp |
| SEA | +1.1 pp | +3.7 pp | +2.5 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +8.8 pp | +5.7 pp | +13.8 pp |
| SEA | +7.1 pp | +10.2 pp | +5.3 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +2.0 pp | +2.9 pp | +4.3 pp |
| SEA | +2.0 pp | +1.4 pp | +0.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +3.3 pp | +1.7 pp | +2.8 pp |
| SEA | +0.9 pp | +0.9 pp | -1.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 48.2 | 48.9 | 48.9 |
| SEA | 48.6 | 49.2 | 46.5 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 50.6 | 52.4 | 50.7 |
| SEA | 49.5 | 46.0 | 48.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | -2.4 | -3.5 | -1.9 |
| SEA | -0.9 | 3.2 | -2.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 2.72 | 2.70 | 2.50 |
| SEA | 2.21 | 2.01 | 1.94 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 2.16 | 2.22 | 2.46 |
| SEA | 2.25 | 2.35 | 2.26 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 0.57 | 0.48 | 0.04 |
| SEA | -0.04 | -0.34 | -0.32 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 8.01 | 8.09 | 7.42 |
| SEA | 7.34 | 6.97 | 7.15 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 49.3% | 47.6% | 48.4% |
| SEA | 47.5% | 53.9% | 55.1% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 50.7% | 52.4% | 51.6% |
| SEA | 52.5% | 46.1% | 44.9% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 3.95 | 3.85 | 3.59 |
| SEA | 3.48 | 3.76 | 3.94 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 4.06 | 4.24 | 3.83 |
| SEA | 3.86 | 3.21 | 3.21 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | KC | SEA |
| --- | ---: | ---: |
| Season-to-date | 1.696 | 1.720 |
| Last 5 games | 1.707 | 1.705 |
| Last 3 games | 1.739 | 1.680 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA KC | 0.178 | 0.083 | 0.056 | - declining |
| Off EPA SEA | 0.112 | -0.008 | -0.104 | - declining |
| Def EPA KC | 0.160 | -0.047 | -0.015 | + improving |
| Def EPA SEA | 0.029 | 0.122 | 0.013 | + improving |
| Off SR KC | 55.2% | 47.7% | 53.8% | - declining |
| Off SR SEA | 50.6% | 40.8% | 43.8% | - declining |
| Def SR KC | 52.3% | 34.4% | 36.2% | + improving |
| Def SR SEA | 45.7% | 50.5% | 40.2% | + improving |
| Tempo KC | 8.375 | 6.143 | 7.750 | - declining |
| Tempo SEA | 7.083 | 7.100 | 7.273 | + improving |

## Matchup Analogs

**KC analogs vs SEA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 10 | JAX (H) | 27-17 | KC | 0.887 | 0.093 | 50.7% | 0.909 |
| 6 | BUF (H) | 20-24 | BUF | 0.729 | 0.037 | 42.7% | -0.400 |
| 13 | CIN (A) | 24-27 | CIN | 0.726 | 0.178 | 55.2% | -0.375 |

**SEA analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | DET (A) | 42-41 | SEA | 0.840 | 0.326 | 52.8% | 0.109 |
| 3 | ATL (H) | 23-24 | ATL | 0.710 | 0.146 | 55.2% | -0.125 |
| 8 | NYG (H) | 27-15 | SEA | 0.687 | 0.090 | 41.2% | 0.895 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.132 | 0.122 | 0.106 |
| SEA | 0.037 | 0.021 | 0.000 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.027 | 0.015 | 0.032 |
| SEA | 0.030 | 0.074 | 0.055 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 49.3% | 51.4% | 52.0% |
| SEA | 46.4% | 48.3% | 45.3% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 43.2% | 41.5% | 40.9% |
| SEA | 42.2% | 46.1% | 45.7% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 12.5% | 14.0% | 14.2% |
| SEA | 11.5% | 10.3% | 9.3% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 46.0% | 47.4% | 50.0% |
| SEA | 37.9% | 31.7% | 38.3% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.518 | 0.396 | 0.079 |
| SEA | 0.015 | -0.340 | -0.307 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 1.125 | 1.638 | 1.483 |
| SEA | 0.014 | -0.301 | 0.156 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.000 | 0.000 | -1.000 |
| SEA | 0.071 | -0.400 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 16.1% | 15.3% | 19.0% |
| SEA | 11.3% | 15.3% | 16.0% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 8.0% | 8.6% | 6.0% |
| SEA | 7.1% | 5.4% | 8.0% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 8.006 | 8.094 | 7.423 |
| SEA | 7.343 | 6.972 | 7.152 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 49.7% | 47.8% | 48.8% |
| SEA | 47.1% | 53.7% | 55.1% |
