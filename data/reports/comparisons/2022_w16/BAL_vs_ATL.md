# Matchup Report - BAL vs ATL

## Metric Comparison

| Metric | BAL | ATL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.032 | 0.010 | 0.022 |
| Core EPA Defense | -0.028 | 0.034 | -0.062 |
| Success Rate Offense | 0.445 | 0.454 | -0.010 |
| Success Rate Defense | 0.433 | 0.471 | -0.038 |
| Explosive Play Rate (Off) | 0.119 | 0.106 | 0.013 |
| Third Down Conversion | 0.392 | 0.388 | 0.005 |
| Points per Drive Differential | 0.155 | -0.397 | 0.552 |
| Yards per Play Differential | 0.141 | -0.448 | 0.589 |
| Turnover Margin | 0.357 | 0.071 | 0.286 |
| Red Zone TD Rate (Off) | 0.113 | 0.143 | -0.031 |
| Pressure Rate (Def) | 0.082 | 0.043 | 0.039 |
| Tempo | 7.222 | 9.111 | -1.889 |
| PowerScore | 1.501 | 1.862 | -0.361 |
| PowerScore | 1.501 | 1.862 | -0.361 |
| Tempo | 7.222 | 9.111 | -1.889 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.045 | -0.140 | +0.095 |
| EPA Defense | 20% | -0.140 | -0.045 | -0.095 |
| Success Rate Offense | 25% | 40.0% | 39.0% | +1.0 pp |
| Tempo | 20% | 0.123 | 0.085 | +0.038 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | ATL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.045 | -0.140 | +0.095 |
| EPA Defense | 20% | -0.140 | -0.045 | -0.095 |
| Success Rate Offense | 15% | 40.0% | 39.0% | +1.0 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.123 | 0.085 | +0.038 |
| Red Zone TD Rate (Off) | 10% | 5.3% | 0.0% | +5.3 pp |
| Pressure Rate (Def) | 10% | 5.7% | 0.0% | +5.7 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.024 (BAL +0.081 vs ATL +0.056)
**7 metrics version:**
BAL edge: +0.221 (BAL +0.144 vs ATL -0.077)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.221), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - ATL) | -6.7 pts (favours ATL) |
| Model Win% BAL | 24.7% |
| Model Win% ATL | 75.3% |
| SOS Differential (BAL-ATL) | -0.161 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 30.8% | 44.3% | -13.6% | 25.0% | 46.7% | Run heavy |
| ATL | 42.7% | 44.3% | -1.6% | 42.9% | 36.2% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -6.3 pp | -7.8 pp | -12.8 pp |
| ATL | +7.7 pp | +4.3 pp | +9.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -4.2 pp | -6.7 pp | -7.5 pp |
| ATL | +3.8 pp | +6.2 pp | +4.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -3.5 pp | -2.4 pp | -7.6 pp |
| ATL | -0.6 pp | +1.6 pp | +0.7 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.5 pp | -0.1 pp | -1.1 pp |
| ATL | +1.2 pp | +3.0 pp | +2.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.7 pp | -0.1 pp | -5.1 pp |
| ATL | +5.5 pp | +8.0 pp | +8.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -8.4 pp | -9.3 pp | -14.1 pp |
| ATL | -1.7 pp | -3.7 pp | -9.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.5 pp | -0.1 pp | -1.1 pp |
| ATL | +1.2 pp | +3.0 pp | +2.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -3.5 pp | -2.4 pp | -7.6 pp |
| ATL | -0.6 pp | +1.6 pp | +0.7 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 45.9 | 42.1 | 39.7 |
| ATL | 49.8 | 47.9 | 46.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 47.6 | 44.0 | 41.6 |
| ATL | 47.5 | 47.2 | 46.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.7 | -1.8 | -1.9 |
| ATL | 2.3 | 0.7 | 0.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.99 | 1.26 | 0.92 |
| ATL | 1.99 | 1.84 | 1.75 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.83 | 1.33 | 1.27 |
| ATL | 2.39 | 2.20 | 2.11 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.16 | -0.07 | -0.35 |
| ATL | -0.40 | -0.36 | -0.36 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 7.49 | 7.62 | 7.62 |
| ATL | 7.56 | 7.82 | 8.47 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 40.2% | 41.0% | 40.9% |
| ATL | 34.1% | 36.7% | 35.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 59.8% | 59.0% | 59.1% |
| ATL | 65.9% | 63.3% | 64.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 3.01 | 3.12 | 3.12 |
| ATL | 2.58 | 2.87 | 3.01 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.49 | 4.50 | 4.50 |
| ATL | 4.98 | 4.95 | 5.46 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | BAL | ATL |
| --- | ---: | ---: |
| Season-to-date | 1.632 | 1.793 |
| Last 5 games | 1.515 | 1.777 |
| Last 3 games | 1.564 | 1.800 |

## Trend Summary (last 3 weeks)

| Metric | W12 | W13 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BAL | 0.089 | -0.103 | -0.236 | - declining |
| Off EPA ATL | 0.004 | 0.009 | 0.008 | = stable |
| Def EPA BAL | 0.052 | -0.103 | -0.083 | + improving |
| Def EPA ATL | 0.077 | 0.081 | 0.024 | + improving |
| Off SR BAL | 45.5% | 38.6% | 43.2% | - declining |
| Off SR ATL | 44.9% | 50.0% | 43.3% | - declining |
| Def SR BAL | 40.7% | 36.8% | 48.7% | - declining |
| Def SR ATL | 50.6% | 48.2% | 50.0% | = stable |
| Tempo BAL | 8.000 | 7.333 | 8.222 | + improving |
| Tempo ATL | 7.667 | 8.750 | 9.000 | + improving |

## Matchup Analogs

**BAL analogs vs ATL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | JAX (A) | 27-28 | JAX | 0.910 | 0.089 | 45.5% | -0.091 |
| 7 | CLE (H) | 23-22 | BAL | 0.900 | 0.029 | 43.8% | 0.091 |
| 15 | CLE (A) | 3-13 | CLE | 0.900 | -0.236 | 43.2% | -1.111 |

**ATL analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | CIN (A) | 17-35 | CIN | 0.770 | -0.006 | 38.7% | -2.000 |
| 6 | SF (H) | 22-13 | ATL | 0.746 | 0.060 | 52.8% | 1.044 |
| 3 | SEA (A) | 27-26 | ATL | 0.719 | 0.280 | 49.3% | 0.125 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.010 | -0.014 | 0.007 |
| BAL | 0.032 | -0.068 | -0.099 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.034 | 0.043 | 0.061 |
| BAL | -0.028 | -0.095 | -0.076 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 45.4% | 44.9% | 45.9% |
| BAL | 44.6% | 42.7% | 41.3% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 46.9% | 45.8% | 49.6% |
| BAL | 43.3% | 41.8% | 44.2% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 10.5% | 11.3% | 10.0% |
| BAL | 11.8% | 9.3% | 8.9% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 39.1% | 33.2% | 40.7% |
| BAL | 39.4% | 35.0% | 28.8% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.413 | -0.391 | -0.366 |
| BAL | 0.175 | -0.012 | -0.302 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.448 | -0.031 | -0.188 |
| BAL | 0.141 | 0.152 | -0.182 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.071 | -0.200 | -0.333 |
| BAL | 0.357 | 0.000 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 14.3% | 13.2% | 11.9% |
| BAL | 11.3% | 8.0% | 7.0% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 4.3% | 6.2% | 3.0% |
| BAL | 8.2% | 8.7% | 7.7% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 7.562 | 7.817 | 8.472 |
| BAL | 7.494 | 7.621 | 7.619 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 34.3% | 36.7% | 35.4% |
| BAL | 40.2% | 41.2% | 41.3% |
