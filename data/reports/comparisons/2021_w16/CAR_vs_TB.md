# Matchup Report - CAR vs TB

## Metric Comparison

| Metric | CAR | TB | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.115 | 0.050 | -0.165 |
| Core EPA Defense | -0.085 | -0.051 | -0.033 |
| Success Rate Offense | 0.407 | 0.472 | -0.065 |
| Success Rate Defense | 0.399 | 0.428 | -0.029 |
| Explosive Play Rate (Off) | 0.094 | 0.108 | -0.014 |
| Third Down Conversion | 0.325 | 0.437 | -0.112 |
| Points per Drive Differential | -0.129 | 0.595 | -0.723 |
| Yards per Play Differential | 0.167 | 0.711 | -0.544 |
| Turnover Margin | -0.571 | 0.357 | -0.929 |
| Red Zone TD Rate (Off) | 0.156 | 0.138 | 0.018 |
| Pressure Rate (Def) | 0.082 | 0.064 | 0.018 |
| Tempo | 8.000 | 7.455 | 0.545 |
| PowerScore | 1.626 | 1.602 | 0.024 |
| PowerScore | 1.626 | 1.602 | 0.024 |
| Tempo | 8.000 | 7.455 | 0.545 |

## PowerScore Breakdown (Model)

| Component | Weight | CAR | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.239 | 0.148 | -0.387 |
| EPA Defense | 20% | 0.148 | -0.239 | +0.387 |
| Success Rate Offense | 25% | 31.8% | 42.7% | -10.9 pp |
| Tempo | 20% | 0.080 | 0.110 | -0.030 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.239 | 0.148 | -0.387 |
| EPA Defense | 20% | 0.148 | -0.239 | +0.387 |
| Success Rate Offense | 15% | 31.8% | 42.7% | -10.9 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.080 | 0.110 | -0.030 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 13.3% | -13.3 pp |
| Pressure Rate (Def) | 10% | 0.0% | 13.5% | -13.5 pp |

## PowerScore Summary

**Model (4 metrics):**
TB edge: +0.091 (CAR +0.042 vs TB +0.133)
**7 metrics version:**
TB edge: +0.265 (CAR -0.074 vs TB +0.191)
**Verdict:** TB holds the edge in the extended breakdown (lead 0.265), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - TB) | +0.4 pts (favours CAR) |
| Model Win% CAR | 51.8% |
| Model Win% TB | 48.2% |
| SOS Differential (CAR-TB) | -0.150 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 59.1% | 45.2% | 13.9% | 28.8% | 36.1% | Pass heavy |
| TB | 39.0% | 45.2% | -6.2% | 43.8% | 42.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.1 pp | +5.4 pp | +3.9 pp |
| TB | +10.7 pp | +2.0 pp | +5.5 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +2.6 pp | +6.7 pp | +9.2 pp |
| TB | -6.2 pp | -6.6 pp | -6.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.2 pp | +0.0 pp | -2.5 pp |
| TB | +5.0 pp | +4.5 pp | +2.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.4 pp | +1.2 pp | +0.5 pp |
| TB | +1.4 pp | +1.3 pp | +4.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.8 pp | -1.6 pp | -2.2 pp |
| TB | +4.5 pp | +4.9 pp | -3.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -2.9 pp | +1.8 pp | -4.9 pp |
| TB | +9.3 pp | +5.9 pp | +0.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.4 pp | +1.2 pp | +0.5 pp |
| TB | +1.4 pp | +1.3 pp | +4.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.2 pp | +0.0 pp | -2.5 pp |
| TB | +5.0 pp | +4.5 pp | +2.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.7 | 49.0 | 50.6 |
| TB | 47.4 | 42.4 | 38.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 48.7 | 49.2 | 48.7 |
| TB | 49.5 | 48.3 | 44.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -3.0 | -0.3 | 1.9 |
| TB | -2.1 | -6.0 | -5.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.68 | 1.93 | 1.57 |
| TB | 2.46 | 2.17 | 1.73 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.80 | 2.09 | 2.31 |
| TB | 1.87 | 1.43 | 1.29 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.13 | -0.16 | -0.74 |
| TB | 0.59 | 0.74 | 0.44 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 7.27 | 7.18 | 7.29 |
| TB | 7.53 | 7.39 | 7.28 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 45.5% | 45.3% | 48.1% |
| TB | 53.3% | 53.7% | 57.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 54.5% | 54.7% | 51.9% |
| TB | 46.7% | 46.3% | 43.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.30 | 3.25 | 3.50 |
| TB | 4.01 | 3.97 | 4.15 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.96 | 3.93 | 3.79 |
| TB | 3.52 | 3.42 | 3.13 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | CAR | TB |
| --- | ---: | ---: |
| Season-to-date | 1.442 | 1.592 |
| Last 5 games | 1.564 | 1.530 |
| Last 3 games | 1.581 | 1.601 |

## Trend Summary (last 3 weeks)

| Metric | W12 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | -0.334 | -0.138 | -0.152 | + improving |
| Off EPA TB | 0.147 | 0.143 | -0.324 | - declining |
| Def EPA CAR | 0.026 | 0.033 | 0.059 | - declining |
| Def EPA TB | 0.023 | 0.067 | -0.183 | + improving |
| Off SR CAR | 29.3% | 47.4% | 39.5% | + improving |
| Off SR TB | 60.0% | 46.1% | 32.6% | - declining |
| Def SR CAR | 41.8% | 37.6% | 46.0% | - declining |
| Def SR TB | 46.6% | 49.0% | 35.0% | + improving |
| Tempo CAR | 6.250 | 7.800 | 7.818 | + improving |
| Tempo TB | 6.538 | 6.800 | 6.846 | + improving |

## Matchup Analogs

**CAR analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 13 | LA (H) | n/a | n/a | 0.935 | n/a | n/a | n/a |
| 2 | ARI (A) | 26-7 | CAR | 0.926 | 0.008 | 45.7% | 1.583 |
| 8 | BUF (H) | 19-14 | CAR | 0.862 | 0.008 | 46.2% | 0.427 |

**TB analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | NO (A) | 27-30 | NO | 0.967 | -0.082 | 41.0% | -0.250 |
| 14 | NO (H) | 33-31 | TB | 0.967 | 0.143 | 46.1% | 0.123 |
| 1 | DAL (H) | 31-29 | TB | 0.737 | 0.060 | 51.7% | 0.154 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.115 | -0.103 | -0.208 |
| TB | 0.050 | 0.032 | -0.012 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.085 | -0.024 | 0.039 |
| TB | -0.051 | -0.082 | -0.059 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 40.9% | 42.4% | 38.9% |
| TB | 47.3% | 47.1% | 41.4% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 40.1% | 41.5% | 41.8% |
| TB | 43.1% | 42.1% | 42.4% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.4% | 10.7% | 10.0% |
| TB | 10.9% | 10.0% | 10.6% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 32.9% | 33.9% | 33.5% |
| TB | 43.7% | 40.0% | 44.5% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.147 | -0.133 | -0.767 |
| TB | 0.577 | 0.627 | 0.312 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.167 | 0.224 | -0.215 |
| TB | 0.711 | 0.576 | 0.593 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.571 | -0.600 | -1.333 |
| TB | 0.357 | 0.400 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 15.6% | 18.0% | 18.0% |
| TB | 13.8% | 12.0% | 10.6% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 8.2% | 8.3% | 6.1% |
| TB | 6.4% | 6.9% | 7.7% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 7.266 | 7.177 | 7.289 |
| TB | 7.529 | 7.386 | 7.282 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 45.3% | 45.2% | 48.1% |
| TB | 53.3% | 53.5% | 56.4% |
