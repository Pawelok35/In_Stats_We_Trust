# Matchup Report - IND vs JAX

## Metric Comparison

| Metric | IND | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.048 | 0.056 | ↓ -0.008 |
| Core EPA Defense | 0.056 | 0.048 | ↑ +0.008 |
| Success Rate Offense | 54.0% | 47.2% | ↑ +6.8 pp |
| Success Rate Defense | 47.2% | 54.0% | ↓ -6.8 pp |
| Explosive Play Rate (Off) | 9.0% | 9.0% | ↑ +0.0 pp |
| Third Down Conversion | 33.3% | 33.3% | ± 0.0 pp |
| Points per Drive Differential | 0.231 | -0.231 | ↑ +0.462 |
| Yards per Play Differential | 0.706 | -0.706 | ↑ +1.411 |
| Turnover Margin | 1.000 | -1.000 | ↑ +2.000 |
| Red Zone TD Rate (Off) | 8.3% | 14.3% | ↓ -6.0 pp |
| Pressure Rate (Def) | 5.6% | 7.0% | ↓ -1.4 pp |
| Tempo | 7.692 | 6.846 | ↑ +0.846 |
| PowerScore | 0.181 | 0.165 | ↑ +0.016 |

## PowerScore Breakdown (Model)

| Component | Weight | IND | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.048 | 0.056 | -0.008 |
| EPA Defense | 20% | 0.056 | 0.048 | +0.008 |
| Success Rate Offense | 25% | 54.0% | 47.2% | +6.8 pp |
| Tempo | 20% | 0.090 | 0.090 | +0.000 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | IND | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.048 | 0.056 | -0.008 |
| EPA Defense | 20% | 0.056 | 0.048 | +0.008 |
| Success Rate Offense | 15% | 54.0% | 47.2% | +6.8 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.090 | 0.090 | +0.000 |
| Red Zone TD Rate (Off) | 10% | 8.3% | 14.3% | -6.0 pp |
| Pressure Rate (Def) | 10% | 5.6% | 7.0% | -1.4 pp |

## PowerScore Summary

**Model (4 metrics):**
IND edge: +0.016 (IND +0.181 vs JAX +0.165)
**7 metrics version:**
IND edge: +0.202 (IND +0.227 vs JAX +0.025)
**Verdict:** IND holds the edge in the extended breakdown (lead 0.202), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (IND - JAX) | +0.3 pts (favours IND) |
| Model Win% IND | 51.2% |
| Model Win% JAX | 48.8% |
| SOS Differential (IND-JAX) | -0.045 |
*Neutral-field assumption. Based on schedule through Week 17.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| IND | 43.0% | 44.5% | -1.5% | 44.2% | 61.4% | Slight run lean |
| JAX | 40.4% | 44.5% | -4.1% | 41.7% | 50.9% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -4.7 pp | +0.1 pp | +8.2 pp |
| JAX | -7.6 pp | -4.9 pp | +5.4 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -3.7 pp | -5.9 pp | -11.3 pp |
| JAX | -2.6 pp | -6.2 pp | -8.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -0.6 pp | +1.8 pp | -0.6 pp |
| JAX | -0.1 pp | +1.3 pp | -3.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -0.8 pp | -2.3 pp | -2.1 pp |
| JAX | -1.0 pp | -1.4 pp | -1.0 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -1.0 pp | +1.2 pp | +1.3 pp |
| JAX | +2.3 pp | +1.0 pp | +5.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -6.3 pp | +3.2 pp | +2.8 pp |
| JAX | -4.4 pp | -7.6 pp | -3.7 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -0.8 pp | -2.3 pp | -2.1 pp |
| JAX | -1.0 pp | -1.4 pp | -1.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -0.6 pp | +1.8 pp | -0.6 pp |
| JAX | -0.1 pp | +1.3 pp | -3.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 46.6 | 46.8 | 48.1 |
| JAX | 47.6 | 42.6 | 44.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 47.5 | 50.5 | 51.8 |
| JAX | 43.7 | 45.2 | 44.5 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -1.0 | -3.7 | -3.7 |
| JAX | 3.9 | -2.6 | -0.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 1.88 | 1.98 | 2.18 |
| JAX | 1.70 | 1.86 | 2.06 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 2.07 | 2.43 | 2.44 |
| JAX | 2.47 | 1.81 | 2.10 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -0.19 | -0.44 | -0.26 |
| JAX | -0.77 | 0.05 | -0.04 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 6.96 | 7.11 | 6.81 |
| JAX | 7.39 | 8.43 | 8.48 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 40.1% | 35.4% | 33.8% |
| JAX | 45.4% | 46.2% | 44.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 59.9% | 64.6% | 66.2% |
| JAX | 54.6% | 53.8% | 55.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 2.79 | 2.52 | 2.30 |
| JAX | 3.35 | 3.90 | 3.81 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 4.16 | 4.59 | 4.51 |
| JAX | 4.04 | 4.54 | 4.68 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 17)

| Window | IND | JAX |
| --- | ---: | ---: |
| Season-to-date | 0.132 | 0.177 |
| Last 5 games | 0.153 | 0.146 |
| Last 3 games | 0.134 | 0.159 |

## Trend Summary (last 3 weeks)

| Metric | W15 | W16 | W17 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA IND | -0.382 | 0.135 | 0.075 | + improving |
| Off EPA JAX | 0.137 | -0.230 | 0.131 | = stable |
| Def EPA IND | -0.190 | 0.048 | 0.255 | - declining |
| Def EPA JAX | 0.291 | -0.132 | 0.016 | + improving |
| Off SR IND | 35.9% | 51.2% | 54.3% | + improving |
| Off SR JAX | 57.7% | 33.3% | 48.6% | - declining |
| Def SR IND | 39.0% | 50.6% | 46.8% | - declining |
| Def SR JAX | 45.5% | 40.2% | 49.4% | - declining |
| Tempo IND | 6.133 | 6.462 | 7.833 | + improving |
| Tempo JAX | 9.700 | 6.750 | 9.000 | - declining |

## Matchup Analogs

**IND analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | JAX (A) | 34-40 | JAX | 1.000 | 0.172 | 49.5% | -0.468 |
| 11 | NYJ (A) | 28-27 | IND | 0.975 | 0.045 | 41.9% | 0.083 |
| 13 | NE (A) | 25-22 | IND | 0.846 | 0.118 | 56.8% | 0.378 |

**JAX analogs vs IND profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | IND (H) | 37-31 | JAX | 1.000 | 0.235 | 52.6% | 0.468 |
| 14 | TEN (A) | 10-6 | JAX | 0.889 | -0.025 | 42.5% | 0.444 |
| 17 | TEN (H) | 20-13 | JAX | 0.889 | 0.131 | 48.6% | 0.875 |

## Core EPA Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.045 | -0.045 | -0.057 |
| JAX | -0.001 | 0.004 | 0.012 |

## Core EPA Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 0.008 | 0.063 | 0.038 |
| JAX | 0.102 | 0.033 | 0.058 |

## Success Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 44.6% | 47.0% | 47.0% |
| JAX | 45.8% | 45.2% | 47.2% |

## Success Rate Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 46.6% | 47.9% | 45.3% |
| JAX | 47.5% | 44.1% | 44.8% |

## Explosive Play Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 10.8% | 9.2% | 9.6% |
| JAX | 9.8% | 9.0% | 8.4% |

## Third Down Conversion Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 36.1% | 39.1% | 43.4% |
| JAX | 34.2% | 42.7% | 46.7% |

## Points per Drive Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.187 | -0.433 | -0.274 |
| JAX | -0.740 | -0.009 | -0.118 |

## Yards per Play Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.154 | 0.028 | 0.707 |
| JAX | -0.504 | 0.028 | -0.151 |

## Turnover Margin Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.312 | -0.800 | -1.000 |
| JAX | -0.875 | -1.200 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 10.3% | 7.9% | 8.6% |
| JAX | 14.2% | 14.3% | 16.9% |

## Pressure Rate (Def) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 5.6% | 4.9% | 1.8% |
| JAX | 5.1% | 5.5% | 5.5% |

## Tempo Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 6.956 | 7.110 | 6.809 |
| JAX | 7.393 | 8.432 | 8.483 |

## Pass Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 40.3% | 35.6% | 34.4% |
| JAX | 45.7% | 46.7% | 45.6% |
