# Matchup Report - IND vs DEN

## Metric Comparison

| Metric | IND | DEN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.095 | -0.133 | 0.228 |
| Core EPA Defense | 0.126 | -0.047 | 0.173 |
| Success Rate Offense | 0.542 | 0.376 | 0.166 |
| Success Rate Defense | 0.505 | 0.465 | 0.040 |
| Explosive Play Rate (Off) | 0.102 | 0.054 | 0.048 |
| Third Down Conversion | 0.455 | 0.263 | 0.191 |
| Points per Drive Differential | 0.364 | -0.667 | 1.030 |
| Yards per Play Differential | 0.837 | -1.051 | 1.888 |
| Turnover Margin | -1.000 | -3.000 | 2.000 |
| Red Zone TD Rate (Off) | 0.167 | 0.059 | 0.108 |
| Pressure Rate (Def) | 0.105 | 0.065 | 0.041 |
| Tempo | 6.900 | 6.417 | 0.483 |
| PowerScore | 1.462 | 1.350 | 0.111 |
| PowerScore | 1.462 | 1.350 | 0.111 |
| Tempo | 6.900 | 6.417 | 0.483 |

## PowerScore Breakdown (Model)

| Component | Weight | IND | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.098 | -0.118 | +0.020 |
| EPA Defense | 20% | 0.001 | -0.009 | +0.010 |
| Success Rate Offense | 25% | 46.4% | 44.2% | +2.2 pp |
| Tempo | 20% | 0.174 | 0.091 | +0.083 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | IND | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.098 | -0.118 | +0.020 |
| EPA Defense | 20% | 0.001 | -0.009 | +0.010 |
| Success Rate Offense | 15% | 46.4% | 44.2% | +2.2 pp |
| Turnover Margin | 10% | -2.000 | -2.000 | +0.000 |
| Tempo | 10% | 0.174 | 0.091 | +0.083 |
| Red Zone TD Rate (Off) | 10% | 10.0% | 0.0% | +10.0 pp |
| Pressure Rate (Def) | 10% | 0.0% | 9.1% | -9.1 pp |

## PowerScore Summary

**Model (4 metrics):**
IND edge: +0.031 (IND +0.117 vs DEN +0.085)
**7 metrics version:**
IND edge: +0.020 (IND -0.127 vs DEN -0.147)
**Verdict:** IND holds the edge in the extended breakdown (lead 0.020), driven by Red Zone TD Rate (Off) (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (IND - DEN) | +2.1 pts (favours IND) |
| Model Win% IND | 58.5% |
| Model Win% DEN | 41.5% |
| SOS Differential (IND-DEN) | +0.733 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| IND | 50.7% | 42.1% | 8.6% | 40.9% | 36.7% | Pass heavy |
| DEN | 48.1% | 42.1% | 6.0% | 57.1% | 35.7% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | +14.7 pp | +14.7 pp | +14.7 pp |
| DEN | -12.6 pp | -12.6 pp | -12.6 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | +16.7 pp | +16.7 pp | +16.7 pp |
| DEN | -14.1 pp | -14.1 pp | -14.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -3.1 pp | -3.1 pp | -3.1 pp |
| DEN | +6.0 pp | +6.0 pp | +6.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | +2.0 pp | +2.0 pp | +2.0 pp |
| DEN | -4.9 pp | -4.9 pp | -4.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | +17.5 pp | +17.5 pp | +17.5 pp |
| DEN | -7.0 pp | -7.0 pp | -7.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -10.1 pp | -10.1 pp | -10.1 pp |
| DEN | -17.1 pp | -17.1 pp | -17.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | +2.0 pp | +2.0 pp | +2.0 pp |
| DEN | -4.9 pp | -4.9 pp | -4.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | -3.1 pp | -3.1 pp | -3.1 pp |
| DEN | +6.0 pp | +6.0 pp | +6.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 64.8 | 64.8 | 64.8 |
| DEN | 60.3 | 60.3 | 60.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 42.4 | 42.4 | 42.4 |
| DEN | 36.1 | 36.1 | 36.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 22.4 | 22.4 | 22.4 |
| DEN | 24.2 | 24.2 | 24.2 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 3.00 | 3.00 | 3.00 |
| DEN | 1.07 | 1.07 | 1.07 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 2.64 | 2.64 | 2.64 |
| DEN | 1.73 | 1.73 | 1.73 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 0.36 | 0.36 | 0.36 |
| DEN | -0.67 | -0.67 | -0.67 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 6.56 | 6.56 | 6.56 |
| DEN | 6.20 | 6.20 | 6.20 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 35.6% | 35.6% | 35.6% |
| DEN | 47.3% | 47.3% | 47.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 64.4% | 64.4% | 64.4% |
| DEN | 52.7% | 52.7% | 52.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 2.33 | 2.33 | 2.33 |
| DEN | 2.93 | 2.93 | 2.93 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| IND | 4.22 | 4.22 | 4.22 |
| DEN | 3.27 | 3.27 | 3.27 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | IND | DEN |
| --- | ---: | ---: |
| Season-to-date | 1.953 | 1.220 |
| Last 5 games | 1.953 | 1.220 |
| Last 3 games | 1.953 | 1.220 |

## Matchup Analogs

**IND analogs vs DEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | HOU (H) | 27-24 | IND | 0.175 | 0.095 | 54.2% | 0.364 |

**DEN analogs vs IND profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | SEA (A) | 16-26 | SEA | 0.178 | -0.133 | 37.6% | -0.667 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.133 | -0.133 | -0.133 |
| IND | 0.095 | 0.095 | 0.095 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.047 | -0.047 | -0.047 |
| IND | 0.126 | 0.126 | 0.126 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 37.6% | 37.6% | 37.6% |
| IND | 54.2% | 54.2% | 54.2% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 46.5% | 46.5% | 46.5% |
| IND | 50.5% | 50.5% | 50.5% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 5.4% | 5.4% | 5.4% |
| IND | 10.2% | 10.2% | 10.2% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 26.3% | 26.3% | 26.3% |
| IND | 45.5% | 45.5% | 45.5% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.667 | -0.667 | -0.667 |
| IND | 0.364 | 0.364 | 0.364 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -1.051 | -1.051 | -1.051 |
| IND | 0.837 | 0.837 | 0.837 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -3.000 | -3.000 | -3.000 |
| IND | -1.000 | -1.000 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 5.9% | 5.9% | 5.9% |
| IND | 16.7% | 16.7% | 16.7% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 6.5% | 6.5% | 6.5% |
| IND | 10.5% | 10.5% | 10.5% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 6.200 | 6.200 | 6.200 |
| IND | 6.556 | 6.556 | 6.556 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 47.3% | 47.3% | 47.3% |
| IND | 35.6% | 35.6% | 35.6% |
