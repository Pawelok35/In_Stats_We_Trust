# Matchup Report - DEN vs PIT

## Metric Comparison

| Metric | DEN | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.133 | -0.041 | -0.092 |
| Core EPA Defense | -0.047 | -0.240 | 0.192 |
| Success Rate Offense | 0.376 | 0.429 | -0.052 |
| Success Rate Defense | 0.465 | 0.471 | -0.006 |
| Explosive Play Rate (Off) | 0.054 | 0.048 | 0.006 |
| Third Down Conversion | 0.263 | 0.421 | -0.158 |
| Points per Drive Differential | -0.667 | 0.800 | -1.467 |
| Yards per Play Differential | -1.051 | -0.014 | -1.037 |
| Turnover Margin | -3.000 | 3.000 | -6.000 |
| Red Zone TD Rate (Off) | 0.059 | 0.000 | 0.059 |
| Pressure Rate (Def) | 0.065 | 0.071 | -0.007 |
| Tempo | 6.417 | 7.455 | -1.038 |
| PowerScore | 1.350 | 1.558 | -0.208 |
| PowerScore | 1.350 | 1.558 | -0.208 |
| Tempo | 6.417 | 7.455 | -1.038 |

## PowerScore Breakdown (Model)

| Component | Weight | DEN | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.118 | -0.009 | -0.109 |
| EPA Defense | 20% | -0.009 | -0.118 | +0.109 |
| Success Rate Offense | 25% | 44.2% | 37.8% | +6.4 pp |
| Tempo | 20% | 0.091 | 0.061 | +0.030 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | DEN | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.118 | -0.009 | -0.109 |
| EPA Defense | 20% | -0.009 | -0.118 | +0.109 |
| Success Rate Offense | 15% | 44.2% | 37.8% | +6.4 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.091 | 0.061 | +0.030 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 11.1% | -11.1 pp |
| Pressure Rate (Def) | 10% | 9.1% | 5.4% | +3.7 pp |

## PowerScore Summary

**Model (4 metrics):**
DEN edge: +0.006 (DEN +0.085 vs PIT +0.080)
**7 metrics version:**
PIT edge: +0.400 (DEN -0.147 vs PIT +0.253)
**Verdict:** PIT holds the edge in the extended breakdown (lead 0.400), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (DEN - PIT) | -3.8 pts (favours PIT) |
| Model Win% DEN | 34.5% |
| Model Win% PIT | 65.5% |
| SOS Differential (DEN-PIT) | -0.206 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| DEN | 48.1% | 42.1% | 6.0% | 35.1% | 52.5% | Pass heavy |
| PIT | 26.8% | 42.1% | -15.2% | 40.9% | 36.7% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | +8.1 pp | +8.1 pp | +8.1 pp |
| PIT | +11.3 pp | +11.3 pp | +11.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -6.6 pp | -6.6 pp | -6.6 pp |
| PIT | +0.0 pp | +0.0 pp | +0.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | +2.6 pp | +2.6 pp | +2.6 pp |
| PIT | -1.5 pp | -1.5 pp | -1.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -3.2 pp | -3.2 pp | -3.2 pp |
| PIT | -3.4 pp | -3.4 pp | -3.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -1.0 pp | -1.0 pp | -1.0 pp |
| PIT | -5.1 pp | -5.1 pp | -5.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -17.9 pp | -17.9 pp | -17.9 pp |
| PIT | -2.7 pp | -2.7 pp | -2.7 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -3.2 pp | -3.2 pp | -3.2 pp |
| PIT | -3.4 pp | -3.4 pp | -3.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | +2.6 pp | +2.6 pp | +2.6 pp |
| PIT | -1.5 pp | -1.5 pp | -1.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 60.3 | 60.3 | 60.3 |
| PIT | 47.8 | 47.8 | 47.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 36.1 | 36.1 | 36.1 |
| PIT | 49.5 | 49.5 | 49.5 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 24.2 | 24.2 | 24.2 |
| PIT | -1.7 | -1.7 | -1.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 1.07 | 1.07 | 1.07 |
| PIT | 1.80 | 1.80 | 1.80 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 1.73 | 1.73 | 1.73 |
| PIT | 1.00 | 1.00 | 1.00 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | -0.67 | -0.67 | -0.67 |
| PIT | 0.80 | 0.80 | 0.80 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 6.20 | 6.20 | 6.20 |
| PIT | 8.40 | 8.40 | 8.40 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 47.3% | 47.3% | 47.3% |
| PIT | 29.8% | 29.8% | 29.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 52.7% | 52.7% | 52.7% |
| PIT | 70.2% | 70.2% | 70.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 2.93 | 2.93 | 2.93 |
| PIT | 2.50 | 2.50 | 2.50 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DEN | 3.27 | 3.27 | 3.27 |
| PIT | 5.90 | 5.90 | 5.90 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | DEN | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.220 | 1.426 |
| Last 5 games | 1.220 | 1.426 |
| Last 3 games | 1.220 | 1.426 |

## Matchup Analogs

**DEN analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | SEA (A) | 16-26 | SEA | 0.754 | -0.133 | 37.6% | -0.667 |

**PIT analogs vs DEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | ATL (A) | 18-10 | PIT | 0.753 | -0.041 | 42.9% | 0.800 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.133 | -0.133 | -0.133 |
| PIT | -0.041 | -0.041 | -0.041 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.047 | -0.047 | -0.047 |
| PIT | -0.240 | -0.240 | -0.240 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 37.6% | 37.6% | 37.6% |
| PIT | 42.9% | 42.9% | 42.9% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 46.5% | 46.5% | 46.5% |
| PIT | 47.1% | 47.1% | 47.1% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 5.4% | 5.4% | 5.4% |
| PIT | 4.8% | 4.8% | 4.8% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 26.3% | 26.3% | 26.3% |
| PIT | 42.1% | 42.1% | 42.1% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -0.667 | -0.667 | -0.667 |
| PIT | 0.800 | 0.800 | 0.800 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -1.051 | -1.051 | -1.051 |
| PIT | -0.014 | -0.014 | -0.014 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | -3.000 | -3.000 | -3.000 |
| PIT | 3.000 | 3.000 | 3.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 5.9% | 5.9% | 5.9% |
| PIT | 0.0% | 0.0% | 0.0% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 6.5% | 6.5% | 6.5% |
| PIT | 7.1% | 7.1% | 7.1% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 6.200 | 6.200 | 6.200 |
| PIT | 8.400 | 8.400 | 8.400 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DEN | 47.3% | 47.3% | 47.3% |
| PIT | 29.8% | 29.8% | 29.8% |
