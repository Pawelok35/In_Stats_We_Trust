# Matchup Report - BAL vs LV

## Metric Comparison

| Metric | BAL | LV | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.034 | 0.030 | ↓ -0.064 |
| Core EPA Defense | 0.030 | -0.034 | ↑ +0.064 |
| Success Rate Offense | 44.9% | 49.4% | ↓ -4.4 pp |
| Success Rate Defense | 49.4% | 44.9% | ↑ +4.4 pp |
| Explosive Play Rate (Off) | 11.2% | 7.6% | ↑ +3.6 pp |
| Third Down Conversion | 21.4% | 30.8% | ↓ -9.3 pp |
| Points per Drive Differential | -0.250 | 0.250 | ↓ -0.500 |
| Yards per Play Differential | 0.731 | -0.731 | ↑ +1.463 |
| Turnover Margin | 0.000 | 0.000 | ± 0.000 |
| Red Zone TD Rate (Off) | 28.6% | 14.3% | ↑ +14.3 pp |
| Pressure Rate (Def) | 11.4% | 5.6% | ↑ +5.8 pp |
| Tempo | 7.417 | 6.583 | ↑ +0.833 |
| PowerScore | 0.129 | 0.142 | ↓ -0.013 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | LV | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.034 | 0.030 | -0.064 |
| EPA Defense | 20% | 0.030 | -0.034 | +0.064 |
| Success Rate Offense | 25% | 44.9% | 49.4% | -4.4 pp |
| Tempo | 20% | 0.112 | 0.076 | +0.036 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | LV | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.034 | 0.030 | -0.064 |
| EPA Defense | 20% | 0.030 | -0.034 | +0.064 |
| Success Rate Offense | 15% | 44.9% | 49.4% | -4.4 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.112 | 0.076 | +0.036 |
| Red Zone TD Rate (Off) | 10% | 28.6% | 14.3% | +14.3 pp |
| Pressure Rate (Def) | 10% | 11.4% | 5.6% | +5.8 pp |

## PowerScore Summary

**Model (4 metrics):**
LV edge: +0.013 (BAL +0.129 vs LV +0.142)
**7 metrics version:**
BAL edge: +0.014 (BAL +0.116 vs LV +0.102)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.014), driven by Red Zone TD Rate (Off) (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - LV) | -0.2 pts (favours LV) |
| Model Win% BAL | 49.0% |
| Model Win% LV | 51.0% |
| SOS Differential (BAL-LV) | +0.192 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 40.4% | 42.1% | -1.6% | 47.2% | 43.4% | Slight run lean |
| LV | 55.7% | 42.1% | 13.6% | 47.7% | 51.4% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +18.8 pp | +18.8 pp | +18.8 pp |
| LV | -6.7 pp | -6.7 pp | -6.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -8.4 pp | -8.4 pp | -8.4 pp |
| LV | -7.7 pp | -7.7 pp | -7.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.0 pp | +1.0 pp | +1.0 pp |
| LV | -4.4 pp | -4.4 pp | -4.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +4.8 pp | +4.8 pp | +4.8 pp |
| LV | -3.4 pp | -3.4 pp | -3.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +27.5 pp | +27.5 pp | +27.5 pp |
| LV | -5.9 pp | -5.9 pp | -5.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +8.6 pp | +8.6 pp | +8.6 pp |
| LV | -11.6 pp | -11.6 pp | -11.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +4.8 pp | +4.8 pp | +4.8 pp |
| LV | -3.4 pp | -3.4 pp | -3.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.0 pp | +1.0 pp | +1.0 pp |
| LV | -4.4 pp | -4.4 pp | -4.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 49.2 | 49.2 | 49.2 |
| LV | 44.2 | 44.2 | 44.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 52.3 | 52.3 | 52.3 |
| LV | 42.2 | 42.2 | 42.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -3.1 | -3.1 | -3.1 |
| LV | 2.0 | 2.0 | 2.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.00 | 2.00 | 2.00 |
| LV | 0.77 | 0.77 | 0.77 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.70 | 2.70 | 2.70 |
| LV | 1.69 | 1.69 | 1.69 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -0.70 | -0.70 | -0.70 |
| LV | -0.92 | -0.92 | -0.92 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 9.60 | 9.60 | 9.60 |
| LV | 5.69 | 5.69 | 5.69 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 43.8% | 43.8% | 43.8% |
| LV | 50.0% | 50.0% | 50.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 56.2% | 56.2% | 56.2% |
| LV | 50.0% | 50.0% | 50.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.20 | 4.20 | 4.20 |
| LV | 2.85 | 2.85 | 2.85 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 5.40 | 5.40 | 5.40 |
| LV | 2.85 | 2.85 | 2.85 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | BAL | LV |
| --- | ---: | ---: |
| Season-to-date | 0.191 | -0.000 |
| Last 5 games | 0.191 | -0.000 |
| Last 3 games | 0.191 | -0.000 |

## Matchup Analogs

**BAL analogs vs LV profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | KC (A) | 20-27 | KC | 0.149 | 0.021 | 55.2% | -0.700 |

**LV analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | LAC (A) | 10-22 | LAC | 0.070 | -0.312 | 39.2% | -0.923 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.021 | 0.021 | 0.021 |
| LV | -0.312 | -0.312 | -0.312 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.124 | 0.124 | 0.124 |
| LV | -0.123 | -0.123 | -0.123 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 55.2% | 55.2% | 55.2% |
| LV | 39.2% | 39.2% | 39.2% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 47.1% | 47.1% | 47.1% |
| LV | 35.9% | 35.9% | 35.9% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 12.5% | 12.5% | 12.5% |
| LV | 9.5% | 9.5% | 9.5% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 43.8% | 43.8% | 43.8% |
| LV | 33.3% | 33.3% | 33.3% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.700 | -0.700 | -0.700 |
| LV | -0.923 | -0.923 | -0.923 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.335 | -0.335 | -0.335 |
| LV | -0.051 | -0.051 | -0.051 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.000 | 0.000 | 0.000 |
| LV | -3.000 | -3.000 | -3.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 5.9% | 5.9% | 5.9% |
| LV | 0.0% | 0.0% | 0.0% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 6.5% | 6.5% | 6.5% |
| LV | 3.3% | 3.3% | 3.3% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 9.600 | 9.600 | 9.600 |
| LV | 5.692 | 5.692 | 5.692 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 43.8% | 43.8% | 43.8% |
| LV | 50.0% | 50.0% | 50.0% |
