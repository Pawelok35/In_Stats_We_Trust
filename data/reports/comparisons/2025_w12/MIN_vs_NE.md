# Matchup Report - MIN vs NE

## PowerScore Breakdown (Model)

| Component | Weight | MIN | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.181 | 0.121 | -0.302 |
| EPA Defense | 20% | -0.084 | -0.052 | -0.032 |
| Success Rate Offense | 25% | 39.4% | 51.2% | -11.8 pp |
| Tempo | 20% | 0.099 | 0.098 | +0.001 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | MIN | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.181 | 0.121 | -0.302 |
| EPA Defense | 20% | -0.084 | -0.052 | -0.032 |
| Success Rate Offense | 15% | 39.4% | 51.2% | -11.8 pp |
| Turnover Margin | 10% | -2.000 | 1.000 | -3.000 |
| Tempo | 10% | 0.099 | 0.098 | +0.001 |
| Red Zone TD Rate (Off) | 10% | 25.0% | 20.0% | +5.0 pp |
| Pressure Rate (Def) | 10% | 5.9% | 6.9% | -1.0 pp |

## PowerScore Summary

**Model (4 metrics):**
NE edge: +0.141 (MIN +0.038 vs NE +0.180)
**7 metrics version:**
NE edge: +0.395 (MIN -0.162 vs NE +0.233)
**Verdict:** NE holds the edge in the extended breakdown (lead 0.395), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (MIN - NE) | -2.6 pts (favours NE) |
| Model Win% MIN | 39.3% |
| Model Win% NE | 60.7% |
| SOS Differential (MIN-NE) | -0.035 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| MIN | 45.1% | n/a | n/a | 34.5% | 65.9% | n/a |
| NE | 45.1% | n/a | n/a | 38.2% | 47.5% | n/a |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.2 pp | +2.3 pp | -1.0 pp |
| NE | +4.5 pp | +5.1 pp | +0.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -1.0 pp | +4.8 pp | +2.4 pp |
| NE | +2.6 pp | +3.5 pp | +4.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.7 pp | -1.7 pp | -5.5 pp |
| NE | -1.1 pp | -2.3 pp | -3.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +0.4 pp | +2.4 pp | +0.5 pp |
| NE | +0.9 pp | +0.6 pp | +2.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -0.9 pp | -3.9 pp | -6.5 pp |
| NE | -2.2 pp | -1.8 pp | -2.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -3.8 pp | +0.9 pp | -1.1 pp |
| NE | +10.5 pp | +8.7 pp | +4.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | +0.4 pp | +2.4 pp | +0.5 pp |
| NE | +0.9 pp | +0.6 pp | +2.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -4.7 pp | -1.7 pp | -5.5 pp |
| NE | -1.1 pp | -2.3 pp | -3.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 48.2 | 48.8 | 53.5 |
| NE | 48.2 | 46.8 | 43.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 47.6 | 46.7 | 44.0 |
| NE | 48.6 | 50.8 | 51.9 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 0.6 | 2.1 | 9.4 |
| NE | -0.4 | -4.0 | -8.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.78 | 1.70 | 1.52 |
| NE | 2.47 | 2.49 | 2.40 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 1.97 | 2.30 | 2.63 |
| NE | 1.82 | 1.67 | 1.65 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | -0.19 | -0.60 | -1.11 |
| NE | 0.65 | 0.82 | 0.75 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 6.76 | 6.76 | 6.30 |
| NE | 7.85 | 7.88 | 7.40 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 45.5% | 48.6% | 46.9% |
| NE | 42.4% | 40.8% | 43.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 54.5% | 51.4% | 53.1% |
| NE | 57.6% | 59.2% | 56.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.08 | 3.29 | 2.96 |
| NE | 3.33 | 3.21 | 3.21 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| MIN | 3.68 | 3.47 | 3.35 |
| NE | 4.52 | 4.67 | 4.18 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | MIN | NE |
| --- | ---: | ---: |
| Season-to-date | 0.101 | 0.136 |
| Last 5 games | 0.126 | 0.113 |
| Last 3 games | 0.115 | 0.096 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA MIN | -0.255 | 0.017 | -0.181 | + improving |
| Off EPA NE | -0.008 | 0.022 | 0.121 | + improving |
| Def EPA MIN | 0.170 | -0.062 | -0.084 | + improving |
| Def EPA NE | -0.306 | 0.016 | -0.052 | - declining |
| Off SR MIN | 37.3% | 42.0% | 39.4% | + improving |
| Off SR NE | 48.2% | 45.1% | 51.2% | + improving |
| Def SR MIN | 54.5% | 44.0% | 44.2% | + improving |
| Def SR NE | 33.8% | 43.8% | 52.9% | - declining |
| Tempo MIN | 6.700 | 6.750 | 5.462 | - declining |
| Tempo NE | 6.538 | 7.455 | 8.200 | + improving |

## Matchup Analogs

**MIN analogs vs NE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 9 | DET (A) | 27-26 | MIN | 0.797 | 0.017 | 42.0% | 0.068 |
| 10 | BUF (A) | n/a | n/a | 0.749 | n/a | n/a | n/a |
| 7 | PHI (A) | 22-22 | TIE | 0.645 | -0.019 | 54.2% | 0.000 |

**NE analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | CLE (A) | 30-14 | NE | 0.956 | -0.008 | 48.2% | 1.224 |
| 6 | NO (H) | 25-19 | NE | 0.817 | 0.091 | 48.1% | 0.667 |
| 9 | ATL (H) | 24-25 | ATL | 0.814 | 0.022 | 45.1% | -0.118 |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | -0.081 | -0.079 | -0.140 |
| NE | 0.075 | 0.077 | 0.045 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | -0.043 | 0.016 | 0.008 |
| NE | -0.044 | -0.093 | -0.114 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 44.9% | 44.4% | 39.7% |
| NE | 47.3% | 47.8% | 48.2% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 44.5% | 45.4% | 47.6% |
| NE | 46.8% | 45.6% | 43.6% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 10.1% | 10.5% | 9.6% |
| NE | 11.4% | 11.9% | 12.9% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 29.1% | 30.9% | 27.0% |
| NE | 37.9% | 40.4% | 44.0% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | -0.100 | -0.524 | -0.968 |
| NE | 0.668 | 0.831 | 0.767 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | -0.169 | -0.440 | -0.798 |
| NE | 0.432 | 0.680 | 0.840 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | -0.444 | -1.200 | -0.667 |
| NE | 0.100 | 0.400 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 15.0% | 16.8% | 19.3% |
| NE | 14.5% | 12.0% | 15.8% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 8.9% | 8.4% | 8.4% |
| NE | 6.2% | 6.4% | 4.1% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 6.756 | 6.759 | 6.304 |
| NE | 7.849 | 7.879 | 7.398 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| MIN | 45.7% | 48.6% | 46.6% |
| NE | 42.7% | 40.8% | 43.4% |
