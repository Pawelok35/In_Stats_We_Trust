# Matchup Report - CAR vs DEN

## PowerScore Breakdown (Model)

| Component | Weight | CAR | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.104 | 0.070 | +0.034 |
| EPA Defense | 20% | 0.043 | 0.044 | -0.001 |
| Success Rate Offense | 25% | 44.7% | 48.8% | -4.1 pp |
| Tempo | 20% | 0.128 | 0.073 | +0.054 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.104 | 0.070 | +0.034 |
| EPA Defense | 20% | 0.043 | 0.044 | -0.001 |
| Success Rate Offense | 15% | 44.7% | 48.8% | -4.1 pp |
| Turnover Margin | 10% | 1.000 | 1.000 | +0.000 |
| Tempo | 10% | 0.128 | 0.073 | +0.054 |
| Red Zone TD Rate (Off) | 10% | 7.1% | 5.3% | +1.9 pp |
| Pressure Rate (Def) | 10% | 0.0% | 6.2% | -6.2 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.012 (CAR +0.182 vs DEN +0.170)
**7 metrics version:**
CAR edge: +0.003 (CAR +0.221 vs DEN +0.218)
**Verdict:** CAR holds the edge in the extended breakdown (lead 0.003), driven by EPA Offense (25% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - DEN) | +0.2 pts (favours CAR) |
| Model Win% CAR | 51.0% |
| Model Win% DEN | 49.0% |
| SOS Differential (CAR-DEN) | +0.032 |
*Neutral-field assumption. Based on schedule through Week 11.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 56.4% | n/a | n/a | 39.6% | 56.4% | n/a |
| DEN | 50.0% | n/a | n/a | 56.7% | 50.0% | n/a |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +7.6 pp | +9.2 pp | +9.8 pp |
| DEN | -0.9 pp | -1.9 pp | +1.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +5.3 pp | +3.8 pp | +3.6 pp |
| DEN | +1.4 pp | -1.1 pp | -9.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +4.9 pp | +3.6 pp | -6.0 pp |
| DEN | +1.1 pp | +2.3 pp | +1.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +4.0 pp | +5.3 pp | +4.6 pp |
| DEN | +1.0 pp | +1.4 pp | -0.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +1.9 pp | +0.8 pp | -3.4 pp |
| DEN | +3.6 pp | +0.5 pp | -1.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +7.1 pp | +9.5 pp | +5.6 pp |
| DEN | -4.8 pp | -5.8 pp | -6.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +4.0 pp | +5.3 pp | +4.6 pp |
| DEN | +1.0 pp | +1.4 pp | -0.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +4.9 pp | +3.6 pp | -6.0 pp |
| DEN | +1.1 pp | +2.3 pp | +1.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 47.3 | 46.6 | 48.1 |
| DEN | 44.7 | 43.9 | 44.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 48.7 | 47.1 | 45.9 |
| DEN | 44.1 | 46.3 | 48.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.4 | -0.5 | 2.3 |
| DEN | 0.6 | -2.4 | -3.6 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.96 | 1.96 | 1.87 |
| DEN | 2.14 | 2.10 | 2.35 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.16 | 2.25 | 2.57 |
| DEN | 1.54 | 1.69 | 1.69 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.21 | -0.29 | -0.70 |
| DEN | 0.60 | 0.41 | 0.66 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 8.26 | 8.35 | 8.36 |
| DEN | 7.26 | 6.76 | 6.76 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 42.4% | 39.7% | 41.9% |
| DEN | 45.4% | 47.1% | 46.2% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 57.6% | 60.3% | 58.1% |
| DEN | 54.6% | 52.9% | 53.8% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.50 | 3.32 | 3.50 |
| DEN | 3.30 | 3.19 | 3.12 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 4.76 | 5.03 | 4.85 |
| DEN | 3.96 | 3.58 | 3.63 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 11)

| Window | CAR | DEN |
| --- | ---: | ---: |
| Season-to-date | 0.145 | 0.113 |
| Last 5 games | 0.156 | 0.124 |
| Last 3 games | 0.184 | 0.136 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W11 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | -0.210 | 0.066 | 0.104 | + improving |
| Off EPA DEN | 0.331 | -0.155 | 0.070 | - declining |
| Def EPA CAR | 0.283 | -0.000 | 0.043 | + improving |
| Def EPA DEN | 0.063 | -0.155 | 0.044 | + improving |
| Off SR CAR | 47.4% | 49.3% | 44.7% | - declining |
| Off SR DEN | 59.5% | 34.6% | 48.8% | - declining |
| Def SR CAR | 51.9% | 50.0% | 52.3% | = stable |
| Def SR DEN | 55.3% | 34.4% | 47.1% | + improving |
| Tempo CAR | 7.091 | 10.143 | 7.833 | + improving |
| Tempo DEN | 6.583 | 6.231 | 7.455 | + improving |

## Matchup Analogs

**CAR analogs vs DEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | NE (A) | 19-36 | NE | 0.716 | -0.179 | 47.1% | -1.545 |
| 9 | GB (A) | 16-11 | CAR | 0.497 | 0.066 | 49.3% | 0.661 |
| 1 | JAX (A) | 10-26 | JAX | 0.481 | -0.108 | 39.5% | -1.600 |

**DEN analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | NYJ (H) | 13-8 | DEN | 0.813 | -0.108 | 34.2% | 0.391 |
| 1 | TEN (H) | 20-12 | DEN | 0.770 | -0.108 | 41.9% | 0.571 |
| 10 | TEN (A) | n/a | n/a | 0.770 | n/a | n/a | n/a |

## Core EPA Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.005 | 0.012 | -0.013 |
| DEN | 0.044 | 0.048 | 0.082 |

## Core EPA Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.045 | 0.061 | 0.108 |
| DEN | -0.034 | -0.010 | -0.016 |

## Success Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 46.6% | 47.4% | 46.9% |
| DEN | 45.1% | 44.4% | 47.5% |

## Success Rate Defense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 45.0% | 47.2% | 51.4% |
| DEN | 42.3% | 42.8% | 45.6% |

## Explosive Play Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.6% | 10.8% | 10.3% |
| DEN | 9.9% | 9.1% | 9.1% |

## Third Down Conversion Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 32.7% | 31.9% | 35.1% |
| DEN | 37.2% | 38.7% | 44.0% |

## Points per Drive Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.249 | -0.359 | -0.843 |
| DEN | 0.557 | 0.392 | 0.664 |

## Yards per Play Differential Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.341 | -0.148 | -0.649 |
| DEN | 0.904 | 1.013 | 0.957 |

## Turnover Margin Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.100 | 0.000 | -0.333 |
| DEN | 0.200 | 0.400 | 0.667 |

## Red Zone TD Rate (Off) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 11.9% | 10.1% | 9.3% |
| DEN | 13.3% | 10.5% | 6.8% |

## Pressure Rate (Def) Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 4.2% | 5.1% | 3.5% |
| DEN | 12.0% | 13.2% | 6.9% |

## Tempo Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 8.264 | 8.350 | 8.356 |
| DEN | 7.258 | 6.763 | 6.756 |

## Pass Rate Offense Form (up to Week 11)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.1% | 40.4% | 43.2% |
| DEN | 45.8% | 47.5% | 46.3% |
