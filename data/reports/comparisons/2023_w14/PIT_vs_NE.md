# Matchup Report - PIT vs NE

## Metric Comparison

| Metric | PIT | NE | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.091 | -0.170 | 0.079 |
| Core EPA Defense | -0.037 | -0.032 | -0.005 |
| Success Rate Offense | 0.389 | 0.382 | 0.006 |
| Success Rate Defense | 0.414 | 0.410 | 0.004 |
| Explosive Play Rate (Off) | 0.086 | 0.071 | 0.014 |
| Third Down Conversion | 0.323 | 0.305 | 0.019 |
| Points per Drive Differential | -0.415 | -0.545 | 0.130 |
| Yards per Play Differential | -0.341 | -0.102 | -0.239 |
| Turnover Margin | 1.000 | -0.750 | 1.750 |
| Red Zone TD Rate (Off) | 0.100 | 0.109 | -0.009 |
| Pressure Rate (Def) | 0.073 | 0.053 | 0.021 |
| Tempo | 6.143 | 5.692 | 0.451 |
| PowerScore | 0.025 | 0.020 | 0.005 |
| PowerScore | 0.025 | 0.020 | 0.005 |
| Tempo | 6.143 | 5.692 | 0.451 |

## PowerScore Breakdown (Model)

| Component | Weight | PIT | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.173 | -0.138 | -0.035 |
| EPA Defense | 20% | -0.138 | -0.173 | +0.035 |
| Success Rate Offense | 25% | 40.7% | 32.4% | +8.3 pp |
| Tempo | 20% | 0.058 | 0.108 | -0.050 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | PIT | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.173 | -0.138 | -0.035 |
| EPA Defense | 20% | -0.138 | -0.173 | +0.035 |
| Success Rate Offense | 15% | 40.7% | 32.4% | +8.3 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.058 | 0.108 | -0.050 |
| Red Zone TD Rate (Off) | 10% | 10.0% | 28.6% | -18.6 pp |
| Pressure Rate (Def) | 10% | 6.7% | 5.0% | +1.7 pp |

## PowerScore Summary

**Model (4 metrics):**
PIT edge: +0.005 (PIT +0.025 vs NE +0.020)
**7 metrics version:**
NE edge: +0.011 (PIT +0.013 vs NE +0.024)
**Verdict:** NE holds the edge in the extended breakdown (lead 0.011), driven by Red Zone TD Rate (Off) (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (PIT - NE) | +0.1 pts (favours PIT) |
| Model Win% PIT | 50.4% |
| Model Win% NE | 49.6% |
| SOS Differential (PIT-NE) | +0.099 |
*Neutral-field assumption. Based on schedule through Week 13.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| PIT | 46.5% | 46.0% | 0.5% | 40.0% | 41.3% | Balanced |
| NE | 40.5% | 46.0% | -5.4% | 43.3% | 25.0% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -3.0 pp | -2.5 pp | +1.5 pp |
| NE | -4.4 pp | -6.1 pp | +0.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.9 pp | +0.5 pp | -5.2 pp |
| NE | -1.2 pp | -1.2 pp | -9.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.4 pp | +2.0 pp | +1.4 pp |
| NE | +0.3 pp | -2.5 pp | -5.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +0.7 pp | +1.2 pp | +1.8 pp |
| NE | -3.1 pp | -2.2 pp | -0.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +1.0 pp | +4.2 pp | +3.7 pp |
| NE | -3.3 pp | -1.9 pp | +3.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -5.9 pp | -0.2 pp | -3.9 pp |
| NE | -2.8 pp | +1.1 pp | +1.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +0.7 pp | +1.2 pp | +1.8 pp |
| NE | -3.1 pp | -2.2 pp | -0.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.4 pp | +2.0 pp | +1.4 pp |
| NE | +0.3 pp | -2.5 pp | -5.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 43.7 | 40.7 | 38.9 |
| NE | 44.2 | 37.3 | 31.0 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 43.8 | 43.8 | 42.7 |
| NE | 42.8 | 39.3 | 37.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.2 | -3.1 | -3.8 |
| NE | 1.4 | -2.1 | -6.1 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 1.31 | 1.52 | 1.09 |
| NE | 1.11 | 0.83 | 0.39 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 1.73 | 1.58 | 1.46 |
| NE | 1.65 | 1.37 | 0.79 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.42 | -0.06 | -0.37 |
| NE | -0.55 | -0.54 | -0.40 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 6.94 | 7.54 | 7.12 |
| NE | 7.10 | 7.00 | 7.35 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 44.1% | 38.3% | 41.1% |
| NE | 46.1% | 45.5% | 41.1% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 55.9% | 61.7% | 58.9% |
| NE | 53.9% | 54.5% | 58.9% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 3.06 | 2.89 | 2.93 |
| NE | 3.27 | 3.18 | 3.02 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 3.88 | 4.65 | 4.19 |
| NE | 3.83 | 3.82 | 4.33 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 13)

| Window | PIT | NE |
| --- | ---: | ---: |
| Season-to-date | 1.613 | 1.513 |
| Last 5 games | 1.645 | 1.494 |
| Last 3 games | 1.487 | 1.333 |

## Trend Summary (last 3 weeks)

| Metric | W10 | W12 | W13 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA PIT | 0.090 | 0.029 | -0.174 | - declining |
| Off EPA NE | -0.053 | -0.189 | -0.267 | - declining |
| Def EPA PIT | 0.050 | -0.118 | 0.051 | = stable |
| Def EPA NE | -0.003 | -0.191 | -0.156 | + improving |
| Off SR PIT | 50.0% | 42.9% | 40.5% | - declining |
| Off SR NE | 48.2% | 39.5% | 35.1% | - declining |
| Def SR PIT | 39.8% | 38.3% | 38.1% | + improving |
| Def SR NE | 41.4% | 33.8% | 40.3% | + improving |
| Tempo PIT | 7.455 | 8.400 | 7.182 | - declining |
| Tempo NE | 8.300 | 6.750 | 7.000 | - declining |

## Matchup Analogs

**PIT analogs vs NE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | LV (A) | 23-20 | PIT | 0.972 | 0.031 | 37.2% | 0.269 |
| 13 | ARI (H) | 10-26 | ARI | 0.704 | -0.174 | 40.5% | -1.491 |
| 2 | CLE (H) | 14-22 | CLE | 0.687 | -0.270 | 29.7% | -0.533 |

**NE analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | NYJ (A) | 13-10 | NE | 0.965 | -0.121 | 31.5% | 0.231 |
| 6 | LV (A) | 17-19 | LV | 0.924 | -0.029 | 45.3% | -0.222 |
| 12 | NYG (H) | 7-10 | NYG | 0.903 | -0.189 | 39.5% | -0.250 |

## Core EPA Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.170 | -0.134 | -0.170 |
| PIT | -0.091 | -0.017 | -0.103 |

## Core EPA Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.032 | -0.054 | -0.117 |
| PIT | -0.037 | -0.014 | -0.066 |

## Success Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 38.5% | 39.5% | 41.1% |
| PIT | 39.0% | 42.8% | 38.2% |

## Success Rate Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 41.1% | 41.0% | 38.6% |
| PIT | 41.4% | 40.0% | 38.6% |

## Explosive Play Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 7.2% | 6.7% | 7.1% |
| PIT | 8.6% | 9.5% | 8.4% |

## Third Down Conversion Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 31.1% | 25.4% | 30.3% |
| PIT | 32.4% | 34.5% | 34.4% |

## Points per Drive Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.560 | -0.536 | -0.394 |
| PIT | -0.417 | -0.103 | -0.399 |

## Yards per Play Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.102 | -0.148 | 0.264 |
| PIT | -0.341 | 0.366 | 0.824 |

## Turnover Margin Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.750 | -0.400 | -1.333 |
| PIT | 1.000 | 0.600 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 10.9% | 10.7% | 6.7% |
| PIT | 10.0% | 10.5% | 5.6% |

## Pressure Rate (Def) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 5.3% | 5.8% | 6.2% |
| PIT | 7.3% | 6.8% | 7.4% |

## Tempo Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 7.098 | 6.999 | 7.350 |
| PIT | 6.943 | 7.539 | 7.117 |

## Pass Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 46.2% | 45.4% | 41.1% |
| PIT | 44.2% | 38.2% | 41.2% |
