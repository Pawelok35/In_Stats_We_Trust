# Matchup Report - CLE vs JAX

## Metric Comparison

| Metric | CLE | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.097 | -0.003 | -0.094 |
| Core EPA Defense | -0.149 | -0.022 | -0.127 |
| Success Rate Offense | 0.393 | 0.433 | -0.039 |
| Success Rate Defense | 0.357 | 0.413 | -0.056 |
| Explosive Play Rate (Off) | 0.084 | 0.091 | -0.007 |
| Third Down Conversion | 0.301 | 0.305 | -0.004 |
| Points per Drive Differential | 0.180 | 0.073 | 0.107 |
| Yards per Play Differential | 0.254 | -0.387 | 0.641 |
| Turnover Margin | -0.500 | 0.083 | -0.583 |
| Red Zone TD Rate (Off) | 0.142 | 0.122 | 0.020 |
| Pressure Rate (Def) | 0.099 | 0.056 | 0.044 |
| Tempo | 5.882 | 5.824 | 0.059 |
| PowerScore | 0.058 | 0.032 | 0.026 |
| PowerScore | 0.058 | 0.032 | 0.026 |
| Tempo | 5.882 | 5.824 | 0.059 |

## PowerScore Breakdown (Model)

| Component | Weight | CLE | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.096 | -0.134 | +0.039 |
| EPA Defense | 20% | -0.134 | -0.096 | -0.039 |
| Success Rate Offense | 25% | 41.0% | 34.3% | +6.7 pp |
| Tempo | 20% | 0.080 | 0.061 | +0.019 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CLE | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.096 | -0.134 | +0.039 |
| EPA Defense | 20% | -0.134 | -0.096 | -0.039 |
| Success Rate Offense | 15% | 41.0% | 34.3% | +6.7 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.080 | 0.061 | +0.019 |
| Red Zone TD Rate (Off) | 10% | 11.1% | 25.0% | -13.9 pp |
| Pressure Rate (Def) | 10% | 8.9% | 2.0% | +6.9 pp |

## PowerScore Summary

**Model (4 metrics):**
CLE edge: +0.026 (CLE +0.058 vs JAX +0.032)
**7 metrics version:**
CLE edge: +0.207 (CLE +0.139 vs JAX -0.068)
**Verdict:** CLE holds the edge in the extended breakdown (lead 0.207), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CLE - JAX) | +0.5 pts (favours CLE) |
| Model Win% CLE | 52.0% |
| Model Win% JAX | 48.0% |
| SOS Differential (CLE-JAX) | -0.259 |
*Neutral-field assumption. Based on schedule through Week 13.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CLE | 49.0% | 46.0% | 3.0% | 42.9% | 39.2% | Pass heavy |
| JAX | 56.6% | 46.0% | 10.6% | 37.5% | 30.2% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -4.6 pp | -2.9 pp | -15.3 pp |
| JAX | +6.3 pp | +8.8 pp | +1.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | +0.9 pp | -2.4 pp | -12.5 pp |
| JAX | +1.5 pp | +2.0 pp | +5.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -1.7 pp | +1.8 pp | +3.1 pp |
| JAX | +3.7 pp | +3.2 pp | +0.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -1.9 pp | -3.2 pp | -4.5 pp |
| JAX | +1.0 pp | +2.4 pp | +1.3 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | +2.9 pp | +0.5 pp | +0.4 pp |
| JAX | +3.6 pp | -1.5 pp | -1.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -8.0 pp | -9.5 pp | -12.3 pp |
| JAX | +13.2 pp | +16.9 pp | +17.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -1.9 pp | -3.2 pp | -4.5 pp |
| JAX | +1.0 pp | +2.4 pp | +1.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -1.7 pp | +1.8 pp | +3.1 pp |
| JAX | +3.7 pp | +3.2 pp | +0.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 42.8 | 44.4 | 42.7 |
| JAX | 47.2 | 47.6 | 48.9 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 48.1 | 48.8 | 47.1 |
| JAX | 45.8 | 47.2 | 50.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | -5.3 | -4.4 | -4.4 |
| JAX | 1.3 | 0.5 | -1.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.63 | 1.61 | 1.22 |
| JAX | 1.91 | 2.06 | 2.72 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 1.45 | 1.58 | 1.88 |
| JAX | 1.84 | 2.07 | 2.16 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 0.18 | 0.03 | -0.66 |
| JAX | 0.07 | -0.01 | 0.56 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 7.25 | 7.52 | 7.50 |
| JAX | 7.44 | 7.91 | 8.17 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 43.3% | 46.0% | 51.1% |
| JAX | 44.4% | 44.2% | 43.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 56.7% | 54.0% | 48.9% |
| JAX | 55.6% | 55.8% | 57.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 3.14 | 3.46 | 3.84 |
| JAX | 3.31 | 3.49 | 3.51 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CLE | 4.11 | 4.06 | 3.66 |
| JAX | 4.14 | 4.41 | 4.66 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 13)

| Window | CLE | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.241 | 1.501 |
| Last 5 games | 1.302 | 1.535 |
| Last 3 games | 1.379 | 1.613 |

## Trend Summary (last 3 weeks)

| Metric | W11 | W12 | W13 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CLE | -0.131 | -0.188 | -0.107 | + improving |
| Off EPA JAX | 0.312 | 0.055 | 0.072 | - declining |
| Def EPA CLE | -0.163 | 0.022 | 0.068 | - declining |
| Def EPA JAX | 0.045 | -0.029 | 0.139 | - declining |
| Off SR CLE | 39.1% | 34.8% | 39.3% | = stable |
| Off SR JAX | 48.8% | 41.8% | 44.7% | - declining |
| Def SR CLE | 30.7% | 43.7% | 42.0% | - declining |
| Def SR JAX | 38.3% | 41.6% | 50.0% | - declining |
| Tempo CLE | 7.667 | 7.417 | 7.417 | - declining |
| Tempo JAX | 8.400 | 8.273 | 7.833 | - declining |

## Matchup Analogs

**CLE analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 13 | LA (A) | 19-31 | LA | 0.818 | -0.107 | 39.3% | -1.032 |
| 8 | SEA (A) | 20-24 | SEA | 0.757 | -0.025 | 43.0% | -0.333 |
| 3 | TEN (H) | 27-3 | CLE | 0.695 | 0.145 | 50.0% | 2.700 |

**JAX analogs vs CLE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | NO (A) | 25-22 | JAX | 0.962 | -0.025 | 37.7% | 0.209 |
| 4 | ATL (H) | 17-8 | JAX | 0.899 | 0.094 | 40.5% | 0.845 |
| 8 | PIT (A) | 20-8 | JAX | 0.710 | -0.019 | 49.4% | 1.049 |

## Core EPA Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.097 | -0.086 | -0.142 |
| JAX | -0.003 | 0.026 | 0.146 |

## Core EPA Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.149 | -0.122 | -0.025 |
| JAX | -0.022 | 0.039 | 0.052 |

## Success Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 39.3% | 38.9% | 37.8% |
| JAX | 43.5% | 43.3% | 45.0% |

## Success Rate Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 36.1% | 37.6% | 39.2% |
| JAX | 41.1% | 43.2% | 44.1% |

## Explosive Play Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 8.4% | 8.1% | 7.0% |
| JAX | 9.3% | 10.3% | 10.4% |

## Third Down Conversion Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 30.0% | 32.2% | 25.5% |
| JAX | 31.1% | 32.0% | 32.0% |

## Points per Drive Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 0.128 | 0.088 | -0.656 |
| JAX | 0.133 | -0.000 | 0.495 |

## Yards per Play Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 0.254 | 0.214 | -0.574 |
| JAX | -0.387 | -0.362 | -0.134 |

## Turnover Margin Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | -0.500 | 0.200 | -1.333 |
| JAX | 0.083 | -1.000 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 14.2% | 12.1% | 9.5% |
| JAX | 12.2% | 9.6% | 16.0% |

## Pressure Rate (Def) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 9.9% | 10.1% | 4.4% |
| JAX | 5.6% | 7.4% | 8.1% |

## Tempo Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 7.252 | 7.517 | 7.500 |
| JAX | 7.441 | 7.907 | 8.169 |

## Pass Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CLE | 43.2% | 45.9% | 51.1% |
| JAX | 44.3% | 44.0% | 43.1% |
