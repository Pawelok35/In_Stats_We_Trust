# Matchup Report - DAL vs TB

## Metric Comparison

| Metric | DAL | TB | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.054 | 0.026 | ↑ +0.028 |
| Core EPA Defense | 0.026 | 0.054 | ↓ -0.028 |
| Success Rate Offense | 42.0% | 57.0% | ↓ -15.0 pp |
| Success Rate Defense | 57.0% | 42.0% | ↑ +15.0 pp |
| Explosive Play Rate (Off) | 7.4% | 14.0% | ↓ -6.5 pp |
| Third Down Conversion | 33.3% | 50.0% | ↓ -16.7 pp |
| Points per Drive Differential | -0.015 | 0.015 | ↓ -0.030 |
| Yards per Play Differential | -0.854 | 0.854 | ↓ -1.708 |
| Turnover Margin | 2.000 | -2.000 | ↑ +4.000 |
| Red Zone TD Rate (Off) | 25.0% | 30.0% | ↓ -5.0 pp |
| Pressure Rate (Def) | 8.5% | 2.6% | ↑ +5.9 pp |
| Tempo | 6.750 | 7.818 | ↓ -1.068 |
| PowerScore | 0.144 | 0.190 | ↓ -0.046 |

## PowerScore Breakdown (Model)

| Component | Weight | DAL | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.054 | 0.026 | +0.028 |
| EPA Defense | 20% | 0.026 | 0.054 | -0.028 |
| Success Rate Offense | 25% | 42.0% | 57.0% | -15.0 pp |
| Tempo | 20% | 0.074 | 0.140 | -0.065 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | DAL | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.054 | 0.026 | +0.028 |
| EPA Defense | 20% | 0.026 | 0.054 | -0.028 |
| Success Rate Offense | 15% | 42.0% | 57.0% | -15.0 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.074 | 0.140 | -0.065 |
| Red Zone TD Rate (Off) | 10% | 25.0% | 30.0% | -5.0 pp |
| Pressure Rate (Def) | 10% | 8.5% | 2.6% | +5.9 pp |

## PowerScore Summary

**Model (4 metrics):**
TB edge: +0.046 (DAL +0.144 vs TB +0.190)
**7 metrics version:**
DAL edge: +0.373 (DAL +0.322 vs TB -0.051)
**Verdict:** DAL holds the edge in the extended breakdown (lead 0.373), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (DAL - TB) | -0.9 pts (favours TB) |
| Model Win% DAL | 46.4% |
| Model Win% TB | 53.6% |
| SOS Differential (DAL-TB) | -0.017 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| DAL | 48.1% | 44.7% | 3.5% | 46.2% | 38.1% | Pass heavy |
| TB | 54.7% | 44.7% | 10.0% | 51.1% | 64.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | -4.7 pp | +1.2 pp | +16.2 pp |
| TB | +11.2 pp | +10.2 pp | +13.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | -2.9 pp | +1.9 pp | +5.8 pp |
| TB | -2.8 pp | -4.9 pp | -8.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | +1.3 pp | +2.1 pp | +1.1 pp |
| TB | +1.6 pp | +3.5 pp | +3.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | +0.1 pp | +2.9 pp | +6.4 pp |
| TB | +2.0 pp | +3.8 pp | +6.0 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | -2.3 pp | -2.7 pp | +3.3 pp |
| TB | +1.5 pp | +1.4 pp | -0.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | -6.5 pp | +0.1 pp | -0.9 pp |
| TB | +7.1 pp | +7.0 pp | +8.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | +0.1 pp | +2.9 pp | +6.4 pp |
| TB | +2.0 pp | +3.8 pp | +6.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | +1.3 pp | +2.1 pp | +1.1 pp |
| TB | +1.6 pp | +3.5 pp | +3.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 48.4 | 46.0 | 47.4 |
| TB | 47.8 | 45.6 | 45.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 48.0 | 47.7 | 47.3 |
| TB | 51.9 | 51.9 | 55.1 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 0.4 | -1.7 | 0.1 |
| TB | -4.1 | -6.3 | -9.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 1.80 | 1.92 | 2.16 |
| TB | 2.64 | 2.66 | 2.72 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 2.29 | 1.88 | 1.77 |
| TB | 2.06 | 1.47 | 1.46 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | -0.49 | 0.04 | 0.39 |
| TB | 0.58 | 1.19 | 1.27 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 7.73 | 7.59 | 7.87 |
| TB | 7.57 | 7.62 | 7.48 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 47.4% | 43.4% | 39.8% |
| TB | 43.6% | 38.3% | 37.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 52.6% | 56.6% | 60.2% |
| TB | 56.4% | 61.7% | 62.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 3.67 | 3.30 | 3.13 |
| TB | 3.30 | 2.92 | 2.82 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DAL | 4.07 | 4.30 | 4.74 |
| TB | 4.27 | 4.70 | 4.67 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | DAL | TB |
| --- | ---: | ---: |
| Season-to-date | 0.144 | 0.161 |
| Last 5 games | 0.129 | 0.121 |
| Last 3 games | 0.138 | 0.094 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA DAL | -0.057 | 0.028 | 0.182 | + improving |
| Off EPA TB | -0.026 | 0.062 | 0.194 | + improving |
| Def EPA DAL | -0.144 | 0.124 | -0.090 | - declining |
| Def EPA TB | -0.075 | -0.128 | -0.221 | + improving |
| Off SR DAL | 41.5% | 50.6% | 53.9% | + improving |
| Off SR TB | 50.0% | 48.1% | 57.0% | + improving |
| Def SR DAL | 46.4% | 52.3% | 47.4% | = stable |
| Def SR TB | 50.0% | 43.8% | 45.7% | + improving |
| Tempo DAL | 7.833 | 7.700 | 8.091 | + improving |
| Tempo TB | 6.933 | 7.700 | 7.818 | + improving |

## Matchup Analogs

**DAL analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | BAL (H) | 25-26 | BAL | 0.886 | 0.063 | 41.2% | -0.061 |
| 6 | DET (H) | 9-43 | DET | 0.848 | -0.274 | 43.5% | -3.098 |
| 14 | CIN (H) | 20-22 | CIN | 0.838 | 0.028 | 50.6% | -0.250 |

**TB analogs vs DAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | NYG (A) | 30-6 | TB | 0.969 | 0.211 | 50.0% | 2.633 |
| 13 | CAR (A) | 26-25 | TB | 0.882 | -0.026 | 50.0% | 0.090 |
| 14 | LV (H) | 28-12 | TB | 0.863 | 0.062 | 48.1% | 1.618 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | -0.047 | -0.008 | 0.051 |
| TB | 0.086 | 0.100 | 0.076 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 0.043 | -0.013 | -0.037 |
| TB | 0.003 | -0.094 | -0.141 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 45.5% | 46.2% | 48.5% |
| TB | 50.8% | 50.9% | 51.7% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 46.5% | 46.9% | 48.8% |
| TB | 49.9% | 48.1% | 46.7% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 9.3% | 10.0% | 12.3% |
| TB | 12.7% | 12.8% | 15.0% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 31.7% | 31.5% | 37.8% |
| TB | 43.5% | 37.2% | 37.3% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | -0.485 | 0.060 | 0.397 |
| TB | 0.552 | 1.052 | 1.126 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | -0.541 | -0.120 | 0.403 |
| TB | 0.400 | 1.032 | 1.674 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | -0.214 | 1.400 | 2.000 |
| TB | -0.429 | -0.600 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 12.0% | 12.4% | 17.6% |
| TB | 17.8% | 17.1% | 15.8% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 8.5% | 10.0% | 12.5% |
| TB | 7.0% | 6.7% | 5.1% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 7.733 | 7.592 | 7.875 |
| TB | 7.569 | 7.616 | 7.484 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DAL | 47.7% | 43.8% | 39.6% |
| TB | 43.7% | 38.2% | 37.5% |
