# Matchup Report - LV vs JAX

## Metric Comparison

| Metric | LV | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.122 | 0.006 | -0.127 |
| Core EPA Defense | 0.007 | 0.125 | -0.118 |
| Success Rate Offense | 0.451 | 0.463 | -0.012 |
| Success Rate Defense | 0.455 | 0.478 | -0.023 |
| Explosive Play Rate (Off) | 0.078 | 0.100 | -0.022 |
| Third Down Conversion | 0.311 | 0.324 | -0.012 |
| Points per Drive Differential | -0.793 | -0.917 | 0.124 |
| Yards per Play Differential | -0.289 | -0.606 | 0.317 |
| Turnover Margin | -1.357 | -0.929 | -0.429 |
| Red Zone TD Rate (Off) | 0.130 | 0.134 | -0.004 |
| Pressure Rate (Def) | 0.067 | 0.049 | 0.018 |
| Tempo | 7.077 | 6.750 | 0.327 |
| PowerScore | 0.024 | -0.006 | 0.030 |
| PowerScore | 0.024 | -0.006 | 0.030 |
| Tempo | 7.077 | 6.750 | 0.327 |

## PowerScore Breakdown (Model)

| Component | Weight | LV | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.132 | -0.230 | +0.098 |
| EPA Defense | 20% | -0.230 | -0.132 | -0.098 |
| Success Rate Offense | 25% | 40.2% | 33.3% | +6.9 pp |
| Tempo | 20% | 0.076 | 0.086 | -0.010 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LV | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.132 | -0.230 | +0.098 |
| EPA Defense | 20% | -0.230 | -0.132 | -0.098 |
| Success Rate Offense | 15% | 40.2% | 33.3% | +6.9 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.076 | 0.086 | -0.010 |
| Red Zone TD Rate (Off) | 10% | 33.3% | 16.7% | +16.7 pp |
| Pressure Rate (Def) | 10% | 2.5% | 4.5% | -2.0 pp |

## PowerScore Summary

**Model (4 metrics):**
LV edge: +0.030 (LV +0.024 vs JAX -0.006)
**7 metrics version:**
LV edge: +0.429 (LV +0.225 vs JAX -0.204)
**Verdict:** LV holds the edge in the extended breakdown (lead 0.429), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LV - JAX) | +0.6 pts (favours LV) |
| Model Win% LV | 52.3% |
| Model Win% JAX | 47.7% |
| SOS Differential (LV-JAX) | -0.178 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LV | 47.8% | 44.7% | 3.1% | 38.6% | 41.7% | Pass heavy |
| JAX | 49.4% | 44.7% | 4.7% | 30.0% | 36.6% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -10.1 pp | -11.6 pp | -15.2 pp |
| JAX | -3.9 pp | -3.9 pp | +3.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.0 pp | +3.2 pp | +9.3 pp |
| JAX | -2.1 pp | -2.5 pp | -2.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -3.3 pp | -4.2 pp | -2.7 pp |
| JAX | +1.1 pp | +4.4 pp | +9.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -4.1 pp | -4.3 pp | -3.8 pp |
| JAX | -0.1 pp | -3.3 pp | -2.9 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -2.6 pp | -2.5 pp | -5.0 pp |
| JAX | +4.8 pp | +8.1 pp | +13.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -2.5 pp | -4.2 pp | +2.0 pp |
| JAX | -2.6 pp | -7.3 pp | +1.9 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -4.1 pp | -4.3 pp | -3.8 pp |
| JAX | -0.1 pp | -3.3 pp | -2.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -3.3 pp | -4.2 pp | -2.7 pp |
| JAX | +1.1 pp | +4.4 pp | +9.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 49.0 | 49.3 | 44.2 |
| JAX | 48.4 | 47.5 | 43.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 49.2 | 49.8 | 46.4 |
| JAX | 43.6 | 40.0 | 45.5 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.2 | -0.6 | -2.3 |
| JAX | 4.8 | 7.5 | -2.4 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 1.56 | 1.54 | 1.14 |
| JAX | 1.68 | 1.40 | 1.87 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 2.35 | 2.61 | 1.86 |
| JAX | 2.60 | 2.57 | 1.99 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | -0.79 | -1.07 | -0.72 |
| JAX | -0.92 | -1.17 | -0.12 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 7.40 | 8.22 | 7.21 |
| JAX | 7.32 | 7.87 | 8.80 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 50.2% | 52.5% | 48.3% |
| JAX | 45.9% | 47.4% | 49.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 49.8% | 47.5% | 51.7% |
| JAX | 54.1% | 52.6% | 51.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 3.72 | 4.31 | 3.48 |
| JAX | 3.36 | 3.73 | 4.31 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LV | 3.68 | 3.91 | 3.73 |
| JAX | 3.97 | 4.14 | 4.49 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | LV | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.619 | 1.797 |
| Last 5 games | 1.787 | 1.854 |
| Last 3 games | 1.540 | 1.696 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LV | -0.056 | -0.128 | -0.284 | - declining |
| Off EPA JAX | 0.007 | -0.025 | 0.137 | + improving |
| Def EPA LV | -0.033 | 0.062 | -0.196 | + improving |
| Def EPA JAX | 0.086 | -0.096 | 0.291 | - declining |
| Off SR LV | 43.7% | 43.8% | 38.1% | - declining |
| Off SR JAX | 41.9% | 42.5% | 57.7% | + improving |
| Def SR LV | 36.7% | 48.1% | 34.2% | + improving |
| Def SR JAX | 45.0% | 40.5% | 45.5% | = stable |
| Tempo LV | 7.909 | 7.273 | 6.462 | - declining |
| Tempo JAX | 8.600 | 8.111 | 9.700 | + improving |

## Matchup Analogs

**LV analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 15 | ATL (H) | 9-15 | ATL | 0.895 | -0.284 | 38.1% | -0.490 |
| 3 | CAR (H) | 22-36 | CAR | 0.877 | -0.075 | 52.5% | -1.167 |
| 7 | LA (A) | 15-14 | LV | 0.845 | -0.259 | 38.1% | 0.077 |

**JAX analogs vs LV profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 14 | TEN (A) | 10-6 | JAX | 0.948 | -0.025 | 42.5% | 0.444 |
| 2 | CLE (H) | 13-18 | CLE | 0.946 | -0.034 | 45.3% | -0.478 |
| 6 | CHI (A) | 16-35 | CHI | 0.895 | 0.012 | 43.6% | -1.900 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.006 | -0.053 | 0.040 |
| LV | -0.122 | -0.086 | -0.156 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 0.125 | 0.142 | 0.094 |
| LV | 0.007 | 0.046 | -0.056 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.6% | 46.3% | 48.0% |
| LV | 45.0% | 45.1% | 41.8% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 47.9% | 49.2% | 43.7% |
| LV | 45.7% | 44.4% | 39.4% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 10.0% | 8.5% | 9.4% |
| LV | 7.8% | 8.2% | 8.8% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 32.2% | 36.6% | 39.6% |
| LV | 30.8% | 31.6% | 26.7% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.864 | -1.121 | -0.135 |
| LV | -0.748 | -0.967 | -0.702 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.606 | -1.252 | -0.095 |
| LV | -0.289 | -0.211 | -0.295 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.929 | -1.200 | -1.667 |
| LV | -1.357 | -1.000 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 13.4% | 10.5% | 10.9% |
| LV | 13.0% | 13.2% | 18.4% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 4.9% | 3.3% | 4.7% |
| LV | 6.7% | 8.9% | 12.2% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 7.325 | 7.871 | 8.804 |
| LV | 7.402 | 8.219 | 7.214 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.2% | 47.9% | 49.2% |
| LV | 50.3% | 52.6% | 48.2% |
