# Matchup Report - SF vs TB

## Metric Comparison

| Metric | SF | TB | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.019 | -0.032 | 0.050 |
| Core EPA Defense | -0.102 | -0.025 | -0.077 |
| Success Rate Offense | 0.449 | 0.428 | 0.021 |
| Success Rate Defense | 0.386 | 0.415 | -0.029 |
| Explosive Play Rate (Off) | 0.105 | 0.078 | 0.026 |
| Third Down Conversion | 0.432 | 0.347 | 0.085 |
| Points per Drive Differential | 0.754 | -0.055 | 0.808 |
| Yards per Play Differential | 0.783 | 0.050 | 0.733 |
| Turnover Margin | 0.000 | 0.167 | -0.167 |
| Red Zone TD Rate (Off) | 0.138 | 0.108 | 0.030 |
| Pressure Rate (Def) | 0.080 | 0.087 | -0.007 |
| Tempo | 7.091 | 8.000 | -0.909 |
| PowerScore | 1.547 | 1.641 | -0.094 |
| PowerScore | 1.547 | 1.641 | -0.094 |
| Tempo | 7.091 | 8.000 | -0.909 |

## PowerScore Breakdown (Model)

| Component | Weight | SF | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.116 | -0.234 | +0.349 |
| EPA Defense | 20% | -0.234 | 0.116 | -0.349 |
| Success Rate Offense | 25% | 53.8% | 39.8% | +14.1 pp |
| Tempo | 20% | 0.192 | 0.034 | +0.158 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | SF | TB | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.116 | -0.234 | +0.349 |
| EPA Defense | 20% | -0.234 | 0.116 | -0.349 |
| Success Rate Offense | 15% | 53.8% | 39.8% | +14.1 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.192 | 0.034 | +0.158 |
| Red Zone TD Rate (Off) | 10% | 16.7% | 14.3% | +2.4 pp |
| Pressure Rate (Def) | 10% | 0.0% | 0.0% | +0.0 pp |

## PowerScore Summary

**Model (4 metrics):**
SF edge: +0.119 (SF +0.167 vs TB +0.048)
**7 metrics version:**
SF edge: +0.457 (SF +0.299 vs TB -0.158)
**Verdict:** SF holds the edge in the extended breakdown (lead 0.457), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (SF - TB) | -1.7 pts (favours TB) |
| Model Win% SF | 42.8% |
| Model Win% TB | 57.2% |
| SOS Differential (SF-TB) | -0.015 |
*Neutral-field assumption. Based on schedule through Week 13.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| SF | 30.8% | 46.7% | -15.9% | 50.0% | 55.6% | Run heavy |
| TB | 63.6% | 46.7% | 17.0% | 39.3% | 40.6% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +9.6 pp | +20.0 pp | +19.8 pp |
| TB | -0.0 pp | +6.8 pp | +19.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | -0.8 pp | +4.5 pp | +2.8 pp |
| TB | +0.0 pp | +7.4 pp | +13.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +3.5 pp | +3.9 pp | +4.5 pp |
| TB | +4.4 pp | +2.9 pp | +3.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +1.3 pp | -0.7 pp | -1.8 pp |
| TB | -0.8 pp | -1.6 pp | -1.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +1.2 pp | +3.8 pp | +4.5 pp |
| TB | +2.6 pp | +6.6 pp | +4.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +5.6 pp | +9.4 pp | -1.2 pp |
| TB | +5.0 pp | +5.7 pp | +4.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +1.3 pp | -0.7 pp | -1.8 pp |
| TB | -0.8 pp | -1.6 pp | -1.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | +3.5 pp | +3.9 pp | +4.5 pp |
| TB | +4.4 pp | +2.9 pp | +3.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 44.8 | 43.0 | 41.6 |
| TB | 41.9 | 41.6 | 38.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 47.6 | 47.5 | 46.5 |
| TB | 43.9 | 43.7 | 44.3 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | -2.8 | -4.6 | -4.9 |
| TB | -1.9 | -2.1 | -5.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 2.14 | 2.57 | 2.33 |
| TB | 1.59 | 1.68 | 1.65 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 1.38 | 1.05 | 0.74 |
| TB | 1.65 | 1.61 | 1.63 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 0.75 | 1.52 | 1.59 |
| TB | -0.05 | 0.07 | 0.03 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 7.56 | 7.88 | 7.73 |
| TB | 7.84 | 8.16 | 8.12 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 43.3% | 43.7% | 46.8% |
| TB | 53.2% | 52.7% | 50.3% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 56.7% | 56.3% | 53.2% |
| TB | 46.8% | 47.3% | 49.7% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 3.27 | 3.45 | 3.62 |
| TB | 4.17 | 4.30 | 4.09 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SF | 4.29 | 4.43 | 4.11 |
| TB | 3.67 | 3.86 | 4.03 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 13)

| Window | SF | TB |
| --- | ---: | ---: |
| Season-to-date | 1.537 | 1.552 |
| Last 5 games | 1.476 | 1.483 |
| Last 3 games | 1.415 | 1.517 |

## Trend Summary (last 3 weeks)

| Metric | W10 | W12 | W13 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA SF | -0.028 | -0.045 | 0.013 | + improving |
| Off EPA TB | 0.090 | -0.081 | -0.037 | - declining |
| Def EPA SF | -0.109 | -0.249 | -0.261 | + improving |
| Def EPA TB | 0.049 | -0.001 | -0.029 | + improving |
| Off SR SF | 48.8% | 39.2% | 44.0% | - declining |
| Off SR TB | 45.5% | 34.1% | 43.6% | - declining |
| Def SR SF | 28.9% | 33.8% | 38.5% | - declining |
| Def SR TB | 54.5% | 37.5% | 39.5% | + improving |
| Tempo SF | 7.818 | 8.778 | 7.692 | - declining |
| Tempo TB | 8.800 | 7.000 | 8.545 | - declining |

## Matchup Analogs

**SF analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | NO (H) | 13-0 | SF | 0.780 | -0.045 | 39.2% | 1.444 |
| 5 | CAR (A) | 31-16 | SF | 0.764 | 0.154 | 41.9% | 1.220 |
| 3 | DEN (A) | 10-9 | SF | 0.751 | -0.253 | 41.4% | 0.071 |

**TB analogs vs SF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | DAL (A) | 19-3 | TB | 0.981 | -0.053 | 47.4% | 1.455 |
| 8 | BAL (H) | 22-22 | BAL | 0.737 | 0.080 | 43.0% | -0.050 |
| 2 | NO (A) | 14-11 | TB | 0.560 | -0.155 | 32.2% | 0.231 |

## Core EPA Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 0.019 | 0.100 | 0.087 |
| TB | -0.032 | -0.010 | -0.010 |

## Core EPA Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | -0.102 | -0.160 | -0.211 |
| TB | -0.025 | 0.009 | 0.006 |

## Success Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 44.8% | 47.5% | 45.5% |
| TB | 42.7% | 41.2% | 41.0% |

## Success Rate Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 38.6% | 34.9% | 36.5% |
| TB | 41.6% | 41.6% | 42.9% |

## Explosive Play Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 10.6% | 9.6% | 8.3% |
| TB | 7.7% | 6.9% | 8.1% |

## Third Down Conversion Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 43.5% | 46.7% | 42.2% |
| TB | 35.1% | 41.5% | 45.7% |

## Points per Drive Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 0.689 | 1.453 | 1.545 |
| TB | -0.053 | 0.058 | -0.004 |

## Yards per Play Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 0.783 | 0.893 | 0.234 |
| TB | 0.050 | -0.049 | 0.008 |

## Turnover Margin Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 0.000 | 1.400 | 2.333 |
| TB | 0.167 | -0.200 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 13.8% | 13.7% | 13.2% |
| TB | 10.8% | 13.2% | 17.3% |

## Pressure Rate (Def) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 8.0% | 6.2% | 5.9% |
| TB | 8.7% | 8.8% | 8.1% |

## Tempo Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 7.560 | 7.878 | 7.732 |
| TB | 7.839 | 8.158 | 8.115 |

## Pass Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SF | 43.3% | 43.8% | 47.0% |
| TB | 53.2% | 52.9% | 50.5% |
