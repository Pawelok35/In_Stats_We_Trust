# Matchup Report - SEA vs CAR

## Metric Comparison

| Metric | SEA | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.008 | 0.122 | ↓ -0.130 |
| Core EPA Defense | 0.122 | -0.008 | ↑ +0.130 |
| Success Rate Offense | 40.8% | 50.5% | ↓ -9.7 pp |
| Success Rate Defense | 50.5% | 40.8% | ↑ +9.7 pp |
| Explosive Play Rate (Off) | 12.7% | 9.9% | ↑ +2.8 pp |
| Third Down Conversion | 42.9% | 46.2% | ↓ -3.3 pp |
| Points per Drive Differential | -0.327 | 0.327 | ↓ -0.655 |
| Yards per Play Differential | 0.438 | -0.438 | ↑ +0.876 |
| Turnover Margin | -2.000 | 2.000 | ↓ -4.000 |
| Red Zone TD Rate (Off) | 15.4% | 11.1% | ↑ +4.3 pp |
| Pressure Rate (Def) | 7.4% | 7.7% | ↓ -0.3 pp |
| Tempo | 7.100 | 8.273 | ↓ -1.173 |
| PowerScore | 0.149 | 0.187 | ↓ -0.038 |

## PowerScore Breakdown (Model)

| Component | Weight | SEA | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.008 | 0.122 | -0.130 |
| EPA Defense | 20% | 0.122 | -0.008 | +0.130 |
| Success Rate Offense | 25% | 40.8% | 50.5% | -9.7 pp |
| Tempo | 20% | 0.127 | 0.099 | +0.028 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | SEA | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.008 | 0.122 | -0.130 |
| EPA Defense | 20% | 0.122 | -0.008 | +0.130 |
| Success Rate Offense | 15% | 40.8% | 50.5% | -9.7 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.127 | 0.099 | +0.028 |
| Red Zone TD Rate (Off) | 10% | 15.4% | 11.1% | +4.3 pp |
| Pressure Rate (Def) | 10% | 7.4% | 7.7% | -0.3 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.038 (SEA +0.149 vs CAR +0.187)
**7 metrics version:**
CAR edge: +0.414 (SEA -0.081 vs CAR +0.334)
**Verdict:** CAR holds the edge in the extended breakdown (lead 0.414), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (SEA - CAR) | -0.7 pts (favours CAR) |
| Model Win% SEA | 47.1% |
| Model Win% CAR | 52.9% |
| SOS Differential (SEA-CAR) | +0.049 |
*Neutral-field assumption. Based on schedule through Week 13.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| SEA | 54.9% | 46.7% | 8.2% | 43.6% | 37.5% | Pass heavy |
| CAR | 29.7% | 46.7% | -17.0% | 40.7% | 54.7% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +1.3 pp | -2.4 pp | -0.5 pp |
| CAR | -17.4 pp | -17.5 pp | -26.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -1.7 pp | +3.4 pp | +3.5 pp |
| CAR | -1.1 pp | -2.9 pp | -7.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -1.1 pp | +1.0 pp | +1.9 pp |
| CAR | +0.8 pp | +4.6 pp | -0.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +1.3 pp | +0.6 pp | +1.5 pp |
| CAR | -1.9 pp | -1.3 pp | -3.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +2.4 pp | +1.2 pp | +5.6 pp |
| CAR | +3.2 pp | +1.9 pp | -0.5 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +6.9 pp | +7.5 pp | +19.0 pp |
| CAR | -9.3 pp | -7.5 pp | -14.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | +1.3 pp | +0.6 pp | +1.5 pp |
| CAR | -1.9 pp | -1.3 pp | -3.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -1.1 pp | +1.0 pp | +1.9 pp |
| CAR | +0.8 pp | +4.6 pp | -0.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 49.0 | 48.6 | 51.3 |
| CAR | 45.4 | 45.1 | 41.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 49.8 | 47.7 | 44.9 |
| CAR | 45.2 | 47.4 | 44.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | -0.8 | 1.0 | 6.3 |
| CAR | 0.2 | -2.3 | -2.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.28 | 2.22 | 2.15 |
| CAR | 1.51 | 1.77 | 1.44 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 2.25 | 1.96 | 2.42 |
| CAR | 1.78 | 1.82 | 1.02 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 0.03 | 0.26 | -0.27 |
| CAR | -0.27 | -0.05 | 0.42 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 7.37 | 6.83 | 6.83 |
| CAR | 6.50 | 6.84 | 6.86 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 45.9% | 49.2% | 52.0% |
| CAR | 40.9% | 35.7% | 31.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 54.1% | 50.8% | 48.0% |
| CAR | 59.1% | 64.3% | 68.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 3.38 | 3.36 | 3.55 |
| CAR | 2.66 | 2.44 | 2.19 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| SEA | 3.99 | 3.47 | 3.28 |
| CAR | 3.84 | 4.40 | 4.67 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 13)

| Window | SEA | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.145 | 0.096 |
| Last 5 games | 0.140 | 0.106 |
| Last 3 games | 0.182 | 0.050 |

## Trend Summary (last 3 weeks)

| Metric | W9 | W10 | W12 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA SEA | 0.043 | 0.049 | 0.053 | = stable |
| Off EPA CAR | -0.015 | 0.007 | 0.028 | + improving |
| Def EPA SEA | -0.139 | 0.090 | 0.118 | - declining |
| Def EPA CAR | 0.224 | -0.139 | -0.171 | + improving |
| Off SR SEA | 47.8% | 54.5% | 51.8% | + improving |
| Off SR CAR | 37.9% | 40.0% | 45.1% | + improving |
| Def SR SEA | 35.7% | 45.5% | 48.0% | - declining |
| Def SR CAR | 58.1% | 40.0% | 35.9% | + improving |
| Tempo SEA | 7.500 | 7.333 | 6.071 | - declining |
| Tempo CAR | 6.000 | 7.727 | 6.308 | + improving |

## Matchup Analogs

**SEA analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | DEN (H) | 17-16 | SEA | 0.976 | 0.071 | 44.8% | 0.125 |
| 13 | LA (A) | 27-28 | LA | 0.845 | 0.112 | 50.6% | -0.050 |
| 10 | TB (A) | 16-19 | TB | 0.764 | 0.049 | 54.5% | -0.322 |

**CAR analogs vs SEA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 11 | BAL (A) | 3-12 | BAL | 0.850 | -0.298 | 36.1% | -0.811 |
| 1 | CLE (H) | 24-28 | CLE | 0.848 | 0.023 | 36.5% | -0.364 |
| 9 | CIN (A) | 21-42 | CIN | 0.825 | -0.015 | 37.9% | -1.909 |

## Core EPA Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.089 | -0.055 | -0.088 |
| SEA | 0.053 | 0.070 | 0.072 |

## Core EPA Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.038 | -0.043 | -0.147 |
| SEA | 0.024 | 0.003 | 0.079 |

## Success Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 39.7% | 40.1% | 40.6% |
| SEA | 47.0% | 49.0% | 52.1% |

## Success Rate Defense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 42.5% | 44.8% | 40.0% |
| SEA | 41.6% | 41.7% | 46.4% |

## Explosive Play Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 9.2% | 9.7% | 9.2% |
| SEA | 12.0% | 11.1% | 11.4% |

## Third Down Conversion Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 24.6% | 26.6% | 25.1% |
| SEA | 38.5% | 35.8% | 30.0% |

## Points per Drive Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.255 | 0.004 | 0.459 |
| SEA | 0.089 | 0.280 | -0.271 |

## Yards per Play Differential Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.155 | -0.092 | 0.230 |
| SEA | 0.079 | 0.323 | -0.253 |

## Turnover Margin Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.167 | 0.000 | 0.667 |
| SEA | 0.333 | -0.400 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 13.2% | 11.1% | 7.4% |
| SEA | 10.2% | 15.1% | 13.8% |

## Pressure Rate (Def) Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.0% | 8.5% | 10.1% |
| SEA | 7.3% | 8.5% | 5.3% |

## Tempo Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.497 | 6.839 | 6.860 |
| SEA | 7.369 | 6.828 | 6.829 |

## Pass Rate Offense Form (up to Week 13)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 40.5% | 34.9% | 31.0% |
| SEA | 45.5% | 48.8% | 51.7% |
