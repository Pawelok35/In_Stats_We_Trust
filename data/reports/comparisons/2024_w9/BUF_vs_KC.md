# Matchup Report - BUF vs KC

## Metric Comparison

| Metric | BUF | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.119 | 0.045 | 0.074 |
| Core EPA Defense | -0.031 | -0.056 | 0.025 |
| Success Rate Offense | 0.479 | 0.522 | -0.044 |
| Success Rate Defense | 0.459 | 0.460 | -0.001 |
| Explosive Play Rate (Off) | 0.109 | 0.084 | 0.025 |
| Third Down Conversion | 0.328 | 0.459 | -0.131 |
| Points per Drive Differential | 0.988 | 0.609 | 0.379 |
| Yards per Play Differential | 0.182 | 0.460 | -0.278 |
| Turnover Margin | 1.125 | -0.429 | 1.554 |
| Red Zone TD Rate (Off) | 0.167 | 0.115 | 0.052 |
| Pressure Rate (Def) | 0.070 | 0.061 | 0.009 |
| Tempo | 10.000 | 8.417 | 1.583 |
| PowerScore | 2.264 | 1.893 | 0.371 |
| PowerScore | 2.264 | 1.893 | 0.371 |
| Tempo | 10.000 | 8.417 | 1.583 |

## PowerScore Breakdown (Model)

| Component | Weight | BUF | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.215 | 0.140 | +0.075 |
| EPA Defense | 20% | 0.210 | 0.133 | +0.076 |
| Success Rate Offense | 25% | 58.8% | 53.5% | +5.3 pp |
| Tempo | 20% | 0.075 | 0.089 | -0.014 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BUF | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.215 | 0.140 | +0.075 |
| EPA Defense | 20% | 0.210 | 0.133 | +0.076 |
| Success Rate Offense | 15% | 58.8% | 53.5% | +5.3 pp |
| Turnover Margin | 10% | 0.000 | -1.000 | +1.000 |
| Tempo | 10% | 0.075 | 0.089 | -0.014 |
| Red Zone TD Rate (Off) | 10% | 10.5% | 22.2% | -11.7 pp |
| Pressure Rate (Def) | 10% | 3.3% | 6.1% | -2.7 pp |

## PowerScore Summary

**Model (4 metrics):**
BUF edge: +0.052 (BUF +0.279 vs KC +0.227)
**7 metrics version:**
BUF edge: +0.126 (BUF +0.205 vs KC +0.079)
**Verdict:** BUF holds the edge in the extended breakdown (lead 0.126), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BUF - KC) | +6.9 pts (favours BUF) |
| Model Win% BUF | 75.9% |
| Model Win% KC | 24.1% |
| SOS Differential (BUF-KC) | +0.031 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BUF | 51.2% | 43.6% | 7.7% | 48.5% | 41.0% | Pass heavy |
| KC | 47.5% | 43.6% | 4.0% | 60.0% | 55.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +2.6 pp | +2.1 pp | +6.5 pp |
| KC | +10.0 pp | +19.2 pp | +33.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +4.0 pp | +0.5 pp | +1.0 pp |
| KC | -0.9 pp | -1.7 pp | +7.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.8 pp | +1.1 pp | +1.5 pp |
| KC | +1.9 pp | +1.3 pp | +2.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.4 pp | +4.0 pp | +6.2 pp |
| KC | -2.3 pp | -3.4 pp | -1.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.8 pp | +6.7 pp | +5.2 pp |
| KC | +6.8 pp | +10.3 pp | +10.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -0.7 pp | -3.3 pp | +5.4 pp |
| KC | +3.8 pp | +2.0 pp | +7.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +3.4 pp | +4.0 pp | +6.2 pp |
| KC | -2.3 pp | -3.4 pp | -1.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | +1.8 pp | +1.1 pp | +1.5 pp |
| KC | +1.9 pp | +1.3 pp | +2.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 46.7 | 42.7 | 41.1 |
| KC | 46.4 | 42.3 | 43.5 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 49.0 | 46.1 | 50.5 |
| KC | 51.2 | 51.9 | 56.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | -2.3 | -3.4 | -9.4 |
| KC | -4.8 | -9.5 | -13.2 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.71 | 2.26 | 2.92 |
| KC | 2.34 | 2.34 | 2.72 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 1.72 | 1.80 | 1.39 |
| KC | 1.73 | 1.57 | 1.72 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 0.99 | 0.47 | 1.53 |
| KC | 0.61 | 0.77 | 1.00 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 7.60 | 7.87 | 8.51 |
| KC | 8.13 | 8.40 | 9.03 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 39.2% | 41.0% | 40.9% |
| KC | 43.4% | 44.8% | 43.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 60.8% | 59.0% | 59.1% |
| KC | 56.6% | 55.2% | 56.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 2.98 | 3.22 | 3.48 |
| KC | 3.53 | 3.76 | 3.92 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BUF | 4.62 | 4.65 | 5.02 |
| KC | 4.60 | 4.64 | 5.11 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | BUF | KC |
| --- | ---: | ---: |
| Season-to-date | 1.678 | 1.647 |
| Last 5 games | 1.594 | 1.564 |
| Last 3 games | 1.725 | 1.563 |

## Trend Summary (last 3 weeks)

| Metric | W5 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BUF | -0.025 | 0.168 | 0.186 | + improving |
| Off EPA KC | 0.093 | 0.052 | 0.071 | - declining |
| Def EPA BUF | -0.027 | -0.148 | -0.099 | + improving |
| Def EPA KC | -0.086 | -0.112 | -0.042 | - declining |
| Off SR BUF | 36.6% | 49.3% | 52.1% | + improving |
| Off SR KC | 56.1% | 54.1% | 52.3% | - declining |
| Def SR BUF | 43.5% | 42.4% | 48.5% | - declining |
| Def SR KC | 37.7% | 49.3% | 50.0% | - declining |
| Tempo BUF | 6.833 | 6.455 | 9.400 | + improving |
| Tempo KC | 9.800 | 7.727 | 9.556 | - declining |

## Matchup Analogs

**BUF analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | ATL (A) | 23-20 | BUF | 0.541 | 0.126 | 50.6% | 0.333 |
| 1 | ARI (H) | 34-28 | BUF | 0.497 | 0.189 | 60.5% | 0.600 |
| 4 | NO (H) | 10-35 | NO | 0.452 | -0.146 | 39.0% | -2.273 |

**KC analogs vs BUF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | DET (H) | n/a | n/a | 0.908 | n/a | n/a | n/a |
| 2 | PHI (H) | 20-23 | PHI | 0.902 | -0.006 | 53.2% | -0.273 |
| 8 | WAS (H) | 27-20 | KC | 0.860 | 0.071 | 52.3% | 0.778 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.119 | 0.062 | 0.160 |
| KC | 0.045 | 0.040 | 0.072 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | -0.031 | 0.002 | -0.058 |
| KC | -0.056 | -0.093 | -0.080 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 48.2% | 45.7% | 50.8% |
| KC | 52.6% | 53.3% | 54.3% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 45.7% | 46.2% | 44.8% |
| KC | 46.5% | 43.1% | 45.9% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 10.9% | 10.9% | 11.9% |
| KC | 8.2% | 8.1% | 8.6% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 33.1% | 30.6% | 37.3% |
| KC | 46.2% | 53.7% | 56.6% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.952 | 0.434 | 1.591 |
| KC | 0.605 | 0.756 | 1.000 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.182 | -0.341 | 0.932 |
| KC | 0.460 | 0.643 | 0.682 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 1.125 | 0.800 | 0.667 |
| KC | -0.429 | -0.200 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 16.7% | 15.4% | 16.5% |
| KC | 11.5% | 13.3% | 14.6% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.0% | 5.7% | 6.9% |
| KC | 6.1% | 6.6% | 6.6% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.600 | 7.871 | 8.507 |
| KC | 8.127 | 8.398 | 9.028 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 39.1% | 40.6% | 40.5% |
| KC | 43.5% | 44.8% | 43.5% |
