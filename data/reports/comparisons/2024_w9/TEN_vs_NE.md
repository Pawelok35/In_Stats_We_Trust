# Matchup Report - TEN vs NE

## Metric Comparison

| Metric | TEN | NE | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.149 | -0.095 | -0.054 |
| Core EPA Defense | 0.008 | 0.055 | -0.046 |
| Success Rate Offense | 0.433 | 0.444 | -0.011 |
| Success Rate Defense | 0.446 | 0.484 | -0.038 |
| Explosive Play Rate (Off) | 0.089 | 0.080 | 0.010 |
| Third Down Conversion | 0.269 | 0.322 | -0.053 |
| Points per Drive Differential | -0.577 | -0.698 | 0.122 |
| Yards per Play Differential | -0.181 | -1.250 | 1.069 |
| Turnover Margin | -1.857 | -0.375 | -1.482 |
| Red Zone TD Rate (Off) | 0.135 | 0.091 | 0.044 |
| Pressure Rate (Def) | 0.081 | 0.053 | 0.029 |
| Tempo | 7.750 | 7.818 | -0.068 |
| PowerScore | 0.086 | 0.090 | -0.004 |
| PowerScore | 0.086 | 0.090 | -0.004 |
| Tempo | 7.750 | 7.818 | -0.068 |

## PowerScore Breakdown (Model)

| Component | Weight | TEN | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.052 | -0.086 | +0.034 |
| EPA Defense | 20% | -0.086 | -0.052 | -0.034 |
| Success Rate Offense | 25% | 40.9% | 46.5% | -5.7 pp |
| Tempo | 20% | 0.097 | 0.070 | +0.027 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | TEN | NE | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.052 | -0.086 | +0.034 |
| EPA Defense | 20% | -0.086 | -0.052 | -0.034 |
| Success Rate Offense | 15% | 40.9% | 46.5% | -5.7 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.097 | 0.070 | +0.027 |
| Red Zone TD Rate (Off) | 10% | 10.0% | 14.3% | -4.3 pp |
| Pressure Rate (Def) | 10% | 8.9% | 2.9% | +5.9 pp |

## PowerScore Summary

**Model (4 metrics):**
NE edge: +0.004 (TEN +0.086 vs NE +0.090)
**7 metrics version:**
TEN edge: +0.398 (TEN +0.260 vs NE -0.138)
**Verdict:** TEN holds the edge in the extended breakdown (lead 0.398), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (TEN - NE) | -0.1 pts (favours NE) |
| Model Win% TEN | 49.7% |
| Model Win% NE | 50.3% |
| SOS Differential (TEN-NE) | -0.286 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| TEN | 36.6% | 43.6% | -7.0% | 52.9% | 33.9% | Run heavy |
| NE | 52.3% | 43.6% | 8.8% | 42.2% | 51.2% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -14.1 pp | -10.6 pp | -10.3 pp |
| NE | +3.9 pp | +3.3 pp | +3.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -1.2 pp | +0.9 pp | -4.0 pp |
| NE | -4.0 pp | -2.9 pp | -1.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -2.5 pp | -2.4 pp | -0.2 pp |
| NE | -1.6 pp | +0.3 pp | +0.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -1.2 pp | -3.8 pp | -3.8 pp |
| NE | -0.7 pp | -1.9 pp | -1.4 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -1.5 pp | -0.3 pp | -5.4 pp |
| NE | +0.9 pp | -0.9 pp | -3.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -11.3 pp | -7.7 pp | -12.6 pp |
| NE | +1.0 pp | +0.2 pp | -7.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -1.2 pp | -3.8 pp | -3.8 pp |
| NE | -0.7 pp | -1.9 pp | -1.4 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -2.5 pp | -2.4 pp | -0.2 pp |
| NE | -1.6 pp | +0.3 pp | +0.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 46.7 | 47.5 | 46.5 |
| NE | 48.5 | 50.8 | 48.1 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 45.6 | 46.2 | 45.5 |
| NE | 43.5 | 46.1 | 51.3 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 1.1 | 1.3 | 1.0 |
| NE | 5.0 | 4.7 | -3.2 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 1.52 | 1.46 | 1.27 |
| NE | 1.50 | 1.67 | 2.15 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 2.09 | 2.30 | 2.79 |
| NE | 2.19 | 2.28 | 2.67 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | -0.58 | -0.83 | -1.52 |
| NE | -0.70 | -0.60 | -0.52 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 6.88 | 6.90 | 7.20 |
| NE | 7.39 | 7.32 | 7.69 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 41.5% | 41.7% | 40.7% |
| NE | 43.2% | 45.1% | 44.7% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 58.5% | 58.3% | 59.3% |
| NE | 56.8% | 54.9% | 55.3% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 2.86 | 2.88 | 2.93 |
| NE | 3.19 | 3.30 | 3.44 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TEN | 4.02 | 4.02 | 4.27 |
| NE | 4.20 | 4.02 | 4.26 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | TEN | NE |
| --- | ---: | ---: |
| Season-to-date | 1.393 | 1.679 |
| Last 5 games | 1.403 | 1.579 |
| Last 3 games | 1.431 | 1.619 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA TEN | -0.029 | -0.148 | -0.320 | - declining |
| Off EPA NE | -0.219 | -0.019 | 0.014 | + improving |
| Def EPA TEN | 0.024 | 0.168 | 0.182 | - declining |
| Def EPA NE | 0.114 | 0.229 | 0.018 | + improving |
| Off SR TEN | 40.3% | 42.4% | 43.3% | + improving |
| Off SR NE | 44.8% | 41.0% | 46.3% | + improving |
| Def SR TEN | 42.9% | 49.3% | 57.8% | - declining |
| Def SR NE | 44.0% | 56.2% | 49.4% | - declining |
| Tempo TEN | 7.000 | 7.667 | 6.929 | - declining |
| Tempo NE | 6.214 | 8.667 | 8.200 | + improving |

## Matchup Analogs

**TEN analogs vs NE profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | MIA (A) | 29-12 | TEN | 0.743 | 0.042 | 41.9% | 1.308 |
| 2 | NYJ (H) | 17-24 | NYJ | 0.536 | -0.066 | 46.2% | -0.636 |
| 6 | IND (H) | 17-22 | IND | 0.522 | -0.029 | 40.3% | -0.455 |

**NE analogs vs TEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | MIA (H) | 10-16 | MIA | 0.930 | -0.162 | 40.7% | -0.530 |
| 3 | NYJ (A) | 3-27 | NYJ | 0.804 | -0.217 | 42.2% | -2.667 |
| 8 | NYJ (H) | 25-22 | NE | 0.804 | 0.014 | 46.3% | 0.300 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.095 | -0.130 | -0.075 |
| TEN | -0.149 | -0.140 | -0.166 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 0.055 | 0.059 | 0.121 |
| TEN | 0.008 | 0.034 | 0.125 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 44.5% | 43.3% | 44.1% |
| TEN | 43.2% | 43.2% | 42.1% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 48.3% | 46.3% | 49.6% |
| TEN | 44.2% | 44.7% | 49.3% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 7.9% | 7.8% | 7.7% |
| TEN | 8.9% | 8.1% | 7.5% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 32.7% | 33.1% | 36.3% |
| TEN | 26.8% | 26.4% | 29.7% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.703 | -0.673 | -0.636 |
| TEN | -0.566 | -0.838 | -1.570 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -1.250 | -1.367 | -1.157 |
| TEN | -0.181 | -0.491 | -0.546 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | -0.375 | -0.400 | -1.000 |
| TEN | -1.857 | -1.800 | -2.000 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 9.1% | 11.1% | 14.8% |
| TEN | 13.5% | 15.3% | 12.9% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 5.3% | 4.2% | 3.0% |
| TEN | 8.1% | 9.0% | 7.8% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 7.393 | 7.316 | 7.694 |
| TEN | 6.877 | 6.897 | 7.198 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| NE | 42.8% | 45.0% | 44.5% |
| TEN | 41.1% | 41.1% | 41.0% |
