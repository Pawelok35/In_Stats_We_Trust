# Matchup Report - GB vs CAR

## Metric Comparison

| Metric | GB | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.056 | 0.031 | ↓ -0.087 |
| Core EPA Defense | 0.106 | 0.064 | ↑ +0.042 |
| Success Rate Offense | 47.5% | 43.1% | ↑ +4.4 pp |
| Success Rate Defense | 49.3% | 43.8% | ↑ +5.6 pp |
| Explosive Play Rate (Off) | 11.2% | 11.1% | ↑ +0.1 pp |
| Third Down Conversion | 23.1% | 40.0% | ↓ -16.9 pp |
| Points per Drive Differential | -0.444 | 0.091 | ↓ -0.535 |
| Yards per Play Differential | 1.587 | -1.031 | ↑ +2.618 |
| Turnover Margin | -1.000 | -1.000 | ± 0.000 |
| Red Zone TD Rate (Off) | 6.2% | 27.3% | ↓ -21.0 pp |
| Pressure Rate (Def) | 3.8% | 2.9% | ↑ +0.9 pp |
| Tempo | 8.889 | 6.545 | ↑ +2.343 |
| PowerScore | 0.143 | 0.154 | ↓ -0.011 |

## PowerScore Breakdown (Model)

| Component | Weight | GB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.056 | 0.031 | -0.087 |
| EPA Defense | 20% | 0.106 | 0.064 | +0.042 |
| Success Rate Offense | 25% | 47.5% | 43.1% | +4.4 pp |
| Tempo | 20% | 0.113 | 0.111 | +0.001 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | GB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.056 | 0.031 | -0.087 |
| EPA Defense | 20% | 0.106 | 0.064 | +0.042 |
| Success Rate Offense | 15% | 47.5% | 43.1% | +4.4 pp |
| Turnover Margin | 10% | -1.000 | -1.000 | +0.000 |
| Tempo | 10% | 0.113 | 0.111 | +0.001 |
| Red Zone TD Rate (Off) | 10% | 6.2% | 27.3% | -21.0 pp |
| Pressure Rate (Def) | 10% | 3.8% | 2.9% | +0.9 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.011 (GB +0.143 vs CAR +0.154)
**7 metrics version:**
CAR edge: +0.027 (GB -0.000 vs CAR +0.027)
**Verdict:** CAR holds the edge in the extended breakdown (lead 0.027), driven by EPA Offense (25% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (GB - CAR) | -0.2 pts (favours CAR) |
| Model Win% GB | 49.2% |
| Model Win% CAR | 50.8% |
| SOS Differential (GB-CAR) | -0.019 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| GB | 50.0% | 43.6% | 6.4% | 38.2% | 46.8% | Pass heavy |
| CAR | 41.7% | 43.6% | -1.9% | 50.0% | 48.9% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -13.9 pp | -18.6 pp | -16.5 pp |
| CAR | -2.2 pp | +2.5 pp | +9.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -2.6 pp | +4.3 pp | +1.3 pp |
| CAR | -3.1 pp | -1.9 pp | +0.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -1.6 pp | -1.8 pp | -1.1 pp |
| CAR | +1.3 pp | +1.2 pp | +1.3 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -0.2 pp | -1.8 pp | -3.0 pp |
| CAR | -3.6 pp | -2.3 pp | -3.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -5.0 pp | -3.4 pp | -7.3 pp |
| CAR | -2.1 pp | +1.7 pp | -1.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -2.3 pp | +0.2 pp | +4.7 pp |
| CAR | -11.4 pp | -10.7 pp | -6.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -0.2 pp | -1.8 pp | -3.0 pp |
| CAR | -3.6 pp | -2.3 pp | -3.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -1.6 pp | -1.8 pp | -1.1 pp |
| CAR | +1.3 pp | +1.2 pp | +1.3 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 44.9 | 43.4 | 45.5 |
| CAR | 51.4 | 52.1 | 53.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 48.7 | 49.5 | 49.5 |
| CAR | 47.1 | 47.5 | 47.7 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | -3.8 | -6.2 | -4.1 |
| CAR | 4.3 | 4.6 | 6.2 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 2.22 | 2.30 | 2.47 |
| CAR | 1.38 | 1.40 | 1.32 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 1.74 | 1.72 | 1.74 |
| CAR | 2.98 | 3.12 | 3.06 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 0.48 | 0.59 | 0.73 |
| CAR | -1.60 | -1.72 | -1.74 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 7.22 | 6.98 | 7.21 |
| CAR | 7.00 | 7.24 | 7.30 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 38.0% | 43.5% | 39.6% |
| CAR | 45.1% | 45.7% | 43.1% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 62.0% | 56.5% | 60.4% |
| CAR | 54.9% | 54.3% | 56.9% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 2.74 | 3.04 | 2.85 |
| CAR | 3.16 | 3.31 | 3.15 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| GB | 4.47 | 3.95 | 4.35 |
| CAR | 3.84 | 3.94 | 4.15 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | GB | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.142 | 0.161 |
| Last 5 games | 0.149 | 0.180 |
| Last 3 games | 0.163 | 0.177 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA GB | 0.156 | 0.030 | 0.131 | - declining |
| Off EPA CAR | -0.088 | -0.309 | -0.123 | - declining |
| Def EPA GB | -0.131 | 0.040 | 0.061 | - declining |
| Def EPA CAR | 0.199 | 0.218 | 0.059 | + improving |
| Off SR GB | 53.3% | 47.4% | 48.9% | - declining |
| Off SR CAR | 45.3% | 45.2% | 44.9% | = stable |
| Def SR GB | 49.3% | 48.1% | 50.0% | = stable |
| Def SR CAR | 53.0% | 57.6% | 47.1% | + improving |
| Tempo GB | 8.364 | 6.333 | 6.923 | - declining |
| Tempo CAR | 8.600 | 6.200 | 7.091 | - declining |

## Matchup Analogs

**GB analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | DAL (A) | 29-31 | DAL | 0.876 | -0.073 | 46.2% | -0.133 |
| 5 | LA (-) | 24-14 | GB | 0.788 | 0.007 | 43.6% | 0.882 |
| 3 | CLE (A) | 24-14 | GB | 0.680 | 0.062 | 40.7% | 0.909 |

**CAR analogs vs GB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | BUF (A) | 14-26 | BUF | 0.755 | -0.123 | 44.9% | -1.061 |
| 2 | ARI (A) | 3-24 | ARI | 0.646 | -0.383 | 30.8% | -1.894 |
| 3 | ATL (H) | 36-22 | CAR | 0.554 | 0.161 | 51.6% | 1.167 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.158 | -0.134 | -0.173 |
| GB | 0.051 | 0.050 | 0.106 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.124 | 0.163 | 0.159 |
| GB | -0.043 | -0.021 | -0.010 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 44.3% | 46.2% | 45.1% |
| GB | 45.9% | 48.0% | 50.0% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 50.4% | 50.2% | 52.5% |
| GB | 50.1% | 50.2% | 49.1% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 7.8% | 8.2% | 7.5% |
| GB | 11.3% | 10.3% | 9.3% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 27.6% | 33.6% | 34.2% |
| GB | 32.9% | 30.3% | 37.7% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -1.611 | -1.715 | -1.717 |
| GB | 0.446 | 0.523 | 0.686 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -1.112 | -1.067 | -1.411 |
| GB | 0.487 | 0.502 | 0.517 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -1.000 | -1.200 | -1.000 |
| GB | 0.875 | 0.000 | 0.667 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 11.9% | 13.2% | 16.8% |
| GB | 14.6% | 18.6% | 15.7% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 3.3% | 2.2% | 2.7% |
| GB | 7.3% | 6.1% | 6.1% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.995 | 7.245 | 7.297 |
| GB | 7.216 | 6.982 | 7.207 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 45.5% | 46.2% | 43.8% |
| GB | 38.2% | 43.6% | 39.1% |
