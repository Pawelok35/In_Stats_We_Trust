# Matchup Report - WAS vs SEA

## Metric Comparison

| Metric | WAS | SEA | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.217 | -0.153 | ↑ +0.370 |
| Core EPA Defense | 0.125 | -0.040 | ↑ +0.165 |
| Success Rate Offense | 58.7% | 43.1% | ↑ +15.5 pp |
| Success Rate Defense | 48.7% | 41.5% | ↑ +7.2 pp |
| Explosive Play Rate (Off) | 12.0% | 12.7% | ↓ -0.7 pp |
| Third Down Conversion | 41.7% | 23.5% | ↑ +18.1 pp |
| Points per Drive Differential | 0.625 | -0.083 | ↑ +0.708 |
| Yards per Play Differential | 0.594 | 0.263 | ↑ +0.331 |
| Turnover Margin | 1.000 | -2.000 | ↑ +3.000 |
| Red Zone TD Rate (Off) | 12.0% | 15.4% | ↓ -3.4 pp |
| Pressure Rate (Def) | 13.3% | 0.0% | ↑ +13.3 pp |
| Tempo | 9.375 | 6.375 | ↑ +3.000 |
| PowerScore | 0.272 | 0.072 | ↑ +0.200 |

## PowerScore Breakdown (Model)

| Component | Weight | WAS | SEA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.217 | -0.153 | +0.370 |
| EPA Defense | 20% | 0.125 | -0.040 | +0.165 |
| Success Rate Offense | 25% | 58.7% | 43.1% | +15.5 pp |
| Tempo | 20% | 0.120 | 0.127 | -0.007 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | WAS | SEA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.217 | -0.153 | +0.370 |
| EPA Defense | 20% | 0.125 | -0.040 | +0.165 |
| Success Rate Offense | 15% | 58.7% | 43.1% | +15.5 pp |
| Turnover Margin | 10% | 1.000 | -2.000 | +3.000 |
| Tempo | 10% | 0.120 | 0.127 | -0.007 |
| Red Zone TD Rate (Off) | 10% | 12.0% | 15.4% | -3.4 pp |
| Pressure Rate (Def) | 10% | 13.3% | 0.0% | +13.3 pp |

## PowerScore Summary

**Model (4 metrics):**
WAS edge: +0.200 (WAS +0.272 vs SEA +0.072)
**7 metrics version:**
WAS edge: +0.458 (WAS +0.305 vs SEA -0.153)
**Verdict:** WAS holds the edge in the extended breakdown (lead 0.458), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (WAS - SEA) | +3.7 pts (favours WAS) |
| Model Win% WAS | 64.9% |
| Model Win% SEA | 35.1% |
| SOS Differential (WAS-SEA) | +0.047 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| WAS | 33.3% | 43.6% | -10.2% | 47.7% | 36.0% | Run heavy |
| SEA | 41.2% | 43.6% | -2.4% | 43.3% | 52.1% | Slight run lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +6.2 pp | -0.7 pp | -9.8 pp |
| SEA | -5.0 pp | +2.9 pp | -4.4 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +2.1 pp | -3.3 pp | -8.1 pp |
| SEA | -2.6 pp | -1.9 pp | -3.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +0.8 pp | +0.8 pp | -2.1 pp |
| SEA | +3.0 pp | +6.7 pp | +6.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +1.8 pp | +0.5 pp | +0.9 pp |
| SEA | -2.1 pp | -1.2 pp | -3.3 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +2.8 pp | -0.5 pp | -0.8 pp |
| SEA | -1.9 pp | +0.9 pp | -4.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +3.4 pp | -4.6 pp | -1.5 pp |
| SEA | +2.3 pp | +8.1 pp | +6.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +1.8 pp | +0.5 pp | +0.9 pp |
| SEA | -2.1 pp | -1.2 pp | -3.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | +0.8 pp | +0.8 pp | -2.1 pp |
| SEA | +3.0 pp | +6.7 pp | +6.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 49.2 | 44.8 | 46.7 |
| SEA | 46.5 | 51.9 | 50.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 54.3 | 51.7 | 50.2 |
| SEA | 47.8 | 45.0 | 43.4 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | -5.1 | -6.9 | -3.5 |
| SEA | -1.2 | 7.0 | 6.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 3.02 | 2.97 | 2.45 |
| SEA | 1.96 | 2.01 | 1.97 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 2.41 | 1.58 | 1.80 |
| SEA | 1.99 | 2.59 | 2.46 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 0.61 | 1.39 | 0.65 |
| SEA | -0.03 | -0.59 | -0.49 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 8.52 | 8.08 | 8.08 |
| SEA | 7.42 | 7.95 | 7.78 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 39.2% | 40.0% | 41.7% |
| SEA | 49.2% | 50.8% | 46.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 60.8% | 60.0% | 58.3% |
| SEA | 50.8% | 49.2% | 53.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 3.33 | 3.23 | 3.37 |
| SEA | 3.66 | 4.04 | 3.61 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| WAS | 5.18 | 4.85 | 4.71 |
| SEA | 3.77 | 3.91 | 4.17 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | WAS | SEA |
| --- | ---: | ---: |
| Season-to-date | 0.189 | 0.142 |
| Last 5 games | 0.136 | 0.195 |
| Last 3 games | 0.141 | 0.189 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA WAS | 0.144 | 0.218 | -0.006 | - declining |
| Off EPA SEA | 0.029 | 0.171 | -0.099 | - declining |
| Def EPA WAS | 0.183 | -0.309 | -0.081 | + improving |
| Def EPA SEA | 0.254 | -0.139 | 0.186 | + improving |
| Off SR WAS | 51.3% | 57.6% | 41.3% | - declining |
| Off SR SEA | 45.4% | 45.5% | 48.5% | + improving |
| Def SR WAS | 53.6% | 45.2% | 43.2% | + improving |
| Def SR SEA | 51.8% | 48.4% | 52.1% | = stable |
| Tempo WAS | 8.667 | 8.500 | 7.077 | - declining |
| Tempo SEA | 8.083 | 7.700 | 7.556 | - declining |

## Matchup Analogs

**WAS analogs vs SEA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | ATL (A) | 42-14 | WAS | 0.746 | 0.374 | 60.7% | 3.111 |
| 7 | DAL (H) | 34-7 | WAS | 0.694 | 0.218 | 57.6% | 2.700 |
| 1 | TB (A) | 20-37 | TB | 0.558 | 0.052 | 50.0% | -1.889 |

**SEA analogs vs WAS profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | TB (H) | 20-21 | TB | 0.899 | -0.171 | 51.2% | -0.103 |
| 8 | BUF (-) | 10-28 | BUF | 0.860 | -0.099 | 48.5% | -1.989 |
| 4 | ARI (A) | 27-42 | ARI | 0.793 | 0.081 | 52.3% | -1.364 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | -0.006 | 0.002 | 0.034 |
| WAS | 0.173 | 0.156 | 0.119 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 0.019 | 0.124 | 0.101 |
| WAS | 0.026 | -0.091 | -0.069 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 48.1% | 48.7% | 46.3% |
| WAS | 50.5% | 49.9% | 49.8% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 47.0% | 51.4% | 50.7% |
| WAS | 48.5% | 45.7% | 47.6% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 10.1% | 10.0% | 8.3% |
| WAS | 11.3% | 11.6% | 11.0% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 31.9% | 31.8% | 28.1% |
| WAS | 43.0% | 43.9% | 33.5% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 0.036 | -0.571 | -0.472 |
| WAS | 0.662 | 1.342 | 0.634 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | -0.106 | -0.896 | -1.000 |
| WAS | 0.539 | 1.156 | 0.537 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 0.000 | -0.200 | 0.333 |
| WAS | 0.500 | 0.600 | 1.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 13.6% | 13.2% | 11.6% |
| WAS | 14.5% | 13.2% | 8.1% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 7.6% | 6.1% | 3.4% |
| WAS | 9.1% | 12.3% | 9.0% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 7.423 | 7.947 | 7.780 |
| WAS | 8.517 | 8.082 | 8.081 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| SEA | 49.7% | 51.5% | 47.1% |
| WAS | 39.2% | 39.9% | 41.6% |
