# Matchup Report - BAL vs DEN

## Metric Comparison

| Metric | BAL | DEN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.137 | -0.036 | 0.174 |
| Core EPA Defense | 0.076 | -0.130 | 0.206 |
| Success Rate Offense | 0.518 | 0.464 | 0.054 |
| Success Rate Defense | 0.475 | 0.423 | 0.052 |
| Explosive Play Rate (Off) | 0.144 | 0.102 | 0.041 |
| Third Down Conversion | 0.414 | 0.300 | 0.115 |
| Points per Drive Differential | 0.432 | 0.417 | 0.015 |
| Yards per Play Differential | 1.083 | 0.451 | 0.632 |
| Turnover Margin | 0.250 | -0.250 | 0.500 |
| Red Zone TD Rate (Off) | 0.229 | 0.181 | 0.048 |
| Pressure Rate (Def) | 0.074 | 0.096 | -0.022 |
| Tempo | 7.300 | 8.600 | -1.300 |
| PowerScore | 0.273 | 0.136 | 0.137 |
| PowerScore | 0.273 | 0.136 | 0.137 |
| Tempo | 7.300 | 8.600 | -1.300 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.333 | -0.118 | +0.452 |
| EPA Defense | 20% | -0.118 | 0.333 | -0.452 |
| Success Rate Offense | 25% | 58.9% | 36.0% | +22.9 pp |
| Tempo | 20% | 0.164 | 0.105 | +0.060 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | DEN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.333 | -0.118 | +0.452 |
| EPA Defense | 20% | -0.118 | 0.333 | -0.452 |
| Success Rate Offense | 15% | 58.9% | 36.0% | +22.9 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.164 | 0.105 | +0.060 |
| Red Zone TD Rate (Off) | 10% | 17.4% | 4.3% | +13.0 pp |
| Pressure Rate (Def) | 10% | 10.3% | 4.8% | +5.5 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.137 (BAL +0.273 vs DEN +0.136)
**7 metrics version:**
BAL edge: +0.281 (BAL +0.292 vs DEN +0.011)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.281), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - DEN) | +2.5 pts (favours BAL) |
| Model Win% BAL | 60.4% |
| Model Win% DEN | 39.6% |
| SOS Differential (BAL-DEN) | +0.206 |
*Neutral-field assumption. Based on schedule through Week 8.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 28.8% | 43.6% | -14.8% | 61.9% | 57.7% | Run heavy |
| DEN | 45.3% | 43.6% | 1.8% | 33.3% | 38.3% | Slight pass lean |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +9.4 pp | +10.0 pp | -3.6 pp |
| DEN | -9.9 pp | -7.0 pp | -2.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +10.2 pp | +7.4 pp | -0.3 pp |
| DEN | +4.3 pp | +13.5 pp | +19.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +4.9 pp | +2.8 pp | +1.1 pp |
| DEN | +3.8 pp | +3.4 pp | +3.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +7.7 pp | +7.9 pp | +8.3 pp |
| DEN | -0.5 pp | +1.9 pp | +2.0 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +5.3 pp | +6.1 pp | -1.3 pp |
| DEN | +5.9 pp | +4.7 pp | +3.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +15.6 pp | +20.8 pp | +21.5 pp |
| DEN | -11.2 pp | -12.1 pp | -8.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +7.7 pp | +7.9 pp | +8.3 pp |
| DEN | -0.5 pp | +1.9 pp | +2.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +4.9 pp | +2.8 pp | +1.1 pp |
| DEN | +3.8 pp | +3.4 pp | +3.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 47.4 | 47.1 | 49.6 |
| DEN | 45.1 | 45.1 | 43.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 51.7 | 53.2 | 54.8 |
| DEN | 42.3 | 43.3 | 46.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -4.3 | -6.1 | -5.2 |
| DEN | 2.8 | 1.8 | -3.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.73 | 3.12 | 3.08 |
| DEN | 1.66 | 1.82 | 2.06 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.30 | 2.25 | 2.52 |
| DEN | 1.24 | 1.28 | 1.40 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.43 | 0.87 | 0.56 |
| DEN | 0.42 | 0.54 | 0.66 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 7.74 | 7.62 | 8.08 |
| DEN | 6.80 | 6.74 | 7.06 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 36.6% | 37.6% | 38.0% |
| DEN | 43.6% | 41.5% | 44.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 63.4% | 62.4% | 62.0% |
| DEN | 56.4% | 58.5% | 55.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.84 | 2.86 | 3.07 |
| DEN | 2.97 | 2.80 | 3.16 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 4.91 | 4.75 | 5.01 |
| DEN | 3.84 | 3.94 | 3.90 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 8)

| Window | BAL | DEN |
| --- | ---: | ---: |
| Season-to-date | 1.689 | 1.483 |
| Last 5 games | 1.718 | 1.549 |
| Last 3 games | 1.816 | 1.620 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA BAL | 0.183 | 0.293 | 0.054 | - declining |
| Off EPA DEN | -0.048 | 0.064 | 0.059 | + improving |
| Def EPA BAL | 0.144 | 0.088 | 0.117 | + improving |
| Def EPA DEN | 0.086 | -0.289 | -0.123 | + improving |
| Off SR BAL | 53.6% | 52.6% | 48.9% | - declining |
| Off SR DEN | 49.3% | 53.7% | 47.1% | - declining |
| Def SR BAL | 51.3% | 52.0% | 51.2% | = stable |
| Def SR DEN | 45.8% | 40.9% | 44.9% | = stable |
| Tempo BAL | 9.333 | 6.909 | 8.000 | - declining |
| Tempo DEN | 7.100 | 6.833 | 7.250 | + improving |

## Matchup Analogs

**BAL analogs vs DEN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | CLE (A) | 24-27 | CLE | 0.754 | 0.054 | 48.9% | -0.235 |
| 2 | LV (H) | 23-26 | LV | 0.730 | -0.034 | 44.9% | -0.250 |
| 1 | KC (A) | 20-27 | KC | 0.592 | 0.021 | 55.2% | -0.700 |

**DEN analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | TB (A) | 26-6 | DEN | 0.924 | 0.089 | 56.8% | 1.964 |
| 1 | SEA (A) | 16-26 | SEA | 0.596 | -0.133 | 37.6% | -0.667 |
| 2 | PIT (H) | 6-14 | PIT | 0.374 | -0.118 | 44.2% | -0.682 |

## Core EPA Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.137 | 0.198 | 0.177 |
| DEN | -0.036 | -0.025 | 0.025 |

## Core EPA Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.076 | 0.078 | 0.116 |
| DEN | -0.130 | -0.144 | -0.109 |

## Success Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 51.9% | 53.7% | 51.6% |
| DEN | 46.3% | 46.7% | 50.0% |

## Success Rate Defense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 47.6% | 48.7% | 51.5% |
| DEN | 42.1% | 41.2% | 43.9% |

## Explosive Play Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 14.2% | 14.9% | 15.3% |
| DEN | 10.1% | 11.9% | 13.3% |

## Third Down Conversion Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 41.4% | 45.4% | 34.3% |
| DEN | 30.6% | 35.3% | 43.9% |

## Points per Drive Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.424 | 0.855 | 0.548 |
| DEN | 0.369 | 0.572 | 0.730 |

## Yards per Play Differential Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.083 | 1.391 | 1.147 |
| DEN | 0.451 | 0.537 | 1.142 |

## Turnover Margin Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.250 | 0.200 | 0.333 |
| DEN | -0.250 | 0.400 | -0.333 |

## Red Zone TD Rate (Off) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 22.9% | 23.0% | 19.0% |
| DEN | 18.1% | 25.6% | 27.2% |

## Pressure Rate (Def) Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.4% | 7.2% | 6.8% |
| DEN | 9.6% | 8.7% | 8.5% |

## Tempo Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.743 | 7.619 | 8.081 |
| DEN | 6.805 | 6.744 | 7.061 |

## Pass Rate Offense Form (up to Week 8)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 37.2% | 38.3% | 38.3% |
| DEN | 43.6% | 41.4% | 44.6% |
