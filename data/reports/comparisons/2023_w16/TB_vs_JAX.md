# Matchup Report - TB vs JAX

## Metric Comparison

| Metric | TB | JAX | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.016 | -0.029 | 0.013 |
| Core EPA Defense | -0.023 | -0.023 | 0.000 |
| Success Rate Offense | 0.397 | 0.421 | -0.024 |
| Success Rate Defense | 0.437 | 0.421 | 0.015 |
| Explosive Play Rate (Off) | 0.098 | 0.088 | 0.010 |
| Third Down Conversion | 0.375 | 0.303 | 0.072 |
| Points per Drive Differential | 0.024 | -0.069 | 0.092 |
| Yards per Play Differential | -0.493 | -0.430 | -0.063 |
| Turnover Margin | 0.429 | -0.071 | 0.500 |
| Red Zone TD Rate (Off) | 0.124 | 0.123 | 0.002 |
| Pressure Rate (Def) | 0.077 | 0.057 | 0.019 |
| Tempo | 8.273 | 7.600 | 0.673 |
| PowerScore | 1.740 | 1.575 | 0.165 |
| PowerScore | 1.740 | 1.575 | 0.165 |
| Tempo | 8.273 | 7.600 | 0.673 |

## PowerScore Breakdown (Model)

| Component | Weight | TB | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.063 | -0.189 | +0.252 |
| EPA Defense | 20% | -0.189 | 0.063 | -0.252 |
| Success Rate Offense | 25% | 40.7% | 43.4% | -2.8 pp |
| Tempo | 20% | 0.099 | 0.092 | +0.007 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | TB | JAX | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.063 | -0.189 | +0.252 |
| EPA Defense | 20% | -0.189 | 0.063 | -0.252 |
| Success Rate Offense | 15% | 40.7% | 43.4% | -2.8 pp |
| Turnover Margin | 10% | 4.000 | -4.000 | +8.000 |
| Tempo | 10% | 0.099 | 0.092 | +0.007 |
| Red Zone TD Rate (Off) | 10% | 8.7% | 20.0% | -11.3 pp |
| Pressure Rate (Def) | 10% | 8.0% | 7.5% | +0.5 pp |

## PowerScore Summary

**Model (4 metrics):**
TB edge: +0.032 (TB +0.105 vs JAX +0.073)
**7 metrics version:**
TB edge: +0.798 (TB +0.465 vs JAX -0.333)
**Verdict:** TB holds the edge in the extended breakdown (lead 0.798), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (TB - JAX) | +3.1 pts (favours TB) |
| Model Win% TB | 62.5% |
| Model Win% JAX | 37.5% |
| SOS Differential (TB-JAX) | +0.109 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| TB | 44.0% | 46.6% | -2.7% | 55.0% | 29.4% | Slight run lean |
| JAX | 65.8% | 46.6% | 19.2% | 48.0% | 34.6% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | +3.6 pp | +6.4 pp | +3.8 pp |
| JAX | -8.9 pp | -0.3 pp | -3.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.6 pp | -1.4 pp | +6.0 pp |
| JAX | +1.2 pp | +0.1 pp | -0.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.8 pp | -1.5 pp | -0.9 pp |
| JAX | +1.6 pp | +3.4 pp | +0.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.9 pp | -1.8 pp | -1.5 pp |
| JAX | -1.8 pp | -2.3 pp | -0.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -2.8 pp | -1.6 pp | -7.7 pp |
| JAX | -2.9 pp | -10.4 pp | -9.1 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -1.6 pp | -4.1 pp | -8.4 pp |
| JAX | +0.6 pp | +2.0 pp | +1.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.9 pp | -1.8 pp | -1.5 pp |
| JAX | -1.8 pp | -2.3 pp | -0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.8 pp | -1.5 pp | -0.9 pp |
| JAX | +1.6 pp | +3.4 pp | +0.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 47.3 | 50.7 | 46.1 |
| JAX | 47.2 | 48.3 | 51.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 45.4 | 46.3 | 49.3 |
| JAX | 45.4 | 47.5 | 43.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 2.0 | 4.4 | -3.2 |
| JAX | 1.8 | 0.8 | 7.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 1.89 | 2.21 | 2.55 |
| JAX | 1.80 | 2.09 | 1.62 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 1.87 | 2.14 | 1.84 |
| JAX | 1.87 | 2.12 | 2.32 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 0.02 | 0.07 | 0.70 |
| JAX | -0.07 | -0.03 | -0.70 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 7.56 | 7.65 | 7.35 |
| JAX | 7.35 | 7.63 | 7.15 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 45.5% | 45.3% | 39.8% |
| JAX | 46.1% | 48.4% | 52.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 54.5% | 54.7% | 60.2% |
| JAX | 53.9% | 51.6% | 47.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 3.44 | 3.46 | 2.93 |
| JAX | 3.39 | 3.69 | 3.78 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 4.12 | 4.18 | 4.42 |
| JAX | 3.96 | 3.94 | 3.37 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | TB | JAX |
| --- | ---: | ---: |
| Season-to-date | 1.609 | 1.500 |
| Last 5 games | 1.618 | 1.567 |
| Last 3 games | 1.584 | 1.562 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA TB | -0.081 | -0.005 | 0.272 | + improving |
| Off EPA JAX | 0.072 | -0.134 | -0.237 | - declining |
| Def EPA TB | -0.085 | -0.066 | 0.036 | - declining |
| Def EPA JAX | 0.139 | -0.096 | 0.034 | + improving |
| Off SR TB | 31.2% | 39.8% | 49.4% | + improving |
| Off SR JAX | 44.7% | 34.3% | 35.9% | - declining |
| Def SR TB | 45.5% | 41.6% | 41.6% | + improving |
| Def SR JAX | 50.0% | 41.0% | 53.6% | - declining |
| Tempo TB | 6.154 | 7.333 | 8.556 | + improving |
| Tempo JAX | 7.833 | 5.824 | 7.800 | - declining |

## Matchup Analogs

**TB analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 10 | TEN (H) | 20-6 | TB | 0.839 | -0.020 | 34.2% | 1.273 |
| 13 | CAR (H) | 21-18 | TB | 0.729 | -0.081 | 31.2% | 0.231 |
| 12 | IND (A) | 20-25 | IND | 0.714 | -0.088 | 43.6% | -0.455 |

**JAX analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 11 | TEN (H) | 34-16 | JAX | 0.818 | 0.312 | 48.8% | 1.844 |
| 13 | CIN (H) | 31-34 | CIN | 0.777 | 0.072 | 44.7% | -0.250 |
| 1 | IND (A) | 31-15 | JAX | 0.623 | -0.215 | 48.2% | 1.067 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.029 | 0.013 | -0.100 |
| TB | -0.016 | 0.009 | 0.062 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.023 | 0.019 | 0.026 |
| TB | -0.023 | 0.005 | -0.038 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 42.3% | 41.0% | 38.4% |
| TB | 39.7% | 42.0% | 40.0% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 42.0% | 45.3% | 47.8% |
| TB | 43.8% | 45.8% | 42.9% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 8.9% | 9.0% | 8.9% |
| TB | 9.7% | 10.1% | 10.6% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 30.9% | 31.2% | 30.0% |
| TB | 37.4% | 42.4% | 41.9% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.007 | -0.061 | -0.590 |
| TB | 0.044 | 0.063 | 0.615 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.430 | -0.355 | -0.944 |
| TB | -0.493 | -0.414 | 0.299 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | -0.071 | -0.400 | -0.333 |
| TB | 0.429 | -0.400 | 0.333 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 12.3% | 14.6% | 13.9% |
| TB | 12.4% | 16.2% | 19.6% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 5.7% | 7.5% | 6.0% |
| TB | 7.7% | 8.1% | 7.7% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 7.351 | 7.626 | 7.152 |
| TB | 7.558 | 7.649 | 7.348 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| JAX | 46.1% | 48.4% | 52.8% |
| TB | 45.2% | 45.2% | 39.6% |
