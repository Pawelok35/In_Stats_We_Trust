# Matchup Report - ATL vs IND

## Metric Comparison

| Metric | ATL | IND | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.086 | -0.037 | -0.049 |
| Core EPA Defense | -0.058 | -0.038 | -0.020 |
| Success Rate Offense | 0.401 | 0.405 | -0.004 |
| Success Rate Defense | 0.392 | 0.419 | -0.027 |
| Explosive Play Rate (Off) | 0.099 | 0.083 | 0.015 |
| Third Down Conversion | 0.383 | 0.324 | 0.058 |
| Points per Drive Differential | -0.121 | -0.015 | -0.106 |
| Yards per Play Differential | 0.085 | 0.004 | 0.081 |
| Turnover Margin | -0.429 | 0.643 | -1.071 |
| Red Zone TD Rate (Off) | 0.136 | 0.126 | 0.010 |
| Pressure Rate (Def) | 0.062 | 0.087 | -0.025 |
| Tempo | 8.300 | 7.818 | 0.482 |
| PowerScore | 1.795 | 1.623 | 0.172 |
| PowerScore | 1.795 | 1.623 | 0.172 |
| Tempo | 8.300 | 7.818 | 0.482 |

## PowerScore Breakdown (Model)

| Component | Weight | ATL | IND | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.117 | -0.146 | +0.263 |
| EPA Defense | 20% | -0.146 | 0.117 | -0.263 |
| Success Rate Offense | 25% | 49.4% | 34.9% | +14.5 pp |
| Tempo | 20% | 0.157 | 0.093 | +0.064 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | ATL | IND | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.117 | -0.146 | +0.263 |
| EPA Defense | 20% | -0.146 | 0.117 | -0.263 |
| Success Rate Offense | 15% | 49.4% | 34.9% | +14.5 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.157 | 0.093 | +0.064 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 9.1% | -9.1 pp |
| Pressure Rate (Def) | 10% | 13.6% | 2.9% | +10.7 pp |

## PowerScore Summary

**Model (4 metrics):**
ATL edge: +0.088 (ATL +0.167 vs IND +0.078)
**7 metrics version:**
ATL edge: +0.243 (ATL +0.203 vs IND -0.039)
**Verdict:** ATL holds the edge in the extended breakdown (lead 0.243), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (ATL - IND) | +3.2 pts (favours ATL) |
| Model Win% ATL | 63.0% |
| Model Win% IND | 37.0% |
| SOS Differential (ATL-IND) | -0.039 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| ATL | 41.0% | 46.6% | -5.6% | 55.9% | 44.9% | Run heavy |
| IND | 51.2% | 46.6% | 4.6% | 36.4% | 33.3% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | +4.5 pp | +8.6 pp | +8.3 pp |
| IND | +1.2 pp | -1.4 pp | +3.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | -0.5 pp | +4.7 pp | -0.4 pp |
| IND | +2.5 pp | +2.9 pp | +1.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | +1.4 pp | +6.4 pp | +3.9 pp |
| IND | -0.1 pp | +0.8 pp | +0.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | -0.3 pp | -1.0 pp | -4.1 pp |
| IND | -0.7 pp | -2.1 pp | -1.3 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | -3.5 pp | -1.1 pp | -0.3 pp |
| IND | -0.7 pp | +0.9 pp | +3.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | +0.6 pp | -0.8 pp | +0.3 pp |
| IND | +3.9 pp | +5.9 pp | +9.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | -0.3 pp | -1.0 pp | -4.1 pp |
| IND | -0.7 pp | -2.1 pp | -1.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | +1.4 pp | +6.4 pp | +3.9 pp |
| IND | -0.1 pp | +0.8 pp | +0.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 46.0 | 47.2 | 41.5 |
| IND | 49.7 | 48.9 | 53.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 45.1 | 43.5 | 42.2 |
| IND | 47.5 | 47.4 | 47.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 0.9 | 3.7 | -0.7 |
| IND | 2.2 | 1.5 | 5.9 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 1.52 | 1.51 | 1.14 |
| IND | 1.91 | 1.77 | 1.79 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 1.65 | 1.46 | 1.18 |
| IND | 1.93 | 1.75 | 2.06 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | -0.12 | 0.04 | -0.03 |
| IND | -0.01 | 0.01 | -0.26 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 7.25 | 6.97 | 6.15 |
| IND | 7.30 | 7.24 | 7.09 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 40.0% | 35.3% | 39.3% |
| IND | 44.3% | 47.5% | 48.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 60.0% | 64.7% | 60.7% |
| IND | 55.7% | 52.5% | 51.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 2.90 | 2.46 | 2.42 |
| IND | 3.24 | 3.44 | 3.44 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| ATL | 4.35 | 4.51 | 3.74 |
| IND | 4.07 | 3.80 | 3.65 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | ATL | IND |
| --- | ---: | ---: |
| Season-to-date | 1.486 | 1.526 |
| Last 5 games | 1.538 | 1.520 |
| Last 3 games | 1.418 | 1.400 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA ATL | -0.170 | -0.066 | -0.232 | - declining |
| Off EPA IND | 0.025 | -0.127 | -0.059 | - declining |
| Def EPA ATL | -0.267 | -0.005 | -0.164 | - declining |
| Def EPA IND | -0.051 | 0.157 | -0.324 | + improving |
| Off SR ATL | 33.3% | 41.6% | 35.9% | + improving |
| Off SR IND | 44.9% | 39.5% | 43.4% | - declining |
| Def SR ATL | 30.6% | 39.8% | 37.5% | - declining |
| Def SR IND | 39.1% | 43.0% | 30.1% | + improving |
| Tempo ATL | 5.800 | 6.846 | 5.818 | + improving |
| Tempo IND | 6.357 | 7.364 | 7.545 | + improving |

## Matchup Analogs

**ATL analogs vs IND profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 13 | NYJ (A) | 13-7 | ATL | 0.757 | -0.170 | 33.3% | 0.405 |
| 1 | CAR (H) | 24-11 | ATL | 0.753 | -0.064 | 40.0% | 1.013 |
| 15 | CAR (A) | 7-9 | CAR | 0.753 | -0.232 | 35.9% | -0.182 |

**IND analogs vs ATL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | NO (H) | 27-38 | NO | 0.912 | 0.034 | 36.8% | -0.917 |
| 7 | CLE (H) | 38-31 | IND | 0.885 | -0.103 | 41.1% | 0.434 |
| 10 | NE (A) | 10-6 | IND | 0.808 | -0.003 | 41.4% | 0.400 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.086 | -0.086 | -0.156 |
| IND | -0.037 | -0.032 | -0.054 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.058 | -0.094 | -0.145 |
| IND | -0.038 | -0.072 | -0.073 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 40.2% | 39.6% | 37.1% |
| IND | 40.4% | 43.4% | 42.7% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 39.2% | 39.9% | 36.0% |
| IND | 41.9% | 40.9% | 37.8% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 10.0% | 9.4% | 6.2% |
| IND | 8.3% | 7.8% | 6.7% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 38.2% | 39.9% | 35.2% |
| IND | 32.4% | 27.4% | 28.9% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.121 | 0.053 | -0.004 |
| IND | -0.067 | 0.001 | -0.248 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 0.085 | -0.198 | 0.145 |
| IND | 0.004 | 0.188 | 0.186 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | -0.429 | 0.200 | 0.000 |
| IND | 0.643 | 1.400 | 1.667 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 13.6% | 15.6% | 15.9% |
| IND | 12.6% | 11.0% | 8.6% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 6.2% | 6.2% | 7.6% |
| IND | 8.7% | 12.0% | 9.4% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 7.251 | 6.973 | 6.155 |
| IND | 7.304 | 7.235 | 7.089 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| ATL | 40.4% | 35.6% | 40.0% |
| IND | 44.5% | 47.8% | 48.6% |
