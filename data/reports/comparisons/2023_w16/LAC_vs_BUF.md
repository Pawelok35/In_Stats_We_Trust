# Matchup Report - LAC vs BUF

## Metric Comparison

| Metric | LAC | BUF | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.009 | 0.003 | ↑ +0.005 |
| Core EPA Defense | 0.003 | 0.009 | ↓ -0.005 |
| Success Rate Offense | 34.8% | 38.0% | ↓ -3.2 pp |
| Success Rate Defense | 38.0% | 34.8% | ↑ +3.2 pp |
| Explosive Play Rate (Off) | 9.0% | 11.3% | ↓ -2.3 pp |
| Third Down Conversion | 25.0% | 50.0% | ↓ -25.0 pp |
| Points per Drive Differential | -0.567 | 0.567 | ↓ -1.133 |
| Yards per Play Differential | -1.685 | 1.685 | ↓ -3.369 |
| Turnover Margin | 1.000 | -1.000 | ↑ +2.000 |
| Red Zone TD Rate (Off) | 9.1% | 15.4% | ↓ -6.3 pp |
| Pressure Rate (Def) | 4.5% | 13.2% | ↓ -8.6 pp |
| Tempo | 7.417 | 7.100 | ↑ +0.317 |
| PowerScore | 0.109 | 0.120 | ↓ -0.012 |

## PowerScore Breakdown (Model)

| Component | Weight | LAC | BUF | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.009 | 0.003 | +0.005 |
| EPA Defense | 20% | 0.003 | 0.009 | -0.005 |
| Success Rate Offense | 25% | 34.8% | 38.0% | -3.2 pp |
| Tempo | 20% | 0.090 | 0.113 | -0.023 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LAC | BUF | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.009 | 0.003 | +0.005 |
| EPA Defense | 20% | 0.003 | 0.009 | -0.005 |
| Success Rate Offense | 15% | 34.8% | 38.0% | -3.2 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.090 | 0.113 | -0.023 |
| Red Zone TD Rate (Off) | 10% | 9.1% | 15.4% | -6.3 pp |
| Pressure Rate (Def) | 10% | 4.5% | 13.2% | -8.6 pp |

## PowerScore Summary

**Model (4 metrics):**
BUF edge: +0.012 (LAC +0.109 vs BUF +0.120)
**7 metrics version:**
LAC edge: +0.178 (LAC +0.178 vs BUF -0.001)
**Verdict:** LAC holds the edge in the extended breakdown (lead 0.178), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LAC - BUF) | -0.2 pts (favours BUF) |
| Model Win% LAC | 49.1% |
| Model Win% BUF | 50.9% |
| SOS Differential (LAC-BUF) | -0.003 |
*Neutral-field assumption. Based on schedule through Week 15.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LAC | 42.7% | 46.6% | -3.9% | 39.5% | 31.4% | Run heavy |
| BUF | 31.0% | 46.6% | -15.6% | 54.5% | 30.6% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +4.2 pp | +4.4 pp | -10.7 pp |
| BUF | +9.6 pp | +2.5 pp | +2.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -2.4 pp | -10.1 pp | -8.9 pp |
| BUF | +3.6 pp | +3.2 pp | +4.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +5.8 pp | +7.2 pp | +1.2 pp |
| BUF | +1.5 pp | -0.6 pp | +0.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -2.0 pp | -1.9 pp | -4.4 pp |
| BUF | +0.9 pp | +3.6 pp | +4.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -3.1 pp | -2.8 pp | -10.7 pp |
| BUF | +5.0 pp | +2.9 pp | +4.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -0.0 pp | +1.3 pp | +1.2 pp |
| BUF | +2.3 pp | -3.9 pp | -2.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -2.0 pp | -1.9 pp | -4.4 pp |
| BUF | +0.9 pp | +3.6 pp | +4.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +5.8 pp | +7.2 pp | +1.2 pp |
| BUF | +1.5 pp | -0.6 pp | +0.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 45.2 | 43.9 | 42.8 |
| BUF | 45.4 | 44.8 | 45.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 46.6 | 43.4 | 40.2 |
| BUF | 49.6 | 51.8 | 51.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.3 | 0.5 | 2.6 |
| BUF | -4.3 | -7.1 | -5.6 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 1.91 | 1.02 | 0.80 |
| BUF | 2.51 | 2.44 | 2.56 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 2.12 | 1.88 | 1.83 |
| BUF | 1.62 | 1.59 | 1.81 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -0.21 | -0.85 | -1.03 |
| BUF | 0.89 | 0.85 | 0.76 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 7.38 | 6.62 | 6.24 |
| BUF | 7.82 | 7.83 | 8.64 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 48.3% | 51.7% | 49.9% |
| BUF | 45.2% | 39.5% | 39.7% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 51.7% | 48.3% | 50.1% |
| BUF | 54.8% | 60.5% | 60.3% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 3.57 | 3.42 | 3.12 |
| BUF | 3.53 | 3.09 | 3.43 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 3.81 | 3.20 | 3.13 |
| BUF | 4.29 | 4.74 | 5.21 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 15)

| Window | LAC | BUF |
| --- | ---: | ---: |
| Season-to-date | 0.117 | 0.120 |
| Last 5 games | 0.064 | 0.091 |
| Last 3 games | 0.042 | 0.136 |

## Trend Summary (last 3 weeks)

| Metric | W12 | W14 | W15 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LAC | -0.258 | -0.389 | -0.308 | - declining |
| Off EPA BUF | 0.042 | -0.027 | 0.224 | + improving |
| Def EPA LAC | -0.073 | -0.126 | 0.233 | - declining |
| Def EPA BUF | 0.087 | -0.102 | -0.066 | + improving |
| Off SR LAC | 35.4% | 35.8% | 39.8% | + improving |
| Off SR BUF | 42.7% | 39.8% | 52.4% | + improving |
| Def SR LAC | 44.8% | 38.6% | 51.2% | - declining |
| Def SR BUF | 43.8% | 45.0% | 42.9% | = stable |
| Tempo LAC | 7.182 | 6.231 | 5.500 | - declining |
| Tempo BUF | 8.357 | 8.455 | 9.111 | + improving |

## Matchup Analogs

**LAC analogs vs BUF profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | DAL (H) | 17-20 | DAL | 0.963 | -0.059 | 36.0% | -0.300 |
| 1 | MIA (H) | 34-33 | LAC | 0.953 | 0.131 | 54.0% | 0.127 |
| 10 | DET (H) | 38-41 | DET | 0.935 | 0.219 | 47.8% | -0.300 |

**BUF analogs vs LAC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 10 | DEN (H) | 22-24 | DEN | 0.913 | -0.119 | 44.8% | -0.167 |
| 3 | WAS (A) | 31-3 | BUF | 0.905 | 0.161 | 44.4% | 2.545 |
| 9 | CIN (A) | 18-24 | CIN | 0.838 | 0.031 | 50.0% | -0.667 |

## Core EPA Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.076 | 0.037 | 0.080 |
| LAC | -0.053 | -0.218 | -0.284 |

## Core EPA Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | -0.060 | -0.091 | -0.027 |
| LAC | -0.003 | -0.043 | -0.053 |

## Success Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 47.0% | 43.0% | 44.5% |
| LAC | 40.9% | 38.4% | 38.6% |

## Success Rate Defense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 41.7% | 39.0% | 43.9% |
| LAC | 43.2% | 42.8% | 41.8% |

## Explosive Play Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 11.2% | 11.5% | 11.3% |
| LAC | 8.8% | 6.9% | 5.3% |

## Third Down Conversion Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 45.2% | 42.9% | 45.5% |
| LAC | 37.7% | 34.9% | 28.3% |

## Points per Drive Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.858 | 0.758 | 0.603 |
| LAC | -0.280 | -0.988 | -1.207 |

## Yards per Play Differential Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.662 | 1.076 | 0.252 |
| LAC | -0.387 | -0.286 | -0.463 |

## Turnover Margin Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 0.214 | 0.600 | 1.000 |
| LAC | -0.143 | -1.600 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 16.6% | 14.7% | 16.1% |
| LAC | 14.2% | 10.3% | 12.5% |

## Pressure Rate (Def) Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 12.1% | 14.1% | 9.0% |
| LAC | 7.8% | 7.4% | 8.2% |

## Tempo Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 7.818 | 7.835 | 8.641 |
| LAC | 7.379 | 6.619 | 6.244 |

## Pass Rate Offense Form (up to Week 15)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BUF | 45.1% | 40.1% | 40.8% |
| LAC | 48.0% | 51.5% | 49.6% |
