# Matchup Report - LAC vs DET

## Metric Comparison

| Metric | LAC | DET | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.219 | 0.325 | ↓ -0.106 |
| Core EPA Defense | 0.325 | 0.219 | ↑ +0.106 |
| Success Rate Offense | 47.8% | 52.4% | ↓ -4.7 pp |
| Success Rate Defense | 52.4% | 47.8% | ↑ +4.7 pp |
| Explosive Play Rate (Off) | 11.1% | 14.6% | ↓ -3.5 pp |
| Third Down Conversion | 41.2% | 30.8% | ↑ +10.4 pp |
| Points per Drive Differential | -0.300 | 0.300 | ↓ -0.600 |
| Yards per Play Differential | -1.822 | 1.822 | ↓ -3.644 |
| Turnover Margin | -1.000 | 1.000 | ↓ -2.000 |
| Red Zone TD Rate (Off) | 16.7% | 13.6% | ↑ +3.0 pp |
| Pressure Rate (Def) | 0.0% | 0.0% | ± 0.0 pp |
| Tempo | 9.000 | 8.200 | ↑ +0.800 |
| PowerScore | 0.283 | 0.318 | ↓ -0.035 |

## PowerScore Breakdown (Model)

| Component | Weight | LAC | DET | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.219 | 0.325 | -0.106 |
| EPA Defense | 20% | 0.325 | 0.219 | +0.106 |
| Success Rate Offense | 25% | 47.8% | 52.4% | -4.7 pp |
| Tempo | 20% | 0.111 | 0.146 | -0.035 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LAC | DET | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.219 | 0.325 | -0.106 |
| EPA Defense | 20% | 0.325 | 0.219 | +0.106 |
| Success Rate Offense | 15% | 47.8% | 52.4% | -4.7 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.111 | 0.146 | -0.035 |
| Red Zone TD Rate (Off) | 10% | 16.7% | 13.6% | +3.0 pp |
| Pressure Rate (Def) | 10% | 0.0% | 0.0% | +0.0 pp |

## PowerScore Summary

**Model (4 metrics):**
DET edge: +0.035 (LAC +0.283 vs DET +0.318)
**7 metrics version:**
DET edge: +0.213 (LAC +0.119 vs DET +0.332)
**Verdict:** DET holds the edge in the extended breakdown (lead 0.213), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LAC - DET) | -0.6 pts (favours DET) |
| Model Win% LAC | 47.3% |
| Model Win% DET | 52.7% |
| SOS Differential (LAC-DET) | +0.036 |
*Neutral-field assumption. Based on schedule through Week 9.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LAC | 44.4% | 43.9% | 0.5% | 57.5% | 40.0% | Balanced |
| DET | 43.9% | 43.9% | -0.0% | 55.6% | 50.0% | Balanced |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +10.6 pp | +12.9 pp | +21.7 pp |
| DET | +5.2 pp | +9.7 pp | +6.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +3.4 pp | +3.0 pp | +5.7 pp |
| DET | -4.6 pp | -6.3 pp | -12.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +0.4 pp | +0.4 pp | -1.0 pp |
| DET | +4.3 pp | +3.5 pp | +0.5 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -0.0 pp | -0.4 pp | -2.7 pp |
| DET | -0.9 pp | +1.3 pp | +0.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -2.4 pp | -7.5 pp | -9.1 pp |
| DET | +0.5 pp | +7.7 pp | +5.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.0 pp | -3.8 pp | +1.8 pp |
| DET | +1.8 pp | +2.1 pp | +1.3 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -0.0 pp | -0.4 pp | -2.7 pp |
| DET | -0.9 pp | +1.3 pp | +0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +0.4 pp | +0.4 pp | -1.0 pp |
| DET | +4.3 pp | +3.5 pp | +0.5 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 45.3 | 45.0 | 45.0 |
| DET | 47.5 | 48.2 | 45.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 47.0 | 44.9 | 43.7 |
| DET | 47.5 | 47.8 | 43.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.7 | 0.1 | 1.2 |
| DET | 0.0 | 0.4 | 1.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 2.23 | 1.95 | 2.02 |
| DET | 2.09 | 2.25 | 1.53 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 2.02 | 1.69 | 1.64 |
| DET | 1.78 | 1.83 | 1.69 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 0.21 | 0.26 | 0.37 |
| DET | 0.31 | 0.42 | -0.16 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 7.65 | 7.23 | 6.98 |
| DET | 7.61 | 7.99 | 8.38 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 46.7% | 44.5% | 46.8% |
| DET | 46.4% | 47.5% | 53.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 53.3% | 55.5% | 53.2% |
| DET | 53.6% | 52.5% | 46.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 3.57 | 3.22 | 3.27 |
| DET | 3.53 | 3.80 | 4.52 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 4.08 | 4.01 | 3.71 |
| DET | 4.07 | 4.19 | 3.86 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 9)

| Window | LAC | DET |
| --- | ---: | ---: |
| Season-to-date | 0.125 | 0.089 |
| Last 5 games | 0.084 | 0.095 |
| Last 3 games | 0.081 | 0.082 |

## Trend Summary (last 3 weeks)

| Metric | W6 | W7 | W8 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LAC | -0.059 | 0.040 | 0.058 | + improving |
| Off EPA DET | 0.038 | -0.126 | -0.123 | - declining |
| Def EPA LAC | 0.000 | 0.201 | -0.187 | + improving |
| Def EPA DET | -0.200 | 0.359 | -0.436 | + improving |
| Off SR LAC | 36.0% | 36.2% | 44.4% | + improving |
| Off SR DET | 46.0% | 38.6% | 42.6% | - declining |
| Def SR LAC | 38.8% | 47.0% | 34.6% | + improving |
| Def SR DET | 36.6% | 52.1% | 39.3% | - declining |
| Tempo LAC | 8.600 | 7.273 | 8.100 | - declining |
| Tempo DET | 7.909 | 8.800 | 8.417 | + improving |

## Matchup Analogs

**LAC analogs vs DET profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 6 | DAL (H) | 17-20 | DAL | 0.956 | -0.059 | 36.0% | -0.300 |
| 7 | KC (A) | 17-31 | KC | 0.830 | 0.040 | 36.2% | -1.273 |
| 3 | MIN (A) | 28-24 | LAC | 0.779 | 0.050 | 45.7% | 0.364 |

**DET analogs vs LAC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | KC (A) | 15-22 | KC | 0.692 | -0.046 | 35.6% | -0.568 |
| 6 | TB (A) | 20-7 | DET | 0.635 | 0.038 | 46.0% | 1.218 |
| 7 | BAL (A) | 6-38 | BAL | 0.562 | -0.126 | 38.6% | -3.200 |

## Core EPA Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.013 | 0.014 | -0.071 |
| LAC | 0.016 | -0.020 | 0.008 |

## Core EPA Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.094 | -0.096 | -0.092 |
| LAC | -0.020 | -0.087 | -0.101 |

## Success Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 43.3% | 45.2% | 42.4% |
| LAC | 41.5% | 37.4% | 38.9% |

## Success Rate Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 42.1% | 43.1% | 42.9% |
| LAC | 42.3% | 40.1% | 38.8% |

## Explosive Play Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 10.3% | 10.3% | 9.4% |
| LAC | 9.6% | 9.2% | 8.4% |

## Third Down Conversion Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 38.9% | 40.8% | 42.3% |
| LAC | 38.9% | 38.5% | 43.5% |

## Points per Drive Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.368 | 0.518 | -0.018 |
| LAC | 0.216 | 0.258 | 0.350 |

## Yards per Play Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.676 | 0.409 | -0.061 |
| LAC | -0.271 | -0.244 | -0.335 |

## Turnover Margin Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.000 | 0.600 | -0.333 |
| LAC | 0.875 | 0.800 | 1.333 |

## Red Zone TD Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 9.2% | 7.2% | 1.6% |
| LAC | 16.3% | 16.6% | 19.2% |

## Pressure Rate (Def) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 7.1% | 7.8% | 8.3% |
| LAC | 9.0% | 9.5% | 6.2% |

## Tempo Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 7.609 | 7.992 | 8.375 |
| LAC | 7.652 | 7.225 | 6.981 |

## Pass Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 46.3% | 47.2% | 53.3% |
| LAC | 46.4% | 44.6% | 46.9% |
