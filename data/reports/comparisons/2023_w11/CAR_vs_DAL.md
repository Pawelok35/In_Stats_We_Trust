# Matchup Report - CAR vs DAL

## Metric Comparison

| Metric | CAR | DAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.105 | 0.055 | -0.160 |
| Core EPA Defense | 0.025 | -0.118 | 0.143 |
| Success Rate Offense | 0.389 | 0.449 | -0.060 |
| Success Rate Defense | 0.457 | 0.393 | 0.063 |
| Explosive Play Rate (Off) | 0.061 | 0.116 | -0.055 |
| Third Down Conversion | 0.317 | 0.432 | -0.114 |
| Points per Drive Differential | -0.905 | 0.674 | -1.579 |
| Yards per Play Differential | -0.749 | 0.960 | -1.709 |
| Turnover Margin | -0.667 | 0.667 | -1.333 |
| Red Zone TD Rate (Off) | 0.121 | 0.106 | 0.016 |
| Pressure Rate (Def) | 0.064 | 0.094 | -0.030 |
| Tempo | 7.600 | 8.700 | -1.100 |
| PowerScore | 0.075 | 0.145 | -0.070 |
| PowerScore | 0.075 | 0.145 | -0.070 |
| Tempo | 7.600 | 8.700 | -1.100 |

## PowerScore Breakdown (Model)

| Component | Weight | CAR | DAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.196 | 0.160 | -0.356 |
| EPA Defense | 20% | 0.160 | -0.196 | +0.356 |
| Success Rate Offense | 25% | 38.2% | 43.7% | -5.5 pp |
| Tempo | 20% | 0.079 | 0.092 | -0.013 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | CAR | DAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.196 | 0.160 | -0.356 |
| EPA Defense | 20% | 0.160 | -0.196 | +0.356 |
| Success Rate Offense | 15% | 38.2% | 43.7% | -5.5 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.079 | 0.092 | -0.013 |
| Red Zone TD Rate (Off) | 10% | 12.5% | 10.5% | +2.0 pp |
| Pressure Rate (Def) | 10% | 0.0% | 18.9% | -18.9 pp |

## PowerScore Summary

**Model (4 metrics):**
DAL edge: +0.070 (CAR +0.075 vs DAL +0.145)
**7 metrics version:**
DAL edge: +0.444 (CAR -0.139 vs DAL +0.305)
**Verdict:** DAL holds the edge in the extended breakdown (lead 0.444), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (CAR - DAL) | -1.3 pts (favours DAL) |
| Model Win% CAR | 44.6% |
| Model Win% DAL | 55.4% |
| SOS Differential (CAR-DAL) | +0.035 |
*Neutral-field assumption. Based on schedule through Week 10.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CAR | 48.7% | 45.8% | 2.8% | 27.0% | 48.7% | Slight pass lean |
| DAL | 50.6% | 45.8% | 4.7% | 38.6% | 48.8% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +4.1 pp | +3.9 pp | +4.8 pp |
| DAL | +11.6 pp | +2.9 pp | +11.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +1.5 pp | -5.0 pp | -9.4 pp |
| DAL | -6.6 pp | -2.8 pp | +3.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.4 pp | -0.6 pp | +0.7 pp |
| DAL | -0.0 pp | -5.4 pp | -2.3 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -2.1 pp | -3.3 pp | -4.1 pp |
| DAL | +1.8 pp | +5.0 pp | +9.1 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +1.0 pp | -0.6 pp | -0.7 pp |
| DAL | -3.2 pp | -3.4 pp | +5.4 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -1.9 pp | -7.5 pp | -6.0 pp |
| DAL | +1.7 pp | +4.9 pp | +13.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -2.1 pp | -3.3 pp | -4.1 pp |
| DAL | +1.8 pp | +5.0 pp | +9.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | +0.4 pp | -0.6 pp | +0.7 pp |
| DAL | -0.0 pp | -5.4 pp | -2.3 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 47.6 | 46.6 | 42.2 |
| DAL | 45.7 | 44.1 | 45.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 46.5 | 48.4 | 46.0 |
| DAL | 50.7 | 50.8 | 50.5 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.1 | -1.8 | -3.8 |
| DAL | -5.0 | -6.7 | -5.4 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 1.35 | 1.36 | 1.05 |
| DAL | 2.40 | 2.39 | 3.06 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 2.25 | 2.45 | 1.65 |
| DAL | 1.73 | 2.15 | 1.94 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | -0.91 | -1.09 | -0.60 |
| DAL | 0.67 | 0.24 | 1.12 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 7.93 | 7.79 | 7.62 |
| DAL | 7.95 | 7.17 | 7.66 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 48.2% | 48.4% | 48.3% |
| DAL | 46.6% | 49.6% | 50.9% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 51.8% | 51.6% | 51.7% |
| DAL | 53.4% | 50.4% | 49.1% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 3.82 | 3.77 | 3.68 |
| DAL | 3.70 | 3.56 | 3.90 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| CAR | 4.10 | 4.02 | 3.94 |
| DAL | 4.25 | 3.61 | 3.76 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 10)

| Window | CAR | DAL |
| --- | ---: | ---: |
| Season-to-date | 1.546 | 1.511 |
| Last 5 games | 1.628 | 1.577 |
| Last 3 games | 1.665 | 1.525 |

## Trend Summary (last 3 weeks)

| Metric | W8 | W9 | W10 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA CAR | -0.021 | -0.178 | -0.142 | - declining |
| Off EPA DAL | 0.185 | 0.150 | 0.245 | + improving |
| Def EPA CAR | -0.043 | -0.002 | -0.114 | + improving |
| Def EPA DAL | -0.123 | 0.209 | -0.163 | + improving |
| Off SR CAR | 41.0% | 37.1% | 30.4% | - declining |
| Off SR DAL | 48.8% | 47.8% | 52.7% | + improving |
| Def SR CAR | 39.4% | 40.7% | 41.6% | - declining |
| Def SR DAL | 36.2% | 49.4% | 33.3% | + improving |
| Tempo CAR | 7.545 | 7.417 | 7.900 | + improving |
| Tempo DAL | 6.833 | 9.000 | 7.154 | + improving |

## Matchup Analogs

**CAR analogs vs DAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | MIN (H) | 7-15 | MIN | 0.907 | -0.182 | 38.2% | -0.800 |
| 5 | DET (A) | 24-38 | DET | 0.885 | -0.061 | 45.6% | -1.418 |
| 2 | NO (H) | 17-20 | NO | 0.693 | -0.051 | 44.7% | -0.273 |

**DAL analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 3 | ARI (A) | 16-25 | ARI | 0.967 | 0.051 | 45.8% | -1.111 |
| 1 | NYG (A) | 28-5 | DAL | 0.935 | 0.102 | 45.8% | 2.300 |
| 10 | NYG (H) | 49-16 | DAL | 0.935 | 0.245 | 52.7% | 2.555 |

## Core EPA Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.105 | -0.102 | -0.114 |
| DAL | 0.055 | 0.046 | 0.193 |

## Core EPA Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.025 | 0.041 | -0.053 |
| DAL | -0.118 | -0.000 | -0.026 |

## Success Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 38.9% | 38.7% | 36.3% |
| DAL | 45.2% | 45.6% | 49.8% |

## Success Rate Defense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 45.7% | 44.5% | 40.7% |
| DAL | 39.7% | 42.8% | 39.7% |

## Explosive Play Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.2% | 5.6% | 4.4% |
| DAL | 11.7% | 13.7% | 15.1% |

## Third Down Conversion Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 32.2% | 32.2% | 29.6% |
| DAL | 43.6% | 41.4% | 43.5% |

## Points per Drive Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.904 | -1.092 | -0.572 |
| DAL | 0.713 | 0.248 | 1.240 |

## Yards per Play Differential Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.749 | -0.632 | -0.167 |
| DAL | 0.960 | 1.278 | 2.259 |

## Turnover Margin Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.667 | -1.000 | -1.000 |
| DAL | 0.667 | -0.600 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 12.1% | 10.2% | 5.9% |
| DAL | 10.6% | 13.4% | 16.8% |

## Pressure Rate (Def) Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.4% | 3.4% | 3.6% |
| DAL | 9.4% | 8.3% | 11.7% |

## Tempo Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 7.926 | 7.789 | 7.621 |
| DAL | 7.946 | 7.167 | 7.662 |

## Pass Rate Offense Form (up to Week 10)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 48.4% | 48.4% | 48.2% |
| DAL | 46.6% | 49.8% | 50.9% |
