# Matchup Report - LAC vs PIT

## Metric Comparison

| Metric | LAC | PIT | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.034 | 0.077 | -0.043 |
| Core EPA Defense | -0.004 | 0.053 | -0.057 |
| Success Rate Offense | 0.487 | 0.480 | 0.007 |
| Success Rate Defense | 0.442 | 0.495 | -0.053 |
| Explosive Play Rate (Off) | 0.101 | 0.086 | 0.015 |
| Third Down Conversion | 0.463 | 0.355 | 0.108 |
| Points per Drive Differential | 0.429 | 0.183 | 0.246 |
| Yards per Play Differential | 0.601 | -0.555 | 1.155 |
| Turnover Margin | -0.444 | 0.875 | -1.319 |
| Red Zone TD Rate (Off) | 0.112 | 0.182 | -0.070 |
| Pressure Rate (Def) | 0.092 | 0.070 | 0.021 |
| Tempo | 7.077 | 5.667 | 1.410 |
| PowerScore | 1.444 | 1.123 | 0.322 |
| PowerScore | 1.444 | 1.123 | 0.322 |
| Tempo | 7.077 | 5.667 | 1.410 |

## PowerScore Breakdown (Model)

| Component | Weight | LAC | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.052 | -0.295 | +0.243 |
| EPA Defense | 20% | -0.295 | -0.052 | -0.243 |
| Success Rate Offense | 25% | 42.4% | 41.2% | +1.2 pp |
| Tempo | 20% | 0.109 | 0.103 | +0.006 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | LAC | PIT | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.052 | -0.295 | +0.243 |
| EPA Defense | 20% | -0.295 | -0.052 | -0.243 |
| Success Rate Offense | 15% | 42.4% | 41.2% | +1.2 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.109 | 0.103 | +0.006 |
| Red Zone TD Rate (Off) | 10% | 15.4% | 0.0% | +15.4 pp |
| Pressure Rate (Def) | 10% | 8.8% | 12.8% | -4.0 pp |

## PowerScore Summary

**Model (4 metrics):**
LAC edge: +0.041 (LAC +0.050 vs PIT +0.010)
**7 metrics version:**
LAC edge: +0.226 (LAC +0.127 vs PIT -0.099)
**Verdict:** LAC holds the edge in the extended breakdown (lead 0.226), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (LAC - PIT) | +5.9 pts (favours LAC) |
| Model Win% LAC | 72.9% |
| Model Win% PIT | 27.1% |
| SOS Differential (LAC-PIT) | -0.204 |
*Neutral-field assumption. Based on schedule through Week 9.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| LAC | 42.4% | 42.4% | -0.0% | 35.9% | 47.2% | Balanced |
| PIT | 50.0% | 42.4% | 7.6% | 35.3% | 47.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | +6.7 pp | +15.2 pp | +11.6 pp |
| PIT | +5.8 pp | +9.5 pp | +9.1 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.8 pp | +3.1 pp | +4.0 pp |
| PIT | +7.2 pp | +2.4 pp | +3.3 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.0 pp | -2.0 pp | -5.4 pp |
| PIT | +3.3 pp | +3.3 pp | +4.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.3 pp | +1.0 pp | -0.4 pp |
| PIT | -1.7 pp | -2.7 pp | -3.8 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -0.9 pp | -5.0 pp | -2.4 pp |
| PIT | +3.4 pp | +3.4 pp | +6.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.6 pp | +3.5 pp | +2.9 pp |
| PIT | +3.2 pp | -1.2 pp | +1.2 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.3 pp | +1.0 pp | -0.4 pp |
| PIT | -1.7 pp | -2.7 pp | -3.8 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -1.0 pp | -2.0 pp | -5.4 pp |
| PIT | +3.3 pp | +3.3 pp | +4.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 46.2 | 47.9 | 45.0 |
| PIT | 50.3 | 51.1 | 54.5 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 51.8 | 52.6 | 56.4 |
| PIT | 47.6 | 46.8 | 46.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | -5.6 | -4.7 | -11.4 |
| PIT | 2.7 | 4.3 | 7.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 2.51 | 2.80 | 3.37 |
| PIT | 2.39 | 2.44 | 2.48 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 2.08 | 2.33 | 2.01 |
| PIT | 2.21 | 2.23 | 2.68 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 0.43 | 0.47 | 1.36 |
| PIT | 0.18 | 0.21 | -0.20 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 8.60 | 9.12 | 9.61 |
| PIT | 6.91 | 7.12 | 7.00 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 47.5% | 46.0% | 45.7% |
| PIT | 45.0% | 45.0% | 47.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 52.5% | 54.0% | 54.3% |
| PIT | 55.0% | 55.0% | 52.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 4.08 | 4.20 | 4.39 |
| PIT | 3.11 | 3.21 | 3.35 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| LAC | 4.51 | 4.93 | 5.22 |
| PIT | 3.80 | 3.91 | 3.65 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 9)

| Window | LAC | PIT |
| --- | ---: | ---: |
| Season-to-date | 1.655 | 1.859 |
| Last 5 games | 1.627 | 1.958 |
| Last 3 games | 1.543 | 1.849 |

## Trend Summary (last 3 weeks)

| Metric | W7 | W8 | W9 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA LAC | 0.090 | 0.170 | -0.002 | - declining |
| Off EPA PIT | 0.262 | 0.017 | 0.046 | - declining |
| Def EPA LAC | 0.367 | -0.255 | -0.151 | + improving |
| Def EPA PIT | 0.219 | 0.169 | -0.042 | + improving |
| Off SR LAC | 47.8% | 54.5% | 54.2% | + improving |
| Off SR PIT | 54.7% | 43.8% | 45.7% | - declining |
| Def SR LAC | 55.4% | 37.3% | 37.5% | + improving |
| Def SR PIT | 53.7% | 48.8% | 52.2% | + improving |
| Tempo LAC | 11.500 | 9.778 | 7.545 | - declining |
| Tempo PIT | 7.500 | 7.273 | 6.231 | - declining |

## Matchup Analogs

**LAC analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | NYG (A) | 18-21 | NYG | 0.793 | -0.066 | 46.8% | -0.273 |
| 5 | WAS (H) | 10-34 | WAS | 0.784 | -0.082 | 39.3% | -2.375 |
| 1 | KC (H) | 27-21 | LAC | 0.751 | 0.211 | 56.8% | 0.750 |

**PIT analogs vs LAC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 8 | GB (H) | 25-32 | GB | 0.758 | 0.017 | 43.8% | -0.644 |
| 2 | SEA (H) | 23-27 | SEA | 0.738 | -0.208 | 36.8% | -0.314 |
| 3 | NE (A) | 21-16 | PIT | 0.704 | 0.037 | 51.5% | 0.544 |

## Core EPA Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 0.034 | 0.061 | 0.086 |
| PIT | 0.077 | 0.116 | 0.109 |

## Core EPA Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | -0.004 | 0.042 | -0.013 |
| PIT | 0.053 | 0.064 | 0.116 |

## Success Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 48.6% | 49.2% | 52.1% |
| PIT | 47.8% | 48.7% | 47.9% |

## Success Rate Defense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 44.5% | 47.6% | 43.9% |
| PIT | 49.4% | 49.0% | 51.7% |

## Explosive Play Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 10.1% | 10.9% | 11.8% |
| PIT | 8.5% | 8.7% | 7.2% |

## Third Down Conversion Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 46.7% | 51.8% | 54.2% |
| PIT | 35.1% | 37.0% | 35.6% |

## Points per Drive Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 0.435 | 0.524 | 1.515 |
| PIT | 0.143 | 0.170 | -0.168 |

## Yards per Play Differential Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 0.601 | 0.357 | 0.873 |
| PIT | -0.555 | -0.059 | -0.877 |

## Turnover Margin Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | -0.444 | -0.400 | -1.000 |
| PIT | 0.875 | 0.400 | 0.000 |

## Red Zone TD Rate (Off) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 11.2% | 13.5% | 17.3% |
| PIT | 18.2% | 16.2% | 15.5% |

## Pressure Rate (Def) Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 9.2% | 8.4% | 10.8% |
| PIT | 7.0% | 6.9% | 4.3% |

## Tempo Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 8.598 | 9.125 | 9.608 |
| PIT | 6.907 | 7.122 | 7.001 |

## Pass Rate Offense Form (up to Week 9)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| LAC | 47.7% | 46.2% | 46.0% |
| PIT | 45.3% | 45.3% | 47.9% |
