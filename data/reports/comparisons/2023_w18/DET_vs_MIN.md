# Matchup Report - DET vs MIN

## Metric Comparison

| Metric | DET | MIN | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.017 | -0.054 | 0.071 |
| Core EPA Defense | -0.039 | -0.048 | 0.009 |
| Success Rate Offense | 0.436 | 0.418 | 0.018 |
| Success Rate Defense | 0.416 | 0.411 | 0.005 |
| Explosive Play Rate (Off) | 0.110 | 0.105 | 0.006 |
| Third Down Conversion | 0.417 | 0.360 | 0.057 |
| Points per Drive Differential | 0.353 | 0.003 | 0.350 |
| Yards per Play Differential | 0.488 | 0.280 | 0.208 |
| Turnover Margin | -0.125 | -0.625 | 0.500 |
| Red Zone TD Rate (Off) | 0.142 | 0.142 | -0.001 |
| Pressure Rate (Def) | 0.065 | 0.071 | -0.006 |
| Tempo | 6.727 | 8.273 | -1.545 |
| PowerScore | 0.152 | 0.127 | 0.025 |
| PowerScore | 0.152 | 0.127 | 0.025 |
| Tempo | 6.727 | 8.273 | -1.545 |

## PowerScore Breakdown (Model)

| Component | Weight | DET | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.100 | -0.036 | +0.136 |
| EPA Defense | 20% | -0.036 | 0.100 | -0.136 |
| Success Rate Offense | 25% | 43.2% | 37.4% | +5.9 pp |
| Tempo | 20% | 0.081 | 0.132 | -0.051 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | DET | MIN | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.100 | -0.036 | +0.136 |
| EPA Defense | 20% | -0.036 | 0.100 | -0.136 |
| Success Rate Offense | 15% | 43.2% | 37.4% | +5.9 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.081 | 0.132 | -0.051 |
| Red Zone TD Rate (Off) | 10% | 20.0% | 0.0% | +20.0 pp |
| Pressure Rate (Def) | 10% | 8.3% | 3.0% | +5.3 pp |

## PowerScore Summary

**Model (4 metrics):**
DET edge: +0.025 (DET +0.152 vs MIN +0.127)
**7 metrics version:**
DET edge: +0.436 (DET +0.319 vs MIN -0.117)
**Verdict:** DET holds the edge in the extended breakdown (lead 0.436), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (DET - MIN) | +0.5 pts (favours DET) |
| Model Win% DET | 51.9% |
| Model Win% MIN | 48.1% |
| SOS Differential (DET-MIN) | -0.070 |
*Neutral-field assumption. Based on schedule through Week 17.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| DET | 44.6% | 42.8% | 1.8% | 48.5% | 39.0% | Slight pass lean |
| MIN | 52.7% | 42.8% | 10.0% | 37.5% | 37.2% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.7 pp | -1.7 pp | -8.8 pp |
| MIN | +2.5 pp | -9.2 pp | -5.7 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.3 pp | +11.9 pp | +5.1 pp |
| MIN | +0.1 pp | -2.3 pp | +0.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.4 pp | +1.3 pp | +0.6 pp |
| MIN | -0.4 pp | -2.0 pp | -2.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.7 pp | +3.0 pp | +3.9 pp |
| MIN | +0.2 pp | -1.9 pp | -1.1 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +1.1 pp | -1.5 pp | -2.8 pp |
| MIN | +2.9 pp | +4.3 pp | +2.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +4.7 pp | +3.2 pp | -3.8 pp |
| MIN | -3.0 pp | -6.4 pp | -1.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.7 pp | +3.0 pp | +3.9 pp |
| MIN | +0.2 pp | -1.9 pp | -1.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | +2.4 pp | +1.3 pp | +0.6 pp |
| MIN | -0.4 pp | -2.0 pp | -2.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 48.9 | 48.8 | 45.6 |
| MIN | 45.7 | 46.0 | 51.6 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 48.1 | 47.4 | 47.9 |
| MIN | 47.4 | 41.6 | 44.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 0.7 | 1.5 | -2.2 |
| MIN | -1.7 | 4.4 | 7.0 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 2.38 | 2.52 | 2.88 |
| MIN | 1.86 | 1.38 | 1.90 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 2.03 | 2.02 | 1.87 |
| MIN | 1.86 | 1.86 | 2.70 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 0.35 | 0.50 | 1.01 |
| MIN | 0.00 | -0.48 | -0.80 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 7.74 | 7.83 | 8.37 |
| MIN | 7.71 | 7.17 | 7.22 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 45.7% | 42.8% | 43.3% |
| MIN | 49.3% | 50.3% | 51.6% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 54.3% | 57.2% | 56.7% |
| MIN | 50.7% | 49.7% | 48.4% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 3.54 | 3.35 | 3.63 |
| MIN | 3.80 | 3.61 | 3.73 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| DET | 4.21 | 4.48 | 4.74 |
| MIN | 3.91 | 3.56 | 3.49 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 17)

| Window | DET | MIN |
| --- | ---: | ---: |
| Season-to-date | 1.532 | 1.602 |
| Last 5 games | 1.486 | 1.575 |
| Last 3 games | 1.521 | 1.796 |

## Trend Summary (last 3 weeks)

| Metric | W15 | W16 | W17 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA DET | 0.241 | 0.111 | -0.049 | - declining |
| Off EPA MIN | 0.083 | 0.015 | -0.180 | - declining |
| Def EPA DET | -0.083 | 0.015 | -0.044 | - declining |
| Def EPA MIN | 0.080 | 0.111 | 0.191 | - declining |
| Off SR DET | 50.0% | 43.6% | 38.8% | - declining |
| Off SR MIN | 47.6% | 39.1% | 35.7% | - declining |
| Def SR DET | 39.5% | 39.1% | 43.4% | - declining |
| Def SR MIN | 47.1% | 43.6% | 51.7% | - declining |
| Tempo DET | 8.200 | 9.182 | 7.727 | - declining |
| Tempo MIN | 8.400 | 6.900 | 6.364 | - declining |

## Matchup Analogs

**DET analogs vs MIN profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 16 | MIN (A) | 30-26 | DET | 1.000 | 0.111 | 43.6% | 0.327 |
| 11 | CHI (H) | 29-26 | DET | 0.884 | 0.016 | 50.6% | 0.273 |
| 14 | CHI (A) | 13-26 | CHI | 0.884 | -0.200 | 34.9% | -1.152 |

**MIN analogs vs DET profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 16 | DET (H) | 24-27 | DET | 1.000 | 0.015 | 39.1% | -0.327 |
| 7 | SF (H) | 22-17 | MIN | 0.968 | 0.055 | 40.7% | 0.556 |
| 8 | GB (A) | 24-10 | MIN | 0.841 | 0.055 | 42.4% | 1.400 |

## Core EPA Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.017 | 0.031 | 0.101 |
| MIN | -0.054 | -0.129 | -0.027 |

## Core EPA Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.039 | -0.031 | -0.037 |
| MIN | -0.048 | -0.043 | 0.127 |

## Success Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 43.4% | 41.3% | 44.0% |
| MIN | 41.9% | 39.1% | 41.3% |

## Success Rate Defense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 41.8% | 39.5% | 40.8% |
| MIN | 41.4% | 41.4% | 47.3% |

## Explosive Play Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 11.0% | 11.9% | 13.8% |
| MIN | 10.4% | 9.5% | 10.3% |

## Third Down Conversion Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 41.5% | 41.4% | 41.2% |
| MIN | 36.8% | 30.2% | 31.3% |

## Points per Drive Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.369 | 0.460 | 0.963 |
| MIN | 0.008 | -0.501 | -0.850 |

## Yards per Play Differential Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 0.488 | -0.022 | 0.145 |
| MIN | 0.280 | -0.121 | 0.038 |

## Turnover Margin Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | -0.125 | 0.600 | 1.333 |
| MIN | -0.625 | -1.200 | -2.333 |

## Red Zone TD Rate (Off) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 14.2% | 20.0% | 18.6% |
| MIN | 14.2% | 12.8% | 13.0% |

## Pressure Rate (Def) Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 6.5% | 7.4% | 7.6% |
| MIN | 7.1% | 6.6% | 5.0% |

## Tempo Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 7.742 | 7.831 | 8.370 |
| MIN | 7.709 | 7.169 | 7.221 |

## Pass Rate Offense Form (up to Week 17)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| DET | 45.6% | 42.9% | 43.3% |
| MIN | 49.2% | 49.9% | 51.1% |
