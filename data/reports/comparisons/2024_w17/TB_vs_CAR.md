# Matchup Report - TB vs CAR

## Metric Comparison

| Metric | TB | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.316 | -0.130 | ↑ +0.446 |
| Core EPA Defense | -0.130 | 0.316 | ↓ -0.446 |
| Success Rate Offense | 59.8% | 47.8% | ↑ +12.0 pp |
| Success Rate Defense | 47.8% | 59.8% | ↓ -12.0 pp |
| Explosive Play Rate (Off) | 19.6% | 10.4% | ↑ +9.1 pp |
| Third Down Conversion | 66.7% | 22.2% | ↑ +44.4 pp |
| Points per Drive Differential | 2.200 | -2.200 | ↑ +4.400 |
| Yards per Play Differential | 2.944 | -2.944 | ↑ +5.889 |
| Turnover Margin | 0.000 | 0.000 | ± 0.000 |
| Red Zone TD Rate (Off) | 17.2% | 25.0% | ↓ -7.8 pp |
| Pressure Rate (Def) | 15.2% | 5.4% | ↑ +9.7 pp |
| Tempo | 9.200 | 6.700 | ↑ +2.500 |
| PowerScore | 0.273 | 0.158 | ↑ +0.115 |

## PowerScore Breakdown (Model)

| Component | Weight | TB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.316 | -0.130 | +0.446 |
| EPA Defense | 20% | -0.130 | 0.316 | -0.446 |
| Success Rate Offense | 25% | 59.8% | 47.8% | +12.0 pp |
| Tempo | 20% | 0.196 | 0.104 | +0.091 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | TB | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.316 | -0.130 | +0.446 |
| EPA Defense | 20% | -0.130 | 0.316 | -0.446 |
| Success Rate Offense | 15% | 59.8% | 47.8% | +12.0 pp |
| Turnover Margin | 10% | 0.000 | 0.000 | +0.000 |
| Tempo | 10% | 0.196 | 0.104 | +0.091 |
| Red Zone TD Rate (Off) | 10% | 17.2% | 25.0% | -7.8 pp |
| Pressure Rate (Def) | 10% | 15.2% | 5.4% | +9.7 pp |

## PowerScore Summary

**Model (4 metrics):**
TB edge: +0.115 (TB +0.273 vs CAR +0.158)
**7 metrics version:**
TB edge: +0.051 (TB +0.195 vs CAR +0.143)
**Verdict:** TB holds the edge in the extended breakdown (lead 0.051), driven by EPA Offense (25% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (TB - CAR) | +2.1 pts (favours TB) |
| Model Win% TB | 58.8% |
| Model Win% CAR | 41.2% |
| SOS Differential (TB-CAR) | -0.012 |
*Neutral-field assumption. Based on schedule through Week 16.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| TB | 40.2% | 43.9% | -3.7% | 59.5% | 60.0% | Run heavy |
| CAR | 49.3% | 43.9% | 5.4% | 36.4% | 58.8% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -0.2 pp | +2.1 pp | +5.9 pp |
| CAR | -5.3 pp | +4.9 pp | +12.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | +2.3 pp | +3.7 pp | +0.6 pp |
| CAR | -3.0 pp | -3.8 pp | -5.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -1.5 pp | +3.0 pp | +1.0 pp |
| CAR | +0.2 pp | -1.9 pp | -4.9 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | +0.9 pp | +2.0 pp | +2.8 pp |
| CAR | -0.6 pp | +2.7 pp | +3.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -2.5 pp | -3.3 pp | +2.7 pp |
| CAR | -2.2 pp | +3.5 pp | +2.7 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | +4.9 pp | +7.6 pp | +9.3 pp |
| CAR | -9.8 pp | -1.6 pp | -4.4 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | +0.9 pp | +2.0 pp | +2.8 pp |
| CAR | -0.6 pp | +2.7 pp | +3.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -1.5 pp | +3.0 pp | +1.0 pp |
| CAR | +0.2 pp | -1.9 pp | -4.9 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 47.5 | 44.0 | 44.4 |
| CAR | 50.9 | 51.3 | 50.4 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 51.6 | 52.5 | 54.9 |
| CAR | 46.4 | 47.3 | 45.6 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | -4.1 | -8.5 | -10.4 |
| CAR | 4.5 | 4.0 | 4.8 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 2.61 | 2.74 | 2.87 |
| CAR | 1.68 | 2.12 | 1.98 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 2.06 | 1.45 | 1.63 |
| CAR | 2.70 | 2.68 | 2.63 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 0.54 | 1.29 | 1.24 |
| CAR | -1.03 | -0.56 | -0.65 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 7.59 | 7.74 | 7.78 |
| CAR | 7.17 | 7.88 | 7.92 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 44.3% | 41.4% | 43.4% |
| CAR | 43.6% | 43.6% | 40.5% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 55.7% | 58.6% | 56.6% |
| CAR | 56.4% | 56.4% | 59.5% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 3.36 | 3.21 | 3.37 |
| CAR | 3.12 | 3.44 | 3.20 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| TB | 4.22 | 4.54 | 4.41 |
| CAR | 4.05 | 4.44 | 4.71 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 16)

| Window | TB | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.160 | 0.172 |
| Last 5 games | 0.105 | 0.199 |
| Last 3 games | 0.105 | 0.195 |

## Trend Summary (last 3 weeks)

| Metric | W14 | W15 | W16 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA TB | 0.062 | 0.194 | 0.026 | - declining |
| Off EPA CAR | -0.001 | -0.090 | 0.176 | + improving |
| Def EPA TB | -0.128 | -0.221 | 0.054 | - declining |
| Def EPA CAR | 0.036 | 0.182 | 0.120 | - declining |
| Off SR TB | 48.1% | 57.0% | 57.0% | + improving |
| Off SR CAR | 39.8% | 47.4% | 45.6% | + improving |
| Def SR TB | 43.8% | 45.7% | 42.0% | + improving |
| Def SR CAR | 44.0% | 53.9% | 51.1% | - declining |
| Tempo TB | 7.700 | 7.818 | 7.818 | + improving |
| Tempo CAR | 9.222 | 7.600 | 6.923 | - declining |

## Matchup Analogs

**TB analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | CAR (A) | 31-45 | CAR | 1.000 | 0.088 | 52.0% | -1.144 |
| 14 | SF (A) | 28-12 | TB | 0.804 | 0.062 | 48.1% | 1.618 |
| 9 | LA (H) | 24-22 | TB | 0.758 | 0.133 | 44.4% | 0.167 |

**CAR analogs vs TB profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 7 | TB (H) | 7-34 | TB | 1.000 | -0.309 | 45.2% | -2.700 |
| 13 | TB (-) | 23-24 | TB | 1.000 | -0.075 | 50.0% | -0.090 |
| 11 | BAL (A) | n/a | n/a | 0.900 | n/a | n/a | n/a |

## Core EPA Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.074 | 0.029 | 0.028 |
| TB | 0.082 | 0.093 | 0.094 |

## Core EPA Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.103 | 0.111 | 0.113 |
| TB | 0.006 | -0.110 | -0.099 |

## Success Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 45.0% | 46.7% | 44.2% |
| TB | 51.2% | 52.4% | 54.2% |

## Success Rate Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 50.8% | 51.9% | 50.0% |
| TB | 49.4% | 44.9% | 43.7% |

## Explosive Play Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 8.6% | 9.2% | 9.2% |
| TB | 12.8% | 14.5% | 15.3% |

## Third Down Conversion Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 31.2% | 33.8% | 33.4% |
| TB | 43.9% | 40.9% | 51.7% |

## Points per Drive Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.994 | -0.486 | -0.585 |
| TB | 0.517 | 1.150 | 1.230 |

## Yards per Play Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.801 | -0.446 | -0.562 |
| TB | 0.431 | 1.691 | 1.891 |

## Turnover Margin Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.533 | -0.200 | -0.667 |
| TB | -0.533 | -0.800 | -1.000 |

## Red Zone TD Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 12.5% | 10.2% | 13.2% |
| TB | 18.6% | 20.5% | 23.2% |

## Pressure Rate (Def) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 5.6% | 9.9% | 9.1% |
| TB | 6.7% | 5.7% | 5.2% |

## Tempo Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 7.174 | 7.879 | 7.915 |
| TB | 7.585 | 7.743 | 7.779 |

## Pass Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.7% | 43.6% | 40.2% |
| TB | 44.5% | 41.3% | 43.4% |
