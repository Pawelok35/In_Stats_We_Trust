# Matchup Report - HOU vs BAL

## Metric Comparison

| Metric | HOU | BAL | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.004 | 0.142 | -0.145 |
| Core EPA Defense | -0.020 | 0.038 | -0.058 |
| Success Rate Offense | 0.435 | 0.512 | -0.077 |
| Success Rate Defense | 0.444 | 0.465 | -0.021 |
| Explosive Play Rate (Off) | 0.109 | 0.140 | -0.031 |
| Third Down Conversion | 0.336 | 0.440 | -0.105 |
| Points per Drive Differential | -0.019 | 0.661 | -0.680 |
| Yards per Play Differential | 0.073 | 1.223 | -1.151 |
| Turnover Margin | 0.600 | 0.200 | 0.400 |
| Red Zone TD Rate (Off) | 0.127 | 0.217 | -0.090 |
| Pressure Rate (Def) | 0.081 | 0.079 | 0.002 |
| Tempo | 6.455 | 7.600 | -1.145 |
| PowerScore | 0.066 | 0.201 | -0.135 |
| PowerScore | 0.066 | 0.201 | -0.135 |
| Tempo | 6.455 | 7.600 | -1.145 |

## PowerScore Breakdown (Model)

| Component | Weight | HOU | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.269 | 0.256 | -0.525 |
| EPA Defense | 20% | 0.256 | -0.269 | +0.525 |
| Success Rate Offense | 25% | 38.0% | 56.6% | -18.6 pp |
| Tempo | 20% | 0.070 | 0.118 | -0.048 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | HOU | BAL | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.269 | 0.256 | -0.525 |
| EPA Defense | 20% | 0.256 | -0.269 | +0.525 |
| Success Rate Offense | 15% | 38.0% | 56.6% | -18.6 pp |
| Turnover Margin | 10% | -1.000 | 1.000 | -2.000 |
| Tempo | 10% | 0.070 | 0.118 | -0.048 |
| Red Zone TD Rate (Off) | 10% | 0.0% | 27.3% | -27.3 pp |
| Pressure Rate (Def) | 10% | 5.6% | 13.5% | -8.0 pp |

## PowerScore Summary

**Model (4 metrics):**
BAL edge: +0.135 (HOU +0.066 vs BAL +0.201)
**7 metrics version:**
BAL edge: +0.294 (HOU -0.046 vs BAL +0.248)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.294), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (HOU - BAL) | -2.5 pts (favours BAL) |
| Model Win% HOU | 39.8% |
| Model Win% BAL | 60.2% |
| SOS Differential (HOU-BAL) | -0.192 |
*Neutral-field assumption. Based on schedule through Week 16.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| HOU | 52.1% | 43.9% | 8.2% | 35.1% | 41.2% | Pass heavy |
| BAL | 23.7% | 43.9% | -20.2% | 50.0% | 58.6% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -2.4 pp | +7.0 pp | +12.0 pp |
| BAL | +11.1 pp | +14.3 pp | +15.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -1.4 pp | -5.4 pp | -8.5 pp |
| BAL | +7.4 pp | +11.3 pp | +3.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -0.7 pp | +1.6 pp | +0.7 pp |
| BAL | +3.2 pp | +1.9 pp | -4.2 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +1.0 pp | +1.7 pp | +2.6 pp |
| BAL | +4.5 pp | +5.2 pp | +6.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -4.1 pp | -5.9 pp | -1.2 pp |
| BAL | +2.2 pp | +0.9 pp | -0.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -2.6 pp | +4.4 pp | +0.5 pp |
| BAL | +13.7 pp | +6.1 pp | +8.3 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | +1.0 pp | +1.7 pp | +2.6 pp |
| BAL | +4.5 pp | +5.2 pp | +6.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -0.7 pp | +1.6 pp | +0.7 pp |
| BAL | +3.2 pp | +1.9 pp | -4.2 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 47.3 | 51.2 | 52.6 |
| BAL | 45.8 | 43.1 | 42.2 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 45.5 | 46.7 | 46.8 |
| BAL | 50.7 | 49.2 | 48.8 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 1.8 | 4.5 | 5.8 |
| BAL | -5.0 | -6.1 | -6.6 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 1.89 | 2.01 | 2.01 |
| BAL | 2.76 | 2.52 | 2.67 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 1.91 | 1.84 | 2.07 |
| BAL | 2.10 | 1.81 | 1.70 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | -0.02 | 0.18 | -0.06 |
| BAL | 0.66 | 0.71 | 0.97 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 7.20 | 6.89 | 7.20 |
| BAL | 7.58 | 7.53 | 7.60 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 45.5% | 48.9% | 51.1% |
| BAL | 37.3% | 39.4% | 39.1% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 54.5% | 51.1% | 48.9% |
| BAL | 62.7% | 60.6% | 60.9% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 3.28 | 3.37 | 3.68 |
| BAL | 2.83 | 2.97 | 2.97 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| HOU | 3.92 | 3.52 | 3.52 |
| BAL | 4.75 | 4.56 | 4.63 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 16)

| Window | HOU | BAL |
| --- | ---: | ---: |
| Season-to-date | 1.538 | 1.730 |
| Last 5 games | 1.747 | 1.726 |
| Last 3 games | 1.883 | 1.651 |

## Trend Summary (last 3 weeks)

| Metric | W13 | W15 | W16 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA HOU | 0.086 | -0.112 | 0.057 | - declining |
| Off EPA BAL | -0.050 | 0.306 | 0.166 | + improving |
| Def EPA HOU | 0.007 | -0.169 | 0.104 | - declining |
| Def EPA BAL | 0.045 | -0.038 | -0.083 | + improving |
| Off SR HOU | 45.0% | 45.5% | 42.7% | - declining |
| Off SR BAL | 40.7% | 57.5% | 55.1% | + improving |
| Def SR HOU | 41.9% | 47.0% | 50.6% | - declining |
| Def SR BAL | 50.0% | 40.5% | 47.4% | + improving |
| Tempo HOU | 7.273 | 6.000 | 8.333 | + improving |
| Tempo BAL | 7.583 | 8.111 | 7.091 | - declining |

## Matchup Analogs

**HOU analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 5 | BUF (H) | 23-23 | TIE | 0.903 | -0.027 | 43.5% | -0.024 |
| 10 | DET (H) | 23-26 | DET | 0.871 | -0.123 | 41.4% | -0.214 |
| 7 | GB (A) | 22-24 | GB | 0.730 | 0.040 | 48.1% | -0.167 |

**BAL analogs vs HOU profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 12 | LAC (A) | 30-23 | BAL | 0.910 | 0.229 | 48.1% | 0.778 |
| 9 | DEN (H) | 41-10 | BAL | 0.855 | 0.333 | 58.9% | 3.100 |
| 8 | CLE (A) | 24-27 | CLE | 0.835 | 0.054 | 48.9% | -0.235 |

## Core EPA Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.142 | 0.097 | 0.141 |
| HOU | -0.004 | 0.008 | 0.011 |

## Core EPA Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.038 | -0.018 | -0.025 |
| HOU | -0.020 | -0.057 | -0.019 |

## Success Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 51.1% | 48.2% | 50.4% |
| HOU | 43.6% | 43.8% | 44.3% |

## Success Rate Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 46.5% | 45.6% | 45.8% |
| HOU | 44.1% | 44.6% | 46.5% |

## Explosive Play Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 13.9% | 13.8% | 14.5% |
| HOU | 11.0% | 10.6% | 11.3% |

## Third Down Conversion Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 43.7% | 46.7% | 49.7% |
| HOU | 33.6% | 33.6% | 39.4% |

## Points per Drive Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.598 | 0.593 | 0.844 |
| HOU | 0.009 | 0.172 | -0.006 |

## Yards per Play Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.223 | 1.620 | 1.793 |
| HOU | 0.073 | -0.036 | -0.135 |

## Turnover Margin Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.200 | -0.200 | 0.333 |
| HOU | 0.600 | 0.400 | 0.667 |

## Red Zone TD Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 21.7% | 20.7% | 15.2% |
| HOU | 12.7% | 11.1% | 11.5% |

## Pressure Rate (Def) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.9% | 8.8% | 8.1% |
| HOU | 8.1% | 8.1% | 3.1% |

## Tempo Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 7.581 | 7.528 | 7.595 |
| HOU | 7.202 | 6.894 | 7.202 |

## Pass Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 37.8% | 39.5% | 39.3% |
| HOU | 45.3% | 48.8% | 51.1% |
