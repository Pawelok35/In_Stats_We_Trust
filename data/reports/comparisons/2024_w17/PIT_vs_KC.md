# Matchup Report - PIT vs KC

## Metric Comparison

| Metric | PIT | KC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.129 | 0.192 | ↓ -0.320 |
| Core EPA Defense | 0.192 | -0.129 | ↑ +0.320 |
| Success Rate Offense | 45.2% | 50.0% | ↓ -4.8 pp |
| Success Rate Defense | 50.0% | 45.2% | ↑ +4.8 pp |
| Explosive Play Rate (Off) | 14.0% | 9.2% | ↑ +4.8 pp |
| Third Down Conversion | 50.0% | 25.0% | ↑ +25.0 pp |
| Points per Drive Differential | -1.991 | 1.991 | ↓ -3.982 |
| Yards per Play Differential | -1.204 | 1.204 | ↓ -2.409 |
| Turnover Margin | -2.000 | 2.000 | ↓ -4.000 |
| Red Zone TD Rate (Off) | 6.2% | 23.5% | ↓ -17.3 pp |
| Pressure Rate (Def) | 0.0% | 11.9% | ↓ -11.9 pp |
| Tempo | 8.455 | 7.600 | ↑ +0.855 |
| PowerScore | 0.134 | 0.185 | ↓ -0.051 |

## PowerScore Breakdown (Model)

| Component | Weight | PIT | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.129 | 0.192 | -0.320 |
| EPA Defense | 20% | 0.192 | -0.129 | +0.320 |
| Success Rate Offense | 25% | 45.2% | 50.0% | -4.8 pp |
| Tempo | 20% | 0.140 | 0.092 | +0.048 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | PIT | KC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.129 | 0.192 | -0.320 |
| EPA Defense | 20% | 0.192 | -0.129 | +0.320 |
| Success Rate Offense | 15% | 45.2% | 50.0% | -4.8 pp |
| Turnover Margin | 10% | -2.000 | 2.000 | -4.000 |
| Tempo | 10% | 0.140 | 0.092 | +0.048 |
| Red Zone TD Rate (Off) | 10% | 6.2% | 23.5% | -17.3 pp |
| Pressure Rate (Def) | 10% | 0.0% | 11.9% | -11.9 pp |

## PowerScore Summary

**Model (4 metrics):**
KC edge: +0.051 (PIT +0.134 vs KC +0.185)
**7 metrics version:**
KC edge: +0.448 (PIT -0.106 vs KC +0.342)
**Verdict:** KC holds the edge in the extended breakdown (lead 0.448), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (PIT - KC) | -0.9 pts (favours KC) |
| Model Win% PIT | 46.1% |
| Model Win% KC | 53.9% |
| SOS Differential (PIT-KC) | -0.013 |
*Neutral-field assumption. Based on schedule through Week 16.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| PIT | 45.2% | 43.9% | 1.3% | 33.3% | 54.9% | Slight pass lean |
| KC | 51.3% | 43.9% | 7.4% | 56.4% | 43.2% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -0.2 pp | +0.7 pp | -7.1 pp |
| KC | +16.1 pp | +14.8 pp | +9.9 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +0.9 pp | +9.1 pp | +10.1 pp |
| KC | -1.6 pp | -5.2 pp | +0.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -1.7 pp | +0.6 pp | +2.1 pp |
| KC | +0.7 pp | +0.1 pp | +3.0 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +1.0 pp | -0.7 pp | -3.2 pp |
| KC | -2.1 pp | -2.7 pp | -4.0 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +3.3 pp | +1.2 pp | +4.4 pp |
| KC | +4.0 pp | +0.3 pp | +2.0 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -4.1 pp | +2.9 pp | -2.8 pp |
| KC | +2.3 pp | -3.0 pp | -0.5 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | +1.0 pp | -0.7 pp | -3.2 pp |
| KC | -2.1 pp | -2.7 pp | -4.0 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | -1.7 pp | +0.6 pp | +2.1 pp |
| KC | +0.7 pp | +0.1 pp | +3.0 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 47.7 | 51.8 | 53.0 |
| KC | 46.1 | 45.7 | 43.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 47.5 | 50.5 | 44.8 |
| KC | 49.6 | 48.3 | 46.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 0.2 | 1.3 | 8.1 |
| KC | -3.4 | -2.6 | -2.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 1.99 | 1.96 | 1.64 |
| KC | 2.31 | 2.33 | 2.10 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 1.82 | 2.40 | 2.24 |
| KC | 1.89 | 1.80 | 1.49 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 0.17 | -0.44 | -0.60 |
| KC | 0.42 | 0.52 | 0.61 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 7.36 | 6.94 | 6.46 |
| KC | 8.34 | 8.64 | 8.19 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 38.9% | 41.8% | 41.0% |
| KC | 47.2% | 49.8% | 48.0% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 61.1% | 58.2% | 59.0% |
| KC | 52.8% | 50.2% | 52.0% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 2.86 | 2.90 | 2.65 |
| KC | 3.94 | 4.30 | 3.93 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| PIT | 4.50 | 4.04 | 3.82 |
| KC | 4.41 | 4.34 | 4.26 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 16)

| Window | PIT | KC |
| --- | ---: | ---: |
| Season-to-date | 0.123 | 0.136 |
| Last 5 games | 0.154 | 0.138 |
| Last 3 games | 0.141 | 0.113 |

## Trend Summary (last 3 weeks)

| Metric | W14 | W15 | W16 | Trend |
| --- | --- | --- | --- | --- |
| Off EPA PIT | -0.028 | -0.085 | -0.083 | - declining |
| Off EPA KC | 0.089 | -0.079 | 0.104 | + improving |
| Def EPA PIT | -0.181 | 0.112 | 0.166 | - declining |
| Def EPA KC | 0.081 | -0.337 | 0.057 | + improving |
| Off SR PIT | 42.7% | 46.6% | 47.4% | + improving |
| Off SR KC | 52.9% | 41.9% | 50.6% | - declining |
| Def SR PIT | 41.3% | 47.4% | 55.1% | - declining |
| Def SR KC | 41.9% | 44.0% | 42.7% | = stable |
| Tempo PIT | 5.857 | 6.444 | 7.091 | + improving |
| Tempo KC | 8.700 | 6.200 | 9.667 | + improving |

## Matchup Analogs

**PIT analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 10 | WAS (A) | 28-32 | WAS | 0.826 | 0.056 | 40.2% | -0.250 |
| 15 | PHI (A) | 13-27 | PHI | 0.793 | -0.085 | 46.6% | -1.556 |
| 2 | DEN (A) | 13-6 | PIT | 0.674 | -0.009 | 37.8% | 0.682 |

**KC analogs vs PIT profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 4 | LAC (A) | 17-11 | KC | 0.930 | -0.103 | 46.1% | 0.545 |
| 14 | LAC (H) | 19-19 | TIE | 0.930 | 0.089 | 52.9% | 0.011 |
| 10 | DEN (H) | 16-14 | KC | 0.904 | 0.019 | 42.0% | 0.222 |

## Core EPA Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.056 | 0.065 | 0.038 |
| PIT | 0.014 | -0.012 | -0.065 |

## Core EPA Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | -0.017 | -0.024 | -0.066 |
| PIT | -0.051 | 0.028 | 0.032 |

## Success Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 50.5% | 48.2% | 48.3% |
| PIT | 45.8% | 47.7% | 45.4% |

## Success Rate Defense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 45.5% | 44.7% | 42.9% |
| PIT | 47.1% | 49.3% | 47.5% |

## Explosive Play Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 8.3% | 8.7% | 7.5% |
| PIT | 10.0% | 10.7% | 7.8% |

## Third Down Conversion Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 46.9% | 44.4% | 44.3% |
| PIT | 35.2% | 37.1% | 34.3% |

## Points per Drive Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.424 | 0.529 | 0.650 |
| PIT | 0.181 | -0.327 | -0.422 |

## Yards per Play Differential Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.094 | -0.299 | -0.169 |
| PIT | -0.106 | -0.269 | -0.912 |

## Turnover Margin Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.133 | 1.400 | 2.000 |
| PIT | 0.733 | 0.400 | -0.667 |

## Red Zone TD Rate (Off) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 12.1% | 10.6% | 12.9% |
| PIT | 12.9% | 15.8% | 19.1% |

## Pressure Rate (Def) Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 6.5% | 8.0% | 8.8% |
| PIT | 6.8% | 6.3% | 6.1% |

## Tempo Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 8.344 | 8.638 | 8.189 |
| PIT | 7.360 | 6.941 | 6.464 |

## Pass Rate Offense Form (up to Week 16)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 47.2% | 49.8% | 47.9% |
| PIT | 38.9% | 41.9% | 40.8% |
