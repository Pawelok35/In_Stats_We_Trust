# Matchup Report - NYG vs CAR

## Metric Comparison

| Metric | NYG | CAR | Delta |
|---|---:|---:|---:|
| Core EPA Offense | -0.061 | -0.101 | ↑ +0.040 |
| Core EPA Defense | -0.101 | -0.061 | ↓ -0.040 |
| Success Rate Offense | 35.9% | 46.7% | ↓ -10.8 pp |
| Success Rate Defense | 46.7% | 35.9% | ↑ +10.8 pp |
| Explosive Play Rate (Off) | 9.8% | 8.0% | ↑ +1.8 pp |
| Third Down Conversion | 30.0% | 15.4% | ↑ +14.6 pp |
| Points per Drive Differential | 0.129 | -0.129 | ↑ +0.258 |
| Yards per Play Differential | -0.786 | 0.786 | ↓ -1.572 |
| Turnover Margin | 2.000 | -2.000 | ↑ +4.000 |
| Red Zone TD Rate (Off) | 9.1% | 6.7% | ↑ +2.4 pp |
| Pressure Rate (Def) | 6.5% | 7.7% | ↓ -1.2 pp |
| Tempo | 7.667 | 6.818 | ↑ +0.848 |
| PowerScore | 0.068 | 0.085 | ↓ -0.017 |

## PowerScore Breakdown (Model)

| Component | Weight | NYG | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | -0.061 | -0.101 | +0.040 |
| EPA Defense | 20% | -0.101 | -0.061 | -0.040 |
| Success Rate Offense | 25% | 35.9% | 46.7% | -10.8 pp |
| Tempo | 20% | 0.098 | 0.080 | +0.018 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | NYG | CAR | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | -0.061 | -0.101 | +0.040 |
| EPA Defense | 20% | -0.101 | -0.061 | -0.040 |
| Success Rate Offense | 15% | 35.9% | 46.7% | -10.8 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.098 | 0.080 | +0.018 |
| Red Zone TD Rate (Off) | 10% | 9.1% | 6.7% | +2.4 pp |
| Pressure Rate (Def) | 10% | 6.5% | 7.7% | -1.2 pp |

## PowerScore Summary

**Model (4 metrics):**
CAR edge: +0.017 (NYG +0.068 vs CAR +0.085)
**7 metrics version:**
NYG edge: +0.389 (NYG +0.244 vs CAR -0.145)
**Verdict:** NYG holds the edge in the extended breakdown (lead 0.389), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (NYG - CAR) | -0.3 pts (favours CAR) |
| Model Win% NYG | 48.7% |
| Model Win% CAR | 51.3% |
| SOS Differential (NYG-CAR) | -0.025 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| NYG | 42.4% | 45.7% | -3.3% | 28.2% | 41.5% | Run heavy |
| CAR | 41.3% | 45.7% | -4.4% | 29.0% | 59.1% | Run heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | -19.9 pp | -19.9 pp | -19.9 pp |
| CAR | +10.8 pp | +10.8 pp | +10.8 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | +8.0 pp | +8.0 pp | +8.0 pp |
| CAR | +6.7 pp | +6.7 pp | +6.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | -15.7 pp | -15.7 pp | -15.7 pp |
| CAR | -9.6 pp | -9.6 pp | -9.6 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | +3.1 pp | +3.1 pp | +3.1 pp |
| CAR | -2.5 pp | -2.5 pp | -2.5 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | -5.5 pp | -5.5 pp | -5.5 pp |
| CAR | +3.9 pp | +3.9 pp | +3.9 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | +3.6 pp | +3.6 pp | +3.6 pp |
| CAR | -21.7 pp | -21.7 pp | -21.7 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | +3.1 pp | +3.1 pp | +3.1 pp |
| CAR | -2.5 pp | -2.5 pp | -2.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | -15.7 pp | -15.7 pp | -15.7 pp |
| CAR | -9.6 pp | -9.6 pp | -9.6 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 45.9 | 45.9 | 45.9 |
| CAR | 45.8 | 45.8 | 45.8 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 40.9 | 40.9 | 40.9 |
| CAR | 50.2 | 50.2 | 50.2 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 5.0 | 5.0 | 5.0 |
| CAR | -4.4 | -4.4 | -4.4 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 1.75 | 1.75 | 1.75 |
| CAR | 2.00 | 2.00 | 2.00 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 1.67 | 1.67 | 1.67 |
| CAR | 2.36 | 2.36 | 2.36 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 0.08 | 0.08 | 0.08 |
| CAR | -0.36 | -0.36 | -0.36 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 6.25 | 6.25 | 6.25 |
| CAR | 6.17 | 6.17 | 6.17 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 36.0% | 36.0% | 36.0% |
| CAR | 43.2% | 43.2% | 43.2% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 64.0% | 64.0% | 64.0% |
| CAR | 56.8% | 56.8% | 56.8% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 2.25 | 2.25 | 2.25 |
| CAR | 2.67 | 2.67 | 2.67 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| NYG | 4.00 | 4.00 | 4.00 |
| CAR | 3.50 | 3.50 | 3.50 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | NYG | CAR |
| --- | ---: | ---: |
| Season-to-date | 0.141 | 0.166 |
| Last 5 games | 0.141 | 0.166 |
| Last 3 games | 0.141 | 0.166 |

## Matchup Analogs

**NYG analogs vs CAR profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | TEN (A) | 21-20 | NYG | 0.671 | 0.041 | 42.7% | 0.083 |

**CAR analogs vs NYG profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | CLE (H) | 24-28 | CLE | 0.600 | 0.023 | 36.5% | -0.364 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.023 | 0.023 | 0.023 |
| NYG | 0.041 | 0.041 | 0.041 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 0.081 | 0.081 | 0.081 |
| NYG | 0.018 | 0.018 | 0.018 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 36.5% | 36.5% | 36.5% |
| NYG | 42.7% | 42.7% | 42.7% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 44.9% | 44.9% | 44.9% |
| NYG | 43.0% | 43.0% | 43.0% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.8% | 6.8% | 6.8% |
| NYG | 13.3% | 13.3% | 13.3% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 30.8% | 30.8% | 30.8% |
| NYG | 18.2% | 18.2% | 18.2% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.364 | -0.364 | -0.364 |
| NYG | 0.083 | 0.083 | 0.083 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -0.095 | -0.095 | -0.095 |
| NYG | 1.106 | 1.106 | 1.106 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | -1.000 | -1.000 | -1.000 |
| NYG | -3.000 | -3.000 | -3.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 16.7% | 16.7% | 16.7% |
| NYG | 14.3% | 14.3% | 14.3% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 2.9% | 2.9% | 2.9% |
| NYG | 2.9% | 2.9% | 2.9% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 6.167 | 6.167 | 6.167 |
| NYG | 6.250 | 6.250 | 6.250 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| CAR | 43.2% | 43.2% | 43.2% |
| NYG | 36.0% | 36.0% | 36.0% |
