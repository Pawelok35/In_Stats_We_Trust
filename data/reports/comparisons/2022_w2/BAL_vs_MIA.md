# Matchup Report - BAL vs MIA

## Metric Comparison

| Metric | BAL | MIA | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.016 | 0.028 | -0.012 |
| Core EPA Defense | -0.195 | -0.163 | -0.033 |
| Success Rate Offense | 0.408 | 0.486 | -0.078 |
| Success Rate Defense | 0.364 | 0.435 | -0.071 |
| Explosive Play Rate (Off) | 0.099 | 0.095 | 0.004 |
| Third Down Conversion | 0.385 | 0.400 | -0.015 |
| Points per Drive Differential | 1.096 | 0.778 | 0.318 |
| Yards per Play Differential | 0.021 | 0.221 | -0.200 |
| Turnover Margin | 1.000 | 3.000 | -2.000 |
| Red Zone TD Rate (Off) | 0.100 | 0.000 | 0.100 |
| Pressure Rate (Def) | 0.048 | 0.061 | -0.012 |
| Tempo | 7.000 | 7.909 | -0.909 |
| PowerScore | 1.688 | 1.889 | -0.202 |
| PowerScore | 1.688 | 1.889 | -0.202 |
| Tempo | 7.000 | 7.909 | -0.909 |

## PowerScore Breakdown (Model)

| Component | Weight | BAL | MIA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.293 | 0.342 | -0.049 |
| EPA Defense | 20% | 0.342 | 0.293 | +0.049 |
| Success Rate Offense | 25% | 46.8% | 51.7% | -5.0 pp |
| Tempo | 20% | 0.169 | 0.103 | +0.065 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | BAL | MIA | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.293 | 0.342 | -0.049 |
| EPA Defense | 20% | 0.342 | 0.293 | +0.049 |
| Success Rate Offense | 15% | 46.8% | 51.7% | -5.0 pp |
| Turnover Margin | 10% | 2.000 | -2.000 | +4.000 |
| Tempo | 10% | 0.169 | 0.103 | +0.065 |
| Red Zone TD Rate (Off) | 10% | 13.3% | 25.0% | -11.7 pp |
| Pressure Rate (Def) | 10% | 2.0% | 0.0% | +2.0 pp |

## PowerScore Summary

**Model (4 metrics):**
MIA edge: +0.007 (BAL +0.321 vs MIA +0.328)
**7 metrics version:**
BAL edge: +0.387 (BAL +0.444 vs MIA +0.057)
**Verdict:** BAL holds the edge in the extended breakdown (lead 0.387), driven by Turnover Margin (10% weight).
### Risk flags

- Score relies ~20% on volatile stats (TO/Red Zone) – treat edge with caution.


## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (BAL - MIA) | -3.7 pts (favours MIA) |
| Model Win% BAL | 34.9% |
| Model Win% MIA | 65.1% |
| SOS Differential (BAL-MIA) | +0.085 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| BAL | 37.7% | 45.7% | -8.1% | 58.6% | 39.6% | Run heavy |
| MIA | 58.6% | 45.7% | 12.9% | 56.9% | 44.4% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +2.1 pp | +2.1 pp | +2.1 pp |
| MIA | +28.2 pp | +28.2 pp | +28.2 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -15.0 pp | -15.0 pp | -15.0 pp |
| MIA | -7.1 pp | -7.1 pp | -7.1 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +0.2 pp | +0.2 pp | +0.2 pp |
| MIA | -3.1 pp | -3.1 pp | -3.1 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -0.3 pp | -0.3 pp | -0.3 pp |
| MIA | -0.6 pp | -0.6 pp | -0.6 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +1.6 pp | +1.6 pp | +1.6 pp |
| MIA | +20.3 pp | +20.3 pp | +20.3 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -7.2 pp | -7.2 pp | -7.2 pp |
| MIA | +7.0 pp | +7.0 pp | +7.0 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -0.3 pp | -0.3 pp | -0.3 pp |
| MIA | -0.6 pp | -0.6 pp | -0.6 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | +0.2 pp | +0.2 pp | +0.2 pp |
| MIA | -3.1 pp | -3.1 pp | -3.1 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 40.2 | 40.2 | 40.2 |
| MIA | 39.3 | 39.3 | 39.3 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 41.8 | 41.8 | 41.8 |
| MIA | 44.0 | 44.0 | 44.0 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | -1.6 | -1.6 | -1.6 |
| MIA | -4.7 | -4.7 | -4.7 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.85 | 1.85 | 1.85 |
| MIA | 1.56 | 1.56 | 1.56 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 0.75 | 0.75 | 0.75 |
| MIA | 0.78 | 0.78 | 0.78 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 1.10 | 1.10 | 1.10 |
| MIA | 0.78 | 0.78 | 0.78 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 5.46 | 5.46 | 5.46 |
| MIA | 8.22 | 8.22 | 8.22 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 47.9% | 47.9% | 47.9% |
| MIA | 51.4% | 51.4% | 51.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 52.1% | 52.1% | 52.1% |
| MIA | 48.6% | 48.6% | 48.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.62 | 2.62 | 2.62 |
| MIA | 4.22 | 4.22 | 4.22 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| BAL | 2.85 | 2.85 | 2.85 |
| MIA | 4.00 | 4.00 | 4.00 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | BAL | MIA |
| --- | ---: | ---: |
| Season-to-date | 1.676 | 1.591 |
| Last 5 games | 1.676 | 1.591 |
| Last 3 games | 1.676 | 1.591 |

## Matchup Analogs

**BAL analogs vs MIA profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | NYJ (A) | 24-10 | BAL | 0.233 | 0.016 | 40.8% | 1.096 |

**MIA analogs vs BAL profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | NE (H) | 14-7 | MIA | 0.181 | 0.028 | 48.6% | 0.778 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.016 | 0.016 | 0.016 |
| MIA | 0.028 | 0.028 | 0.028 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | -0.195 | -0.195 | -0.195 |
| MIA | -0.163 | -0.163 | -0.163 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 40.8% | 40.8% | 40.8% |
| MIA | 48.6% | 48.6% | 48.6% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 36.4% | 36.4% | 36.4% |
| MIA | 43.5% | 43.5% | 43.5% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 9.9% | 9.9% | 9.9% |
| MIA | 9.5% | 9.5% | 9.5% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 38.5% | 38.5% | 38.5% |
| MIA | 40.0% | 40.0% | 40.0% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.096 | 1.096 | 1.096 |
| MIA | 0.778 | 0.778 | 0.778 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 0.021 | 0.021 | 0.021 |
| MIA | 0.221 | 0.221 | 0.221 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 1.000 | 1.000 | 1.000 |
| MIA | 3.000 | 3.000 | 3.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 10.0% | 10.0% | 10.0% |
| MIA | 0.0% | 0.0% | 0.0% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 4.8% | 4.8% | 4.8% |
| MIA | 6.1% | 6.1% | 6.1% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 5.462 | 5.462 | 5.462 |
| MIA | 8.222 | 8.222 | 8.222 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| BAL | 47.9% | 47.9% | 47.9% |
| MIA | 51.4% | 51.4% | 51.4% |
