# Matchup Report - KC vs LAC

## Metric Comparison

| Metric | KC | LAC | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.059 | 0.023 | ↑ +0.035 |
| Core EPA Defense | 0.023 | 0.059 | ↓ -0.035 |
| Success Rate Offense | 46.1% | 48.9% | ↓ -2.9 pp |
| Success Rate Defense | 48.9% | 46.1% | ↑ +2.9 pp |
| Explosive Play Rate (Off) | 7.9% | 7.4% | ↑ +0.4 pp |
| Third Down Conversion | 30.8% | 29.4% | ↑ +1.4 pp |
| Points per Drive Differential | -0.250 | 0.250 | ↓ -0.500 |
| Yards per Play Differential | -0.069 | 0.069 | ↓ -0.137 |
| Turnover Margin | 1.000 | -1.000 | ↑ +2.000 |
| Red Zone TD Rate (Off) | 8.3% | 26.7% | ↓ -18.3 pp |
| Pressure Rate (Def) | 3.9% | 2.6% | ↑ +1.3 pp |
| Tempo | 6.333 | 7.833 | ↓ -1.500 |
| PowerScore | 0.156 | 0.157 | ↓ -0.001 |

## PowerScore Breakdown (Model)

| Component | Weight | KC | LAC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.059 | 0.023 | +0.035 |
| EPA Defense | 20% | 0.023 | 0.059 | -0.035 |
| Success Rate Offense | 25% | 46.1% | 48.9% | -2.9 pp |
| Tempo | 20% | 0.079 | 0.074 | +0.004 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | KC | LAC | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.059 | 0.023 | +0.035 |
| EPA Defense | 20% | 0.023 | 0.059 | -0.035 |
| Success Rate Offense | 15% | 46.1% | 48.9% | -2.9 pp |
| Turnover Margin | 10% | 1.000 | -1.000 | +2.000 |
| Tempo | 10% | 0.079 | 0.074 | +0.004 |
| Red Zone TD Rate (Off) | 10% | 8.3% | 26.7% | -18.3 pp |
| Pressure Rate (Def) | 10% | 3.9% | 2.6% | +1.3 pp |

## PowerScore Summary

**Model (4 metrics):**
LAC edge: +0.001 (KC +0.156 vs LAC +0.157)
**7 metrics version:**
KC edge: +0.181 (KC +0.209 vs LAC +0.028)
**Verdict:** KC holds the edge in the extended breakdown (lead 0.181), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (KC - LAC) | -0.0 pts (favours LAC) |
| Model Win% KC | 49.9% |
| Model Win% LAC | 50.1% |
| SOS Differential (KC-LAC) | +0.094 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| KC | 50.0% | 45.7% | 4.3% | 44.7% | 47.4% | Pass heavy |
| LAC | 54.3% | 45.7% | 8.5% | 47.1% | 51.2% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +10.1 pp | +10.1 pp | +10.1 pp |
| LAC | +15.0 pp | +15.0 pp | +15.0 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +7.3 pp | +7.3 pp | +7.3 pp |
| LAC | -11.9 pp | -11.9 pp | -11.9 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +13.6 pp | +13.6 pp | +13.6 pp |
| LAC | +6.8 pp | +6.8 pp | +6.8 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +4.0 pp | +4.0 pp | +4.0 pp |
| LAC | -0.2 pp | -0.2 pp | -0.2 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +4.7 pp | +4.7 pp | +4.7 pp |
| LAC | -3.2 pp | -3.2 pp | -3.2 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +33.4 pp | +33.4 pp | +33.4 pp |
| LAC | +14.1 pp | +14.1 pp | +14.1 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +4.0 pp | +4.0 pp | +4.0 pp |
| LAC | -0.2 pp | -0.2 pp | -0.2 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | +13.6 pp | +13.6 pp | +13.6 pp |
| LAC | +6.8 pp | +6.8 pp | +6.8 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 41.2 | 41.2 | 41.2 |
| LAC | 51.0 | 51.0 | 51.0 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 56.3 | 56.3 | 56.3 |
| LAC | 43.5 | 43.5 | 43.5 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | -15.1 | -15.1 | -15.1 |
| LAC | 7.5 | 7.5 | 7.5 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 4.40 | 4.40 | 4.40 |
| LAC | 2.18 | 2.18 | 2.18 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 1.91 | 1.91 | 1.91 |
| LAC | 1.73 | 1.73 | 1.73 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 2.49 | 2.49 | 2.49 |
| LAC | 0.45 | 0.45 | 0.45 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 8.30 | 8.30 | 8.30 |
| LAC | 7.73 | 7.73 | 7.73 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 47.0% | 47.0% | 47.0% |
| LAC | 42.4% | 42.4% | 42.4% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 53.0% | 53.0% | 53.0% |
| LAC | 57.6% | 57.6% | 57.6% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 3.90 | 3.90 | 3.90 |
| LAC | 3.27 | 3.27 | 3.27 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| KC | 4.40 | 4.40 | 4.40 |
| LAC | 4.45 | 4.45 | 4.45 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | KC | LAC |
| --- | ---: | ---: |
| Season-to-date | 0.187 | 0.094 |
| Last 5 games | 0.187 | 0.094 |
| Last 3 games | 0.187 | 0.094 |

## Matchup Analogs

**KC analogs vs LAC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | ARI (A) | 44-19 | KC | 0.201 | 0.366 | 59.0% | 2.491 |

**LAC analogs vs KC profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | LV (H) | 24-19 | LAC | 0.364 | -0.038 | 41.2% | 0.455 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.366 | 0.366 | 0.366 |
| LAC | -0.038 | -0.038 | -0.038 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 0.012 | 0.012 | 0.012 |
| LAC | -0.082 | -0.082 | -0.082 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 59.0% | 59.0% | 59.0% |
| LAC | 41.2% | 41.2% | 41.2% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 37.3% | 37.3% | 37.3% |
| LAC | 40.8% | 40.8% | 40.8% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 18.1% | 18.1% | 18.1% |
| LAC | 8.2% | 8.2% | 8.2% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 55.6% | 55.6% | 55.6% |
| LAC | 40.0% | 40.0% | 40.0% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 2.491 | 2.491 | 2.491 |
| LAC | 0.455 | 0.455 | 0.455 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 2.458 | 2.458 | 2.458 |
| LAC | -0.331 | -0.331 | -0.331 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | -1.000 | -1.000 | -1.000 |
| LAC | 3.000 | 3.000 | 3.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 24.0% | 24.0% | 24.0% |
| LAC | 15.4% | 15.4% | 15.4% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 6.8% | 6.8% | 6.8% |
| LAC | 13.6% | 13.6% | 13.6% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 8.300 | 8.300 | 8.300 |
| LAC | 7.727 | 7.727 | 7.727 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| KC | 47.0% | 47.0% | 47.0% |
| LAC | 42.4% | 42.4% | 42.4% |
