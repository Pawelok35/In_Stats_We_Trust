# Matchup Report - JAX vs IND

## Metric Comparison

| Metric | JAX | IND | Delta |
|---|---:|---:|---:|
| Core EPA Offense | 0.081 | -0.333 | ↑ +0.414 |
| Core EPA Defense | -0.333 | 0.081 | ↓ -0.414 |
| Success Rate Offense | 42.2% | 28.1% | ↑ +14.0 pp |
| Success Rate Defense | 28.1% | 42.2% | ↓ -14.0 pp |
| Explosive Play Rate (Off) | 3.6% | 9.4% | ↓ -5.8 pp |
| Third Down Conversion | 40.0% | 18.2% | ↑ +21.8 pp |
| Points per Drive Differential | 2.000 | -2.000 | ↑ +4.000 |
| Yards per Play Differential | 0.582 | -0.582 | ↑ +1.163 |
| Turnover Margin | 3.000 | -3.000 | ↑ +6.000 |
| Red Zone TD Rate (Off) | 16.7% | 0.0% | ↑ +16.7 pp |
| Pressure Rate (Def) | 14.3% | 0.0% | ↑ +14.3 pp |
| Tempo | 6.917 | 5.818 | ↑ +1.098 |
| PowerScore | 0.074 | -0.011 | ↑ +0.086 |

## PowerScore Breakdown (Model)

| Component | Weight | JAX | IND | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 35% | 0.081 | -0.333 | +0.414 |
| EPA Defense | 20% | -0.333 | 0.081 | -0.414 |
| Success Rate Offense | 25% | 42.2% | 28.1% | +14.0 pp |
| Tempo | 20% | 0.036 | 0.094 | -0.058 |

## PowerScore Breakdown (7 Metrics)

| Component | Weight | JAX | IND | Delta |
| --- | ---: | ---: | ---: | ---: |
| EPA Offense | 25% | 0.081 | -0.333 | +0.414 |
| EPA Defense | 20% | -0.333 | 0.081 | -0.414 |
| Success Rate Offense | 15% | 42.2% | 28.1% | +14.0 pp |
| Turnover Margin | 10% | 3.000 | -3.000 | +6.000 |
| Tempo | 10% | 0.036 | 0.094 | -0.058 |
| Red Zone TD Rate (Off) | 10% | 16.7% | 0.0% | +16.7 pp |
| Pressure Rate (Def) | 10% | 14.3% | 0.0% | +14.3 pp |

## PowerScore Summary

**Model (4 metrics):**
JAX edge: +0.086 (JAX +0.074 vs IND -0.011)
**7 metrics version:**
JAX edge: +0.667 (JAX +0.351 vs IND -0.315)
**Verdict:** JAX holds the edge in the extended breakdown (lead 0.667), driven by Turnover Margin (10% weight).

## Model Outlook

| Metric | Value |
| --- | --- |
| Model Spread (JAX - IND) | +1.6 pts (favours JAX) |
| Model Win% JAX | 56.6% |
| Model Win% IND | 43.4% |
| SOS Differential (JAX-IND) | +0.058 |
*Neutral-field assumption. Based on schedule through Week 1.*

## PROE Tendencies

| Team | Pass Rate | Expected | PROE | Opp Pass SR Allowed | Opp Rush SR Allowed | Lean |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| JAX | 37.3% | 45.7% | -8.4% | 61.3% | 30.8% | Run heavy |
| IND | 54.7% | 45.7% | 9.0% | 31.4% | 24.1% | Pass heavy |

## Situational Edges

### 3rd Down Conversion

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -6.2 pp | -6.2 pp | -6.2 pp |
| IND | -16.3 pp | -16.3 pp | -16.3 pp |

### Red Zone TD Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -22.8 pp | -22.8 pp | -22.8 pp |
| IND | -15.5 pp | -15.5 pp | -15.5 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +2.9 pp | +2.9 pp | +2.9 pp |
| IND | +0.4 pp | +0.4 pp | +0.4 pp |

### Explosive Plays

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +7.5 pp | +7.5 pp | +7.5 pp |
| IND | +4.7 pp | +4.7 pp | +4.7 pp |

_Positive values indicate the offense exceeding the opponent's defensive rate (pass protection uses defense minus pressure allowed)._

## Matchup Edges

### Rush Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +4.4 pp | +4.4 pp | +4.4 pp |
| IND | +8.8 pp | +8.8 pp | +8.8 pp |

### Pass Success Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +0.5 pp | +0.5 pp | +0.5 pp |
| IND | +3.6 pp | +3.6 pp | +3.6 pp |

### Explosive Rate Edge

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +7.5 pp | +7.5 pp | +7.5 pp |
| IND | +4.7 pp | +4.7 pp | +4.7 pp |

### Pass Protection vs Pressure

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | +2.9 pp | +2.9 pp | +2.9 pp |
| IND | +0.4 pp | +0.4 pp | +0.4 pp |

_Positive values favour the listed offense; pass protection uses defense minus pressure allowed._

## Drive Context

### Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 52.0 | 52.0 | 52.0 |
| IND | 41.7 | 41.7 | 41.7 |

### Opponent Avg Start (own yardline)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 42.8 | 42.8 | 42.8 |
| IND | 40.4 | 40.4 | 40.4 |

### Field Position Edge (own - opp)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 9.2 | 9.2 | 9.2 |
| IND | 1.3 | 1.3 | 1.3 |

### Points per Drive (offense)

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.00 | 2.00 | 2.00 |
| IND | 1.54 | 1.54 | 1.54 |

### Points per Drive Allowed

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 2.33 | 2.33 | 2.33 |
| IND | 1.43 | 1.43 | 1.43 |

### Points per Drive Differential

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | -0.33 | -0.33 | -0.33 |
| IND | 0.11 | 0.11 | 0.11 |

_Starting field position expressed as own-yard line (higher = shorter field)._

## Game Script Projection

### Tempo

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 7.73 | 7.73 | 7.73 |
| IND | 8.54 | 8.54 | 8.54 |

### Pass Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 52.9% | 52.9% | 52.9% |
| IND | 46.8% | 46.8% | 46.8% |

### Run Rate

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 47.1% | 47.1% | 47.1% |
| IND | 53.2% | 53.2% | 53.2% |

### Passes per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 4.09 | 4.09 | 4.09 |
| IND | 4.00 | 4.00 | 4.00 |

### Runs per Drive

| Team | Season-to-date | Last 5 | Last 3 |
| --- | ---: | ---: | ---: |
| JAX | 3.64 | 3.64 | 3.64 |
| IND | 4.54 | 4.54 | 4.54 |

_Derived using aggregated tempo and pass rate (Run Rate = 1 - Pass Rate)._

## Strength of Schedule (through Week 1)

| Window | JAX | IND |
| --- | ---: | ---: |
| Season-to-date | 0.151 | 0.094 |
| Last 5 games | 0.151 | 0.094 |
| Last 3 games | 0.151 | 0.094 |

## Matchup Analogs

**JAX analogs vs IND profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | WAS (A) | 22-26 | WAS | 0.318 | 0.040 | 45.9% | -0.333 |

**IND analogs vs JAX profile**

| Week | Opponent | Score | Winner | Similarity | EPA Off | Success Rate | PPD Diff |
| ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | HOU (A) | 20-19 | IND | 0.334 | -0.020 | 45.9% | 0.110 |

## Core EPA Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.020 | -0.020 | -0.020 |
| JAX | 0.040 | 0.040 | 0.040 |

## Core EPA Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -0.062 | -0.062 | -0.062 |
| JAX | 0.070 | 0.070 | 0.070 |

## Success Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 45.9% | 45.9% | 45.9% |
| JAX | 45.9% | 45.9% | 45.9% |

## Success Rate Defense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 43.5% | 43.5% | 43.5% |
| JAX | 39.8% | 39.8% | 39.8% |

## Explosive Play Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 14.4% | 14.4% | 14.4% |
| JAX | 12.9% | 12.9% | 12.9% |

## Third Down Conversion Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 37.5% | 37.5% | 37.5% |
| JAX | 25.0% | 25.0% | 25.0% |

## Points per Drive Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 0.110 | 0.110 | 0.110 |
| JAX | -0.333 | -0.333 | -0.333 |

## Yards per Play Differential Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 1.408 | 1.408 | 1.408 |
| JAX | 0.291 | 0.291 | 0.291 |

## Turnover Margin Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | -1.000 | -1.000 | -1.000 |
| JAX | 2.000 | 2.000 | 2.000 |

## Red Zone TD Rate (Off) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 9.5% | 9.5% | 9.5% |
| JAX | 10.5% | 10.5% | 10.5% |

## Pressure Rate (Def) Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 7.3% | 7.3% | 7.3% |
| JAX | 4.3% | 4.3% | 4.3% |

## Tempo Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 8.538 | 8.538 | 8.538 |
| JAX | 7.727 | 7.727 | 7.727 |

## Pass Rate Offense Form (up to Week 1)

| Team | Season-to-date | Last 5 | Last 3 |
|------|---------------:|-------:|-------:|
| IND | 46.8% | 46.8% | 46.8% |
| JAX | 52.9% | 52.9% | 52.9% |
